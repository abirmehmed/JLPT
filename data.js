// JLPT vocabulary data — [Word, Kana reading, Meaning] tuples per level.
// Romaji is generated automatically in app.js from the kana reading.
// One word per line — edit meanings directly here.
const JLPT_DATA = {
  "n5": [
    ["ああ", "ああ", "Ah!, Oh!"],
    ["会う", "あう", "to meet, to see"],
    ["青", "あお", "blue"],
    ["青い", "あおい", "blue"],
    ["赤", "あか", "red"],
    ["赤い", "あかい", "red"],
    ["明るい", "あかるい", "bright (in reference to personality or weather); cheerful"],
    ["秋", "あき", "fall (season)"],
    ["開く", "あく", "to open, to become open"],
    ["開ける", "あける", "to open (v.t.)"],
    ["上げる", "あげる", "to raise, to lift"],
    ["朝", "あさ", "morning"],
    ["朝御飯", "あさごはん", "breakfast"],
    ["明後日", "あさって", "day after tomorrow"],
    ["足; 脚", "あし", "foot; leg"],
    ["明日", "あした", "tomorrow"],
    ["あそこ", "あそこ", "there, over there, that place"],
    ["遊ぶ", "あそぶ", "to play; to spend time pleasantly; to hang out"],
    ["暖かい", "あたたかい", "warm"],
    ["頭", "あたま", "head"],
    ["新しい", "あたらしい", "new"],
    ["あちら", "あちら", "this way (polite)"],
    ["暑い", "あつい", "hot (in reference to weather), warm"],
    ["熱い", "あつい", "hot (objects)"],
    ["厚い", "あつい", "kind, warm(hearted), thick, deep"],
    ["あっち", "あっち", "over there"],
    ["後", "あと", "afterwards (later); in the future; the rest; since then"],
    ["あなた", "あなた", "you"],
    ["兄", "あに", "(my) older brother (humble)"],
    ["姉", "あね", "(my) older sister (humble)"],
    ["アパート", "アパート", "apartment (abbr.)"],
    ["あの", "あの", "that over there; like that, that way; um..."],
    ["浴びる", "あびる", "to bathe, to shower"],
    ["危ない", "あぶない", "dangerous, critical"],
    ["甘い", "あまい", "generous, sweet"],
    ["余り", "あまり", "not very; surplus"],
    ["雨", "あめ", "rain"],
    ["飴", "あめ", "(hard) candy"],
    ["洗う", "あらう", "to wash"],
    ["在る", "ある", "to be, to have"],
    ["有る", "ある", "to be, to have"],
    ["歩く", "あるく", "to walk"],
    ["あれ", "あれ", "that one (over there)"],
    ["いい; よい", "いい; よい", "good"],
    ["いいえ", "いいえ", "no, not at all"],
    ["言う", "いう", "to say"],
    ["家", "いえ", "house, home"],
    ["いかが", "いかが", "how, in what way"],
    ["行く", "いく; ゆく", "to go"],
    ["いくつ", "いくつ", "how many, how old"],
    ["いくら", "いくら", "how much, how many"],
    ["池", "いけ", "pond"],
    ["医者", "いしゃ", "doctor; physician"],
    ["椅子", "いす", "chair"],
    ["忙しい", "いそがしい", "busy (people, days)"],
    ["痛い", "いたい", "hurt; painful; sore"],
    ["一", "いち", "one"],
    ["一日", "いちにち", "one day (duration)"],
    ["一番", "いちばん", "best (most), first, number one"],
    ["いつ", "いつ", "when"],
    ["五日", "いつか", "five days; fifth day of the month"],
    ["一緒", "いっしょ", "together"],
    ["五つ", "いつつ", "five things"],
    ["いつも", "いつも", "always, usually, every time, never (with neg. verb)"],
    ["犬", "いぬ", "dog"],
    ["今", "いま", "now"],
    ["意味", "いみ", "meaning, significance"],
    ["妹", "いもうと", "younger sister (humble)"],
    ["嫌", "いや", "disagreeable, detestable, unpleasant"],
    ["入口", "いりぐち", "entrance"],
    ["居る", "いる", "(humble) to be (animate), to exist"],
    ["要る", "いる", "to need"],
    ["入れる", "いれる", "to put in"],
    ["色", "いろ", "color"],
    ["色々", "いろいろ", "various"],
    ["上", "うえ", "above (up, top, etc.), over, on top of"],
    ["後ろ", "うしろ", "back, behind, rear"],
    ["薄い", "うすい", "thin, weak"],
    ["歌", "うた", "a song"],
    ["歌う", "うたう", "to sing"],
    ["うち", "うち", "home; house; my place"],
    ["生まれる", "うまれる", "to be born"],
    ["海", "うみ", "sea, beach"],
    ["売る", "うる", "to sell (v.t.)"],
    ["うるさい", "うるさい", "noisy; annoying"],
    ["上着", "うわぎ", "coat, jacket"],
    ["絵", "え", "a painting; a picture; a drawing"],
    ["映画", "えいが", "movie, film"],
    ["映画館", "えいがかん", "movie theater, cinema"],
    ["英語", "えいご", "English (language)"],
    ["ええ", "ええ", "yes"],
    ["駅", "えき", "station"],
    ["エレベーター", "エレベーター", "elevator"],
    ["～円", "～えん", "Yen"],
    ["鉛筆", "えんぴつ", "pencil"],
    ["お～", "お～", "honorable ~ (honorific)"],
    ["美味しい", "おいしい", "delicious, tasty"],
    ["多い", "おおい", "many; there are a lot"],
    ["大きい", "おおきい", "big, large"],
    ["大きな", "おおきな", "big"],
    ["大勢", "おおぜい", "great number of people"],
    ["お母さん", "おかあさん", "mother (formal)"],
    ["お菓子", "おかし", "confections, sweets, snack"],
    ["お金", "おかね", "money"],
    ["起きる", "おきる", "to get up (e.g., from sleeping); to happen"],
    ["置く", "おく", "to put; to lay; to place"],
    ["奥さん", "おくさん", "(someone else's) wife (hon.)"],
    ["お酒", "おさけ", "sake; alcohol"],
    ["お皿", "おさら", "plate, dish"],
    ["伯父; 叔父さん", "おじさん", "uncle, middle-aged man"],
    ["おじいさん", "おじいさん", "grandfather, male senior citizen"],
    ["教える", "おしえる", "to teach, to inform, to instruct"],
    ["押す", "おす", "to push, to press, to stamp (e.g., a passport)"],
    ["遅い", "おそい", "slow; (to be) late"],
    ["お茶", "おちゃ", "(green) tea"],
    ["お手洗い", "おてあらい", "toilet, restroom, bathroom (lit., a place to wash one's hands)"],
    ["お父さん", "おとうさん", "father (formal)"],
    ["弟", "おとうと", "younger brother"],
    ["男", "おとこ", "man, male"],
    ["男の子", "おとこのこ", "boy"],
    ["一昨日", "おととい", "the day before yesterday"],
    ["おととし", "おととし", "year before last"],
    ["大人", "おとな", "adult"],
    ["お腹", "おなか", "stomach"],
    ["同じ", "おなじ", "same, identical"],
    ["お兄さん", "おにいさん", "(someone else's) older brother (formal)"],
    ["お姉さん", "おねえさん", "older sister (formal)"],
    ["伯母さん; 叔母さん", "おばさん", "aunt"],
    ["おばあさん", "おばあさん", "grandmother, female senior-citizen"],
    ["お風呂", "おふろ", "a bath"],
    ["お弁当", "おべんとう", "a boxed lunch"],
    ["覚える", "おぼえる", "to learn, to commit to memory, to remember, to memorize"],
    ["おまわりさん", "おまわりさん", "policeman (friendly term)"],
    ["重い", "おもい", "heavy; serious"],
    ["面白い", "おもしろい", "interesting, amusing"],
    ["泳ぐ", "およぐ", "to swim"],
    ["降りる", "おりる", "to get off"],
    ["終る", "おわる", "to finish, to close"],
    ["音楽", "おんがく", "Music"],
    ["女", "おんな", "woman, female"],
    ["女の子", "おんなのこ", "girl"],
    ["～回", "～かい", "counter for occurrences (~ times)"],
    ["～階", "～かい", "counter for stories (floors) of a building"],
    ["外国", "がいこく", "foreign country; abroad"],
    ["外国人", "がいこくじん", "foreigner"],
    ["会社", "かいしゃ", "company, corporation"],
    ["階段", "かいだん", "stairs"],
    ["買い物", "かいもの", "shopping"],
    ["買う", "かう", "to buy"],
    ["返す", "かえす", "to return something"],
    ["帰る", "かえる", "to go back, to go home, to return"],
    ["顔", "かお", "face (body part)"],
    ["かかる", "かかる", "it takes (amount of time, money) (v.i.)"],
    ["鍵", "かぎ", "a lock; a key"],
    ["書く", "かく", "to write"],
    ["学生", "がくせい", "student"],
    ["～か月", "～かげつ", "(number of) months"],
    ["掛ける", "かける", "to put on (e.g., glasses); to hang (e.g., on a wall)"],
    ["かける", "かける", "to dial/call (e.g., phone); to sit down"],
    ["傘", "かさ", "umbrella, parasol"],
    ["貸す", "かす", "to lend"],
    ["風", "かぜ", "wind, breeze"],
    ["風邪", "かぜ", "cold, flu"],
    ["方", "かた", "-- honorific form for 人 (ひと) --; way of doing"],
    ["家族", "かぞく", "family, members of a family"],
    ["片仮名", "かたかな", "katakana"],
    ["～月", "～がつ", "month of year"],
    ["学校", "がっこう", "a school"],
    ["カップ", "カップ", "cup"],
    ["家庭", "かてい", "home; family"],
    ["角", "かど", "corner (e.g., desk, pavement)"],
    ["かばん", "かばん", "bag, basket"],
    ["花瓶", "かびん", "(flower) vase"],
    ["かぶる", "かぶる", "to wear, to put on (e.g., a hat on the head)"],
    ["紙", "かみ", "paper"],
    ["カメラ", "カメラ", "camera"],
    ["火曜日", "かようび", "Tuesday"],
    ["辛い", "からい", "hot and spicy; salty"],
    ["体", "からだ", "body; health"],
    ["借りる", "かりる", "to borrow, to owe"],
    ["～がる", "～がる", "feel"],
    ["軽い", "かるい", "light, non-serious, minor"],
    ["カレー", "カレー", "curry (abbr. for curry and rice)"],
    ["カレンダー", "カレンダー", "calendar"],
    ["川; 河", "かわ", "river"],
    ["～側", "～がわ", "~ side"],
    ["可愛い", "かわいい", "cute, adorable"],
    ["漢字", "かんじ", "kanji; Chinese character"],
    ["木", "き", "tree, wood, timber"],
    ["黄色", "きいろ", "yellow"],
    ["黄色い", "きいろい", "yellow"],
    ["消える", "きえる", "to vanish, to disappear"],
    ["聞く", "きく", "to hear, to listen, to ask"],
    ["北", "きた", "north"],
    ["ギター", "ギター", "guitar"],
    ["汚い", "きたない", "dirty, unclean, filthy"],
    ["喫茶店", "きっさてん", "café"],
    ["切手", "きって", "postal (postage) stamps"],
    ["切符", "きっぷ", "a ticket"],
    ["昨日", "きのう", "yesterday"],
    ["九", "きゅう", "nine"],
    ["牛肉", "ぎゅうにく", "beef"],
    ["牛乳", "ぎゅうにゅう", "milk"],
    ["今日", "きょう", "today, this day"],
    ["教室", "きょうしつ", "classroom"],
    ["兄弟", "きょうだい", "siblings (humble), brothers and sisters"],
    ["去年", "きょねん", "last year"],
    ["嫌い", "きらい", "dislike"],
    ["切る", "きる", "to cut; to hang up (a phone)"],
    ["着る", "きる", "to put on (clothes above your waist); to wear"],
    ["綺麗", "きれい", "pretty, clean, tidy"],
    ["キロ; キログラム", "キロ; キログラム", "(abbr.) kilo (kilogram)"],
    ["キロ; キロメートル", "キロ; キロメートル", "(abbr.) kilo (kilometer)"],
    ["銀行", "ぎんこう", "bank"],
    ["金曜日", "きんようび", "Friday"],
    ["九", "く", "nine"],
    ["薬", "くすり", "medicine"],
    ["下さい", "ください", "(with te-form verb) please do for me"],
    ["果物", "くだもの", "fruit"],
    ["口", "くち", "job opening; mouth"],
    ["靴", "くつ", "shoes, footwear"],
    ["靴下", "くつした", "socks"],
    ["国", "くに", "country; place of origin"],
    ["曇り", "くもり", "cloudiness, cloudy weather"],
    ["曇る", "くもる", "to become cloudy, to become dim"],
    ["暗い", "くらい", "dark, gloomy"],
    ["～くらい; ぐらい", "～くらい; ぐらい", "approximate (quantity)"],
    ["クラス", "クラス", "a class"],
    ["グラム", "グラム", "gram"],
    ["来る", "くる", "to come"],
    ["車", "くるま", "car, vehicle"],
    ["黒", "くろ", "black"],
    ["黒い", "くろい", "black; dark"],
    ["警官", "けいかん", "police officer"],
    ["今朝", "けさ", "this morning"],
    ["消す", "けす", "to erase, to delete, to turn off power"],
    ["結構", "けっこう", "splendid; enough, tolerably"],
    ["結婚", "けっこん (する)", "marriage (get married)"],
    ["月曜日", "げつようび", "Monday"],
    ["玄関", "げんかん", "entrance (to a house or a building)"],
    ["元気", "げんき", "health(y), energetic"],
    ["～個", "～こ", "counter for small items (e.g., fruits, cups)"],
    ["五", "ご", "five"],
    ["～語", "～ご", "word, language"],
    ["公園", "こうえん", "a park"],
    ["交差点", "こうさてん", "intersection"],
    ["紅茶", "こうちゃ", "black tea"],
    ["交番", "こうばん", "police box"],
    ["声", "こえ", "voice"],
    ["コート", "コート", "coat; court (e.g., tennis)"],
    ["コーヒー", "コーヒー", "coffee"],
    ["ここ", "ここ", "here, this place"],
    ["午後", "ごご", "afternoon, P.M."],
    ["九日", "ここのか", "nine days; ninth day of the month"],
    ["九つ", "ここのつ", "nine things"],
    ["午前", "ごぜん", "morning, A.M."],
    ["答える", "こたえる", "to answer, to reply"],
    ["こちら", "こちら", "this person (polite); this way (polite)"],
    ["こっち", "こっち", "this person; this direction; this side"],
    ["コップ", "コップ", "a tumbler; a glass"],
    ["今年", "ことし", "this year"],
    ["言葉", "ことば", "language; word(s); expression(s)"],
    ["子供", "こども", "child(ren)"],
    ["この", "この", "this"],
    ["御飯", "ごはん", "rice (cooked); meal"],
    ["コピーする", "コピーする", "to copy"],
    ["困る", "こまる", "to be bothered, to have difficulty"],
    ["これ", "これ", "this one"],
    ["～ころ; ～ごろ", "～ころ; ～ごろ", "about, toward, approximately (time)"],
    ["今月", "こんげつ", "this month"],
    ["今週", "こんしゅう", "this week"],
    ["こんな", "こんな", "such, like this"],
    ["今晩", "こんばん", "tonight, this evening"],
    ["さあ", "さあ", "come now, well"],
    ["～歳", "～さい", "~ years old"],
    ["財布", "さいふ", "wallet"],
    ["魚", "さかな", "fish"],
    ["先", "さき", "future; recent, previous"],
    ["咲く", "さく", "to bloom"],
    ["作文", "さくぶん", "essay; composition"],
    ["差す", "さす", "to raise (stretch out) hands, to raise (e.g., umbrella)"],
    ["～冊", "～さつ", "counter for books"],
    ["雑誌", "ざっし", "magazine, journal"],
    ["砂糖", "さとう", "sugar"],
    ["寒い", "さむい", "cold (in reference to weather)"],
    ["さ来年", "さらいねん", "year after next"],
    ["～さん", "～さん", "Mr. ~, Ms. ~"],
    ["三", "さん", "three"],
    ["散歩", "さんぽ (する)", "walk, stroll"],
    ["四", "し", "four"],
    ["～時", "～じ", "~ o'clock (time)"],
    ["塩", "しお", "salt"],
    ["しかし", "しかし", "however; but"],
    ["時間", "じかん", "time"],
    ["～時間", "～じかん", "~ hours"],
    ["仕事", "しごと", "work, job, occupation, employment"],
    ["辞書", "じしょ", "dictionary"],
    ["静か", "しずか", "quiet, calm"],
    ["下", "した", "under, below, beneath"],
    ["七", "しち", "seven"],
    ["質問", "しつもん", "question, inquiry"],
    ["自転車", "じてんしゃ", "bicycle"],
    ["自動車", "じどうしゃ", "automobile"],
    ["死ぬ", "しぬ", "to die"],
    ["字引", "じびき", "dictionary"],
    ["自分", "じぶん", "myself, oneself"],
    ["閉まる", "しまる", "to close, to be closed"],
    ["閉める", "しめる", "to close, to shut"],
    ["締める", "しめる", "to tie, to fasten, to tighten"],
    ["じゃ; じゃあ", "じゃ; じゃあ", "well, well then"],
    ["写真", "しゃしん", "a picture; a photograph"],
    ["シャツ", "シャツ", "shirt"],
    ["シャワー", "シャワー", "shower"],
    ["十", "じゅう", "ten"],
    ["～中", "～じゅう", "during, while"],
    ["～週間", "～しゅうかん", "~ weeks"],
    ["授業", "じゅぎょう", "a class (of school)"],
    ["宿題", "しゅくだい", "homework"],
    ["上手", "じょうず", "be good at, skillful"],
    ["丈夫", "じょうぶ", "strong, solid, durable"],
    ["醤油", "しょうゆ", "soy sauce"],
    ["食堂", "しょくどう", "cafeteria, dining hall"],
    ["知る", "しる", "to know, to understand"],
    ["白", "しろ", "white"],
    ["白い", "しろい", "white"],
    ["～人", "～じん", "counter for people"],
    ["新聞", "しんぶん", "newspaper"],
    ["水曜日", "すいようび", "Wednesday"],
    ["吸う", "すう", "to breathe in, to suck"],
    ["スカート", "スカート", "skirt"],
    ["好き", "すき", "liking, fondness, love"],
    ["～すぎ", "～すぎ", "past; to exceed, ~ too much"],
    ["少ない", "すくない", "a little; a few"],
    ["すぐに", "すぐに", "immediately, soon"],
    ["少し", "すこし", "little, few"],
    ["涼しい", "すずしい", "cool, refreshing (in reference to weather)"],
    ["～ずつ", "～ずつ", "at a time"],
    ["ストーブ", "ストーブ", "heater (lit: stove)"],
    ["スプーン", "スプーン", "spoon"],
    ["スポーツ", "スポーツ", "sport(s)"],
    ["ズボン", "ズボン", "trousers"],
    ["住む", "すむ", "to reside, to live in"],
    ["する", "する", "to do, to try; to wear small items (e.g., necktie, watch, etc.)"],
    ["座る", "すわる", "to sit"],
    ["背", "せい", "(one's) height, stature"],
    ["生徒", "せいと", "student; pupil"],
    ["セーター", "セーター", "sweater"],
    ["石鹸", "せっけん", "soap"],
    ["背広", "せびろ", "men's suit"],
    ["狭い", "せまい", "narrow; not spacious"],
    ["ゼロ", "ゼロ", "zero"],
    ["千", "せん", "thousand"],
    ["先月", "せんげつ", "last month"],
    ["先週", "せんしゅう", "last week"],
    ["先生", "せんせい", "teacher, professor; master; doctor"],
    ["洗濯", "せんたく", "washing, laundry"],
    ["全部", "ぜんぶ", "all, entire, whole"],
    ["そう; そうです", "そう; そうです", "yes; appears, to be the case"],
    ["掃除", "そうじ (する)", "cleaning, sweeping"],
    ["そうして; そして", "そうして; そして", "and, like that"],
    ["そこ", "そこ", "that place, there; bottom, sole"],
    ["そちら", "そちら", "over there"],
    ["そっち", "そっち", "over there"],
    ["外", "そと", "outside, exterior"],
    ["その", "その", "that"],
    ["そば", "そば", "near, close, beside; Japanese traditional buckwheat noodle"],
    ["空", "そら", "sky"],
    ["それ", "それ", "that one"],
    ["それから", "それから", "and then, after that"],
    ["それでは", "それでは", "in that situation, well then..."],
    ["～台", "～だい", "counter for vehicles; machines"],
    ["大学", "だいがく", "college; university"],
    ["大使館", "たいしかん", "embassy"],
    ["大丈夫", "だいじょうぶ", "It's ok (all right); No need to worry; Everything is under control"],
    ["大好き", "だいすき", "very like-able, like very much"],
    ["大切", "たいせつ", "important"],
    ["台所", "だいどころ", "kitchen"],
    ["大変", "たいへん", "very; difficult, hard"],
    ["高い", "たかい", "tall, high; expensive"],
    ["～だけ", "～だけ", "only ~, just ~, as ~"],
    ["沢山", "たくさん", "many, much"],
    ["タクシー", "タクシー", "taxi"],
    ["出す", "だす", "to take (something) out; to hand in (something)"],
    ["～たち", "～たち", "plural suffix"],
    ["立つ", "たつ", "to stand up"],
    ["たて", "たて", "length, height"],
    ["建物", "たてもの", "building"],
    ["楽しい", "たのしい", "enjoyable, fun"],
    ["頼む", "たのむ", "to request, to ask (a favor)"],
    ["たばこ", "たばこ", "tobacco, cigarettes"],
    ["多分", "たぶん", "perhaps, probably, maybe"],
    ["食べ物", "たべもの", "food"],
    ["食べる", "たべる", "to eat"],
    ["卵", "たまご", "egg"],
    ["誰", "だれ", "who"],
    ["誰か", "だれか", "someone"],
    ["誕生日", "たんじょうび", "birthday"],
    ["段々", "だんだん", "gradually, by degrees"],
    ["小さい", "ちいさい", "small, little"],
    ["小さな", "ちいさな", "small, little"],
    ["近い", "ちかい", "near, close by, short"],
    ["違う", "ちがう", "to be different; to differ; wrong"],
    ["近く", "ちかく", "nearby; in the neighborhood"],
    ["地下鉄", "ちかてつ", "underground train, subway"],
    ["地図", "ちず", "a map"],
    ["父", "ちち", "(my) father"],
    ["茶色", "ちゃいろ", "brown"],
    ["茶碗", "ちゃわん", "rice bowl"],
    ["～中", "～ちゅう", "during, while ~ing"],
    ["丁度", "ちょうど", "just, right, exactly"],
    ["ちょっと", "ちょっと", "a little, somewhat; just a little, somewhat"],
    ["一日", "ついたち", "one day; first day of the month"],
    ["使う", "つかう", "to use"],
    ["疲れる", "つかれる", "to get (become) tired; to become fatigued"],
    ["次", "つぎ", "next"],
    ["着く", "つく", "to arrive at, to reach"],
    ["机", "つくえ", "desk"],
    ["作る", "つくる", "to make, to create"],
    ["つける", "つける", "to turn on (e.g., a light); to take"],
    ["勤める", "つとめる", "to work (for)"],
    ["つまらない", "つまらない", "boring, dull; insignificant"],
    ["冷たい", "つめたい", "cold (things, people)"],
    ["強い", "つよい", "strong, powerful"],
    ["手", "て", "hand"],
    ["テープ", "テープ", "tape"],
    ["テープレコーダー", "テープレコーダー", "tape recorder"],
    ["テーブル", "テーブル", "table"],
    ["出かける", "でかける", "to go out; to depart"],
    ["手紙", "てがみ", "letter"],
    ["できる", "できる", "to be able to (to accomplish)"],
    ["出口", "でぐち", "an exit"],
    ["テスト", "テスト", "test"],
    ["では", "では", "then, well, so"],
    ["デパート", "デパート", "(abbr.) department store"],
    ["でも", "でも", "but, however"],
    ["出る", "でる", "to appear, to leave"],
    ["テレビ", "テレビ", "television, TV"],
    ["天気", "てんき", "weather"],
    ["電気", "でんき", "electricity, (electric) light"],
    ["電車", "でんしゃ", "electric train"],
    ["電話", "でんわ", "a telephone"],
    ["戸", "と", "door (Japanese style)"],
    ["～度", "～ど", "counter for occurrences; ~ degree; ~ point"],
    ["ドア", "ドア", "door (Western style)"],
    ["トイレ", "トイレ", "bathroom; toilet"],
    ["どう", "どう", "how, in what way"],
    ["どうして", "どうして", "why, for what reason"],
    ["どうぞ", "どうぞ", "please, kindly, by all means"],
    ["動物", "どうぶつ", "animal"],
    ["どうも", "どうも", "Thank you; somehow; no matter how hard one may try"],
    ["十", "(〜を) とお", "ten (~)"],
    ["遠い", "とおい", "far (away), distant"],
    ["十日", "とおか", "ten days; tenth day of the month"],
    ["～時", "～とき", "at the time of ~"],
    ["時々", "ときどき", "sometimes"],
    ["時計", "とけい", "a watch; a clock"],
    ["どこ", "どこ", "where, what place"],
    ["所", "ところ", "place"],
    ["年", "とし", "year, age"],
    ["図書館", "としょかん", "library"],
    ["どちら", "どちら", "which (one) (way); where (polite)"],
    ["どっち", "どっち", "which one, which way"],
    ["とても", "とても", "very (much), greatly, exceedingly"],
    ["どなた", "どなた", "who"],
    ["隣", "となり", "next to, next door to"],
    ["どの", "どの", "which"],
    ["飛ぶ", "とぶ", "to fly, to hop"],
    ["止まる", "とまる", "to come to a halt"],
    ["友達", "ともだち", "friend"],
    ["土曜日", "どようび", "Saturday"],
    ["鳥", "とり", "chicken (lit., bird)"],
    ["鶏肉", "とりにく", "chicken meat"],
    ["取る", "とる", "to take (a class); to get (a grade)"],
    ["撮る", "とる", "to take (a photo), to make (a film)"],
    ["どれ", "どれ", "which one"],
    ["どんな", "どんな", "what, what kind of"],
    ["ない", "ない", "there isn't, doesn't have"],
    ["ナイフ", "ナイフ", "knife"],
    ["中", "なか", "inside, middle, among"],
    ["長い", "ながい", "long, lengthy"],
    ["鳴く", "なく", "to make sound (animal)"],
    ["無くす", "なくす", "to lose something"],
    ["なぜ", "なぜ", "why (same as どうして)"],
    ["夏", "なつ", "summer"],
    ["夏休み", "なつやすみ", "summer vacation"],
    ["～など", "～など", "et cetera"],
    ["七つ", "ななつ", "seven things"],
    ["何", "なん; なに", "what"],
    ["七日", "なのか", "seven days; seventh day (of the month)"],
    ["名前", "なまえ", "name"],
    ["習う", "ならう", "to learn"],
    ["並ぶ", "ならぶ", "to line up, to stand in a line (v.i.)"],
    ["並べる", "ならべる", "to put (things) side by side; to line up"],
    ["なる", "なる", "to become"],
    ["何～", "なん～", "what sort of ~"],
    ["二", "に", "two"],
    ["にぎやか", "にぎやか", "bustling, busy"],
    ["肉", "にく", "meat"],
    ["西", "にし", "west"],
    ["～日", "～にち", "~ day of the month, for ~ days"],
    ["日曜日", "にちようび", "Sunday"],
    ["荷物", "にもつ", "luggage; baggage"],
    ["ニュース", "ニュース", "news"],
    ["庭", "にわ", "garden"],
    ["～人", "～にん", "counter for people"],
    ["脱ぐ", "ぬぐ", "to take off (clothes)"],
    ["温い", "ぬるい", "lukewarm"],
    ["ネクタイ", "ネクタイ", "tie, necktie"],
    ["猫", "ねこ", "cat"],
    ["寝る", "ねる", "to sleep; to go to sleep; to go to bed"],
    ["～年", "～ねん", "~ years"],
    ["ノート", "ノート", "notebook"],
    ["登る", "のぼる", "to climb"],
    ["飲み物", "のみもの", "drink, beverage"],
    ["飲む", "のむ", "to drink"],
    ["乗る", "のる", "to get on, to ride in, to board"],
    ["歯", "は", "tooth"],
    ["パーティー", "パーティー", "a party"],
    ["はい", "はい", "yes"],
    ["～杯", "～はい", "counter for cupfuls"],
    ["灰皿", "はいざら", "ashtray"],
    ["入る", "はいる", "to enter, to contain, to hold"],
    ["葉書", "はがき", "postcard"],
    ["はく", "はく", "to put on (items below your waist)"],
    ["箱", "はこ", "box"],
    ["橋", "はし", "bridge"],
    ["箸", "はし", "chopsticks"],
    ["始まる", "はじまる", "(something) begins"],
    ["初め; 始め", "はじめ", "beginning, start"],
    ["初めて", "はじめて", "for the first time"],
    ["走る", "はしる", "to run"],
    ["バス", "バス", "bus; bath; bass"],
    ["バター", "バター", "butter"],
    ["二十歳", "はたち", "20 years old"],
    ["働く", "はたらく", "to work"],
    ["八", "はち", "eight"],
    ["二十日", "はつか", "twenty days, twentieth (day of the month)"],
    ["花", "はな", "flower"],
    ["鼻", "はな", "nose"],
    ["話", "はなし", "talk (chat), story"],
    ["話す", "はなす", "to speak"],
    ["母", "はは", "(my) mother"],
    ["早い", "はやい", "early"],
    ["速い", "はやい", "fast, quick"],
    ["春", "はる", "spring"],
    ["貼る", "はる", "to post; to paste; to attach"],
    ["晴れ", "はれ", "clear (sunny) weather"],
    ["晴れる", "はれる", "to be sunny"],
    ["半", "はん", "half (e.g., にじはん | half-past two)"],
    ["晩", "ばん", "evening"],
    ["～番", "～ばん", "~st; ~th best"],
    ["パン", "パン", "bread"],
    ["ハンカチ", "ハンカチ", "handkerchief"],
    ["番号", "ばんごう", "number, series of digits"],
    ["晩御飯", "ばんごはん", "dinner, evening meal"],
    ["半分", "はんぶん", "half"],
    ["東", "ひがし", "east"],
    ["～匹", "～ひき", "counter for small animals"],
    ["引く", "ひく", "to pull, to draw; subtract"],
    ["弾く", "ひく", "to play (a string instrument or piano)"],
    ["低い", "ひくい", "short, low"],
    ["飛行機", "ひこうき", "airplane"],
    ["左", "ひだり", "left hand side"],
    ["人", "ひと", "man, person"],
    ["一つ", "ひとつ", "one thing"],
    ["一月", "ひとつき", "one month"],
    ["一人", "ひとり", "one person"],
    ["暇", "ひま", "free time, leisure"],
    ["百", "ひゃく", "hundred"],
    ["病院", "びょういん", "hospital"],
    ["病気", "びょうき", "illness; sickness"],
    ["平仮名", "ひらがな", "hiragana"],
    ["昼", "ひる", "noon, daytime"],
    ["昼御飯", "ひるごはん", "lunch, midday meal"],
    ["広い", "ひろい", "spacious; wide; broad"],
    ["フィルム", "フィルム", "film (roll of)"],
    ["封筒", "ふうとう", "envelope"],
    ["プール", "プール", "swimming pool"],
    ["フォーク", "フォーク", "fork"],
    ["吹く", "ふく", "to blow (wind, etc.)"],
    ["服", "ふく", "clothes"],
    ["二つ", "ふたつ", "two things"],
    ["豚肉", "ぶたにく", "pork"],
    ["二人", "ふたり", "two people"],
    ["二日", "ふつか", "two days; second day of the month"],
    ["太い", "ふとい", "fat, thick"],
    ["冬", "ふゆ", "winter"],
    ["降る", "ふる", "to precipitate, to fall (e.g., rain, snow, etc.)"],
    ["古い", "ふるい", "old (in reference to objects, not people), aged, ancient"],
    ["～分", "～ふん", "~ minutes"],
    ["文章", "ぶんしょう", "sentence, text"],
    ["ページ", "ページ", "a page"],
    ["下手", "へた", "unskillful, poor"],
    ["ベッド", "ベッド", "bed"],
    ["ペット", "ペット", "pet"],
    ["部屋", "へや", "a room"],
    ["辺", "へん", "area, vicinity"],
    ["ペン", "ペン", "pen"],
    ["勉強", "べんきょう (する)", "study"],
    ["便利", "べんり", "convenient, handy"],
    ["帽子", "ぼうし", "hat; cap"],
    ["ボールペン", "ボールペン", "ball-point pen"],
    ["外", "ほか", "other, the rest"],
    ["ポケット", "ポケット", "pocket"],
    ["欲しい", "ほしい", "to want, in need of"],
    ["ポスト", "ポスト", "mailbox; post, position"],
    ["細い", "ほそい", "thin, slender, fine"],
    ["ボタン", "ボタン", "button"],
    ["ホテル", "ホテル", "hotel"],
    ["本", "ほん", "book"],
    ["～本", "～ほん", "counter for long cylindrical things"],
    ["本棚", "ほんだな", "bookshelf"],
    ["本当", "ほんとう", "real, true"],
    ["～枚", "～まい", "counter for flat things"],
    ["毎朝", "まいあさ", "every morning"],
    ["毎月", "まいげつ; まいつき", "every month, monthly"],
    ["毎週", "まいしゅう", "every week"],
    ["毎日", "まいにち", "every day"],
    ["毎年", "まいねん; まいとし", "every year, yearly, annually"],
    ["毎晩", "まいばん", "every night"],
    ["前", "まえ", "before, in front"],
    ["～前", "～まえ", "in front of ~"],
    ["曲る", "まがる", "to turn, to bend"],
    ["まずい", "まずい", "terrible (in reference to food), unappetizing, unpleasant (taste)"],
    ["また", "また", "and; furthermore"],
    ["まだ", "まだ", "yet, still, besides"],
    ["町", "まち", "town; city"],
    ["待つ", "まつ", "to wait"],
    ["まっすぐ", "まっすぐ", "straight (ahead), direct"],
    ["マッチ", "マッチ", "match"],
    ["窓", "まど", "window"],
    ["丸い; 円い", "まるい", "round, circular"],
    ["万", "まん", "ten thousand"],
    ["万年筆", "まんねんひつ", "fountain pen"],
    ["磨く", "みがく", "to brush (teeth); to polish"],
    ["右", "みぎ", "right hand side"],
    ["短い", "みじかい", "short (length)"],
    ["水", "みず", "water"],
    ["店", "みせ", "store, shop"],
    ["見せる", "みせる", "to show, to display"],
    ["道", "みち", "road, street; way, directions"],
    ["三日", "みっか", "three days, third day of the month"],
    ["三つ", "みっつ", "three things"],
    ["緑", "みどり", "green"],
    ["皆さん", "みなさん", "all of you, everyone"],
    ["南", "みなみ", "South"],
    ["耳", "みみ", "ear"],
    ["見る", "みる", "to see, to look"],
    ["みんな", "みんな", "all, everyone, everybody"],
    ["六日", "むいか", "six days; sixth day of month"],
    ["向こう", "むこう", "beyond, over there"],
    ["難しい", "むずかしい", "difficult"],
    ["六つ", "むっつ", "six things"],
    ["村", "むら", "village"],
    ["目", "め", "eye(s)"],
    ["メートル", "メートル", "meter"],
    ["眼鏡", "めがね", "eye glasses"],
    ["もう", "もう", "already; again; more"],
    ["木曜日", "もくようび", "Thursday"],
    ["もしもし", "もしもし", "Hello? (used on the phone)"],
    ["持つ", "もつ", "to hold, to carry; to possess"],
    ["もっと", "もっと", "more"],
    ["物", "もの", "thing (concrete object)"],
    ["門", "もん", "gate"],
    ["問題", "もんだい", "a problem"],
    ["～屋", "～や", "~ shop"],
    ["八百屋", "やおや", "greengrocer"],
    ["野菜", "やさい", "vegetable"],
    ["易しい", "やさしい", "easy, plain, simple"],
    ["安い", "やすい", "inexpensive; cheap (things)"],
    ["休み", "やすみ", "holiday; day off; absence"],
    ["休む", "やすむ", "to rest, to have a break, to get time off"],
    ["八つ", "やっつ", "eight things"],
    ["山", "やま", "mountain"],
    ["やる", "やる", "to do; to give (to pets, parents, siblings, etc.)"],
    ["夕方", "ゆうがた", "late afternoon (typically just before dinner time), evening"],
    ["夕飯", "ゆうはん", "dinner, supper, evening meal"],
    ["郵便局", "ゆうびんきょく", "post office"],
    ["昨夜", "ゆうべ", "last night"],
    ["有名", "ゆうめい", "famous"],
    ["雪", "ゆき", "snow"],
    ["ゆっくりと", "ゆっくりと", "slowly, at ease"],
    ["八日", "ようか", "eight days; eighth day of the month"],
    ["洋服", "ようふく", "Western-style clothes"],
    ["よく", "よく", "frequently, often (much); well, skillfully"],
    ["横", "よこ", "beside; side; width"],
    ["四日", "よっか", "four days; fourth day of the month"],
    ["四つ", "よっつ", "four things"],
    ["呼ぶ", "よぶ", "to call (one's name); to invite"],
    ["読む", "よむ", "to read"],
    ["夜", "よる", "evening, night"],
    ["弱い", "よわい", "weak"],
    ["来月", "らいげつ", "next month"],
    ["来週", "らいしゅう", "next week"],
    ["来年", "らいねん", "next year"],
    ["ラジオ", "ラジオ", "radio"],
    ["ラジオカセ", "ラジオカセ", "radio cassette player"],
    ["りっぱ", "りっぱ", "splendid, fine"],
    ["留学生", "りゅうがくせい", "international student"],
    ["両親", "りょうしん", "parents (lit., both parents)"],
    ["料理", "りょうり", "cooking; cuisine"],
    ["旅行", "りょこう", "travel, trip"],
    ["零", "れい", "zero, nought"],
    ["冷蔵庫", "れいぞうこ", "refrigerator"],
    ["レコード", "レコード", "record"],
    ["レストラン", "レストラン", "restaurant"],
    ["練習", "れんしゅう (する)", "(to) practice"],
    ["廊下", "ろうか", "corridor"],
    ["六", "ろく", "six"],
    ["ワイシャツ", "ワイシャツ", "shirt (lit: white shirt), business shirt"],
    ["若い", "わかい", "young"],
    ["分かる", "わかる", "to understand"],
    ["忘れる", "わすれる", "to forget"],
    ["私", "わたし", "I, myself"],
    ["私", "わたくし", "I (formal), myself, private affairs"],
    ["渡す", "わたす", "to hand (something) over (v.t.); to get across"],
    ["渡る", "わたる", "to cross over, to go across"],
    ["悪い", "わるい", "bad, sinful; inferior"]
  ],
  "n4": [
    ["踏む", "ふむ", "to step on, to tread on"],
    ["～区", "～く", "~ district, ~ ward, ~ borough"],
    ["すっと", "すっと", "straight, quickly"],
    ["盗む", "ぬすむ", "to steal; to rob"],
    ["大抵", "たいてい", "generally, usually"],
    ["とうとう", "とうとう", "finally, at last"],
    ["ガソリン", "ガソリン", "gasoline, petrol"],
    ["鳴る", "なる", "to sound, to ring (v.i.)"],
    ["しっかり", "しっかり", "firmly, steady"],
    ["生きる", "いきる", "to live"],
    ["苦い", "にがい", "bitter"],
    ["沸く", "わく", "to boil, to grow hot"],
    ["意見", "いけん", "opinion, view, idea"],
    ["やはり; やっぱり", "やはり; やっぱり", "as I thought, absolutely"],
    ["漫画", "まんが", "comic (book), cartoon"],
    ["ステレオ", "ステレオ", "stereo"],
    ["医学", "いがく", "medical science"],
    ["テキスト", "テキスト", "text; text book"],
    ["～月", "～つき", "month"],
    ["折る", "おる", "to snap, to break; to bend"],
    ["～会", "～かい", "~ meeting"],
    ["うかがう", "うかがう", "to ask"],
    ["聞こえる", "きこえる", "to be heard, to be audible"],
    ["僕", "ぼく", "I (used by men towards those of equal or lower status)"],
    ["必ず", "かならず", "surely, certainly"],
    ["壊す", "こわす", "to break, to break down"],
    ["怒る", "おこる", "to get angry; to scold angrily"],
    ["床屋", "とこや", "barber's (shop)"],
    ["オートバイ", "オートバイ", "motorcycle (lit: auto-bi(ke))"],
    ["運動", "うんどうする", "exercise"],
    ["止む", "やむ", "to cease, to stop"],
    ["もし", "もし", "if"],
    ["表", "おもて", "surface; front; outside"],
    ["大学生", "だいがくせい", "college student, university student"],
    ["運転手", "うんてんしゅ", "driver (by occupation)"],
    ["予習", "よしゅう", "preparation of lessons (for class)"],
    ["心配", "しんぱいする", "worry, concern"],
    ["別", "べつ", "distinction, different"],
    ["非常に", "ひじょうに", "extremely; very"],
    ["お宅", "おたく", "(someone else's) house; home -- polite word for 家 (いえ) --"],
    ["柔らかい", "やわらかい", "soft (in reference to texture), tender"],
    ["拾う", "ひろう", "to pick up (something), to find"],
    ["～ございます", "～ございます", "to be (polite), to exist"],
    ["気", "き", "spirit, mood"],
    ["比べる", "くらべる", "to compare"],
    ["ほとんど", "ほとんど", "mostly, almost"],
    ["つもり", "つもり", "intention, plan"],
    ["郊外", "こうがい", "suburb, outskirts"],
    ["だめ", "だめ", "useless, no good, hopeless"],
    ["売り場", "うりば", "place where things are sold"],
    ["正月", "しょうがつ", "New Year, New Year's Day"],
    ["規則", "きそく", "rule, regulation"],
    ["うん", "うん", "yes (informal), all right (ok)"],
    ["発音", "はつおん", "pronunciation"],
    ["焼く", "やく", "to bake, to grill"],
    ["屋上", "おくじょう", "rooftop"],
    ["失礼", "しつれい", "discourtesy, impoliteness; Excuse me"],
    ["ごみ", "ごみ", "trash, garbage"],
    ["アフリカ", "アフリカ", "Africa"],
    ["点", "てん", "mark, score, grade; point, dot"],
    ["一生懸命", "いっしょうけんめい", "very hard (as in \"to work hard\"), with utmost effort"],
    ["今度", "こんど", "now, this time, near future, one of these days, next time"],
    ["機会", "きかい", "chance, opportunity"],
    ["建てる", "たてる", "to build"],
    ["複雑", "ふくざつ", "complexity, complication"],
    ["彼", "かれ", "he, boyfriend"],
    ["いらっしゃる", "いらっしゃる", "-- honorific expression for いく, くる, and いる --"],
    ["布団", "ふとん", "futon"],
    ["大事", "だいじ", "important, valuable, serious matter"],
    ["贈り物", "おくりもの", "a gift; a present"],
    ["泥棒", "どろぼう", "thief; burglar"],
    ["～製", "～せい", "made in ~"],
    ["注意", "ちゅうい", "caution, attention"],
    ["台風", "たいふう", "typhoon"],
    ["日", "ひ", "sun, sunshine, day"],
    ["～軒", "～けん", "counter for houses"],
    ["そう", "そう", "really, (is that) so; yes, right"],
    ["通る", "とおる", "to pass (by), to go through"],
    ["過ぎる", "すぎる", "to exceed, to go beyond"],
    ["レポート; リポート", "レポート; リポート", "report"],
    ["葉", "は", "leaf"],
    ["必要", "ひつよう", "necessary"],
    ["課長", "かちょう", "section manager"],
    ["地震", "じしん", "earthquake"],
    ["すると", "すると", "and, then"],
    ["止める", "やめる", "to end, to stop"],
    ["ガラス", "ガラス", "glass, pane"],
    ["～学部", "～がくぶ", "department of a university"],
    ["厳しい", "きびしい", "hard; rigorous; strict"],
    ["エスカレーター", "エスカレーター", "escalator"],
    ["人口", "じんこう", "population"],
    ["月", "つき", "moon"],
    ["絹", "きぬ", "silk"],
    ["ちっとも", "ちっとも", "not at all (neg. verb)"],
    ["深い", "ふかい", "deep, profound"],
    ["壊れる", "こわれる", "to be broken, to break"],
    ["揺れる", "ゆれる", "to shake, to sway"],
    ["落る", "おちる", "to fall, to drop"],
    ["できるだけ", "できるだけ", "if at all possible, as much as possible"],
    ["悲しい", "かなしい", "sad, sorrowful"],
    ["中学校", "ちゅうがっこう", "junior high school pupil"],
    ["ガス", "ガス", "gas"],
    ["祈る", "いのる", "to pray; to wish"],
    ["盛ん", "さかん", "prosperous, active, thriving"],
    ["アルバイト", "アルバイト", "part-time job"],
    ["起こす", "おこす", "to wake (someone) up"],
    ["致す", "いたす", "-- extra-modest expression for する --"],
    ["噛む", "かむ", "to bite, to chew"],
    ["赤ちゃん", "あかちゃん", "baby, infant"],
    ["浅い", "あさい", "shallow, superficial"],
    ["うそ", "嘘", "lie"],
    ["小説", "しょうせつ", "novel"],
    ["親", "おや", "a parent"],
    ["それに", "それに", "moreover; besides"],
    ["西洋", "せいよう", "the West, Western countries"],
    ["思う", "おもう", "to think, to feel"],
    ["パート (タイム)", "パート (タイム)", "part time (esp. female part time employees)"],
    ["時代", "じだい", "age, period, epoch, era"],
    ["申し上げる", "もうしあげる", "(humble)to say, to tell"],
    ["～式", "～しき", "~ ceremony; ~ style"],
    ["出席", "しゅっせきする", "attendance"],
    ["～家", "～か", "person who is specialized in ~"],
    ["迎える", "むかえる", "to welcome; to meet; to greet"],
    ["触る", "さわる", "to touch, to feel"],
    ["建て", "～だて", "~ storied, separate housing"],
    ["社長", "しゃちょう", "president of a company"],
    ["動物園", "どうぶつえん", "zoo"],
    ["捕まえる", "つかまえる", "to catch, to arrest"],
    ["季節", "きせつ", "season (in reference to weather)"],
    ["寄る", "よる", "to stop by"],
    ["決まる", "きまる", "to be set; fixed (v.i.)"],
    ["開く", "ひらく", "to open; to hold (an event)"],
    ["逃げる", "にげる", "to escape, to run away"],
    ["だから", "だから", "so; therefore"],
    ["残念", "ざんねん", "regret; regrettable"],
    ["畳", "たたみ", "tatami mat (Japanese straw mat)"],
    ["丁寧", "ていねい", "polite, courteous, careful"],
    ["地理", "ちり", "geography"],
    ["さっき", "さっき", "a little while ago"],
    ["怖い", "こわい", "scary, frightening"],
    ["包む", "つつむ", "to wrap, to cover"],
    ["なるべく", "なるべく", "if possible, as much as possible"],
    ["無理", "むり", "unreasonable, impossible"],
    ["サンドイッチ", "サンドイッチ", "sandwich"],
    ["会議室", "かいぎしつ", "conference room"],
    ["品物", "しなもの", "goods"],
    ["人形", "にんぎょう", "doll, figure"],
    ["利用", "りよう", "use, utilization"],
    ["飾る", "かざる", "to decorate, to adorn"],
    ["止める", "とめる", "to stop (something)"],
    ["恥ずかしい", "はずかしい", "ashamed, embarrassed"],
    ["いくら～ても", "いくら～ても", "however much one may ~"],
    ["用事", "ようじ", "business to take care of; tasks; errands"],
    ["ビル", "ビル", "(abbr.) building"],
    ["けんかする", "けんかする", "quarrel"],
    ["頑張る", "がんばる", "to try one's best, to try hard, to persist"],
    ["投げる", "なげる", "to pitch, to cast away"],
    ["故障", "こしょうする", "breakdown"],
    ["力", "ちから", "strength, power"],
    ["受ける", "うける", "to take (an examination, interview, etc.); to receive"],
    ["気分", "きぶん", "feeling, mood"],
    ["間違える", "まちがえる", "to make a mistake"],
    ["そんな", "そんな", "such, like that, that sort of"],
    ["星", "ほし", "star"],
    ["場合", "ばあい", "case, situation"],
    ["やっと", "やっと", "at last, finally"],
    ["足りる", "たりる", "to be sufficient; to be enough"],
    ["行う", "おこなう", "to carry out; to conduct (typically used in written language)"],
    ["ぶどう", "ぶどう", "grapes"],
    ["無くなる", "なくなる", "to disappear, to get lost"],
    ["準備", "じゅんびする", "prepare"],
    ["世界", "せかい", "world"],
    ["住所", "じゅうしょ", "address; place of residence"],
    ["再来月", "さらいげつ", "the month after next"],
    ["林", "はやし", "woods, forest"],
    ["倍", "ばい", "double"],
    ["痩せる", "やせる", "to lose weight"],
    ["線", "せん", "line, wire"],
    ["戦争", "せんそう", "war"],
    ["決める", "きめる", "to decide (v.t.)"],
    ["調べる", "しらべる", "to check; to look up; to inquire; to search"],
    ["寝坊", "ねぼう", "sleeping in late"],
    ["パパ", "パパ", "papa, daddy"],
    ["光る", "ひかる", "to shine, to glitter"],
    ["夫", "おっと", "husband"],
    ["雲", "くも", "cloud"],
    ["坂", "さか", "slope, hill"],
    ["～(て) しまう", "～(て) しまう", "to end up ~"],
    ["飛行場", "ひこうじょう", "airport"],
    ["柔道", "じゅうどう", "judo"],
    ["決して", "けっして", "never"],
    ["事務所", "じむしょ", "office"],
    ["連絡", "れんらく", "communication, contact, connection"],
    ["小学校", "しょうがっこう", "elementary school"],
    ["客", "きゃく", "guest, customer"],
    ["昔", "むかし", "old days; past"],
    ["美しい", "うつくしい", "beautiful, lovely"],
    ["捨てる", "すてる", "throw away (trash), dump, discard"],
    ["なさる", "なさる", "-- honorific expression for する --"],
    ["事", "こと", "thing(s), matter(s), fact"],
    ["どんどん", "どんどん", "quickly and steadily; at a rapid pace"],
    ["試合", "しあい", "match, game, competition"],
    ["適当", "てきとう", "fitness, suitability"],
    ["素晴らしい", "すばらしい", "wonderful; terrific"],
    ["美術館", "びじゅつかん", "art gallery, art museum"],
    ["文法", "ぶんぽう", "grammar"],
    ["終わり", "おわり", "end"],
    ["壁", "かべ", "wall"],
    ["一度", "いちど", "once, one time"],
    ["お礼", "おれい", "expression of gratitude; thanking; gift of appreciation; bow"],
    ["親切", "しんせつ", "kindness"],
    ["知らせる", "しらせる", "to notify"],
    ["歯医者", "はいしゃ", "dentist"],
    ["熱心", "ねっしん", "enthusiasm"],
    ["始める", "はじめる", "to start, to begin"],
    ["もらう", "もらう", "to receive"],
    ["泣く", "なく", "to cry"],
    ["治る", "なおる", "to get better; to recover from illness (v.i.)"],
    ["熱", "ねつ", "fever, temperature"],
    ["お祭り", "おまつり", "festival"],
    ["水道", "すいどう", "water service, water line"],
    ["匂い", "におい", "odor, smell"],
    ["ベル", "ベル", "bell"],
    ["赤ん坊", "あかんぼう", "baby"],
    ["おかしい", "おかしい", "strange; odd; funny"],
    ["事故", "じこ", "accident"],
    ["変", "へん", "strange, odd"],
    ["辞典", "じてん", "encyclopedia, reference book"],
    ["残る", "のこる", "to remain (v.i.), to be left"],
    ["立てる", "たてる", "to stand (something) up, to erect (something)"],
    ["くれる", "くれる", "to give, to do for"],
    ["～員", "～いん", "member of ~"],
    ["原因", "げんいん", "cause, origin, source"],
    ["驚く", "おどろく", "to be surprised, to be astonished"],
    ["いただく", "頂く", "-- extra-modest expression for たべる and のむ; humble expression for もらう --"],
    ["祖母", "そぼ", "grandmother"],
    ["場所", "ばしょ", "place, location"],
    ["答", "こたえ", "answer, response"],
    ["もちろん", "もちろん", "certainly, of course"],
    ["漬ける", "つける", "to soak, to moisten, to pickle"],
    ["受付", "うけつけ", "reception(ist) desk"],
    ["内", "うち", "within, inside"],
    ["スクリーン", "スクリーン", "screen"],
    ["増える", "ふえる", "to increase, to multiply"],
    ["または", "または", "or, otherwise"],
    ["けがする", "けがする", "injury (to animate object), hurt"],
    ["以下", "いか", "less than, below"],
    ["選ぶ", "えらぶ", "to choose; to select"],
    ["～ばかり", "～ばかり", "just did ~, only"],
    ["心", "こころ", "heart, mind"],
    ["～だす", "～だす", "to start doing ~"],
    ["サラダ", "サラダ", "salad"],
    ["届ける", "とどける", "to deliver (v.t.)"],
    ["あいさつする", "挨拶", "greet(ing)"],
    ["景色", "けしき", "scenery, landscape"],
    ["確か", "たしか", "if I remember correctly; certain, definite"],
    ["ステーキ", "ステーキ", "steak"],
    ["食料品", "しょくりょうひん", "foodstuff, groceries"],
    ["森", "もり", "forest"],
    ["以内", "いない", "within, less (no more) than"],
    ["予定", "よてい", "plans, arrangement, schedule"],
    ["オーバー", "オーバー", "overcoat; over, exceeding, exaggeration"],
    ["乾く", "かわく", "to get dry"],
    ["石", "いし", "stone"],
    ["思い出す", "おもいだす", "to recall, to remember"],
    ["踊る", "おどる", "to dance"],
    ["細かい", "こまかい", "small; fine, minute"],
    ["塗る", "ぬる", "to paint, to plaster"],
    ["ご主人", "ごしゅじん", "(your, her) husband"],
    ["珍しい", "めずらしい", "unusual, rare"],
    ["用", "よう", "errand, task, business (to take care of)"],
    ["公務員", "こうむいん", "government worker, public servant"],
    ["お嬢さん", "おじょうさん", "(someone's) daughter (polite)"],
    ["用意", "ようい", "preparation"],
    ["探す", "さがす", "to search, to seek, to look for"],
    ["形", "かたち", "shape"],
    ["運転", "うんてんする", "driving"],
    ["すっかり", "すっかり", "all, completely"],
    ["アナウンサー", "アナウンサー", "announcer"],
    ["お土産", "おみやげ", "souvenir"],
    ["消しゴム", "けしゴム", "eraser"],
    ["旅館", "りょかん", "a Japanese inn"],
    ["海岸", "かいがん", "coast, seashore"],
    ["寂しい", "さびしい", "lonely, lonesome"],
    ["火", "ひ", "fire"],
    ["育てる", "そだてる", "to raise (v.t.); to bring up"],
    ["味噌", "みそ", "miso, bean paste"],
    ["お祝い", "おいわい", "congratulation, celebration"],
    ["乗り物", "のりもの", "vehicle"],
    ["案内", "あんないする", "information, guidance"],
    ["通う", "かよう", "to go back and forth; to commute"],
    ["連れる", "つれる", "to lead, to take (a person)"],
    ["技術", "ぎじゅつ", "technique, technology, skill"],
    ["小鳥", "ことり", "small bird"],
    ["下宿", "げしゅく", "lodging, boarding house"],
    ["ジャム", "ジャム", "jam"],
    ["招待", "しょうたいする", "invitation"],
    ["鏡", "かがみ", "mirror"],
    ["はず", "はず", "it should be so"],
    ["法律", "ほうりつ", "law"],
    ["進む", "すすむ", "to advance, to proceed"],
    ["楽む", "たのしむ", "to enjoy"],
    ["貿易", "ぼうえき", "trade"],
    ["反対", "はんたい", "oppose, opposition, resistance"],
    ["おる", "おる", "-- extra-modest expression for いる --"],
    ["申す", "もうす", "-- extra-modest (humble) expression for 言う (いう) --"],
    ["試験", "しけん", "an exam"],
    ["真面目", "まじめ", "diligent, serious"],
    ["店員", "てんいん", "clerk; shop-employee"],
    ["泊まる", "とまる", "to stay (over night) (v.i.)"],
    ["よろしい", "よろしい", "(hon.) good, OK, all right"],
    ["今夜", "こんや", "this evening, tonight"],
    ["おつり", "おつり", "change; balance of money returned to the purchaser"],
    ["チェックする", "チェックする", "check"],
    ["会話", "かいわ", "conversation"],
    ["空気", "くうき", "air, atmosphere"],
    ["交通", "こうつう", "traffic, transportation"],
    ["ワープロ", "ワープロ", "word processor"],
    ["喜ぶ", "よろこぶ", "to rejoice, to be delighted, to be glad"],
    ["急行", "きゅうこう", "express train or bus"],
    ["皆", "みな", "everyone"],
    ["味", "あじ", "flavor, taste"],
    ["空港", "くうこう", "airport"],
    ["手袋", "てぶくろ", "glove(s)"],
    ["校長", "こうちょう", "principal, headmaster"],
    ["ごちそう", "ごちそう", "feast, treating (someone)"],
    ["踊り", "おどり", "dance"],
    ["興味", "きょうみ", "interest (in something)"],
    ["引っ越す", "ひっこす", "to move to a new place of residence"],
    ["冷房", "れいぼう", "cooling, air conditioning"],
    ["都合", "つごう", "circumstances, convenience"],
    ["遠慮", "えんりょする", "restraint, reserve, hesitate"],
    ["亡くなる", "なくなる", "to pass away"],
    ["科学", "かがく", "science"],
    ["はっきり", "はっきり", "clearly, distinctly"],
    ["差し上げる", "さしあげる", "-- humble expression for あげる --"],
    ["気持ち", "きもち", "feeling, sensation, mood"],
    ["祖父", "そふ", "grandfather"],
    ["港", "みなと", "harbor, port"],
    ["予約", "よやく", "reservation"],
    ["凄い", "すごい", "terrific, great"],
    ["入学", "にゅうがくする", "entry to school or university"],
    ["片付ける", "かたづける", "to (clean) tidy up (v.t.), to put away"],
    ["写す", "うつす", "to copy (v.t.); to photograph"],
    ["パソコン", "パソコン", "(personal) computer"],
    ["部長", "ぶちょう", "department (division) manager"],
    ["火事", "かじ", "fire"],
    ["足す", "たす", "to add (numbers)"],
    ["教会", "きょうかい", "church"],
    ["彼ら", "かれら", "they (usually male)"],
    ["いっぱい", "一杯", "full, to the utmost"],
    ["アメリカ", "アメリカ", "America, U.S.A."],
    ["男性", "だんせい", "man; male"],
    ["理由", "りゆう", "reason"],
    ["生産", "せいさんする", "production; to produce"],
    ["着物", "きもの", "kimono; Japanese traditional dress"],
    ["おもちゃ", "おもちゃ", "a toy"],
    ["暮れる", "くれる", "to get dark, to come to an end"],
    ["釣る", "つる", "to fish"],
    ["～ちゃん", "～ちゃん", "suffix for familiar (female) person"],
    ["打つ", "うつ", "to hit, to strike"],
    ["あんな", "あんな", "such, like that"],
    ["謝る", "あやまる", "to apologize"],
    ["昼間", "ひるま", "daytime, during the day"],
    ["教育", "きょういく", "education"],
    ["女性", "じょせい", "woman"],
    ["米", "こめ", "uncooked rice"],
    ["邪魔", "じゃま", "hindrance, intrusion"],
    ["国際", "こくさい", "international"],
    ["隅", "すみ", "corner"],
    ["伺う", "うかがう", "humble form of 行く (いく), 聞く (きく) and 来る (くる)"],
    ["再来週", "さらいしゅう", "the week after next"],
    ["夢", "ゆめ", "a dream"],
    ["喉", "のど", "throat"],
    ["最近", "さいきん", "recently, nowadays, in recent years, most recent, latest"],
    ["周り", "まわり", "surroundings"],
    ["歴史", "れきし", "history"],
    ["不便", "ふべん", "inconvenience"],
    ["血", "ち", "blood"],
    ["～続ける", "～つづける", "to continue doing ~"],
    ["毛", "け", "hair, fur"],
    ["ひどい", "ひどい", "terrible, awful, unfair, cruel"],
    ["例えば", "たとえば", "for example, e.g."],
    ["中々", "なかなか", "very, considerably, quite"],
    ["随分", "ずいぶん", "extremely"],
    ["～やすい", "～やすい", "easy to do ~"],
    ["押し入れ", "おしいれ", "closet"],
    ["電灯", "でんとう", "electric light"],
    ["叱る", "しかる", "to scold"],
    ["サンダル", "サンダル", "sandal"],
    ["びっくりする", "びっくりする", "to be surprised"],
    ["うまい", "うまい", "delicious; skillful; fortunate"],
    ["変える", "かえる", "to change, to alter, to vary"],
    ["講堂", "こうどう", "auditorium"],
    ["子", "こ", "child"],
    ["沸かす", "わかす", "to boil"],
    ["レジ", "レジ", "register"],
    ["しばらく", "しばらく", "little while"],
    ["特に", "とくに", "particularly"],
    ["空く", "あく", "to open, to become empty (vacant)"],
    ["計画", "けいかくする", "plan, project, schedule"],
    ["通り", "とおり", "~ Street, ~ Avenue"],
    ["下着", "したぎ", "underwear"],
    ["経済", "けいざい", "economics, finance, economy"],
    ["こう", "こう", "like this, this way"],
    ["是非", "ぜひ", "certainly, by all means; without fail"],
    ["裏", "うら", "reverse side, back"],
    ["為", "ため", "good, advantage, in order to"],
    ["おいでになる", "おいでになる", "(hon.) to be"],
    ["変わる", "かわる", "to change (v.i.), to be transformed, to vary"],
    ["以外", "いがい", "other than, with the exception of, excepting"],
    ["済む", "すむ", "to finish, to end"],
    ["ハンバーグ", "ハンバーグ", "hamburger steak"],
    ["市", "し", "city"],
    ["引き出し", "ひきだし", "drawer"],
    ["遊び", "あそび", "play"],
    ["支度", "したくする", "preparation"],
    ["～始める", "～はじめる", "to start doing ~"],
    ["見える", "みえる", "to be visible; -- polite verb meaning 来る (くる) --"],
    ["十分", "じゅうぶん", "enough"],
    ["音", "おと", "sound, note"],
    ["きっと", "きっと", "surely, definitely, undoubtedly, certainly"],
    ["まず", "まず", "first (of all), to start with"],
    ["遠く", "とおく", "far away, distant"],
    ["大体", "だいたい", "approximately; in most cases; in general; to begin with (same as もともと)"],
    ["折れる", "おれる", "to break, to be folded, to give in; to turn (a corner)"],
    ["正しい", "ただしい", "correct"],
    ["輸入", "ゆにゅう", "import"],
    ["返事", "へんじ", "reply, answer"],
    ["都", "と", "metropolitan"],
    ["産業", "さんぎょう", "industry"],
    ["伝える", "つたえる", "to convey (a message); to tell, to report"],
    ["お金持ち", "かねもち; おかねもち", "rich person"],
    ["説明", "せつめい", "explanation"],
    ["島", "しま", "island"],
    ["道具", "どうぐ", "tool"],
    ["滑る", "すべる", "to slide, to slip"],
    ["それほど", "それほど", "to that degree; extent"],
    ["以上", "いじょう", "more than; this is all"],
    ["～まま", "～まま", "as it is"],
    ["特急", "とっきゅう", "limited express (train faster than an express)"],
    ["プレゼント", "プレゼント", "present, gift"],
    ["～(に) よると", "～(に) よると", "according to ~"],
    ["妻", "つま", "wife (humble)"],
    ["帰り", "かえり", "return, coming back"],
    ["具合", "ぐあい", "condition, state, health"],
    ["堅; 硬; 固い", "かたい", "solid, hard, firm"],
    ["駐車場", "ちゅうしゃじょう", "parking lot"],
    ["スーツ", "スーツ", "suit"],
    ["危険", "きけん", "danger, risk, hazard"],
    ["髪", "かみ", "hair"],
    ["天気予報", "てんきよほう", "weather forecast"],
    ["彼女", "かのじょ", "girlfriend; she"],
    ["間", "あいだ", "space, interval"],
    ["卒業", "そつぎょう", "graduation"],
    ["それで", "それで", "and (conj.), thereupon, because of that"],
    ["枝", "えだ", "branch, twig"],
    ["専門", "せんもん", "major; speciality"],
    ["そろそろ", "そろそろ", "gradually, soon"],
    ["送る", "おくる", "to send, to dispatch"],
    ["あげる", "あげる", "to give"],
    ["騒ぐ", "さわぐ", "to make noise, to clamor"],
    ["尋ねる", "たずねる", "to inquire (same as 質問する)"],
    ["放送", "ほうそうする", "broadcasting"],
    ["政治", "せいじ", "politics"],
    ["市民", "しみん", "citizen"],
    ["ファックス", "ファックス", "fax"],
    ["負ける", "まける", "to lose (a game) (v.i.), to be defeated"],
    ["指輪", "ゆびわ", "(finger) ring"],
    ["田舎", "いなか", "rural, countryside"],
    ["見つける", "みつける", "to discover, to find (v.t.)"],
    ["高校生", "こうこうせい", "high school student"],
    ["講義", "こうぎ", "a lecture"],
    ["そんなに", "そんなに", "so much, like that"],
    ["昼休み", "ひるやすみ", "lunch break, noon recess"],
    ["忘れ物", "わすれもの", "lost article, something forgotten"],
    ["下りる", "おりる", "to get down, to go; come down"],
    ["腕", "うで", "arm (in reference to body)"],
    ["訳", "わけ", "reason; explanation"],
    ["承知", "しょうちする", "consent, acceptance"],
    ["日記", "にっき", "diary, journal"],
    ["高校; 高等学校", "こうこう; こうとうがっこう", "high school; senior high school"],
    ["似る", "にる", "to resemble, to be similar"],
    ["～おわる", "～おわる", "to finish doing ~"],
    ["暖房", "だんぼう", "heating"],
    ["留守", "るす", "absence; not at home"],
    ["割合", "わりあい", "rate, ratio, percentage"],
    ["寺", "てら", "Buddhist temple"],
    ["慣れる", "なれる", "to grow accustomed to"],
    ["普通", "ふつう", "common; usual"],
    ["手伝う", "てつだう", "to help"],
    ["なるほど", "なるほど", "I see; I now understand"],
    ["くださる", "くださる", "(hon.) to give, to confer"],
    ["息子", "むすこ", "(humble) son"],
    ["お子さん", "おこさん", "(someone else's) child (polite)"],
    ["会場", "かいじょう", "venue, meeting place"],
    ["笑う", "わらう", "to laugh, to smile"],
    ["運ぶ", "はこぶ", "to transport, to carry"],
    ["文学", "ぶんがく", "literature"],
    ["光", "ひかり", "light"],
    ["お見舞い", "おみまい", "calling on someone who is ill, visit"],
    ["席", "せき", "a seat"],
    ["～様", "～さま", "Mr., Mrs., Ms."],
    ["ご存じ", "ごぞんじ", "knowing, acquaintance"],
    ["下る", "さがる", "to descend, to drop, to fall"],
    ["字", "じ", "letter; character"],
    ["アジア", "アジア", "Asia"],
    ["褒める", "ほめる", "to praise; to say nice things"],
    ["空く", "すく", "to be empty (in reference to people), to be less crowded"],
    ["あ", "あ", "Ah"],
    ["最も", "もっとも", "most"],
    ["合う", "あう", "to fit, to match"],
    ["～代", "～だい", "~ age; period"],
    ["最後", "さいご", "last, end"],
    ["値段", "ねだん", "price"],
    ["退院", "たいいんする", "leaving hospital"],
    ["展覧会", "てんらんかい", "exhibition"],
    ["久しぶり", "ひさしぶり", "it has been a long time; for the first time in a long time"],
    ["カーテン", "カーテン", "curtain"],
    ["汽車", "きしゃ", "train (steam)"],
    ["遅れる", "おくれる", "to (be) become late"],
    ["見つかる", "みつかる", "to be found (v.i.), to be discovered"],
    ["召し上がる", "めしあがる", "-- honorific form of 食べる (たべる) and 飲む (のむ) --"],
    ["太る", "ふとる", "to gain weight"],
    ["注射", "ちゅうしゃ", "injection"],
    ["様", "よう", "way, manner, kind"],
    ["～おき", "～おき", "after every ~"],
    ["最初", "さいしょ", "beginning, first"],
    ["御～", "ご～", "honorable ~"],
    ["安心", "あんしん", "peace of mind, relief"],
    ["直る", "なおる", "to be fixed"],
    ["集める", "あつめる", "to collect, to gather (v.t.), to assemble"],
    ["直す", "なおす", "to correct (v.t.); to fix"],
    ["続く", "つづく", "to be continued"],
    ["先輩", "せんぱい", "senior members of a group"],
    ["約束", "やくそく", "arrangement, appointment, promise"],
    ["世話", "せわする", "looking after; to look after"],
    ["近所", "きんじょ", "neighborhood"],
    ["将来", "しょうらい", "(in the) future; prospects"],
    ["億", "おく", "hundred million"],
    ["数学", "すうがく", "mathematics"],
    ["文化", "ぶんか", "culture"],
    ["払う", "はらう", "to pay"],
    ["習慣", "しゅうかん", "custom (in reference to culture)"],
    ["焼ける", "やける", "to burn, to be roasted"],
    ["君", "きみ", "you (informal for men)"],
    ["冷える", "ひえる", "to grow cold, to cool down"],
    ["点く", "つく", "to be started, to be switched on"],
    ["この間", "このあいだ", "the other day, recently"],
    ["かっこう", "格好", "appearance, manner, shape, form, posture"],
    ["続ける", "つづける", "to continue, to keep up"],
    ["落す", "おとす", "to drop, to lose"],
    ["明日", "あす", "tomorrow"],
    ["出発", "しゅっぱつする", "departure"],
    ["拝見", "はいけんする", "(humble) (polite) seeing, look at"],
    ["割れる", "われる", "to break"],
    ["背中", "せなか", "back (of body)"],
    ["新聞社", "しんぶんしゃ", "newspaper company"],
    ["いじめる", "いじめる", "to bully, to torment"],
    ["回る、回す", "まわる、まわす", "to go around, to revolve"],
    ["～君", "～くん", "Mr. (junior) ~, master ~"],
    ["おっしゃる", "おっしゃる", "-- honorific expression for いう --"],
    ["眠い", "ねむい", "sleepy, drowsy"],
    ["濡れる", "ぬれる", "to get wet"],
    ["倒れる", "たおれる", "to collapse, to break down"],
    ["スーパー (マーケット)", "スーパー (マーケット)", "supermarket"],
    ["アクセサリー", "アクセサリー", "accessory"],
    ["考える", "かんがえる", "to think (about); to consider"],
    ["向かう", "むかう", "to face, to go towards"],
    ["自由", "じゆう", "freedom"],
    ["仕方", "しかた", "way (of doing something)"],
    ["首", "くび", "neck"],
    ["程", "ほど", "degree, extent"],
    ["代わり", "かわり", "substitute, replacement"],
    ["失敗", "しっぱい", "failure, mistake"],
    ["工業", "こうぎょう", "(manufacturing) industry"],
    ["移る", "うつる", "to move (from a house); to transfer (from a department); to shift"],
    ["スーツケース", "スーツケース", "suitcase"],
    ["ひげ", "ひげ", "beard"],
    ["研究室", "けんきゅうしつ", "the professor's office; laboratory"],
    ["工場", "こうじょう", "factory"],
    ["紹介", "しょうかい", "an introduction"],
    ["けれど; けれども", "けれど; けれども", "but, although"],
    ["舟", "ふね", "ship, boat"],
    ["動く", "うごく", "to move"],
    ["～(に) ついて", "～(に) ついて", "about, concerning"],
    ["コンサート", "コンサート", "concert"],
    ["虫", "むし", "insect"],
    ["優しい", "やさしい", "kind (person), gentle (person), easy (problem)"],
    ["コンピュータ; コンピューター", "コンピュータ; コンピューター", "computer"],
    ["植える", "うえる", "to plant"],
    ["両方", "りょうほう", "both sides, both parties"],
    ["汚れる", "よごれる", "to become dirty"],
    ["水泳", "すいえい", "swimming"],
    ["経験", "けいけんする", "experience"],
    ["勝つ", "かつ", "to win"],
    ["砂", "すな", "sand"],
    ["警察", "けいさつ", "police; police station"],
    ["取り替える", "とりかえる", "to exchange, to replace"],
    ["急ぐ", "いそぐ", "to hurry, to be in a hurry, to rush"],
    ["簡単", "かんたん", "simple"],
    ["参る", "まいる", "humble expression for 行く and 来る"],
    ["全然", "ぜんぜん", "not at all; wholly, entirely"],
    ["特別", "とくべつ", "special"],
    ["復習", "ふくしゅう", "review (of lessons), revision"],
    ["間に合う", "まにあう", "to be in time for"],
    ["役に立つ", "やくにたつ", "to be helpful, to be useful"],
    ["もうすぐ", "もうすぐ", "very soon; in a few moments; days"],
    ["真中", "まんなか", "middle, center"],
    ["戻る", "もどる", "to return (v.i.); to come back"],
    ["研究", "けんきゅう", "study, research, investigation"],
    ["ケーキ", "ケーキ", "cake"],
    ["草", "くさ", "grass"],
    ["込む", "こむ", "to be crowded"],
    ["この頃", "このごろ", "these days, nowadays"],
    ["訪ねる", "たずねる", "to visit"],
    ["下げる", "さげる", "to lower (v.t.); to hang"],
    ["花見", "はなみ", "cherry-blossom viewing"],
    ["途中", "とちゅう", "on the way, midway"],
    ["入院", "にゅういんする", "hospitalization"],
    ["乗り換える", "のりかえる", "to transfer (trains), to change (bus, train, etc.)"],
    ["別れる", "わかれる", "to part from, to separate"],
    ["～町", "～ちょう", "the town of ~"],
    ["安全", "あんぜん", "safety, security"],
    ["看護婦", "かんごふ", "(female) nurse"],
    ["見物", "けんぶつ", "sightseeing"],
    ["相談", "そうだんする", "consultation"],
    ["ガソリンスタンド", "ガソリンスタンド", "gas station, service station"],
    ["テニス", "テニス", "tennis"],
    ["眠る", "ねむる", "to sleep"],
    ["上がる", "あがる", "to rise, to go up"],
    ["翻訳", "ほんやく", "translation"],
    ["食事", "しょくじする", "meal"],
    ["おかげ", "お陰", "thanks or owing to"],
    ["娘", "むすめ", "daughter (humble)"],
    ["湯", "ゆ", "hot water"],
    ["競争", "きょうそう", "competition, contest"],
    ["会議", "かいぎ", "business meeting; conference"],
    ["湖", "みずうみ", "lake"],
    ["集まる", "あつまる", "to gather (v.i.), to collect"],
    ["～にくい", "～にくい", "difficult to do ~"],
    ["生活", "せいかつする", "living, life; to live"],
    ["糸", "いと", "thread"],
    ["関係", "かんけい", "relation(ship), connection"],
    ["ピアノ", "ピアノ", "piano"],
    ["～目", "～め", "number ~ sequence, ~nd; ~th"],
    ["番組", "ばんぐみ", "broadcast program"],
    ["急", "きゅう", "urgent, sudden; steep"],
    ["棚", "たな", "shelves, rack"],
    ["木綿", "もめん", "cotton"],
    ["輸出", "ゆしゅつする", "export"],
    ["タイプ", "タイプ", "type, style"],
    ["すり", "すり", "pickpocket"],
    ["うれしい", "嬉しい", "to be happy; to be glad"],
    ["アルコール", "アルコール", "alcohol"],
    ["ソフト", "ソフト", "soft; soft hat; software"],
    ["神社", "じんじゃ", "Shinto shrine"],
    ["大分", "だいぶ", "fairly well, to a large extent, considerably, pretty much"],
    ["楽しみ", "たのしみ", "pleasure, joy"],
    ["趣味", "しゅみ", "hobby; pastime"],
    ["電報", "でんぽう", "telegram"],
    ["家内", "かない", "(one's own) wife"],
    ["指", "ゆび", "finger"],
    ["これから", "これから", "from now on, after this"],
    ["たまに", "たまに", "occasionally"],
    ["社会", "しゃかい", "society"]
  ],
  "n3": [
    ["作法", "さほう", "manners, etiquette, propriety"],
    ["様々", "さまざま", "varied, various"],
    ["冷ます", "さます", "to cool, to let cool"],
    ["覚ます", "さます", "to awaken"],
    ["冷める", "さめる", "to become cool, to wear off"],
    ["覚める", "さめる", "to wake, to wake up"],
    ["左右", "さゆう", "left and right; influence"],
    ["皿", "さら", "plate, dish"],
    ["更に", "さらに", "furthermore, moreover"],
    ["去る", "さる", "to leave, to go away"],
    ["猿", "さる", "monkey"],
    ["騒ぎ", "さわぎ", "uproar, disturbance"],
    ["参加", "さんか", "participation"],
    ["参考", "さんこう", "reference, consultation"],
    ["賛成", "さんせい", "approval, agreement"],
    ["酸性", "さんせい", "acidity"],
    ["酸素", "さんそ", "oxygen"],
    ["氏", "し", "family name, lineage"],
    ["詩", "し", "poem; poetry"],
    ["幸せ", "しあわせ", "happiness, blessing"],
    ["ジーンズ", "ジーンズ", "jeans"],
    ["ジェット機", "ジェットき", "jet plane"],
    ["四角", "しかく", "square"],
    ["直に", "じかに", "immediately, readily, directly"],
    ["しかも", "しかも", "moreover, furthermore, besides, plus"],
    ["四季", "しき", "four seasons"],
    ["直", "じき", "immediately, soon, shortly"],
    ["時期", "じき", "time, season, period"],
    ["支給", "しきゅう", "payment, allowance"],
    ["至急", "しきゅう", "urgent, pressing"],
    ["しきりに", "しきりに", "frequently, repeatedly, eagerly"],
    ["刺激", "しげき", "stimulus, impetus, incentive"],
    ["資源", "しげん", "resources"],
    ["事件", "じけん", "event, affair, incident"],
    ["時刻", "じこく", "time, hour"],
    ["自殺", "じさつ", "suicide"],
    ["事実", "じじつ", "fact, truth, reality"],
    ["支出", "ししゅつ", "expenditure, expenses"],
    ["事情", "じじょう", "circumstances, situation, reasons"],
    ["詩人", "しじん", "poet"],
    ["自身", "じしん", "oneself"],
    ["沈む", "しずむ", "to sink; to feel depressed"],
    ["自然", "しぜん", "nature, spontaneous"],
    ["思想", "しそう", "thought, idea"],
    ["舌", "した", "tongue"],
    ["次第", "しだい", "order; circumstances; immediate(ly)"],
    ["従う", "したがう", "to abide (by the rules), to obey"],
    ["したがって", "したがって", "therefore, consequently"],
    ["親しい", "したしい", "intimate, close (e.g., friend)"],
    ["質", "しつ", "quality, nature (of person)"],
    ["失業", "しつぎょう", "unemployment"],
    ["湿気", "しっけ", "moisture, humidity, dampness"],
    ["実験", "じっけん", "lab work; experiment"],
    ["実現", "じつげん", "implementation, materialization, realization"],
    ["実行", "じっこう", "practice, execution (e.g., program), realization"],
    ["実際", "じっさい", "in fact; in actuality"],
    ["実施", "じっし", "enforcement, carry out, operation"],
    ["湿度", "しつど", "humidity"],
    ["じっと", "じっと", "patiently, quietly"],
    ["実に", "じつに", "indeed, truly, surely"],
    ["実は", "じつは", "actually; in fact"],
    ["失望", "しつぼう", "disappointment, despair"],
    ["実力", "じつりょく", "ability; force"],
    ["支店", "してん", "branch store (office)"],
    ["指導", "しどう", "leadership, guidance, coaching"],
    ["自動", "じどう", "automatic, self-motion"],
    ["児童", "じどう", "children, juvenile"],
    ["品", "しな", "thing, article, goods"],
    ["支配", "しはい", "rule, control, direction"],
    ["芝居", "しばい", "play, drama"],
    ["しばしば", "しばしば", "often, again and again, frequently"],
    ["芝生", "しばふ", "lawn"],
    ["支払", "しはらい", "payment"],
    ["支払う", "しはらう", "to pay"],
    ["死亡", "しぼう", "death"],
    ["資本", "しほん", "funds, capital"],
    ["姉妹", "しまい", "sisters"],
    ["しまった (かん)", "しまった (かん)", "Damn it!"],
    ["自慢", "じまん", "pride, boast"],
    ["地味", "じみ", "quiet, plain, conservative"],
    ["示す", "しめす", "to show, to indicate"],
    ["占める", "しめる", "to take up; to account for"],
    ["湿る", "しめる", "to be wet, to be damp"],
    ["下", "しも", "under, below, beneath"],
    ["霜", "しも", "frost"],
    ["借金", "しゃっきん", "debt, loan, liabilities"],
    ["しゃべる", "しゃべる", "to talk, to chat, to chatter (same as 話す (はなす))"],
    ["週", "しゅう", "week"],
    ["州", "しゅう", "state, province"],
    ["銃", "じゅう", "gun"],
    ["周囲", "しゅうい", "surroundings, circumference, environs"],
    ["収穫", "しゅうかく", "harvest, crop, ingathering"],
    ["宗教", "しゅうきょう", "religion"],
    ["重視", "じゅうし", "importance, stress"],
    ["就職", "しゅうしょく", "finding employment"],
    ["ジュース", "ジュース", "juice, soft drink; deuce"],
    ["修正", "しゅうせい", "amendment, correction"],
    ["重体", "じゅうたい", "seriously ill, critical state"],
    ["渋滞", "じゅうたい", "congestion (e.g., traffic), delay"],
    ["重大", "じゅうだい", "serious, important"],
    ["住宅", "じゅうたく", "resident, housing"],
    ["集団", "しゅうだん", "group, mass"],
    ["集中", "しゅうちゅう", "concentration, focusing the mind"],
    ["収入", "しゅうにゅう", "income, revenue"],
    ["住民", "じゅうみん", "inhabitants, residents"],
    ["重要", "じゅうよう", "important, essential"],
    ["修理", "しゅうり", "repairing, mending"],
    ["主義", "しゅぎ", "doctrine, cause, principle"],
    ["宿泊", "しゅくはく", "lodging"],
    ["手術", "しゅじゅつ", "surgical operation"],
    ["首相", "しゅしょう", "Prime Minister"],
    ["手段", "しゅだん", "means, way, measure"],
    ["主張", "しゅちょう", "claim, insistence, assertion"],
    ["出場", "しゅつじょう", "participation"],
    ["出身", "しゅっしん", "come from"],
    ["出版", "しゅっぱん", "publication"],
    ["首都", "しゅと", "capital city"],
    ["主婦", "しゅふ", "housewife"],
    ["主要", "しゅよう", "chief, main"],
    ["需要", "じゅよう", "demand"],
    ["種類", "しゅるい", "variety, kind"],
    ["順", "じゅん", "order, turn"],
    ["瞬間", "しゅんかん", "moment, second"],
    ["順調", "じゅんちょう", "doing well"],
    ["順番", "じゅんばん", "turn (in line), order of things"],
    ["使用", "しよう", "use (same as 使うこと (つかうこと)), application, employment, utilization"],
    ["小", "しょう", "small"],
    ["章", "しょう", "chapter, section; medal"],
    ["賞", "しょう", "prize, award"],
    ["上", "じょう", "first volume; superior quality; governmental"],
    ["障害", "しょうがい", "obstacle, impediment"],
    ["奨学金", "しょうがくきん", "scholarship"],
    ["乗客", "じょうきゃく", "passenger"],
    ["上京", "じょうきょう", "proceeding to the capital (Tokyo)"],
    ["状況", "じょうきょう", "state of affairs, situation"],
    ["条件", "じょうけん", "conditions, terms"],
    ["正午", "しょうご", "noon, mid-day"],
    ["正直", "しょうじき", "honesty, integrity, frankness"],
    ["常識", "じょうしき", "common sense"],
    ["少女", "しょうじょ", "young girl"],
    ["少々", "しょうしょう", "a little; short (time) (formal for 少し (すこし))"],
    ["症状", "しょうじょう", "symptoms, condition"],
    ["生じる", "しょうじる", "to occur, to arise, to be generated"],
    ["状態", "じょうたい", "condition, situation"],
    ["上達", "じょうたつ", "improvement, advance"],
    ["冗談", "じょうだん", "a joke"],
    ["上等", "じょうとう", "first class, very good"],
    ["衝突", "しょうとつ", "collision, conflict"],
    ["商人", "しょうにん", "trader, shopkeeper, merchant"],
    ["承認", "しょうにん", "recognition, approval"],
    ["少年", "しょうねん", "boys, juveniles"],
    ["商売", "しょうばい", "trade, business, commerce"],
    ["消費", "しょうひ", "consumption, expenditure"],
    ["商品", "しょうひん", "commodity, merchandise"],
    ["賞品", "しょうひん", "prize, trophy"],
    ["消防", "しょうぼう", "fire fighting, fire department"],
    ["情報", "じょうほう", "information, (military) intelligence"],
    ["証明", "しょうめい", "proof, verification"],
    ["省略", "しょうりゃく", "omission, abbreviation, abridgment"],
    ["女王", "じょおう", "queen"],
    ["職", "しょく", "employment"],
    ["職業", "しょくぎょう", "occupation, business"],
    ["食卓", "しょくたく", "dining table"],
    ["食品", "しょくひん", "foodstuff"],
    ["植物", "しょくぶつ", "plant, vegetation"],
    ["食物", "しょくもつ", "food, foodstuff"],
    ["食欲", "しょくよく", "appetite (for food)"],
    ["食料", "しょくりょう", "food"],
    ["食糧", "しょくりょう", "provisions, rations"],
    ["書斎", "しょさい", "study, den"],
    ["女子", "じょし", "woman, girl"],
    ["助手", "じょしゅ", "helper, assistant"],
    ["徐々に", "じょじょに", "slowly, little by little"],
    ["署名", "しょめい", "signature"],
    ["書物", "しょもつ", "books"],
    ["女優", "じょゆう", "actress"],
    ["処理", "しょり", "processing, treatment, disposition"],
    ["書類", "しょるい", "documents, official papers"],
    ["知らせ", "しらせ", "notice"],
    ["尻", "しり", "buttocks, bottom"],
    ["知合い", "しりあい", "acquaintance"],
    ["印", "しるし", "mark; symbol; evidence"],
    ["城", "しろ", "castle"],
    ["進学", "しんがく", "going on to university"],
    ["神経", "しんけい", "nerve, sensitivity"],
    ["真剣", "しんけん", "seriousness, earnestness"],
    ["信仰", "しんこう", "(religious) faith, belief"],
    ["信号", "しんごう", "traffic lights, signal, semaphore"],
    ["人工", "じんこう", "artificial, man made, human work,"],
    ["深刻", "しんこく", "serious"],
    ["診察", "しんさつ", "medical examination (of a patient)"],
    ["人種", "じんしゅ", "race (of people)"],
    ["信じる", "しんじる", "to believe"],
    ["人生", "じんせい", "(human) life (e.g., conception to death)"],
    ["親戚", "しんせき", "relative(s)"],
    ["新鮮", "しんせん", "fresh"],
    ["心臓", "しんぞう", "heart"],
    ["身体", "しんたい", "the body"],
    ["身長", "しんちょう", "height (of body), stature"],
    ["慎重", "しんちょう", "careful, prudent, cautious"],
    ["審判", "しんぱん", "umpire, referee, judgment"],
    ["人物", "じんぶつ", "character, personality, talented man"],
    ["進歩", "しんぽ", "improvement, progress, development"],
    ["親友", "しんゆう", "close friend, buddy"],
    ["信用", "しんよう", "confidence, dependence"],
    ["信頼", "しんらい", "reliance, trust, confidence"],
    ["心理", "しんり", "mentality"],
    ["人類", "じんるい", "mankind, humanity"],
    ["巣", "す", "nest, breeding place"],
    ["酢", "す", "vinegar"],
    ["図", "ず", "figure, drawing, illustration"],
    ["水準", "すいじゅん", "level, standard"],
    ["推薦", "すいせん", "recommendation"],
    ["スイッチ", "スイッチ", "switch"],
    ["睡眠", "すいみん", "sleep"],
    ["数", "すう", "number, figure"],
    ["数字", "すうじ", "numeral, figure"],
    ["スープ", "スープ", "(Western) soup"],
    ["末", "すえ", "the end of, powder"],
    ["姿", "すがた", "figure, shape, appearance"],
    ["スキー", "スキー", "ski"],
    ["救う", "すくう", "to rescue from, to help out of"],
    ["すくなくとも", "すくなくとも", "at least"],
    ["優れる", "すぐれる", "to surpass, to outstrip, to excel"],
    ["スケート", "スケート", "skate(s), skating"],
    ["スケジュール", "スケジュール", "schedule"],
    ["少しも", "すこしも", "anything of, not one bit"],
    ["過ごす", "すごす", "to pass, to spend (time)"],
    ["筋", "すじ", "muscle, string, line"],
    ["進める", "すすめる", "to advance, to promote, to hasten"],
    ["勧める", "すすめる", "to urge (someone) to do (something), to recommend"],
    ["スター", "スター", "star"],
    ["スタイル", "スタイル", "style"],
    ["スタンド", "スタンド", "stand"],
    ["頭痛", "ずつう", "headache"],
    ["ずっと", "ずっと", "for a long time; all the time; consecutively"],
    ["すっぱい", "すっぱい", "sour, acid"],
    ["すてき", "すてき", "lovely, great"],
    ["既に", "すでに", "already (same as もう)"],
    ["すなわち", "すなわち", "that is, namely, e.g."],
    ["スピーチ", "スピーチ", "speech"],
    ["全て", "すべて", "all, the whole, entirely"],
    ["済ませる", "すませる", "to be finished"],
    ["角", "すみ", "corner"],
    ["墨", "すみ", "ink"],
    ["すみません (かん)", "すみません (かん)", "sorry, excuse me"],
    ["澄む", "すむ", "to clear (e.g., weather), to become transparent"],
    ["清む", "すむ", "to clear (e.g., weather), to become transparent"],
    ["刷る", "する", "to print"],
    ["為る", "する", "to do, to attempt"],
    ["鋭い", "するどい", "pointed, sharp"],
    ["すれ違う", "すれちがう", "to pass by one another"],
    ["ずれる", "ずれる", "move, off the point"],
    ["正", "せい", "(logical) true, regular"],
    ["生", "せい", "birth"],
    ["性", "せい", "sex, gender"],
    ["姓", "せい", "surname, family name"],
    ["所為", "せい", "cause, reason, fault"],
    ["税", "ぜい", "tax"],
    ["性格", "せいかく", "character, personality"],
    ["正確", "せいかく", "accurate, punctuality, exact"],
    ["世紀", "せいき", "century"],
    ["請求", "せいきゅう", "claim, demand, request"],
    ["税金", "ぜいきん", "tax, duty"],
    ["清潔", "せいけつ", "clean"],
    ["制限", "せいげん", "restriction, restraint, limitation"],
    ["成功", "せいこう", "success, hit"],
    ["正式", "せいしき", "official, formal"],
    ["性質", "せいしつ", "nature, property, disposition"],
    ["精神", "せいしん", "mind, soul, spirit"],
    ["成人", "せいじん", "adult"],
    ["精々", "せいぜい", "at the most, at best"],
    ["成績", "せいせき", "grade (i.e., on a test), academic record"],
    ["製造", "せいぞう", "manufacture, production"],
    ["贅沢", "ぜいたく", "luxury, extravagance"],
    ["成長", "せいちょう", "growth"],
    ["生長", "せいちょう", "growth"],
    ["制度", "せいど", "system, institution"],
    ["青年", "せいねん", "youth, young man"],
    ["生年月日", "せいねんがっぴ", "birth date"],
    ["製品", "せいひん", "manufactured goods, finished goods"],
    ["政府", "せいふ", "government, administration"],
    ["生物", "せいぶつ", "living thing, organism"],
    ["生命", "せいめい", "life"],
    ["整理", "せいり", "sorting, arrangement"],
    ["咳", "せき", "cough"],
    ["石炭", "せきたん", "coal"],
    ["責任", "せきにん", "duty, responsibility"],
    ["石油", "せきゆ", "oil, petroleum, kerosene"],
    ["世間", "せけん", "world, society"],
    ["説", "せつ", "theory"],
    ["積極的", "せっきょくてき", "positive, active, proactive"],
    ["設計", "せっけい", "plan, design"],
    ["絶対", "ぜったい", "definitely, without fail, absoluteness"],
    ["セット", "セット", "set"],
    ["愛", "あい", "love"],
    ["相変わらず", "あいかわらず", "as ever, as usual"],
    ["愛情", "あいじょう", "love, affection"],
    ["合図", "あいず", "sign, signal"],
    ["アイスクリーム", "アイスクリーム", "ice cream"],
    ["愛する", "あいする", "to love"],
    ["相手", "あいて", "partner; addressee; the person you are talking to"],
    ["あいにく", "あいにく", "unfortunately"],
    ["遭う", "あう", "to meet, to encounter (undesirable nuance)"],
    ["アウト", "アウト", "out"],
    ["明かり", "あかり", "lamplight, light (in general)"],
    ["空き", "あき", "vacancy, opening, space"],
    ["明らか", "あきらか", "obvious, clear"],
    ["諦める", "あきらめる", "to give up, to abandon"],
    ["飽きる", "あきる", "to get tired of, to lose interest in"],
    ["握手", "あくしゅ", "handshake"],
    ["悪魔", "あくま", "devil, demon, evil spirit"],
    ["明ける", "あける", "to dawn, to become daylight"],
    ["揚げる", "あげる", "to lift, to fry"],
    ["挙げる", "あげる", "to raise; to list, to cite"],
    ["預かる", "あずかる", "to keep (something) for (someone)"],
    ["預ける", "あずける", "to give into custody, to deposit"],
    ["汗", "あせ", "sweat, perspiration"],
    ["与える", "あたえる", "to give"],
    ["温かい", "あたたかい", "warm"],
    ["暖まる", "あたたまる", "to warm up"],
    ["温まる", "あたたまる", "to warm oneself, to get warm"],
    ["暖める", "あたためる", "to warm (up to someone/something), to heat (up to someone/something)"],
    ["温める", "あたためる", "to warm, to heat"],
    ["辺り", "あたり", "vicinity, nearby"],
    ["当たり前", "あたりまえ", "usual, common, obvious"],
    ["当たる", "あたる", "to be hit, to be successful"],
    ["あちこち", "あちこち", "here and there"],
    ["扱う", "あつかう", "to treat, to handle, to deal with"],
    ["集まり", "あつまり", "gathering, meeting, collection"],
    ["当てる", "あてる", "to hit; to apply to"],
    ["跡", "あと", "trace; remains; scar"],
    ["穴", "あな", "hole"],
    ["油", "あぶら", "oil"],
    ["脂", "あぶら", "fat"],
    ["誤り", "あやまり", "error"],
    ["粗", "あら", "defect, flaw, fault"],
    ["嵐", "あらし", "storm"],
    ["争う", "あらそう", "to dispute, to argue, to fight"],
    ["新た", "あらた", "new, fresh"],
    ["あらゆる", "あらゆる", "all, every"],
    ["表す", "あらわす", "to express, to show"],
    ["現す", "あらわす", "to show, to appear, to reveal"],
    ["著す", "あらわす", "to write, to publish"],
    ["現れ", "あらわれ", "expression, indication, sign"],
    ["現れる", "あらわれる", "to appear (v.i.), to become visible; to express"],
    ["ありがとう", "ありがとう", "Thank you"],
    ["在る; 有る", "ある", "to live, to be, to exist"],
    ["或", "ある", "a certain..., some..."],
    ["あるいは", "あるいは", "or, perhaps"],
    ["アルバム", "アルバム", "album"],
    ["泡", "あわ", "bubble, foam"],
    ["合わせる", "あわせる", "to combine"],
    ["慌てる", "あわてる", "to become confused、to panic"],
    ["哀れ", "あわれ", "helpless, pity, pathetic"],
    ["案", "あん", "plan, scheme, proposal"],
    ["案外", "あんがい", "unexpectedly"],
    ["暗記", "あんき", "memorization, learning by heart"],
    ["安定", "あんてい", "stability, equilibrium"],
    ["あんなに", "あんなに", "to that extent, to that degree"],
    ["あんまり", "あんまり", "not very, not much"],
    ["胃", "い", "stomach"],
    ["委員", "いいん", "committee member"],
    ["意外", "いがい", "unexpected, surprising"],
    ["行き", "いき", "going"],
    ["行き", "ゆき", "going"],
    ["息", "いき", "breath"],
    ["勢い", "いきおい", "force, vigor, momentum"],
    ["生き物", "いきもの", "living thing, creature"],
    ["いけない", "いけない", "must not do, bad, wrong"],
    ["医師", "いし", "doctor, physician"],
    ["意思", "いし", "intention, purpose"],
    ["意志", "いし", "will, volition"],
    ["維持", "いじ", "maintenance, preservation"],
    ["意識", "いしき", "consciousness, senses"],
    ["異常", "いじょう", "strangeness, abnormality, disorder"],
    ["意地悪", "いじわる", "malicious, mean, unkind"],
    ["泉", "いずみ", "spring, fountain"],
    ["いずれ", "いずれ", "where, which, who"],
    ["以前", "いぜん", "in the past; before"],
    ["板", "いた", "board, plank"],
    ["偉大", "いだい", "greatness"],
    ["抱く", "いだく", "to hold (v.t.) (written expression), to embrace, to harbor"],
    ["いたずら", "いたずら", "trick, practical joke"],
    ["いただきます", "いただきます", "expression of gratitude before meals"],
    ["痛み", "いたみ", "pain, ache, sore"],
    ["至る", "いたる", "to come, to arrive"],
    ["市", "いち", "market, fair"],
    ["位置", "いち", "place, position"],
    ["一時", "いちじ", "for a time, temporarily"],
    ["一度に", "いちどに", "all at once"],
    ["市場", "いちば", "market, bazaar"],
    ["いつか", "いつか", "sometime, one day"],
    ["一家", "いっか", "family, clan"],
    ["一種", "いっしゅ", "a species, a kind, a variety"],
    ["一瞬", "いっしゅん", "a moment, an instant"],
    ["一生", "いっしょう", "throughout (one's) life"],
    ["一層", "いっそう", "much more, still more"],
    ["一体", "いったい", "one object; body; what on earth?; generally"],
    ["一致", "いっち", "agreement; conformity"],
    ["いつでも", "いつでも", "(at) any time, always"],
    ["いつのまにか", "いつのまにか", "before one knows"],
    ["一般", "いっぱん", "general, average"],
    ["一方", "いっぽう", "on the other hand; meanwhile"],
    ["いつまでも", "いつまでも", "forever, for good, eternally"],
    ["移動", "いどう", "migration, movement"],
    ["従兄弟", "いとこ", "cousin (male)"],
    ["従姉妹", "いとこ", "cousin (female)"],
    ["稲", "いね", "rice-plant"],
    ["居眠り", "いねむり", "dozing, nodding off"],
    ["命", "いのち", "life"],
    ["違反", "いはん", "violation (of law), infringement"],
    ["衣服", "いふく", "clothes"],
    ["居間", "いま", "living room"],
    ["今に", "いまに", "before long, soon"],
    ["今にも", "いまにも", "at any time, soon"],
    ["イメージ", "イメージ", "one's image"],
    ["否", "いや", "no, the noes"],
    ["以来", "いらい", "since, henceforth"],
    ["依頼", "いらい", "request; dependence"],
    ["いらいら", "いらいら", "getting nervous, irritation"],
    ["いらっしゃい", "いらっしゃい", "welcome"],
    ["医療", "いりょう", "medical care, medical treatment"],
    ["岩", "いわ", "rock"],
    ["祝い", "いわい", "celebration, festival"],
    ["祝う", "いわう", "to congratulate, to celebrate"],
    ["言わば", "いわば", "so to speak"],
    ["いわゆる", "いわゆる", "the so-called, so-to-speak"],
    ["インク", "インク", "ink"],
    ["印刷", "いんさつ", "printing"],
    ["印象", "いんしょう", "impression"],
    ["引退", "いんたい", "retirement"],
    ["インタビュー", "インタビュー", "interview"],
    ["引用", "いんよう", "quotation, citation"],
    ["ウイスキー", "ウイスキー", "whiskey"],
    ["上", "うわ", "upper, outer, surface"],
    ["魚", "うお", "fish"],
    ["うがい", "うがい", "gargling"],
    ["受け取る", "うけとる", "to receive, to get, to accept"],
    ["動かす", "うごかす", "to move, to shift"],
    ["兎", "うさぎ", "rabbit, hare"],
    ["牛", "うし", "cattle, cow"],
    ["失う", "うしなう", "to lose, to part with"],
    ["疑う", "うたがう", "to doubt, to distrust"],
    ["宇宙", "うちゅう", "universe, cosmos, space"],
    ["討つ", "うつ", "to attack, to avenge"],
    ["撃つ", "うつ", "to attack, to shoot"],
    ["うっかり", "うっかり", "carelessly; inadvertently"],
    ["映す", "うつす", "to project, to reflect, to cast (shadow)"],
    ["訴える", "うったえる", "to complain, to appeal, to sue (a person)"],
    ["写る", "うつる", "to be photographed, to be projected"],
    ["映る", "うつる", "to be reflected, to come out (photo)"],
    ["うなる", "うなる", "to groan, to moan"],
    ["奪う", "うばう", "to rob, to deprive"],
    ["馬", "うま", "horse; promoted bishop (in Japanese chess known as shogi)"],
    ["生まれ", "うまれ", "birth, birth-place"],
    ["有無", "うむ", "yes or no, presence or absence"],
    ["梅", "うめ", "plum, lowest (of a three-tier ranking system)"],
    ["埋める", "うめる", "to bury, to fill up, to fill (a seat, a vacant position)"],
    ["裏切る", "うらぎる", "to betray, to turn traitor"],
    ["羨ましい", "うらやましい", "envious, enviable"],
    ["売れる", "うれる", "to be sold"],
    ["噂", "うわさ", "rumor, gossip"],
    ["運", "うん", "fortune, luck"],
    ["運転", "うんてん", "operation, driving"],
    ["柄", "え", "handle (of a sword, dagger, etc.), grip"],
    ["永遠", "えいえん", "eternity, perpetuity, immortality"],
    ["永久", "えいきゅう", "eternity, perpetuity, immortality"],
    ["影響", "えいきょう", "influence, effect"],
    ["営業", "えいぎょう", "business, trade, management"],
    ["衛星", "えいせい", "satellite"],
    ["栄養", "えいよう", "nutrition, nourishment"],
    ["笑顔", "えがお", "smile (on one's face)"],
    ["描く", "えがく", "to draw, to depict, to describe"],
    ["餌", "えさ", "feed, bait"],
    ["エネルギー", "エネルギー", "energy (GER: energie)"],
    ["得る", "える", "to get, to gain, to win, to learn"],
    ["得る", "うる", "to get, to gain, to win"],
    ["円", "えん", "circle, yen"],
    ["延期", "えんき", "postponement, adjournment"],
    ["演技", "えんぎ", "acting, performance"],
    ["援助", "えんじょ", "assistance, aid, support"],
    ["エンジン", "エンジン", "engine"],
    ["演説", "えんぜつ", "speech, address"],
    ["演奏", "えんそう", "musical performance"],
    ["老い", "おい", "old age, the aged"],
    ["追い付く", "おいつく", "to overtake, to catch up (with)"],
    ["王", "おう", "king"],
    ["追う", "おう", "to chase, to run after"],
    ["応援", "おうえん", "aid, assistance, help"],
    ["王様", "おうさま", "king"],
    ["王子", "おうじ", "prince"],
    ["応じる", "おうじる", "to adapt, to respond, to comply with"],
    ["横断", "おうだん", "crossing"],
    ["終える", "おえる", "to finish"],
    ["大いに", "おおいに", "much, considerably (same as 大変 (たいへん)), greatly"],
    ["覆う", "おおう", "to cover, to hide, to conceal"],
    ["大家", "おおや", "landlord"],
    ["丘", "おか", "hill, height"],
    ["沖", "おき", "open sea"],
    ["奥", "おく", "interior, inner part"],
    ["贈る", "おくる", "to present, to give to, to award to"],
    ["起こる", "おこる", "to occur, to happen"],
    ["押える", "おさえる", "to stop, to restrain, to press down"],
    ["幼い", "おさない", "very young, childish"],
    ["収める", "おさめる", "to store to pay, to supply"],
    ["納める", "おさめる", "to store to pay, to supply"],
    ["治める", "おさめる", "to govern, to manage; to subdue"],
    ["お辞儀", "おじぎ", "bow"],
    ["お洒落", "おしゃれ", "smartly dressed, fashion-conscious"],
    ["お喋り", "おしゃべり", "chattering, talk"],
    ["汚染", "おせん", "pollution, contamination"],
    ["恐らく", "おそらく", "perhaps"],
    ["恐れる", "おそれる", "to fear, to be afraid of"],
    ["恐ろしい", "おそろしい", "terrible, dreadful"],
    ["教わる", "おそわる", "to be taught"],
    ["お互い", "おたがい", "mutual, reciprocal, each other"],
    ["穏やか", "おだやか", "calm, gentle, quiet"],
    ["男の人", "おとこのひと", "man"],
    ["大人しい", "おとなしい", "obedient, docile, quiet"],
    ["劣る", "おとる", "to fall behind, to be inferior to"],
    ["鬼", "おに", "ogre, demon, 'it' (e.g.,in a game of tag)"],
    ["帯", "おび", "band, sash"],
    ["お昼", "おひる", "lunch, noon"],
    ["オフィス", "オフィス", "office"],
    ["溺れる", "おぼれる", "to be drowned, to indulge in"],
    ["お前", "おまえ", "you (sing), presence (of a high personage)"],
    ["おめでとう", "おめでとう", "Congratulations!, an auspicious occasion!"],
    ["お目に掛かる", "おめにかかる", "meet ~, see ~"],
    ["思い付く", "おもいつく", "to think of, to hit upon"],
    ["思い出", "おもいで", "memories, recollections, reminiscence"],
    ["主に", "おもに", "mainly, primarily"],
    ["思わず", "おもわず", "unintentional, spontaneous"],
    ["泳ぎ", "およぎ", "swimming"],
    ["およそ", "およそ", "about, roughly, approximately"],
    ["及ぼす", "およぼす", "to exert, to cause, to exercise"],
    ["下す", "おろす", "to lower, to let go down"],
    ["降ろす", "おろす", "to take down, to launch, to drop"],
    ["卸す", "おろす", "to sell wholesale, grated (vegetables)"],
    ["音", "おん", "sound, note"],
    ["恩", "おん", "favor, obligation, debt of gratitude"],
    ["温暖", "おんだん", "warmth"],
    ["温度", "おんど", "temperature"],
    ["可", "か", "passable"],
    ["蚊", "か", "mosquito"],
    ["課", "か", "department, division"],
    ["カー", "カー", "car"],
    ["カード", "カード", "card, curd"],
    ["貝", "かい", "shell, shellfish"],
    ["害", "がい", "harm, damage"],
    ["会員", "かいいん", "member, the membership"],
    ["絵画", "かいが", "picture, painting"],
    ["海外", "かいがい", "foreign, abroad, overseas"],
    ["会計", "かいけい", "account, finance"],
    ["解決", "かいけつ", "settlement, solution, resolution"],
    ["会合", "かいごう", "meeting, assembly"],
    ["外交", "がいこう", "diplomacy"],
    ["開始", "かいし", "start, commencement, beginning"],
    ["解釈", "かいしゃく", "explanation, interpretation"],
    ["外出", "がいしゅつ", "outing, going out"],
    ["改善", "かいぜん", "betterment, improvement"],
    ["快適", "かいてき", "pleasant, agreeable"],
    ["回復", "かいふく", "recovery (from illness), rehabilitation, restoration"],
    ["飼う", "かう", "to keep; to own (a pet); to raise; to feed"],
    ["帰す", "かえす", "to send back"],
    ["代える", "かえる", "to exchange, to interchange, to substitute"],
    ["替える", "かえる", "to exchange, to interchange, to substitute"],
    ["換える", "かえる", "to exchange, to interchange, to substitute"],
    ["反る", "かえる", "to warp, to be warped, to curve"],
    ["香り", "かおり", "aroma, fragrance"],
    ["画家", "がか", "painter"],
    ["抱える", "かかえる", "to hold or carry under or in the arms"],
    ["価格", "かかく", "price, value"],
    ["化学", "かがく", "chemistry"],
    ["輝く", "かがやく", "to shine, to glitter, to sparkle"],
    ["係", "かかり", "person in charge"],
    ["罹る", "かかる", "to suffer from"],
    ["限る", "かぎる", "to restrict, to limit, to confine"],
    ["掻く", "かく", "to scratch"],
    ["描く", "かく", "to draw, to depict, to describe"],
    ["嗅ぐ", "かぐ", "to sniff, to smell"],
    ["家具", "かぐ", "furniture"],
    ["学", "がく", "learning, knowledge"],
    ["額", "がく", "amount; frame"],
    ["覚悟", "かくご", "resolution, resignation, readiness"],
    ["確実", "かくじつ", "certainty, reliability, soundness"],
    ["学者", "がくしゃ", "scholar"],
    ["学習", "がくしゅう", "study, learning"],
    ["隠す", "かくす", "to hide, to conceal"],
    ["拡大", "かくだい", "magnification, enlargement"],
    ["確認", "かくにん", "affirmation, confirmation"],
    ["学問", "がくもん", "scholarship, study, learning"],
    ["隠れる", "かくれる", "to hide, to be hidden"],
    ["影", "かげ", "shade, shadow, other side"],
    ["陰", "かげ", "shade, shadow, other side"],
    ["欠ける", "かける", "to be lacking"],
    ["加減", "かげん", "adjustment; addition and subtraction"],
    ["過去", "かこ", "past"],
    ["籠", "かご", "basket, cage"],
    ["囲む", "かこむ", "to surround, to encircle"],
    ["火災", "かさい", "conflagration, fire"],
    ["重なる", "かさなる", "to be piled up, lie on top of one another"],
    ["重ねる", "かさねる", "to pile up, to put something on another, to heap up"],
    ["飾り", "かざり", "decoration"],
    ["貸し", "かし", "loan, lending"],
    ["菓子", "かし", "confectionery, sweet"],
    ["家事", "かじ", "household matters; housework (same as 家の仕事 (いえのしごと))"],
    ["賢い", "かしこい", "wise, clever, smart"],
    ["歌手", "かしゅ", "singer"],
    ["数", "かず", "number, figure, amount"],
    ["稼ぐ", "かせぐ", "to earn income, to labor"],
    ["数える", "かぞえる", "to count"],
    ["型", "かた", "mold, model, style"],
    ["肩", "かた", "shoulder"],
    ["堅い", "かたい", "hard, firm, solid"],
    ["硬い", "かたい", "hard, firm, solid"],
    ["方々", "かたがた", "persons"],
    ["片付く", "かたづく", "to put in order, to solve"],
    ["刀", "かたな", "sword, saber"],
    ["語る", "かたる", "to talk, to tell, to recite"],
    ["勝ち", "かち", "win, victory"],
    ["価値", "かち", "value, worth, merit"],
    ["がっかり", "がっかり", "feel disappointed, be dejected, lose heart"],
    ["活気", "かっき", "vigor; liveliness; vitality; energy"],
    ["楽器", "がっき", "musical instrument"],
    ["学期", "がっき", "term (school)"],
    ["活動", "かつどう", "action, activity"],
    ["活躍", "かつやく", "activity"],
    ["活用", "かつよう", "conjugation; practical use"],
    ["仮定", "かてい", "assumption, supposition, hypothesis"],
    ["過程", "かてい", "process"],
    ["課程", "かてい", "course, curriculum"],
    ["悲しむ", "かなしむ", "to be sad, to mourn for, to regret"],
    ["必ずしも", "かならずしも", "(not) always, (not) necessarily"],
    ["かなり", "かなり", "considerably, fairly, quite"],
    ["金", "かね", "gold, metal; money"],
    ["鐘", "かね", "bell, chime"],
    ["可能", "かのう", "possible, practicable, feasible"],
    ["株", "かぶ", "stock; stump (of tree)"],
    ["被る", "かぶる", "to wear; to be covered with"],
    ["我慢", "がまん", "patience, endurance, perseverance"],
    ["上", "かみ", "first volume; superior quality; governmental"],
    ["神", "かみ", "god"],
    ["雷", "かみなり", "thunder"],
    ["髪の毛", "かみのけ", "hair (head)"],
    ["科目", "かもく", "(school) subject, curriculum, course"],
    ["かもしれない", "かもしれない", "maybe, perhaps"],
    ["かゆい", "かゆい", "itchy, itching"],
    ["歌謡", "かよう", "song, ballad"],
    ["空", "から", "empty"],
    ["殻", "から", "shell, husk, hull"],
    ["柄", "がら", "pattern; build; character"],
    ["刈る", "かる", "to cut (hair), to mow (grass), to harvest"],
    ["河", "かわ", "river, stream"],
    ["皮", "かわ", "skin, hide, leather"],
    ["革", "かわ", "leather"],
    ["可愛そう", "かわいそう", "poor, pitiable, pathetic"],
    ["可愛らしい", "かわいらしい", "lovely, sweet"],
    ["乾かす", "かわかす", "to dry"],
    ["渇く", "かわく", "to be thirsty"],
    ["代る", "かわる", "to take the place of, to relieve, to be substituted for"],
    ["缶", "かん", "can, tin"],
    ["勘", "かん", "perception, intuition, the sixth sense"],
    ["考え", "かんがえ", "thinking, thought, ideas"],
    ["感覚", "かんかく", "sense, sensation"],
    ["間隔", "かんかく", "space, interval, SPC"],
    ["観客", "かんきゃく", "audience, spectator(s)"],
    ["環境", "かんきょう", "environment, circumstance"],
    ["歓迎", "かんげい", "welcome, reception"],
    ["観光", "かんこう", "sightseeing"],
    ["観察", "かんさつ", "observation, survey"],
    ["感じ", "かんじ", "feeling, sense, impression"],
    ["感謝", "かんしゃ", "thanks, gratitude"],
    ["患者", "かんじゃ", "patient"],
    ["勘定", "かんじょう", "calculation, counting, consideration"],
    ["感情", "かんじょう", "emotion(s), feeling(s), sentiment"],
    ["感じる", "かんじる", "to feel, to sense"],
    ["感心", "かんしん", "admiration"],
    ["関心", "かんしん", "concern, interest"],
    ["関する", "かんする", "to concern, to be related"],
    ["完成", "かんせい", "complete, completion; perfection"],
    ["完全", "かんぜん", "perfection, completeness"],
    ["乾燥", "かんそう", "dry, arid, dehydrated"],
    ["感想", "かんそう", "(one's) impressions, (one's) thoughts"],
    ["感動", "かんどう", "being deeply moved, excitement"],
    ["監督", "かんとく", "supervision, control, (movie) director"],
    ["管理", "かんり", "control, management (e.g., of a business)"],
    ["完了", "かんりょう", "completion, conclusion"],
    ["関連", "かんれん", "relation, connection, relevance"],
    ["議員", "ぎいん", "member of the Diet, congress, or parliament"],
    ["記憶", "きおく", "memory, recollection, remembrance"],
    ["気温", "きおん", "temperature (weather - not used for things)"],
    ["機械", "きかい", "machine, machinery"],
    ["器械", "きかい", "instrument"],
    ["議会", "ぎかい", "Diet, congress, parliament"],
    ["期間", "きかん", "period, term"],
    ["機関", "きかん", "engine; institution, organization"],
    ["企業", "きぎょう", "industry, business, undertaking"],
    ["効く", "きく", "to be effective"],
    ["期限", "きげん", "deadline, term"],
    ["機嫌", "きげん", "humor, temper, mood"],
    ["気候", "きこう", "climate"],
    ["岸", "きし", "bank, coast, shore"],
    ["生地", "きじ", "fabric; dough"],
    ["記事", "きじ", "article, news story"],
    ["技師", "ぎし", "engineer, technician"],
    ["記者", "きしゃ", "reporter"],
    ["傷", "きず", "wound, injury, hurt"],
    ["期待", "きたい", "expectation, anticipation, hope"],
    ["気体", "きたい", "vapor, gas"],
    ["帰宅", "きたく", "returning home"],
    ["貴重", "きちょう", "precious, valuable"],
    ["議長", "ぎちょう", "chairman"],
    ["きちんと", "きちんと", "precisely, accurately"],
    ["きつい", "きつい", "tight, close, intense"],
    ["気付く", "きづく", "to notice, to recognize, to become aware of"],
    ["気に入る", "きにいる", "to like, to be please"],
    ["記入", "きにゅう", "entry, filling in of forms"],
    ["記念", "きねん", "commemoration, memory"],
    ["機能", "きのう", "function, faculty"],
    ["気の毒", "きのどく", "pitiful, a pity"],
    ["寄付", "きふ", "contribution, donation"],
    ["希望", "きぼう", "hope, wish, aspiration"],
    ["基本", "きほん", "basic, basis"],
    ["決まり", "きまり", "settlement, conclusion, rule"],
    ["気味", "きみ", "-like, -looking, -looked"],
    ["奇妙", "きみょう", "strange, queer, curious"],
    ["義務", "ぎむ", "duty, obligation, responsibility"],
    ["疑問", "ぎもん", "question, problem, doubt"],
    ["逆", "ぎゃく", "reverse, opposite"],
    ["キャプテン", "キャプテン", "captain"],
    ["キャンプ", "キャンプ", "camp"],
    ["旧", "きゅう", "ex-"],
    ["級", "きゅう", "class, grade, rank"],
    ["球", "きゅう", "globe, sphere, ball"],
    ["休暇", "きゅうか", "vacation, holiday, day off"],
    ["休憩", "きゅうけい", "rest, break, intermission"],
    ["急激", "きゅうげき", "sudden, precipitous, radical"],
    ["吸収", "きゅうしゅう", "absorption, suction"],
    ["救助", "きゅうじょ", "relief, aid, rescue"],
    ["急速", "きゅうそく", "rapid (e.g., progress)"],
    ["休息", "きゅうそく", "rest, relief, relaxation"],
    ["急に", "きゅうに", "suddenly"],
    ["給料", "きゅうりょう", "salary, wages"],
    ["器用", "きよう", "skillful, handy"],
    ["教科書", "きょうかしょ", "textbook"],
    ["競技", "きょうぎ", "game, match, contest"],
    ["行儀", "ぎょうぎ", "manners"],
    ["供給", "きょうきゅう", "supply, provision"],
    ["教授", "きょうじゅ", "teaching, instruction; professor"],
    ["強調", "きょうちょう", "emphasis, stress, stressed point"],
    ["共通", "きょうつう", "commonness, mutual"],
    ["共同", "きょうどう", "cooperation, association, collaboration"],
    ["恐怖", "きょうふ", "fear, terror"],
    ["協力", "きょうりょく", "cooperation, collaboration"],
    ["強力", "きょうりょく", "powerful, strong"],
    ["許可", "きょか", "permission, approval"],
    ["局", "きょく", "office, bureau, station(TV, radio)"],
    ["巨大", "きょだい", "huge, gigantic, enormous"],
    ["嫌う", "きらう", "to hate, to dislike, to loathe"],
    ["霧", "きり", "fog, mist"],
    ["切れ", "きれ", "cloth, piece, cut"],
    ["切れる", "きれる", "to cut well, to be sharp; to break (off)"],
    ["記録", "きろく", "record, minutes, document"],
    ["議論", "ぎろん", "argument, discussion, dispute"],
    ["金", "きん", "gold, metal; money"],
    ["銀", "ぎん", "silver"],
    ["禁煙", "きんえん", "No Smoking"],
    ["金額", "きんがく", "amount of money"],
    ["金庫", "きんこ", "safe, vault"],
    ["禁止", "きんし", "prohibition, ban"],
    ["金銭", "きんせん", "money, cash"],
    ["金属", "きんぞく", "metal"],
    ["近代", "きんだい", "modern times"],
    ["緊張", "きんちょう", "tension, mental strain, nervousness"],
    ["筋肉", "きんにく", "muscle, sinews"],
    ["金融", "きんゆう", "finance, money and banking"],
    ["句", "く", "phrase"],
    ["食う", "くう", "(male) (vulg.) to eat"],
    ["偶然", "ぐうぜん", "(by) chance, unexpectedly"],
    ["臭い", "くさい", "stinky, smelly, bad-smelling"],
    ["鎖", "くさり", "chain"],
    ["腐る", "くさる", "to rot, to go bad"],
    ["癖", "くせ", "a habit (often a bad habit), peculiarity"],
    ["管", "くだ", "pipe, tube"],
    ["具体", "ぐたい", "concrete, tangible, material"],
    ["下り", "くだり", "down-train (going away from Tokyo)"],
    ["下る", "くだる", "to get down, to descend"],
    ["苦痛", "くつう", "pain, agony"],
    ["ぐっすり", "ぐっすり", "sound asleep, fast asleep"],
    ["区別", "くべつ", "distinction, differentiation, classification"],
    ["組", "くみ", "class, team, set"],
    ["組合", "くみあい", "association, union"],
    ["組む", "くむ", "to put together"],
    ["汲む", "くむ", "to draw, to scoop, to pump"],
    ["酌む", "くむ", "to serve sake"],
    ["悔しい", "くやしい", "regrettable, mortifying, vexing"],
    ["位", "くらい", "grade, rank, about"],
    ["暮らし", "くらし", "living; life style"],
    ["クラシック", "クラシック", "classic(s)"],
    ["暮らす", "くらす", "to live, to get along"],
    ["グラス", "グラス", "glass; grass"],
    ["グランド", "グランド", "gland, grand, (electrical) ground"],
    ["クリーム", "クリーム", "cream"],
    ["繰り返す", "くりかえす", "to repeat, to do something over again"],
    ["クリスマス", "クリスマス", "Christmas"],
    ["狂う", "くるう", "to go mad, to get out of order"],
    ["グループ", "グループ", "group"],
    ["苦しい", "くるしい", "tough; physically strenuous"],
    ["苦しむ", "くるしむ", "to suffer, to groan, to be worried"],
    ["暮れ", "くれ", "year end,"],
    ["苦労", "くろう", "hardship; suffering"],
    ["加える", "くわえる", "to append, to sum up, to add (up)"],
    ["咥える", "くわえる", "to hold something in the mouth"],
    ["詳しい", "くわしい", "detailed; full; accurate"],
    ["加わる", "くわわる", "to join in, to accede to"],
    ["訓", "くん", "native Japanese reading of a Chinese character"],
    ["軍", "ぐん", "army, force, troops"],
    ["郡", "ぐん", "country, district"],
    ["軍隊", "ぐんたい", "army, troops"],
    ["訓練", "くんれん", "practice, training"],
    ["下", "げ", "under, below, beneath"],
    ["計", "けい", "plan; sum, total"],
    ["敬意", "けいい", "respect, honor"],
    ["経営", "けいえい", "management, administration"],
    ["景気", "けいき", "condition, state, business (condition)"],
    ["傾向", "けいこう", "tendency, trend, inclination"],
    ["警告", "けいこく", "warning"],
    ["計算", "けいさん", "calculation, reckoning"],
    ["掲示", "けいじ", "notice, bulletin"],
    ["刑事", "けいじ", "criminal case, (police) detective"],
    ["芸術", "げいじゅつ", "(fine) art, the arts"],
    ["契約", "けいやく", "contract, compact, agreement"],
    ["経由", "けいゆ", "go by the way, via"],
    ["ケース", "ケース", "case"],
    ["ゲーム", "ゲーム", "game"],
    ["劇", "げき", "drama, play"],
    ["劇場", "げきじょう", "theater, playhouse"],
    ["化粧", "けしょう", "make-up (cosmetic)"],
    ["けち", "けち", "stinginess, miser"],
    ["血液", "けつえき", "blood"],
    ["結果", "けっか", "result, consequence"],
    ["欠陥", "けっかん", "defect, fault, deficiency"],
    ["結局", "けっきょく", "after all, eventually"],
    ["決心", "けっしん", "determination, resolution"],
    ["欠席", "けっせき", "absence, non-attendance"],
    ["決定", "けってい", "decision, determination"],
    ["欠点", "けってん", "faults, defect, weakness"],
    ["結論", "けつろん", "conclusion"],
    ["煙", "けむり", "smoke, fumes"],
    ["蹴る", "ける", "to kick"],
    ["券", "けん", "ticket, certificate"],
    ["県", "けん", "prefecture"],
    ["見解", "けんかい", "opinion, point of view"],
    ["限界", "げんかい", "limit, bound"],
    ["現金", "げんきん", "cash"],
    ["言語", "げんご", "language"],
    ["健康", "けんこう", "health(y)"],
    ["検査", "けんさ", "inspection, examination"],
    ["現在", "げんざい", "now (same as 今 (いま)), present, current"],
    ["現実", "げんじつ", "reality"],
    ["現象", "げんしょう", "phenomenon"],
    ["現状", "げんじょう", "present condition, status quo"],
    ["建設", "けんせつ", "construction, foundation"],
    ["現代", "げんだい", "today, present-day"],
    ["建築", "けんちく", "construction, architecture"],
    ["見当", "けんとう", "estimate, guess"],
    ["検討", "けんとう", "consideration, examination, investigation"],
    ["現場", "げんば", "actual spot, scene, field"],
    ["憲法", "けんぽう", "constitution"],
    ["権利", "けんり", "right, privilege"],
    ["後", "ご", "afterwards, since then"],
    ["碁", "ご", "Go (board game of capturing territory)"],
    ["恋", "こい", "love, tender passion"],
    ["濃い", "こい", "thick (as of color, liquid), dense, strong"],
    ["恋人", "こいびと", "lover; sweetheart; girlfriend"],
    ["幸運", "こううん", "good luck, fortune"],
    ["講演", "こうえん", "lecture, talk"],
    ["効果", "こうか", "effect, result"],
    ["硬貨", "こうか", "coin"],
    ["高価", "こうか", "high price"],
    ["豪華", "ごうか", "luxurious, gorgeous, extravagance"],
    ["合格", "ごうかく", "success, passing (e.g., exam)"],
    ["交換", "こうかん", "exchange, swap"],
    ["航空", "こうくう", "aviation, flying"],
    ["光景", "こうけい", "scene, spectacle"],
    ["合計", "ごうけい", "sum total, total amount"],
    ["攻撃", "こうげき", "attack, strike, offensive"],
    ["貢献", "こうけん", "contribution, services"],
    ["広告", "こうこく", "advertisement"],
    ["交際", "こうさい", "friendship, association, acquaintance"],
    ["校舎", "こうしゃ", "school building"],
    ["後者", "こうしゃ", "the latter"],
    ["工場", "こうば", "factory, plant"],
    ["公正", "こうせい", "justice, fairness, impartiality"],
    ["構成", "こうせい", "organization, composition"],
    ["高速", "こうそく", "high speed, high gear"],
    ["行動", "こうどう", "action, conduct, behavior"],
    ["強盗", "ごうとう", "robbery, burglary"],
    ["後輩", "こうはい", "junior members of a group"],
    ["幸福", "こうふく", "happiness, blessedness"],
    ["公平", "こうへい", "fairness, impartial, justice"],
    ["候補", "こうほ", "candidacy"],
    ["考慮", "こうりょ", "consideration, taking into account"],
    ["越える", "こえる", "to exceed, to cross over, to cross"],
    ["超える", "こえる", "to exceed, to cross over, to cross"],
    ["コーチ", "コーチ", "coach"],
    ["コード", "コード", "code; cord; chord"],
    ["氷", "こおり", "ice, hail"],
    ["凍る", "こおる", "to freeze, to be frozen over, to congeal"],
    ["ゴール", "ゴール", "goal"],
    ["誤解", "ごかい", "misunderstanding"],
    ["語学", "ごがく", "language study"],
    ["呼吸", "こきゅう", "breath, respiration"],
    ["故郷", "こきょう", "hometown"],
    ["極", "ごく", "quite, very"],
    ["国語", "こくご", "national language"],
    ["国籍", "こくせき", "nationality"],
    ["黒板", "こくばん", "blackboard"],
    ["克服", "こくふく", "conquest, overcome"],
    ["国民", "こくみん", "national, people, citizen"],
    ["穀物", "こくもつ", "grain, cereal, corn"],
    ["腰", "こし", "hip, waist"],
    ["胡椒", "こしょう", "pepper"],
    ["個人", "こじん", "individual, private person"],
    ["越す", "こす", "to go over (e.g., with audience)"],
    ["超す", "こす", "to cross, to pass, to tide over"],
    ["国家", "こっか", "state, country, nation"],
    ["国会", "こっかい", "National Diet, parliament, congress"],
    ["国境", "こっきょう", "national or state border"],
    ["骨折", "こっせつ", "bone fracture"],
    ["小包", "こづつみ", "parcel, package"],
    ["琴", "こと", "Japanese harp"],
    ["異なる", "ことなる", "to differ, to vary"],
    ["諺", "ことわざ", "proverb, saying"],
    ["断る", "ことわる", "to refuse, to decline, to dismiss"],
    ["粉", "こな", "flour, powder"],
    ["好み", "このみ", "liking, taste, choice"],
    ["好む", "このむ", "to like, to prefer"],
    ["こぼす", "こぼす", "to spill"],
    ["こぼれる", "こぼれる", "to overflow, to spill"],
    ["塵", "ごみ", "garbage, litter"],
    ["小麦", "こむぎ", "wheat"],
    ["ごめんなさい", "ごめんなさい", "I beg your pardon, excuse me, I'm sorry"],
    ["小屋", "こや", "hut, cabin, shed"],
    ["これら", "これら", "these"],
    ["殺す", "ころす", "to kill"],
    ["転ぶ", "ころぶ", "to fall down, to fall over"],
    ["今回", "こんかい", "now, this time, lately"],
    ["今後", "こんご", "from now on, hereafter"],
    ["混雑", "こんざつ", "confusion, congestion"],
    ["こんなに", "こんなに", "so, like this, in this way"],
    ["困難", "こんなん", "difficulty, distress"],
    ["今日", "こんにち", "today, this day"],
    ["こんにちは", "こんにちは", "hello, good day (daytime greeting)"],
    ["婚約", "こんやく", "engagement, betrothal"],
    ["混乱", "こんらん", "chaos, confusion, mayhem"],
    ["差", "さ", "difference, variation"],
    ["サービス", "サービス", "service, support system; goods or services without charge"],
    ["際", "さい", "on the occasion of, circumstances"],
    ["最高", "さいこう", "highest, supreme, the most"],
    ["財産", "ざいさん", "property, fortune, assets"],
    ["最終", "さいしゅう", "last, closing"],
    ["最中", "さいちゅう", "in the middle of"],
    ["最低", "さいてい", "least, lowest, worst"],
    ["才能", "さいのう", "talent, ability"],
    ["裁判", "さいばん", "trial, judgment"],
    ["材料", "ざいりょう", "ingredients, material"],
    ["幸い", "さいわい", "fortunately; luckily"],
    ["サイン", "サイン", "autograph; sign; sine"],
    ["境", "さかい", "border, boundary, mental state"],
    ["逆らう", "さからう", "to go against, to oppose, to disobey"],
    ["盛り", "さかり", "helping, serving"],
    ["作業", "さぎょう", "work, operation, manufacturing"],
    ["裂く", "さく", "to tear, to split"],
    ["昨", "さく", "last (year), yesterday"],
    ["作品", "さくひん", "work, opus, production"],
    ["作物", "さくもつ", "produce (e.g., agricultural), crops"],
    ["桜", "さくら", "cherry blossom, cherry tree"],
    ["酒", "さけ", "alcohol, sake"],
    ["叫ぶ", "さけぶ", "to shout, to cry"],
    ["避ける", "さける", "to avoid (physical contact); to ward off, to avert"],
    ["支える", "ささえる", "support, hold, sustain"],
    ["刺さる", "ささる", "to stick, to be stuck"],
    ["刺す", "さす", "to sting, to bite (e.g., bug), to prick, to stab"],
    ["指す", "さす", "to point,"],
    ["挿す", "さす", "to insert, to put in, to graft"],
    ["注す", "さす", "to pour (drink), to serve (drinks)"],
    ["射す", "さす", "to shine, to strike"],
    ["座席", "ざせき", "seat"],
    ["誘う", "さそう", "to invite (someone to do something with you); to tempt, to lure"],
    ["札", "さつ", "bill, note"],
    ["作家", "さっか", "author, writer, novelist"],
    ["作曲", "さっきょく", "composition (of music)"],
    ["ざっと", "ざっと", "roughly, in round numbers"],
    ["さっぱり", "さっぱり", "feeling refreshed, neat"],
    ["さて", "さて", "well; now (typically used when switching to a new, usually more important topic)"],
    ["砂漠", "さばく", "desert"],
    ["差別", "さべつ", "discrimination, differentiation"],
    ["ママ", "ママ", "Mama"],
    ["豆", "まめ", "beans, peas"],
    ["守る", "まもる", "to protect; to abide (by the rules)"],
    ["迷う", "まよう", "to be puzzled, to be perplexed, to lose one's way"],
    ["丸", "まる", "circle, full (month)"],
    ["円", "まる", "circle"],
    ["まるで", "まるで", "just like"],
    ["万一", "まんいち", "by some chance, if by any chance"],
    ["満足", "まんぞく", "satisfaction"],
    ["身", "み", "body, main part"],
    ["実", "み", "fruit, seed, good result"],
    ["見送り", "みおくり", "seeing one off"],
    ["味方", "みかた", "ally, supporter"],
    ["見事", "みごと", "splendid, magnificent"],
    ["ミス", "ミス", "miss (mistake, error, failure), Miss"],
    ["満ちる", "みちる", "to be full, to mature"],
    ["密", "みつ", "thick, close"],
    ["認める", "みとめる", "to recognize, to notice; to approve"],
    ["見舞い", "みまい", "expression of sympathy, expression of concern"],
    ["土産", "みやげ", "souvenir"],
    ["都", "みやこ", "city; capital"],
    ["妙", "みょう", "strange, unusual"],
    ["明後日", "みょうごにち", "day after tomorrow"],
    ["未来", "みらい", "future (life tense)"],
    ["魅力", "みりょく", "charm, fascination, appeal"],
    ["診る", "みる", "to examine (a patient)"],
    ["ミルク", "ミルク", "milk"],
    ["無", "む", "nothing, naught, zero"],
    ["向かい", "むかい", "facing, opposite, across"],
    ["迎え", "むかえ", "meeting, person sent to pick up an arrival"],
    ["向く", "むく", "to face"],
    ["剥く", "むく", "to peel, to skin"],
    ["向ける", "むける", "to turn towards, to point"],
    ["無視", "むし", "disregard, ignore"],
    ["蒸し暑い", "むしあつい", "humid, sultry"],
    ["虫歯", "むしば", "cavity, tooth decay"],
    ["寧ろ", "むしろ", "rather, better, instead"],
    ["蒸す", "むす", "to steam, to poultice, to be sultry"],
    ["結ぶ", "むすぶ", "to tie, to bind, to link"],
    ["無駄", "むだ", "futility, uselessness"],
    ["夢中", "むちゅう", "crush, crazy, be hooked on"],
    ["胸", "むね", "breast, chest"],
    ["無料", "むりょう", "free, no charge"],
    ["芽", "め", "sprout"],
    ["明確", "めいかく", "clear, definite"],
    ["名刺", "めいし", "(name) card; business card"],
    ["名詞", "めいし", "noun"],
    ["命じる", "めいじる", "to order, to command, to appoint"],
    ["名人", "めいじん", "master, expert"],
    ["命令", "めいれい", "order, command, decree"],
    ["迷惑", "めいわく", "trouble, bother, annoyance"],
    ["目上", "めうえ", "person of higher status; one's senior"],
    ["飯", "めし", "meals, food"],
    ["滅多に", "めったに", "rarely (with neg. verb), seldom"],
    ["メモ", "メモ", "memorandum, note"],
    ["面", "めん", "face, mug, surface, side or facet, corner, page"],
    ["綿", "めん", "cotton"],
    ["免許", "めんきょ", "license, permit, certificate"],
    ["面接", "めんせつ", "interview"],
    ["面倒", "めんどう", "trouble, attention"],
    ["メンバー", "メンバー", "member"],
    ["申し込む", "もうしこむ", "to apply for, to make an application"],
    ["申し訳", "もうしわけ", "apology, excuse"],
    ["毛布", "もうふ", "blanket"],
    ["燃える", "もえる", "to burn"],
    ["目的", "もくてき", "purpose, goal, aim"],
    ["目標", "もくひょう", "mark, objective, target"],
    ["文字", "もじ", "letter (of alphabet), character"],
    ["文字", "もんじ", "letter (of alphabet), character"],
    ["もしかすると", "もしかすると", "perhaps, maybe, by some chance"],
    ["もしも", "もしも", "if"],
    ["持ち上げる", "もちあげる", "to raise, to lift up, to flatter"],
    ["用いる", "もちいる", "to use, to make use of"],
    ["もったいない", "もったいない", "wasteful; more than one deserves, unworthy of"],
    ["尤も", "もっとも", "quite right, plausible, natural"],
    ["元", "もと", "origin, original; former"],
    ["基", "もと", "basis"],
    ["素", "もと", "prime"],
    ["戻す", "もどす", "to restore, to put back, to return"],
    ["基づく", "もとづく", "to be grounded on, to be based on"],
    ["求める", "もとめる", "to request, to ask for; to seek, to search for"],
    ["者", "もの", "person (same as 人 (ひと))"],
    ["物音", "ものおと", "sounds"],
    ["物語", "ものがたり", "tale, story, legend"],
    ["物事", "ものごと", "things, everything"],
    ["燃やす", "もやす", "to burn"],
    ["模様", "もよう", "pattern, figure, design"],
    ["文句", "もんく", "a complaint"],
    ["やがて", "やがて", "before long, soon, at length"],
    ["役", "やく", "role, position"],
    ["約", "やく", "approximately, about, some"],
    ["訳", "やく", "translation"],
    ["訳す", "やくす", "to translate"],
    ["役割", "やくわり", "assigning (allotment of) parts, role, duties"],
    ["家賃", "やちん", "rent"],
    ["厄介", "やっかい", "trouble, burden, care"],
    ["宿", "やど", "inn, lodging"],
    ["雇う", "やとう", "to employ, to hire"],
    ["屋根", "やね", "roof"],
    ["破る", "やぶる", "to tear; to violate; to defeat"],
    ["破れる", "やぶれる", "to get torn, to wear out"],
    ["辞める", "やめる", "to retire"],
    ["やや", "やや", "a little, partially, somewhat"],
    ["唯一", "ゆいいつ", "only, sole, unique"],
    ["勇気", "ゆうき", "courage, bravery, boldness"],
    ["友好", "ゆうこう", "friendship"],
    ["有効", "ゆうこう", "valid, effectual"],
    ["優秀", "ゆうしゅう", "superiority, excellence"],
    ["優勝", "ゆうしょう", "overall victory, championship"],
    ["友情", "ゆうじょう", "friendship, fellowship"],
    ["友人", "ゆうじん", "friend (formal)"],
    ["有能", "ゆうのう", "able, capable, efficient"],
    ["郵便", "ゆうびん", "mail, postal service"],
    ["ユーモア", "ユーモア", "humor"],
    ["有利", "ゆうり", "advantageous, better"],
    ["床", "ゆか", "floor"],
    ["愉快", "ゆかい", "pleasant, happy"],
    ["譲る", "ゆずる", "to turn over, to assign, to hand over"],
    ["豊か", "ゆたか", "abundant, wealthy, plentiful, rich"],
    ["茹でる", "ゆでる", "to boil"],
    ["許す", "ゆるす", "to permit, to allow, to approve"],
    ["夜", "よ", "evening, night"],
    ["夜明け", "よあけ", "dawn, daybreak"],
    ["酔う", "よう", "to get drunk"],
    ["容易", "ようい", "easy, simple, plain"],
    ["容器", "ようき", "container, vessel"],
    ["陽気", "ようき", "season, weather, cheerfulness"],
    ["要求", "ようきゅう", "request, demand"],
    ["用心", "ようじん", "care, precaution, caution"],
    ["様子", "ようす", "aspect, state, appearance"],
    ["要するに", "ようするに", "in a word, after all, in short …"],
    ["要素", "ようそ", "element"],
    ["要点", "ようてん", "gist, main point"],
    ["曜日", "ようび", "day of the week"],
    ["ヨーロッパ", "ヨーロッパ", "Europe"],
    ["予期", "よき", "expectation, forecast"],
    ["横切る", "よこぎる", "to cross (e.g., arms), to traverse"],
    ["汚す", "よごす", "to pollute, to make dirty"],
    ["予算", "よさん", "estimate, budget"],
    ["止す", "よす", "to cease, to give up"],
    ["寄せる", "よせる", "to collect, to gather, to put aside"],
    ["予測", "よそく", "prediction, estimation"],
    ["ヨット", "ヨット", "yacht"],
    ["夜中", "よなか", "midnight, dead of night"],
    ["世の中", "よのなか", "society, the world"],
    ["余分", "よぶん", "extra, excess, surplus"],
    ["予報", "よほう", "forecast, prediction"],
    ["予防", "よぼう", "prevention, protection against"],
    ["読み", "よみ", "reading"],
    ["嫁", "よめ", "bride, daughter-in-law"],
    ["余裕", "よゆう", "excess; surplus"],
    ["より", "より", "twist, ply"],
    ["因る", "よる", "to come from"],
    ["喜び", "よろこび", "joy, pleasure, rejoicing"],
    ["よろしく (かん)", "よろしく (かん)", "best regards, please remember me"],
    ["四", "よん", "four"],
    ["来", "らい～", "next ~"],
    ["ライター", "ライター", "lighter; writer"],
    ["楽", "らく", "comfort, ease"],
    ["ラケット", "ラケット", "racket"],
    ["利益", "りえき", "profits, gains"],
    ["理解", "りかい", "understanding, comprehension"],
    ["陸", "りく", "land, shore"],
    ["利口", "りこう", "clever, shrewd, bright"],
    ["離婚", "りこん", "divorce"],
    ["理想", "りそう", "ideal"],
    ["率", "りつ", "rate, ratio, percentage"],
    ["留学", "りゅうがく", "studying abroad"],
    ["流行", "りゅうこう", "fashionable, fad, prevailing"],
    ["量", "りょう", "quantity, amount"],
    ["寮", "りょう", "hostel, dormitory"],
    ["両替", "りょうがえ", "change, money exchange"],
    ["料金", "りょうきん", "fee, charge, fare"],
    ["例", "れい", "instance, example, case"],
    ["礼", "れい", "expression of gratitude; bow"],
    ["例外", "れいがい", "exception"],
    ["礼儀", "れいぎ", "manners, courtesy, etiquette"],
    ["冷静", "れいせい", "calm, coolness"],
    ["列", "れつ", "queue, line, row"],
    ["列車", "れっしゃ", "train (ordinary)"],
    ["レベル", "レベル", "level"],
    ["連想", "れんそう", "association (of ideas), suggestion"],
    ["連続", "れんぞく", "consecutive, continuity, continuing"],
    ["老人", "ろうじん", "the aged, old person"],
    ["労働", "ろうどう", "labor, work"],
    ["ロケット", "ロケット", "locket, rocket"],
    ["論じる", "ろんじる", "to argue, to discuss"],
    ["論争", "ろんそう", "controversy, dispute"],
    ["論文", "ろんぶん", "thesis, paper"],
    ["輪", "わ", "ring, hoop, circle"],
    ["ワイン", "ワイン", "wine"],
    ["わがまま", "わがまま", "selfishness, egoism, willfulness"],
    ["別れ", "わかれ", "parting, separation, farewell"],
    ["分かれる", "わかれる", "to branch off, to diverge from"],
    ["脇", "わき", "side"],
    ["湧く", "わく", "to boil, to grow hot"],
    ["分ける", "わける", "to divide, to separate"],
    ["わざと", "わざと", "on purpose"],
    ["僅か", "わずか", "only, merely, a little"],
    ["綿", "わた", "cotton"],
    ["話題", "わだい", "topic, subject"],
    ["笑い", "わらい", "laugh, laughter, smile"],
    ["割る", "わる", "to divide, to break"],
    ["悪口", "わるくち", "abuse, insult"],
    ["我々", "われわれ", "we"],
    ["湾", "わん", "bay, gulf, inlet"],
    ["椀", "わん", "bowl"],
    ["碗", "わん", "bowl"],
    ["悪", "あく", "evil, vice"],
    ["当り", "あたり", "hit, success, reaching the mark"],
    ["アップ", "アップ", "up"],
    ["宛てる", "あてる", "to address, to put"],
    ["アンケート", "アンケート", "questionnaire (FRE: enquete), survey"],
    ["異", "い", "objection"],
    ["意", "い", "will"],
    ["医院", "いいん", "doctor's office (surgery), clinic"],
    ["怒り", "いかり", "anger"],
    ["粋", "いき", "chic, style, purity"],
    ["意地", "いじ", "disposition, spirit, obstinacy, appetite"],
    ["依然", "いぜん", "still, as yet"],
    ["傷める", "いためる", "to damage, to impair, to spoil"],
    ["炒める", "いためる", "to stir-fry"],
    ["一帯", "いったい", "a region, the whole place"],
    ["異動", "いどう", "a change, transfer"],
    ["衣料", "いりょう", "clothing"],
    ["渦", "うず", "swirl"],
    ["埋まる", "うまる", "to be buried, to be filled"],
    ["産む", "うむ", "to give birth (v.t.), to deliver, to produce"],
    ["縁", "えん", "chance, tie, relationship"],
    ["尾", "お", "tail, ridge"],
    ["負う", "おう", "to bear, to owe"],
    ["遅れ", "おくれ", "delay, lag"],
    ["教え", "おしえ", "teachings, doctrine"],
    ["驚き", "おどろき", "surprise, astonishment, wonder"],
    ["織る", "おる", "to weave"],
    ["欠く", "かく", "to lack, to crack"],
    ["角", "かく", "angle"],
    ["核", "かく", "nucleus, kernel"],
    ["格", "かく", "status, character, case"],
    ["学歴", "がくれき", "academic background"],
    ["駆ける", "かける", "to run"],
    ["賭ける", "かける", "to bet, to risk, to gamble"],
    ["課題", "かだい", "subject, theme, task"],
    ["片付け", "かたづけ", "tidying up, finishing"],
    ["加味", "かみ", "seasoning, flavoring"],
    ["借り", "かり", "borrowing, debt, loan"],
    ["狩り", "かり", "hunting"],
    ["管", "かん", "pipe, tube"],
    ["～観", "かん", "feeling, view"],
    ["癌", "がん", "cancer"],
    ["刊行", "かんこう", "publication, issue"],
    ["慣行", "かんこう", "customary practice, habit, traditional event"],
    ["歓声", "かんせい", "cheer, shout of joy"],
    ["官僚", "かんりょう", "bureaucrat, bureaucracy"],
    ["器官", "きかん", "organ (of body, instrument)"],
    ["季刊", "きかん", "quarterly (e.g., magazine)"],
    ["起源", "きげん", "origin, beginning, rise"],
    ["機構", "きこう", "mechanism, organization"],
    ["築く", "きずく", "to build, to establish"],
    ["規制", "きせい", "regulation"],
    ["丘陵", "きゅうりょう", "hill"],
    ["協議", "きょうぎ", "conference, discussion, negotiation"],
    ["享受", "きょうじゅ", "enjoyment, being given"],
    ["協調", "きょうちょう", "co-operation, conciliation, harmony"],
    ["切り", "きり", "limits, place to leave off"],
    ["菌", "きん", "germ, bacterium"],
    ["近視", "きんし", "nearsightedness"],
    ["苦", "く", "trouble, worry, difficulty"],
    ["群", "ぐん", "group"],
    ["刑", "けい", "penalty, sentence, punishment"],
    ["経緯", "けいい", "sequence of events, course"],
    ["計器", "けいき", "meter, gauge"],
    ["契機", "けいき", "opportunity, chance"],
    ["携帯", "けいたい", "carrying something; mobile telephone"],
    ["形態", "けいたい", "form, shape, figure"],
    ["血管", "けっかん", "blood vessel"],
    ["決行", "けっこう", "doing (with resolve), carrying out (e.g., a plan)"],
    ["件", "けん", "matter, case, item"],
    ["減少", "げんしょう", "decrease, reduction, decline"],
    ["公演", "こうえん", "public performance"],
    ["後悔", "こうかい", "regret, repentance"],
    ["航海", "こうかい", "sail, voyage"],
    ["拘束", "こうそく", "restriction, restraint"],
    ["荒廃", "こうはい", "ruin"],
    ["降伏", "こうふく", "capitulation, surrender, submission"],
    ["興奮", "こうふん", "excitement, stimulation"],
    ["語句", "ごく", "words, phrases"],
    ["個々", "ここ", "individual, one by one"],
    ["故人", "こじん", "the deceased"],
    ["小銭", "こぜに", "coins, small change"],
    ["ことによると", "ことによると", "(depending on the circumstances)"],
    ["コンテスト", "コンテスト", "contest"],
    ["採集", "さいしゅう", "collecting, gathering"],
    ["作", "さく", "a work, a harvest"],
    ["策", "さく", "plan, policy"],
    ["設備", "せつび", "equipment, device, facilities"],
    ["絶滅", "ぜつめつ", "destruction, extinction"],
    ["節約", "せつやく", "economizing, saving"],
    ["攻める", "せめる", "to attack, to assault"],
    ["責める", "せめる", "to condemn, to blame, to criticize"],
    ["善", "ぜん", "good, virtue"],
    ["全", "ぜん", "all, whole, entire"],
    ["全員", "ぜんいん", "all members"],
    ["専攻", "せんこう", "major subject, special study"],
    ["全国", "ぜんこく", "the entire nation, country-wide, nation-wide"],
    ["洗剤", "せんざい", "detergent"],
    ["先日", "せんじつ", "the other day, a few days ago"],
    ["前者", "ぜんしゃ", "the former"],
    ["選手", "せんしゅ", "player selected for a team (usually athletic)"],
    ["全身", "ぜんしん", "the whole body, full-length (portrait)"],
    ["前進", "ぜんしん", "advance, drive, progress"],
    ["センター", "センター", "center"],
    ["全体", "ぜんたい", "whole, entirety, whatever (is the matter)"],
    ["選択", "せんたく", "selection, choice"],
    ["宣伝", "せんでん", "advertisement, publicity"],
    ["象", "ぞう", "elephant"],
    ["騒音", "そうおん", "noise"],
    ["増加", "ぞうか", "increase, addition"],
    ["操作", "そうさ", "operation, management, processing"],
    ["想像", "そうぞう", "imagination, guess"],
    ["相続", "そうぞく", "succession, inheritance"],
    ["装置", "そうち", "equipment, installation, apparatus"],
    ["相当", "そうとう", "considerably, fairly; worth ~"],
    ["速度", "そくど", "speed, velocity, rate"],
    ["底", "そこ", "bottom, sole"],
    ["そこで", "そこで", "so (conj.), accordingly"],
    ["組織", "そしき", "organization; structure; tissue"],
    ["そして", "そして", "and then"],
    ["注ぐ", "そそぐ", "to pour (into)"],
    ["育つ", "そだつ", "to be brought up, to grow (up)"],
    ["そっくり", "そっくり", "the splitting image of; entirely"],
    ["そっと", "そっと", "softly, gently"],
    ["袖", "そで", "sleeve"],
    ["備える", "そなえる", "to prepare, to furnish, to store"],
    ["具える", "そなえる", "to be furnished with"],
    ["そのうえ", "そのうえ", "in addition, furthermore"],
    ["そのうち", "そのうち", "before long, eventually, sooner or later"],
    ["そのまま", "そのまま", "without change, as it is (e.g., now)"],
    ["蕎麦", "そば", "soba (buckwheat noodles)"],
    ["ソファー", "ソファー", "sofa, couch"],
    ["粗末", "そまつ", "humble, miserable, crude"],
    ["それぞれ", "それぞれ", "each, every, respectively"],
    ["それでも", "それでも", "but (still), and yet, nevertheless"],
    ["それと", "それと", "and"],
    ["それとも", "それとも", "or, or else"],
    ["揃う", "そろう", "to become complete, to be equal"],
    ["揃える", "そろえる", "to put things in order, to arrange"],
    ["損", "そん", "loss, disadvantage"],
    ["損害", "そんがい", "damage, loss"],
    ["尊敬", "そんけい", "respect, reverence, honor"],
    ["存在", "そんざい", "existence, being"],
    ["尊重", "そんちょう", "respect, esteem, regard"],
    ["田", "た", "rice field"],
    ["他", "た", "other (esp. places and things)"],
    ["対", "たい", "pair, couple, set"],
    ["大", "だい", "big, great"],
    ["題", "だい", "title, subject, theme"],
    ["体育", "たいいく", "physical education, gymnastics, athletics"],
    ["体温", "たいおん", "temperature (body)"],
    ["大会", "たいかい", "convention, (big) tournament, mass meeting"],
    ["大気", "たいき", "atmosphere"],
    ["代金", "だいきん", "price, cost"],
    ["退屈", "たいくつ", "tedium, boring"],
    ["滞在", "たいざい", "stay, sojourn"],
    ["大使", "たいし", "ambassador"],
    ["大した", "たいした", "significant, great, considerable"],
    ["体重", "たいじゅう", "(body) weight"],
    ["対象", "たいしょう", "target; object (of worship, study, etc.); subject (i.e., of taxation)"],
    ["対照", "たいしょう", "contrast, antithesis, comparison"],
    ["大臣", "だいじん", "cabinet minister"],
    ["対する", "たいする", "to face, to confront, to oppose"],
    ["大戦", "たいせん", "great war, great battle"],
    ["態度", "たいど", "attitude, manner"],
    ["大統領", "だいとうりょう", "president"],
    ["大半", "たいはん", "most of, majority, mostly"],
    ["代表", "だいひょう", "representative, delegation"],
    ["大部分", "だいぶぶん", "most part, majority"],
    ["タイプライター", "タイプライター", "typewriter"],
    ["逮捕", "たいほ", "arrest, apprehension, capture"],
    ["題名", "だいめい", "title"],
    ["ダイヤ", "ダイヤ", "(railway) schedule; diamond"],
    ["太陽", "たいよう", "sun"],
    ["平ら", "たいら", "flatness, level, smooth"],
    ["代理", "だいり", "representation, proxy, deputy"],
    ["大陸", "たいりく", "continent"],
    ["倒す", "たおす", "to throw down, to beat"],
    ["タオル", "タオル", "(hand) towel"],
    ["だが", "だが", "but, however"],
    ["互い", "たがい", "mutual, each other"],
    ["高める", "たかめる", "to raise, to lift, to boost"],
    ["宝", "たから", "treasure"],
    ["宅", "たく", "house, home"],
    ["炊く", "たく", "to boil, to cook"],
    ["焚く", "たく", "to burn, to kindle, to build a fire"],
    ["抱く", "だく", "to embrace, to hug"],
    ["だけど", "だけど", "however"],
    ["たしか", "たしか", "certain, sure, if I remember correctly"],
    ["確かめる", "たしかめる", "to ascertain, to make sure"],
    ["多少", "たしょう", "a little (same as 少し (すこし))"],
    ["助かる", "たすかる", "to be saved; (something) helps (v.i.)"],
    ["助ける", "たすける", "to help (v.t.), to save, to rescue"],
    ["ただ", "ただ", "free of charge; just (~); only (~)"],
    ["只", "ただ", "free of charge, mere, only"],
    ["唯", "ただ", "mere, sole, plain"],
    ["戦い", "たたかい", "battle, fight"],
    ["戦う", "たたかう", "to fight, to compete, to battle"],
    ["叩く", "たたく", "to strike, to beat"],
    ["直ちに", "ただちに", "at once, immediately"],
    ["畳む", "たたむ", "to fold (clothes)"],
    ["立ち上がる", "たちあがる", "to stand up"],
    ["立場", "たちば", "standpoint, position, situation"],
    ["建つ", "たつ", "to stand; to be built (v.i.), to erect, to be erected"],
    ["経つ", "たつ", "(time) passes"],
    ["達する", "たっする", "to reach, to get to"],
    ["唯", "たった", "only, just"],
    ["だって", "だって", "but, because, even"],
    ["たっぷり", "たっぷり", "full, in plenty, ample"],
    ["たとえ", "たとえ", "even if, no matter, though"],
    ["谷", "たに", "valley"],
    ["他人", "たにん", "unrelated person, stranger"],
    ["種", "たね", "seed; material; cause"],
    ["束", "たば", "bunch, bundle"],
    ["足袋", "たび", "Japanese socks (with split toe)"],
    ["度", "たび", "counter for occurrences"],
    ["旅", "たび", "travel, trip, journey"],
    ["たびたび", "たびたび", "often, repeatedly, frequently"],
    ["玉", "たま", "ball, sphere, coin"],
    ["球", "たま", "globe, sphere, ball"],
    ["弾", "たま", "bullet, shot"],
    ["偶", "たま", "even number, couple, friend"],
    ["騙す", "だます", "to trick, to cheat, to deceive"],
    ["偶々", "たまたま", "unexpectedly, accidentally, by chance"],
    ["たまらない", "たまらない", "intolerable, unbearable, unendurable"],
    ["溜まる", "たまる", "to collect, to gather, to accumulate"],
    ["黙る", "だまる", "to be silent"],
    ["試し", "ためし", "trial, test"],
    ["試す", "ためす", "to attempt, to test"],
    ["溜める", "ためる", "to accumulate, to collect"],
    ["便り", "たより", "news, correspondence, letter"],
    ["頼る", "たよる", "to rely on, to have recourse to, to depend on"],
    ["段", "だん", "step, stair, grade"],
    ["単位", "たんい", "credit (for a course in school); unit, denomination"],
    ["単語", "たんご", "word; vocabulary"],
    ["男子", "だんし", "young man"],
    ["単純", "たんじゅん", "simplicity"],
    ["誕生", "たんじょう", "birth"],
    ["ダンス", "ダンス", "dance"],
    ["団体", "だんたい", "organization, association"],
    ["担当", "たんとう", "(in) charge"],
    ["単なる", "たんなる", "mere, simple, sheer"],
    ["単に", "たんに", "simply, merely"],
    ["地", "ち", "earth"],
    ["地位", "ちい", "(social) position, status"],
    ["地域", "ちいき", "area, region"],
    ["チーズ", "チーズ", "cheese"],
    ["チーム", "チーム", "team"],
    ["知恵", "ちえ", "wisdom, wit, intelligence"],
    ["地下", "ちか", "basement, underground"],
    ["違い", "ちがい", "difference, discrepancy"],
    ["違いない", "ちがいない", "(phrase) sure, no mistaking it, for certain"],
    ["近頃", "ちかごろ", "lately, recently, nowadays"],
    ["地球", "ちきゅう", "the earth"],
    ["地区", "ちく", "district, section"],
    ["遅刻", "ちこく", "lateness, late coming"],
    ["知事", "ちじ", "prefectural governor"],
    ["知識", "ちしき", "knowledge, information"],
    ["父親", "ちちおや", "father"],
    ["知能", "ちのう", "intelligence, brains"],
    ["地平線", "ちへいせん", "horizon"],
    ["地方", "ちほう", "area, locality, region"],
    ["茶", "ちゃ", "tea"],
    ["チャンス", "チャンス", "chance, opportunity"],
    ["ちゃんと", "ちゃんと", "perfectly, properly, exactly"],
    ["中", "ちゅう", "inside, middle, among"],
    ["注", "ちゅう", "annotation, explanatory note"],
    ["中央", "ちゅうおう", "central, center, middle"],
    ["中学", "ちゅうがく", "middle school, junior high school"],
    ["中古", "ちゅうこ", "used, second-hand"],
    ["中止", "ちゅうし", "suspension, stoppage, discontinuance"],
    ["駐車", "ちゅうしゃ", "parking (e.g., car)"],
    ["昼食", "ちゅうしょく", "lunch (same as 昼ご飯 (ひるごはん))"],
    ["中心", "ちゅうしん", "center, core"],
    ["注目", "ちゅうもく", "notice, attention, observation"],
    ["注文", "ちゅうもん", "order, request"],
    ["長期", "ちょうき", "long time period"],
    ["調査", "ちょうさ", "survey; investigation"],
    ["調子", "ちょうし", "condition, state, tune, tone"],
    ["頂上", "ちょうじょう", "top, summit, peak"],
    ["ちょうだい", "ちょうだい", "-- colloquial form of ください; typically used by children or women --"],
    ["貯金", "ちょきん", "(bank) savings"],
    ["直接", "ちょくせつ", "direct, immediate, firsthand"],
    ["著者", "ちょしゃ", "author, writer"],
    ["散らす", "ちらす", "to scatter, to disperse, to distribute"],
    ["散る", "ちる", "to fall, to scatter (e.g., blossoms)"],
    ["遂に", "ついに", "finally, at last"],
    ["通過", "つうか", "passage through, passing"],
    ["通貨", "つうか", "currency"],
    ["通学", "つうがく", "commuting to school"],
    ["通勤", "つうきん", "commuting to work"],
    ["通行", "つうこう", "passage, passing"],
    ["通じる", "つうじる", "to run to, to lead to, to communicate"],
    ["通信", "つうしん", "correspondence, communications"],
    ["通訳", "つうやく", "interpretation; interpreter"],
    ["捕まる", "つかまる", "to be arrested; to be caught"],
    ["掴む", "つかむ", "to seize, to catch, to grasp"],
    ["疲れ", "つかれ", "tiredness, fatigue"],
    ["付き合い", "つきあい", "socialization; friendship; association"],
    ["付合う", "つきあう", "to associate with, to keep company with, to get on with"],
    ["次々", "つぎつぎ", "in succession, one by one"],
    ["付く", "つく", "to adjoin, to be attached, to adhere"],
    ["就く", "つく", "to settle in (place), to take (seat position), to study (under teacher)"],
    ["突く", "つく", "to thrust, to strike; to poke"],
    ["次ぐ", "つぐ", "to rank next to, to come after"],
    ["注ぐ", "つぐ", "to pour (into)"],
    ["付ける", "つける", "to attach, to join, to add"],
    ["着ける", "つける", "to put on, to wear; to draw up"],
    ["伝わる", "つたわる", "to be handed down, to be transmitted"],
    ["土", "つち", "earth, soil"],
    ["続き", "つづき", "sequel, continuation"],
    ["包み", "つつみ", "bundle, package"],
    ["勤め", "つとめ", "work, employment"],
    ["務め", "つとめ", "duty"],
    ["繋がる", "つながる", "to be connected to, to be related to"],
    ["繋ぐ", "つなぐ", "to tie, to fasten, to connect"],
    ["繋げる", "つなげる", "to connect"],
    ["常に", "つねに", "always (same as いつも) (written expression)"],
    ["翼", "つばさ", "wings"],
    ["つまり", "つまり", "namely; in other words; in short"],
    ["罪", "つみ", "crime, fault, indiscretion"],
    ["積む", "つむ", "to pile up, to stack"],
    ["詰める", "つめる", "to pack, to shorten, to work out (details)"],
    ["積もる", "つもる", "to pile up"],
    ["梅雨", "つゆ", "rainy season"],
    ["辛い", "つらい", "painful, heart-breaking"],
    ["釣", "つり", "fishing"],
    ["連れ", "つれ", "companion, company"],
    ["出", "で", "outflow, coming (going) out, graduate (of)"],
    ["出会い", "であい", "meeting, rendezvous, encounter"],
    ["出合い", "であい", "an encounter"],
    ["出会う", "であう", "to meet by chance, to come across, to happen to encounter"],
    ["提案", "ていあん", "proposal, proposition, suggestion"],
    ["定期", "ていき", "fixed term"],
    ["抵抗", "ていこう", "resistance, opposition"],
    ["提出", "ていしゅつ", "presentation, submission, filing"],
    ["停電", "ていでん", "failure of electricity"],
    ["程度", "ていど", "degree, amount, level"],
    ["停留所", "ていりゅうじょ", "bus or tram stop"],
    ["デート", "デート", "date (in the sense of 'social engagement' only)"],
    ["敵", "てき", "enemy, rival"],
    ["出来事", "できごと", "incident happening, event"],
    ["適する", "てきする", "to fit, to suit"],
    ["適切", "てきせつ", "appropriate, adequate, relevance"],
    ["適度", "てきど", "moderate"],
    ["適用", "てきよう", "applying"],
    ["できれば", "できれば", "if possible…"],
    ["手品", "てじな", "conjuring trick, magic"],
    ["ですから", "ですから", "therefore"],
    ["鉄", "てつ", "iron"],
    ["哲学", "てつがく", "philosophy"],
    ["手伝い", "てつだい", "help, helper, assistant"],
    ["徹底", "てってい", "thoroughness, completeness"],
    ["鉄道", "てつどう", "railway; railroad"],
    ["徹夜", "てつや", "staying up all night, sleepless night"],
    ["手間", "てま", "time, labor"],
    ["デモ", "デモ", "demo, demonstration"],
    ["典型", "てんけい", "type, pattern, archetypal"],
    ["天候", "てんこう", "weather"],
    ["電子", "でんし", "electron; electronic"],
    ["テント", "テント", "tent"],
    ["伝統", "でんとう", "tradition, convention"],
    ["天然", "てんねん", "nature, spontaneity"],
    ["問い", "とい", "question, query"],
    ["党", "とう", "party (political)"],
    ["塔", "とう", "tower, pagoda"],
    ["答案", "とうあん", "examination paper, examination script"],
    ["同一", "どういつ", "identity, sameness, similarity"],
    ["銅貨", "どうか", "copper coin"],
    ["当時", "とうじ", "at that time, in those days (same as そのころ)"],
    ["動詞", "どうし", "verb"],
    ["同時", "どうじ", "simultaneous(ly), same time"],
    ["どうしても", "どうしても", "at any cost, no matter what, no matter how hard one tries"],
    ["どうぞよろしく", "どうぞよろしく", "pleased to meet you"],
    ["到着", "とうちゃく", "arrival"],
    ["道徳", "どうとく", "morals"],
    ["投票", "とうひょう", "voting, poll"],
    ["東洋", "とうよう", "Orient"],
    ["同様", "どうよう", "identical, same (kind), like"],
    ["童謡", "どうよう", "children's song, nursery rhyme"],
    ["同僚", "どうりょう", "colleague; co-worker"],
    ["道路", "どうろ", "road"],
    ["通す", "とおす", "to let pass, to overlook, to continue"],
    ["通り過ぎる", "とおりすぎる", "to pass, to pass through"],
    ["都会", "とかい", "city"],
    ["溶く", "とく", "to dissolve (paint)"],
    ["解く", "とく", "to unfasten; answer, solve"],
    ["退く", "どく", "to retreat, to recede, to withdraw"],
    ["毒", "どく", "poison, toxicant"],
    ["得意", "とくい", "pride, triumph, prosperity"],
    ["読書", "どくしょ", "reading"],
    ["独身", "どくしん", "single, unmarried"],
    ["特徴", "とくちょう", "characteristic(s); feature(s); trait(s)"],
    ["特長", "とくちょう", "forte, merit"],
    ["独特", "どくとく", "peculiarity, uniqueness, characteristic"],
    ["独立", "どくりつ", "independence (e.g., Ind. Day), self-support"],
    ["溶ける", "とける", "to melt, to thaw, to dissolve"],
    ["解ける", "とける", "to come untied, to come apart"],
    ["どこか", "どこか", "somewhere, anywhere"],
    ["ところが", "ところが", "however, while, even if"],
    ["ところで", "ところで", "by the way; even if"],
    ["登山", "とざん", "mountain-climbing"],
    ["都市", "とし", "town, city, urban"],
    ["年月", "としつき", "months and years"],
    ["図書", "としょ", "books"],
    ["年寄", "としより", "old people, the aged"],
    ["閉じる", "とじる", "to close (e.g., book, eyes), to shut"],
    ["途端", "とたん", "just (now, at the moment, etc.)"],
    ["土地", "とち", "plot of land, lot, soil"],
    ["突然", "とつぜん", "abruptly, suddenly, unexpectedly"],
    ["トップ", "トップ", "top"],
    ["届く", "とどく", "to reach"],
    ["とにかく", "とにかく", "anyhow, at any rate, anyway"],
    ["飛ばす", "とばす", "to skip over, to omit"],
    ["飛び出す", "とびだす", "to jump out, to rush out, to fly out"],
    ["留める", "とめる", "to fasten, to turn off, to detain"],
    ["泊める", "とめる", "to have someone stay (over night) (v.t.)"],
    ["友", "とも", "friend, companion, pal"],
    ["共に", "ともに", "sharing with, participate in"],
    ["虎", "とら", "tiger"],
    ["ドライブ", "ドライブ", "drive, driving"],
    ["トラック", "トラック", "truck; (running) track"],
    ["ドラマ", "ドラマ", "drama"],
    ["トランプ", "トランプ", "playing cards (lit: trump)"],
    ["取り上げる", "とりあげる", "to take up, to pick up; to confiscate"],
    ["努力", "どりょく", "great effort, exertion, endeavor"],
    ["トレーニング", "トレーニング", "training"],
    ["ドレス", "ドレス", "dress"],
    ["取れる", "とれる", "to come off, to be taken off"],
    ["泥", "どろ", "mud"],
    ["トン", "トン", "ton (1000 lbs.)"],
    ["とんでもない", "とんでもない", "outrageous, No way!"],
    ["どんなに", "どんなに", "how, how much"],
    ["トンネル", "トンネル", "tunnel"],
    ["名", "な", "name, reputation"],
    ["内容", "ないよう", "subject, contents, detail"],
    ["なお", "なお", "still, yet"],
    ["仲", "なか", "relation, relationship"],
    ["流す", "ながす", "to drain, to float, to shed (e.g., blood, tears)"],
    ["半ば", "なかば", "middle, halfway"],
    ["仲間", "なかま", "company, fellow, colleague, associate"],
    ["中身", "なかみ", "content(s); substance"],
    ["中味", "なかみ", "contents, interior, filling"],
    ["眺め", "ながめ", "scene; view; prospect; outlook"],
    ["眺める", "ながめる", "to view, to gaze at"],
    ["流れ", "ながれ", "stream, current"],
    ["流れる", "ながれる", "to flow, to be washed away"],
    ["亡くす", "なくす", "to lose someone"],
    ["殴る", "なぐる", "to strike; to hit; to punch"],
    ["無し", "なし", "without"],
    ["なぜなら", "なぜなら", "because"],
    ["謎", "なぞ", "riddle, puzzle, enigma"],
    ["納得", "なっとく", "consent, assent, understanding"],
    ["何か", "なにか", "something"],
    ["なにも", "なにも", "nothing"],
    ["鍋", "なべ", "saucepan, pot"],
    ["生", "なま", "raw, unprocessed"],
    ["怠ける", "なまける", "to be idle, to neglect"],
    ["波", "なみ", "wave"],
    ["涙", "なみだ", "tear"],
    ["悩む", "なやむ", "to agonize, to be troubled"],
    ["鳴らす", "ならす", "to ring, to sound"],
    ["為る", "なる", "to change, to be of use, to reach to"],
    ["生る", "なる", "to bear fruit"],
    ["馴れる", "なれる", "to become domesticated, to become tame"],
    ["縄", "なわ", "rope"],
    ["何で", "なんで", "Why, What for"],
    ["何でも", "なんでも", "by all means, everything"],
    ["何とか", "なんとか", "somehow, anyhow, one way or another"],
    ["似合う", "にあう", "to suit (a person), to match (clothing), (something) becomes (someone)"],
    ["煮える", "にえる", "to boil, to cook, to be cooked"],
    ["苦手", "にがて", "poor (at), weak (in), dislike (of)"],
    ["握る", "にぎる", "to grasp, to seize, to mold sushi"],
    ["日", "にち", "day"],
    ["日常", "にちじょう", "ordinary, regular, everyday"],
    ["日光", "にっこう", "sunlight"],
    ["日中", "にっちゅう", "daytime, broad daylight"],
    ["にっこり", "にっこり", "smile sweetly, smile, grin"],
    ["日本", "にっぽん", "Japan"],
    ["日本", "にほん", "Japan"],
    ["入場", "にゅうじょう", "entrance, admission, entering"],
    ["煮る", "にる", "to boil, to cook"],
    ["人気", "にんき", "popularity"],
    ["人間", "にんげん", "human being, person"],
    ["抜く", "ぬく", "to extract, to omit, to unplug"],
    ["抜ける", "ぬける", "to come out, to fall out, to be omitted"],
    ["布", "ぬの", "cloth"],
    ["濡らす", "ぬらす", "to wet, to soak"],
    ["根", "ね", "root"],
    ["値", "ね", "value, price, cost"],
    ["願い", "ねがい", "desire, wish, request"],
    ["願う", "ねがう", "to desire, to wish, to request"],
    ["鼠", "ねずみ", "mouse, rat"],
    ["熱帯", "ねったい", "tropics"],
    ["熱中", "ねっちゅう", "enthusiasm, zeal, mania"],
    ["年間", "ねんかん", "year"],
    ["年月", "ねんげつ", "months and years"],
    ["年中", "ねんじゅう", "whole year, always, everyday"],
    ["年代", "ねんだい", "age, era, period"],
    ["年齢", "ねんれい", "age, years"],
    ["野", "の", "field"],
    ["能", "のう", "being skilled in, nicely, properly"],
    ["農家", "のうか", "farmer, farm family"],
    ["農業", "のうぎょう", "agriculture"],
    ["農民", "のうみん", "farmers, peasants"],
    ["能力", "のうりょく", "ability, faculty"],
    ["ノー", "ノー", "no"],
    ["軒", "のき", "eaves"],
    ["残す", "のこす", "to leave (behind, over), to save, to reserve"],
    ["残り", "のこり", "remaining, left-over"],
    ["乗せる", "のせる", "to place on (something), to take on board"],
    ["載せる", "のせる", "to place on (something); to publish"],
    ["覗く", "のぞく", "to peek in, to look in"],
    ["除く", "のぞく", "to remove, to exclude, to except"],
    ["望み", "のぞみ", "wish, desire, (a) hope"],
    ["望む", "のぞむ", "to desire, to wish for; to view"],
    ["後", "のち", "afterwards, since then"],
    ["ノック", "ノック", "knock; fungo (baseball)"],
    ["伸ばす", "のばす", "to extend, to stretch, to reach out"],
    ["延ばす", "のばす", "to extend, to stretch, to reach out"],
    ["伸びる", "のびる", "to extend, to make progress, to grow"],
    ["延びる", "のびる", "to be prolonged"],
    ["述べる", "のべる", "to state, to express, to mention"],
    ["上る", "のぼる", "to ascend, to go up, to climb"],
    ["昇る", "のぼる", "to arise, to ascend, to go up"],
    ["載る", "のる", "to appear (in print), to be recorded"],
    ["のんびり", "のんびり", "carefree, at leisure"],
    ["場", "ば", "place, field (physics)"],
    ["はあ (かん)", "はあ (かん)", "(sigh)"],
    ["パーセント", "パーセント", "percent"],
    ["灰", "はい", "ash"],
    ["梅雨", "ばいう", "rainy season"],
    ["バイオリン", "バイオリン", "violin"],
    ["ハイキング", "ハイキング", "hiking"],
    ["配達", "はいたつ", "delivery, distribution"],
    ["パイプ", "パイプ", "pipe; channels official or otherwise"],
    ["俳優", "はいゆう", "actor, actress, performer"],
    ["パイロット", "パイロット", "pilot"],
    ["生える", "はえる", "to grow, to spring up; to cut (teeth)"],
    ["墓", "はか", "grave, tomb"],
    ["馬鹿", "ばか", "fool, idiot"],
    ["博士", "はかせ", "doctorate, PhD., doctor"],
    ["計る", "はかる", "to measure, to weigh, to survey"],
    ["量る", "はかる", "to measure, to weigh, to survey"],
    ["測る", "はかる", "to measure, to weigh, to survey"],
    ["掃く", "はく", "to sweep, to brush, to gather up"],
    ["吐く", "はく", "to throw up, to vomit"],
    ["拍手", "はくしゅ", "clapping hands, applause"],
    ["莫大", "ばくだい", "enormous, vast"],
    ["爆発", "ばくはつ", "explosion, detonation, eruption"],
    ["博物館", "はくぶつかん", "museum"],
    ["激しい", "はげしい", "violent, vehement, intense"],
    ["はさみ", "はさみ", "scissors"],
    ["破産", "はさん", "(personal) bankruptcy"],
    ["端", "はし", "end (e.g., of street), edge, margin"],
    ["始まり", "はじまり", "origin, beginning"],
    ["パス", "パス", "path, pass (in games)"],
    ["外す", "はずす", "to unfasten, to remove"],
    ["パスポート", "パスポート", "passport"],
    ["外れる", "はずれる", "to be disconnected, to be out (e.g., of gear)"],
    ["旗", "はた", "flag"],
    ["肌", "はだ", "skin"],
    ["裸", "はだか", "naked, nude"],
    ["畑", "はたけ", "field, patch"],
    ["働き", "はたらき", "work, labor"],
    ["バッグ", "バッグ", "bag"],
    ["発見", "はっけん", "discovery, detection, finding"],
    ["発行", "はっこう", "issue (publications)"],
    ["発車", "はっしゃ", "departure of a vehicle"],
    ["発射", "はっしゃ", "firing, shooting, discharge"],
    ["罰する", "ばっする", "to punish, to penalize"],
    ["発達", "はったつ", "development, growth"],
    ["ばったり", "ばったり", "(to meet) by chance"],
    ["発展", "はってん", "development, growth, progress"],
    ["発表", "はっぴょう", "announcement; publication; presentation"],
    ["発明", "はつめい", "invention"],
    ["派手", "はで", "showy, flashy, gaudy"],
    ["話し合う", "はなしあう", "to discuss, to talk together"],
    ["離す", "はなす", "to part, to separate"],
    ["放す", "はなす", "to separate, to set free"],
    ["離れる", "はなれる", "(something, someone) separates; parts from; to be apart"],
    ["放れる", "はなれる", "to leave, to get free, to cut oneself off"],
    ["羽", "はね", "wing"],
    ["羽根", "はね", "feather"],
    ["幅", "はば", "width, breadth"],
    ["母親", "ははおや", "mother"],
    ["省く", "はぶく", "to omit, to eliminate"],
    ["場面", "ばめん", "scene, setting (e.g., of novel)"],
    ["流行る", "はやる", "to be popular, to come into fashion"],
    ["腹", "はら", "abdomen, belly, stomach"],
    ["原", "はら", "field, plain"],
    ["バランス", "バランス", "balance"],
    ["針", "はり", "needle, hand (e.g., clock)"],
    ["範囲", "はんい", "extent, scope, range"],
    ["反抗", "はんこう", "opposition, resistance"],
    ["犯罪", "はんざい", "crime"],
    ["ハンサム", "ハンサム", "handsome"],
    ["反省", "はんせい", "reflection, reconsideration, regret"],
    ["判断", "はんだん", "judgment, decision"],
    ["犯人", "はんにん", "offender, criminal"],
    ["販売", "はんばい", "sale, selling, marketing"],
    ["灯", "ひ", "light"],
    ["ビール", "ビール", "beer"],
    ["被害", "ひがい", "damage"],
    ["比較", "ひかく", "comparison"],
    ["ピクニック", "ピクニック", "picnic"],
    ["悲劇", "ひげき", "tragedy"],
    ["飛行", "ひこう", "aviation, flight"],
    ["膝", "ひざ", "knee, lap"],
    ["非常", "ひじょう", "emergency, extraordinary, unusual"],
    ["美人", "びじん", "beautiful person (woman)"],
    ["額", "ひたい", "forehead, brow"],
    ["日付", "ひづけ", "date, dating"],
    ["引越し", "ひっこし", "moving, changing residence"],
    ["必死", "ひっし", "desperation, frantic, inevitable result"],
    ["ぴったり", "ぴったり", "exactly, neatly, sharp"],
    ["引っ張る", "ひっぱる", "to pull, to stretch, to drag"],
    ["否定", "ひてい", "negation, denial"],
    ["ビデオ", "ビデオ", "video tape; VCR"],
    ["一言", "ひとこと", "one word"],
    ["人込み", "ひとごみ", "crowd of people"],
    ["等しい", "ひとしい", "equal"],
    ["独り", "ひとり", "alone, unmarried"],
    ["一人一人", "ひとりひとり", "one by one, each"],
    ["批判", "ひはん", "criticism"],
    ["批評", "ひひょう", "criticism, review, commentary"],
    ["秘密", "ひみつ", "secret, secrecy"],
    ["微妙", "びみょう", "delicate, subtle"],
    ["紐", "ひも", "string, cord"],
    ["冷やす", "ひやす", "to cool, to refrigerate"],
    ["費用", "ひよう", "cost, expense"],
    ["表", "ひょう", "table (e.g., Tab 1), chart, list"],
    ["秒", "びょう", "second (60th min)"],
    ["評価", "ひょうか", "assessment, evaluation"],
    ["表現", "ひょうげん", "expression, presentation"],
    ["表情", "ひょうじょう", "facial expression"],
    ["平等", "びょうどう", "equality, impartiality, evenness"],
    ["評判", "ひょうばん", "fame, reputation"],
    ["表面", "ひょうめん", "surface, outside, face"],
    ["広がる", "ひろがる", "to spread (out), to extend, to stretch, to reach to, to get around"],
    ["広げる", "ひろげる", "to spread, to extend, to expand, to enlarge"],
    ["広める", "ひろめる", "to broaden, to propagate"],
    ["品", "ひん", "goods; taste, elegance"],
    ["瓶", "びん", "bottle"],
    ["便", "びん", "way, means; flight"],
    ["ピン", "ピン", "pin"],
    ["不", "ふ", "un(~), non(~), negative prefix"],
    ["不", "ぶ", "un(~), non(~), negative prefix"],
    ["無", "ぶ", "nothing, naught, nil, zero"],
    ["分", "ぶ", "dividing, part"],
    ["不安", "ふあん", "anxiety, uneasiness"],
    ["風景", "ふうけい", "scenery"],
    ["夫婦", "ふうふ", "married couple, husband and wife"],
    ["笛", "ふえ", "flute, whistle"],
    ["不可", "ふか", "wrong, bad, impossible"],
    ["深まる", "ふかまる", "to deepen"],
    ["武器", "ぶき", "weapon, arms"],
    ["拭く", "ふく", "to wipe, to dry"],
    ["服装", "ふくそう", "clothes, attire"],
    ["含む", "ふくむ", "to contain, to include"],
    ["含める", "ふくめる", "to include"],
    ["袋", "ふくろ", "bag, sack"],
    ["不幸", "ふこう", "unhappiness, sorrow, misfortune"],
    ["節", "ふし", "tune, tone, knob"],
    ["無事", "ぶじ", "safety, peace"],
    ["不思議", "ふしぎ", "mystery, curiosity"],
    ["不自由", "ふじゆう", "discomfort, disability, inconvenience"],
    ["夫人", "ふじん", "wife, Mrs, madam"],
    ["婦人", "ふじん", "woman (same as 女性 (じょせい)、女の人 (おんなのひと))"],
    ["不正", "ふせい", "injustice, unfairness"],
    ["防ぐ", "ふせぐ", "to defend (against), to protect, to prevent"],
    ["不足", "ふそく", "insufficiency, shortage"],
    ["舞台", "ぶたい", "stage (theater)"],
    ["双子", "ふたご", "twins, a twin"],
    ["再び", "ふたたび", "again, once more, a second time"],
    ["普段", "ふだん", "in everyday situations, usually, ordinarily"],
    ["縁", "ふち", "edge, rim"],
    ["打つ", "ぶつ", "to hit, to strike"],
    ["不通", "ふつう", "blockade, interruption, stoppage"],
    ["物価", "ぶっか", "(commodity/consumer) prices"],
    ["ぶつかる", "ぶつかる", "to strike, to collide with"],
    ["ぶつける", "ぶつける", "to knock, to strike hard, to hit and attack"],
    ["物質", "ぶっしつ", "material, substance"],
    ["物理", "ぶつり", "physics"],
    ["筆", "ふで", "writing brush"],
    ["ふと", "ふと", "suddenly, accidentally, incidentally"],
    ["部分", "ぶぶん", "portion, section, part"],
    ["不平", "ふへい", "complaint, discontent, dissatisfaction"],
    ["不満", "ふまん", "dissatisfaction, discontent, complaints"],
    ["増やす", "ふやす", "to increase (v.t.), to add to"],
    ["殖やす", "ふやす", "to increase, to add to"],
    ["プラス", "プラス", "plus"],
    ["プラスチック", "プラスチック", "plastic"],
    ["プラン", "プラン", "plan"],
    ["不利", "ふり", "disadvantage, drawback"],
    ["振る", "ふる", "to wave, to shake; to sprinkle; to cast (actor)"],
    ["震える", "ふるえる", "to shiver, to shake, to quake"],
    ["ブレーキ", "ブレーキ", "a brake"],
    ["触れる", "ふれる", "to touch, to feel, to violate"],
    ["プロ", "プロ", "professional"],
    ["分", "ぶん", "dividing, part, segment"],
    ["文", "ぶん", "sentence"],
    ["雰囲気", "ふんいき", "atmosphere (e.g., musical), mood, ambiance"],
    ["分析", "ぶんせき", "analysis"],
    ["文明", "ぶんめい", "civilization"],
    ["分野", "ぶんや", "field, sphere"],
    ["塀", "へい", "wall, fence"],
    ["平均", "へいきん", "equilibrium, balance, average"],
    ["平和", "へいわ", "peace"],
    ["別に", "べつに", "(not) particularly, nothing"],
    ["減らす", "へらす", "to decrease, to diminish"],
    ["減る", "へる", "to decrease (in size or number), to diminish"],
    ["ベルト", "ベルト", "Belt for western clothes"],
    ["変化", "へんか", "change, variation, shift"],
    ["ペンキ", "ペンキ", "paint"],
    ["変更", "へんこう", "change, modification, alteration"],
    ["ベンチ", "ベンチ", "bench"],
    ["弁当", "べんとう", "box lunch"],
    ["方", "ほう", "side"],
    ["法", "ほう", "Act (law: the X Act)"],
    ["棒", "ぼう", "pole, rod, stick"],
    ["冒険", "ぼうけん", "risk, venture, adventure"],
    ["方向", "ほうこう", "direction, course, way"],
    ["報告", "ほうこく", "report, information"],
    ["宝石", "ほうせき", "gem, jewel"],
    ["包装", "ほうそう", "packing, wrapping"],
    ["豊富", "ほうふ", "abundance, plenty"],
    ["方法", "ほうほう", "method, means, technique"],
    ["方々", "ほうぼう", "here and there, everywhere"],
    ["訪問", "ほうもん", "call, visit"],
    ["吠える", "ほえる", "to bark, to howl"],
    ["ボーイ", "ボーイ", "porter; boy"],
    ["ボート", "ボート", "rowing boat"],
    ["ホーム", "ホーム", "platform; home"],
    ["ボール", "ボール", "ball; bowl"],
    ["他", "ほか", "other (esp. places and things)"],
    ["誇り", "ほこり", "pride"],
    ["埃", "ほこり", "dust"],
    ["保証", "ほしょう", "guarantee, assurance, warranty"],
    ["保存", "ほぞん", "preservation, conservation"],
    ["歩道", "ほどう", "walkway"],
    ["仏", "ほとけ", "Buddha"],
    ["骨", "ほね", "bone"],
    ["炎", "ほのお", "flame"],
    ["頬", "ほほ", "cheek (of face)"],
    ["頬", "ほお", "cheek (of face)"],
    ["ほぼ", "ほぼ", "almost, roughly"],
    ["微笑む", "ほほえむ", "to smile"],
    ["堀", "ほり", "moat, canal"],
    ["濠", "ほり", "moat"],
    ["本当", "ほんと", "truth, reality"],
    ["本人", "ほんにん", "the person himself"],
    ["本物", "ほんもの", "genuine article"],
    ["ぼんやり", "ぼんやり", "dim, faint, vague"],
    ["間", "ま", "space, room, pause"],
    ["まあ", "まあ", "well (used when making a modest or hesitant statement)"],
    ["マーケット", "マーケット", "market"],
    ["マイク", "マイク", "mic"],
    ["迷子", "まいご", "lost (stray) child"],
    ["マイナス", "マイナス", "minus"],
    ["任せる", "まかせる", "to entrust to another, to leave to"],
    ["巻く", "まく", "to wind, to coil, to roll"],
    ["蒔く", "まく", "to sow (seeds)"],
    ["撒く", "まく", "to scatter, to sprinkle, to sow"],
    ["幕", "まく", "curtain, act (in play)"],
    ["負け", "まけ", "defeat, loss"],
    ["孫", "まご", "grandchild"],
    ["まさか", "まさか", "by no means"],
    ["まさに", "まさに", "correctly, surely"],
    ["混ざる", "まざる", "to be mixed, to mingle with"],
    ["交ざる", "まざる", "to be mixed, to mingle with"],
    ["混じる", "まじる", "to be mixed, to mingle with"],
    ["交じる", "まじる", "to be mixed, to mingle with"],
    ["増す", "ます", "to increase, to gain"],
    ["貧しい", "まずしい", "poor, needy"],
    ["マスター", "マスター", "bar owner, master (e.g., arts, science)"],
    ["ますます", "ますます", "increasingly (same as もっと), more and more"],
    ["混ぜる", "まぜる", "to mix, to stir"],
    ["交ぜる", "まぜる", "to be mixed, to be blended with"],
    ["街", "まち", "town; street, road"],
    ["間違い", "まちがい", "mistake"],
    ["松", "まつ", "pine tree"],
    ["真っ赤", "まっか", "deep red, flushed (of face)"],
    ["全く", "まったく", "really, completely"],
    ["祭", "まつり", "festival, feast"],
    ["まとまる", "まとまる", "to be collected, to be settled, to be in order"],
    ["まとめる", "まとめる", "to put in order, to collect, to bring to a conclusion"],
    ["学ぶ", "まなぶ", "to learn; to study"],
    ["真似", "まね", "mimicry, imitation, behavior"],
    ["招く", "まねく", "to invite"],
    ["まぶしい", "まぶしい", "dazzling, radiant"],
    ["柵", "さく", "fence, paling"],
    ["裂ける", "さける", "to split, to tear, to burst"],
    ["裁く", "さばく", "to judge"],
    ["酸化", "さんか", "oxidation"],
    ["死", "し", "death, decease"],
    ["資格", "しかく", "qualifications, requirements, capabilities"],
    ["視覚", "しかく", "sense of sight, vision"],
    ["指揮", "しき", "command, direction"],
    ["磁気", "じき", "magnetism"],
    ["磁器", "じき", "porcelain, china"],
    ["自己", "じこ", "self, oneself"],
    ["字体", "じたい", "font, lettering"],
    ["辞退", "じたい", "refusal"],
    ["視点", "してん", "opinion, point of view, visual point"],
    ["脂肪", "しぼう", "fat, grease"],
    ["志望", "しぼう", "wish, desire, ambition"],
    ["衆", "しゅう", "masses, people"],
    ["住", "じゅう", "dwelling, living"],
    ["修飾", "しゅうしょく", "ornamentation; modification (gram)"],
    ["私用", "しよう", "personal use, private business"],
    ["仕様", "しよう", "way, method, specification"],
    ["情", "じょう", "feelings, emotion, passion"],
    ["生涯", "しょうがい", "one's lifetime"],
    ["上司", "じょうし", "one's superior"],
    ["正体", "しょうたい", "natural shape, one's true colors, true character"],
    ["照明", "しょうめい", "illumination"],
    ["女史", "じょし", "Ms."],
    ["助詞", "じょし", "(gram) particle(s), postposition"],
    ["ショック", "ショック", "shock"],
    ["進行", "しんこう", "advance"],
    ["新興", "しんこう", "rising, developing, emergent"],
    ["振興", "しんこう", "promotion, encouragement"],
    ["申告", "しんこく", "report, statement"],
    ["真理", "しんり", "truth"],
    ["水洗", "すいせん", "flushing"],
    ["ストレス", "ストレス", "stress"],
    ["擦る", "する", "to rub, to chafe"],
    ["正規", "せいき", "regular, legitimate"],
    ["精巧", "せいこう", "elaborate, delicate, exquisite"],
    ["精算", "せいさん", "exact calculation, adjustment"],
    ["成年", "せいねん", "majority, adult age"],
    ["声明", "せいめい", "declaration, statement, proclamation"],
    ["姓名", "せいめい", "full name"],
    ["生理", "せいり", "physiology, menses"],
    ["節", "せつ", "when, if; section; clause"],
    ["膳", "ぜん", "(small) table, tray, meal"],
    ["禅", "ぜん", "Zen (Buddhism)"],
    ["選挙", "せんきょ", "election"],
    ["先行", "せんこう", "preceding, going first"],
    ["選考", "せんこう", "selection, screening"],
    ["相", "そう", "aspect, phase, countenance"],
    ["沿う", "そう", "to run along, to follow"],
    ["添う", "そう", "to accompany, to comply with"],
    ["僧", "そう", "monk, priest"],
    ["像", "ぞう", "statue, image"],
    ["捜査", "そうさ", "search (esp. in criminal investigations, investigation)"],
    ["操縦", "そうじゅう", "management, control, manipulation"],
    ["創造", "そうぞう", "creation"],
    ["隊", "たい", "party, troops"],
    ["退学", "たいがく", "dropping out of school"],
    ["タイトル", "タイトル", "title"],
    ["ダウン", "ダウン", "down"],
    ["高まる", "たかまる", "to rise; to grow; to mount (v.i.)"],
    ["断つ", "たつ", "to sever, to cut off"],
    ["盾", "たて", "shield"],
    ["例え", "たとえ", "example; even though"],
    ["チャイム", "チャイム", "chime"],
    ["挑戦", "ちょうせん", "challenge, defiance"],
    ["治療", "ちりょう", "medical treatment"],
    ["対", "つい", "pair, couple, set"],
    ["接ぐ", "つぐ", "to join; to piece together; to set (bones); to graft (trees)"],
    ["継ぐ", "つぐ", "to succeed (someone in a business or inheritance"],
    ["摘む", "つむ", "to pluck, to pick, to trim"],
    ["露", "つゆ", "dew"],
    ["強まる", "つよまる", "to get strong, to gain strength"],
    ["強める", "つよめる", "to strengthen, to emphasize"],
    ["データ", "データ", "data"],
    ["デザイン", "デザイン", "design"],
    ["デザート", "デザート", "dessert"],
    ["転校", "てんこう", "change schools"],
    ["伝言", "でんごん", "verbal message"],
    ["と", "と", "and"],
    ["問う", "とう", "to ask, to question"],
    ["棟", "とう", "ridge (of roof"],
    ["倒産", "とうさん", "(corporate) bankruptcy, insolvency"],
    ["同士", "どうし", "one another, companion, comrade"],
    ["同志", "どうし", "same mind, comrade, kindred soul"],
    ["当然", "とうぜん", "obvious; natural"],
    ["動揺", "どうよう", "disturbance, flutter shock"],
    ["説く", "とく", "to explain, to advocate"],
    ["綴じる", "とじる", "to bind, to file"],
    ["供", "とも", "accompanying, attendant, companion, retinue"],
    ["並", "なみ", "medium (e.g., food serving size, quality, price, etc.), ordinary"],
    ["慣らす", "ならす", "to accustom"],
    ["馴らす", "ならす", "to domesticate, to tame"],
    ["難", "なん", "difficulty, hardships, defect"],
    ["音", "ね", "sound, note"],
    ["年鑑", "ねんかん", "yearbook"],
    ["脳", "のう", "brain, memory"],
    ["臨む", "のぞむ", "to look out on, to face, to attend (function)"],
    ["肺", "はい", "lung"],
    ["～敗", "はい", "counter for loss, defeat"],
    ["映える", "はえる", "to shine, to look attractive, to look pretty"],
    ["諮る", "はかる", "to consult with, to confer"],
    ["図る", "はかる", "to plot, to attempt, to devise, to design, to refer A to B"],
    ["生やす", "はやす", "to grow, to cultivate, to wear beard"],
    ["班", "はん", "group, party, section (mil)"],
    ["判", "はん", "size (of paper or books)"],
    ["版", "はん", "edition"],
    ["碑", "ひ", "stone monument bearing an inscription"],
    ["非行", "ひこう", "delinquency, misconduct"],
    ["票", "ひょう", "label, ballot, sign"],
    ["広まる", "ひろまる", "to spread, to be propagated"],
    ["深める", "ふかめる", "to deepen, to heighten, to intensify"],
    ["福", "ふく", "good fortune"],
    ["振り", "ふり", "style, manner"],
    ["経る", "へる", "to pass, to elapse, to experience"],
    ["保護", "ほご", "care, protection, shelter"],
    ["保障", "ほしょう", "guarantee, security, warranty"],
    ["補償", "ほしょう", "compensation, reparation"],
    ["ほっと", "ほっと", "feel relieved"],
    ["前もって", "まえもって", "in advance, beforehand, previously"],
    ["膜", "まく", "membrane, film"],
    ["マスコミ", "マスコミ", "mass communication"],
    ["股", "また", "thigh, crotch"],
    ["末", "まつ", "the end of"],
    ["マッサージ", "マッサージ", "massage"],
    ["見掛ける", "みかける", "to (happen to see), to notice, to catch sight of"],
    ["捲る", "めくる", "to turn over, to turn pages of a book"],
    ["メッセージ", "メッセージ", "message"],
    ["野党", "やとう", "opposition party"],
    ["優", "ゆう", "superiority, high grade"],
    ["有機", "ゆうき", "organic"],
    ["世", "よ", "world, society, generation"],
    ["良い", "よい", "good, nice"],
    ["予想", "よそう", "expectation, anticipation, prediction"],
    ["弱まる", "よわまる", "to weaken, to be emaciated, to be dejected"],
    ["弱める", "よわめる", "to weaken"],
    ["ラベル", "ラベル", "label"],
    ["ルール", "ルール", "rule"],
    ["枠", "わく", "frame, slide"]
  ],
  "n2": [
    ["就任", "しゅうにん", "inauguration, assumption of office"],
    ["周辺", "しゅうへん", "circumference, peripheral"],
    ["重役", "じゅうやく", "director, high executive"],
    ["終了", "しゅうりょう", "end, close, termination"],
    ["重量", "じゅうりょう", "heavyweight"],
    ["重力", "じゅうりょく", "gravity"],
    ["熟語", "じゅくご", "idiom, kanji compound"],
    ["祝日", "しゅくじつ", "national holiday"],
    ["縮小", "しゅくしょう", "reduction, curtailment"],
    ["受験", "じゅけん", "taking an examination"],
    ["主語", "しゅご", "(gram) subject"],
    ["主人", "しゅじん", "(one's own) husband"],
    ["出勤", "しゅっきん", "going to work, at work"],
    ["述語", "じゅつご", "predicate"],
    ["出張", "しゅっちょう", "official tour, business trip"],
    ["寿命", "じゅみょう", "life span"],
    ["主役", "しゅやく", "leading part"],
    ["受話器", "じゅわき", "(telephone) receiver"],
    ["循環", "じゅんかん", "circulation, rotation, cycle"],
    ["巡査", "じゅんさ", "policeman"],
    ["順々", "じゅんじゅん", "in order, in turn"],
    ["順序", "じゅんじょ", "order, sequence, procedure"],
    ["純情", "じゅんじょう", "pure heart"],
    ["純粋", "じゅんすい", "pure, genuine, unmixed"],
    ["初～", "しょ～", "first ~"],
    ["諸～", "しょ～", "various ~"],
    ["～所", "～しょ", "place"],
    ["～所", "～じょ", "place"],
    ["女～", "じょ～", "things done by women"],
    ["～女", "～じょ", "count for sisters"],
    ["省～", "しょう～", "economizing ~"],
    ["～省", "～しょう", "kind of ministry"],
    ["～商", "～しょう", "merchant, business"],
    ["～勝", "～しょう", "count for victory"],
    ["～条", "～じょう", "counter for article"],
    ["～場", "～じょう", "kind of field, ground"],
    ["～畳", "～じょう", "count for tatami, mat"],
    ["消化", "しょうか", "digestion"],
    ["小学生", "しょうがくせい", "elementary school pupil"],
    ["しょうがない", "しょうがない", "It is not worth ~"],
    ["将棋", "しょうぎ", "Japanese chess"],
    ["蒸気", "じょうき", "steam, vapor"],
    ["定規", "じょうぎ", "(measuring) ruler"],
    ["上級", "じょうきゅう", "advanced level, high grade, senior"],
    ["商業", "しょうぎょう", "commerce, trade, business"],
    ["消極的", "しょうきょくてき", "passive"],
    ["賞金", "しょうきん", "prize, monetary award"],
    ["上下", "じょうげ", "high and low, up and down"],
    ["障子", "しょうじ", "paper sliding door"],
    ["商社", "しょうしゃ", "trading company"],
    ["乗車", "じょうしゃ", "taking a train, entraining"],
    ["上旬", "じょうじゅん", "first 10 days of month"],
    ["生ずる", "しょうずる", "to cause, to arise, to be generated"],
    ["小数", "しょうすう", "fraction (part of), decimal"],
    ["商店", "しょうてん", "shop, business firm"],
    ["焦点", "しょうてん", "focus, point"],
    ["消毒", "しょうどく", "disinfection"],
    ["勝敗", "しょうはい", "victory or defeat, issue (of battle)"],
    ["蒸発", "じょうはつ", "evaporation; unexplained disappearance"],
    ["上品", "じょうひん", "refined, elegant, well-mannered"],
    ["勝負", "しょうぶ", "victory or defeat, game"],
    ["小便", "しょうべん", "(col) urine, piss"],
    ["消防署", "しょうぼうしょ", "fire station"],
    ["正味", "しょうみ", "net (weight)"],
    ["正面", "しょうめん", "front"],
    ["消耗", "しょうもう", "exhaustion, consumption"],
    ["初級", "しょきゅう", "elementary level"],
    ["助教授", "じょきょうじゅ", "assistant professor"],
    ["～色", "～しょく", "kind of color"],
    ["食塩", "しょくえん", "table salt"],
    ["職人", "しょくにん", "artisan, craftsman"],
    ["職場", "しょくば", "workplace"],
    ["初旬", "しょじゅん", "first 10 days of the month"],
    ["書籍", "しょせき", "book, publication"],
    ["食器", "しょっき", "tableware"],
    ["ショップ", "ショップ", "a shop"],
    ["書店", "しょてん", "bookshop"],
    ["書道", "しょどう", "calligraphy"],
    ["初歩", "しょほ", "elements, rudiments"],
    ["白髪", "しらが", "white or grey hair, trendy hair bleaching"],
    ["シリーズ", "シリーズ", "series"],
    ["私立", "しりつ", "private (establishment)"],
    ["資料", "しりょう", "materials, data"],
    ["汁", "しる", "juice, soup"],
    ["素人", "しろうと", "layman, amateur, novice"],
    ["しわ (かおの～)", "しわ (かおの～)", "wrinkles, creases"],
    ["芯", "しん", "core, heart, wick"],
    ["新幹線", "しんかんせん", "Shinkansen, \"Bullet Train\""],
    ["真空", "しんくう", "vacuum"],
    ["人事", "じんじ", "human resources, personnel management"],
    ["信ずる", "しんずる", "to believe"],
    ["心身", "しんしん", "mind and body"],
    ["申請", "しんせい", "application, request, petition"],
    ["人造", "じんぞう", "man-made, synthetic, artificial"],
    ["寝台", "しんだい", "bed"],
    ["診断", "しんだん", "diagnosis"],
    ["侵入", "しんにゅう", "invasion, raid, trespass"],
    ["人文科学", "じんぶんかがく", "social sciences, humanities"],
    ["人命", "じんめい", "(human) life"],
    ["深夜", "しんや", "late at night"],
    ["森林", "しんりん", "forest, woods"],
    ["親類", "しんるい", "relative(s) (same as 親戚 (しんせき))"],
    ["針路", "しんろ", "course, direction"],
    ["神話", "しんわ", "myth, legend"],
    ["水産", "すいさん", "marine products, fisheries"],
    ["炊事", "すいじ", "cooking"],
    ["水蒸気", "すいじょうき", "water vapor, steam"],
    ["水素", "すいそ", "hydrogen"],
    ["垂直", "すいちょく", "vertical, perpendicular"],
    ["推定", "すいてい", "presumption, assumption, estimation"],
    ["水滴", "すいてき", "drop of water"],
    ["水筒", "すいとう", "canteen, flask, water bottle"],
    ["随筆", "ずいひつ", "essays, miscellaneous writings"],
    ["水分", "すいぶん", "moisture"],
    ["水平", "すいへい", "level, horizontal"],
    ["水平線", "すいへいせん", "horizon"],
    ["水面", "すいめん", "water's surface"],
    ["図々しい", "ずうずうしい", "impudent, shameless"],
    ["ずうっと", "ずうっと", "all the time, all the way"],
    ["末っ子", "すえっこ", "youngest child"],
    ["スカーフ", "スカーフ", "scarf"],
    ["図鑑", "ずかん", "picture book"],
    ["隙", "すき", "unguarded moment, chance"],
    ["杉", "すぎ", "Japanese cedar"],
    ["好き嫌い", "すききらい", "likes and dislikes, taste"],
    ["好き好き", "すきずき", "matter of taste"],
    ["透き通る", "すきとおる", "to be(come) transparent"],
    ["隙間", "すきま", "crack, gap, opening"],
    ["～過ぎる", "～すぎる", "too much ~"],
    ["スクール", "スクール", "school"],
    ["図形", "ずけい", "figure"],
    ["鈴", "すず", "bell"],
    ["涼む", "すずむ", "to cool oneself, to cool off"],
    ["スタート", "スタート", "start"],
    ["スチュワーデス", "スチュワーデス", "stewardess"],
    ["すっきり", "すっきり", "shapely, clear, neat"],
    ["ステージ", "ステージ", "stage; performance"],
    ["ストッキング", "ストッキング", "stockings"],
    ["ストップ", "ストップ", "stop"],
    ["素直", "すなお", "obedient, meek, docile"],
    ["頭脳", "ずのう", "head, brains, intellect"],
    ["スピーカー", "スピーカー", "speaker"],
    ["図表", "ずひょう", "chart, diagram, graph"],
    ["スマート", "スマート", "smart, stylish, slim"],
    ["住まい", "すまい", "dwelling, house"],
    ["すまない", "すまない", "sorry (phrase)"],
    ["～済", "～ずみ", "finished"],
    ["相撲", "すもう", "sumo wrestling"],
    ["スライド", "スライド", "slide"],
    ["ずらす", "ずらす", "to put off, to delay"],
    ["ずらり", "ずらり", "in a line, in a row"],
    ["スリッパ", "スリッパ", "slippers"],
    ["狡い", "ずるい", "sly, cunning"],
    ["寸法", "すんぽう", "measurement, size, dimension"],
    ["税関", "ぜいかん", "customs"],
    ["製作", "せいさく", "manufacture, production"],
    ["制作", "せいさく", "work (e.g., film, book)"],
    ["清書", "せいしょ", "clean copy"],
    ["青少年", "せいしょうねん", "youth, young person"],
    ["整数", "せいすう", "integer"],
    ["清掃", "せいそう", "cleaning"],
    ["生存", "せいぞん", "existence, being, survival"],
    ["政党", "せいとう", "(member of) political party"],
    ["性能", "せいのう", "ability, capability"],
    ["整備", "せいび", "maintenance, overhaul"],
    ["成分", "せいぶん", "ingredient, component, composition"],
    ["性別", "せいべつ", "sex, gender"],
    ["正方形", "せいほうけい", "square"],
    ["正門", "せいもん", "main gate, main entrance"],
    ["成立", "せいりつ", "formation, establishment, completion"],
    ["西暦", "せいれき", "Christian Era, after (Christ’s) death (A.D.)"],
    ["背負う", "せおう", "to be burdened with; to carry on (one's) back or shoulder(s)"],
    ["～席", "～せき", "counter for seats"],
    ["赤道", "せきどう", "equator"],
    ["折角", "せっかく", "with trouble, at great pains, long-awaited"],
    ["接近", "せっきん", "getting closer, drawing nearer, approaching"],
    ["接する", "せっする", "to attend to (someone); to associate with"],
    ["せっせと", "せっせと", "busily, away"],
    ["接続", "せつぞく", "connection, union, join, link; changing trains"],
    ["瀬戸物", "せともの", "earthenware, crockery, china"],
    ["ぜひとも", "ぜひとも", "by all means (with sense of not taking 'no' for an answer)"],
    ["迫る", "せまる", "to draw near, to press"],
    ["ゼミ", "ゼミ", "seminar"],
    ["せめて", "せめて", "at least"],
    ["セメント", "セメント", "cement"],
    ["台詞", "せりふ", "speech, words, one's lines, remarks"],
    ["栓", "せん", "stopper, cork, stopcock"],
    ["～船", "～せん", "counter for ships"],
    ["～戦", "～せん", "counter for games, matches"],
    ["前～", "ぜん～", "former, late ~, past ~"],
    ["～前", "～ぜん", "before ~"],
    ["前後", "ぜんご", "front and back, before and after"],
    ["全集", "ぜんしゅう", "complete works"],
    ["扇子", "せんす", "folding fan"],
    ["専制", "せんせい", "despotism, autocracy"],
    ["先々月", "せんせんげつ", "month before last"],
    ["先々週", "せんせんしゅう", "2 weeks before"],
    ["先祖", "せんぞ", "ancestor"],
    ["先端", "せんたん", "pointed end, tip"],
    ["センチ", "センチ", "centimeter"],
    ["先頭", "せんとう", "head, lead, vanguard"],
    ["全般", "ぜんぱん", "(the) whole, general"],
    ["扇風機", "せんぷうき", "electric fan"],
    ["洗面", "せんめん", "wash up (one's face), have a wash"],
    ["全力", "ぜんりょく", "all one's power, whole energy"],
    ["線路", "せんろ", "line, track, roadbed"],
    ["～沿い", "～そい", "along"],
    ["総～", "そう～", "gross, general, entire"],
    ["～艘", "～そう", "counter for ships"],
    ["相違", "そうい", "difference, discrepancy, variation"],
    ["そういえば", "そういえば", "which reminds me .."],
    ["雑巾", "ぞうきん", "house-cloth, dust cloth"],
    ["増減", "ぞうげん", "increase and decrease, fluctuation"],
    ["倉庫", "そうこ", "storehouse, warehouse"],
    ["相互", "そうご", "mutual, reciprocal"],
    ["創作", "そうさく", "production, creation, work"],
    ["葬式", "そうしき", "funeral"],
    ["造船", "ぞうせん", "shipbuilding"],
    ["騒々しい", "そうぞうしい", "noisy, boisterous"],
    ["増大", "ぞうだい", "increase, growth"],
    ["そうっと", "そうっと", "softly, cautiously, gently"],
    ["送別", "そうべつ", "farewell, send-off"],
    ["草履", "ぞうり", "Japanese sandals (footwear)"],
    ["総理大臣", "そうりだいじん", "Prime Minister"],
    ["送料", "そうりょう", "postage, carriage"],
    ["～足", "～そく", "counter for shoes"],
    ["属する", "ぞくする", "to belong to, to come under"],
    ["続々", "ぞくぞく", "successively, one after another"],
    ["速達", "そくたつ", "express, special delivery"],
    ["測定", "そくてい", "measurement"],
    ["測量", "そくりょう", "measurement, surveying"],
    ["速力", "そくりょく", "speed"],
    ["素質", "そしつ", "talent, capability"],
    ["祖先", "そせん", "ancestor(s)"],
    ["そそっかしい", "そそっかしい", "careless, thoughtless"],
    ["卒直", "そっちょく", "frank, candid, honest"],
    ["そのころ", "そのころ", "in those days, at that time, then"],
    ["そのため", "そのため", "hence, for that reason"],
    ["その他", "そのほか", "besides"],
    ["剃る", "そる", "to shave"],
    ["それなのに", "それなのに", "though, although"],
    ["それなら", "それなら", "If that's the case..., If so..., That being the case..."],
    ["それはいけませんね (かん)", "それはいけませんね (かん)", "that's not good"],
    ["逸れる", "それる", "to stray (turn) from subject, to go astray"],
    ["算盤", "そろばん", "abacus"],
    ["存じる", "ぞんじる", "(humble) to know"],
    ["存ずる", "ぞんずる", "(humble) to know"],
    ["損得", "そんとく", "loss and gain, advantage and disadvantage"],
    ["田ぼ", "たんぼ", "paddy field, farm"],
    ["第～", "だい～", "~th"],
    ["だいいち (とりわけ)", "だいいち (とりわけ)", "first, foremost,&nbsp;&nbsp;#1"],
    ["大学院", "だいがくいん", "graduate school"],
    ["大工", "だいく", "carpenter"],
    ["体系", "たいけい", "system, organization"],
    ["太鼓", "たいこ", "drum, tambourine"],
    ["対策", "たいさく", "counter-plan, counter-measure"],
    ["大して", "たいして", "(not so) much, (not) very"],
    ["大小", "だいしょう", "size"],
    ["体制", "たいせい", "order, system, structure"],
    ["体積", "たいせき", "capacity, volume"],
    ["大層", "たいそう", "very much, greatly"],
    ["体操", "たいそう", "gymnastics, physical exercises, calisthenics"],
    ["大分", "だいぶん", "considerably, greatly, a lot"],
    ["大木", "たいぼく", "large tree"],
    ["代名詞", "だいめいし", "pronoun"],
    ["タイア", "タイア", "tire, tyre"],
    ["ダイヤグラム", "ダイヤグラム", "diagram"],
    ["ダイヤモンド", "ダイヤモンド", "diamond"],
    ["ダイヤル", "ダイヤル", "dial"],
    ["対立", "たいりつ", "confrontation, opposition, antagonism"],
    ["田植え", "たうえ", "rice planting"],
    ["絶えず", "たえず", "constantly"],
    ["楕円", "だえん", "ellipse"],
    ["耕す", "たがやす", "to till, to plow, to cultivate"],
    ["滝", "たき", "waterfall"],
    ["蓄える", "たくわえる", "to save, to store, to lay in stock"],
    ["竹", "たけ", "bamboo"],
    ["ただいま", "ただいま", "Here I am, I'm home!"],
    ["但し", "ただし", "but, however, provided that"],
    ["立ち止まる", "たちどまる", "to stop, to halt, to stand still"],
    ["たちまち", "たちまち", "instantly, suddenly, all at once"],
    ["発", "はつ", "to depart (e.g., on a plane, train)"],
    ["脱線", "だっせん", "derailment, digression"],
    ["妥当", "だとう", "proper, appropriate"],
    ["例える", "たとえる", "to compare, to liken"],
    ["頼もしい", "たのもしい", "reliable, promising"],
    ["ダブル", "ダブル", "double"],
    ["ダム", "ダム", "dam"],
    ["溜息", "ためいき", "a sigh"],
    ["ためらう", "ためらう", "to be hesitant"],
    ["～だらけ", "～だらけ", "be full of ~, be filled with ~"],
    ["だらしない", "だらしない", "slovenly, loose, a slut"],
    ["足る", "たる", "to be sufficient, to be enough"],
    ["短～", "たん～", "short ~"],
    ["～団", "～だん", "group, corps, party"],
    ["オーケストラ", "オーケストラ", "orchestra"],
    ["おおざっぱ", "おおざっぱ", "rough (not precise), broad, sketchy"],
    ["大通り", "おおどおり", "main street"],
    ["オートメーション", "オートメーション", "automation"],
    ["大凡", "おおよそ", "about, approximately"],
    ["お帰り", "おかえり", "return, welcome"],
    ["おかけください", "おかけください", "please sit down"],
    ["おかげさまで", "おかげさまで", "Thanks to god, thanks to you"],
    ["おかず", "おかず", "side dish, accompaniment for rice dishes"],
    ["おかまいなく", "おかまいなく", "please don't fuss over me"],
    ["拝む", "おがむ", "to worship, to pray"],
    ["お代わり", "おかわり", "second helping, another cup"],
    ["補う", "おぎなう", "to compensate for"],
    ["お気の毒に", "おきのどくに", "I'm sorry to hear that…"],
    ["屋外", "おくがい", "outdoors"],
    ["送り仮名", "おくりがな", "part of word written in kana"],
    ["お元気で", "おげんきで", "Take care'"],
    ["怠る", "おこたる", "to neglect, to fail"],
    ["お先に", "おさきに", "before, after you"],
    ["伯父", "おじ", "(humble) uncle (older than one's parent)"],
    ["叔父", "おじ", "uncle (younger than one's parent)"],
    ["惜しい", "おしい", "regrettable, disappointing"],
    ["伯父さん", "おじさん", "(hon.) middle-aged gentleman, uncle"],
    ["小父さん", "おじさん", "(hon.) middle-aged gentleman, uncle"],
    ["叔父さん", "おじさん", "(hon.) middle-aged gentleman, uncle"],
    ["お邪魔します", "おじゃまします", "Excuse me for disturbing you"],
    ["お世話になりました", "おせわになりました", "I've been in your care"],
    ["お大事に", "おだいじに", "Take care of yourself, Take care!, Get well soon"],
    ["落着く", "おちつく", "to calm down, to settle down"],
    ["お出掛け", "おでかけ", "outing"],
    ["お手伝いさん", "おてつだいさん", "maid"],
    ["脅かす", "おどかす", "to threaten, to coerce"],
    ["落し物", "おとしもの", "lost property"],
    ["驚かす", "おどろかす", "to surprise, to frighten"],
    ["お願いします", "おねがいします", "Please (lit., I request)"],
    ["各々", "おのおの", "each, every, either"],
    ["伯母", "おば", "(humble) aunt (older than one's parent)"],
    ["叔母", "おば", "aunt (younger than one's parent)"],
    ["小母さん", "おばさん", "lady, woman, ma'am"],
    ["おはよう", "おはよう", "(abbr.) Good morning"],
    ["お参り", "おまいり", "worship, shrine visit"],
    ["お待たせしました", "おまたせしました", "Sorry to have kept you waiting"],
    ["お待ちください", "おまちください", "Please wait a moment"],
    ["おまちどおさま", "おまちどおさま", "Sorry to have kept you waiting"],
    ["おめでたい", "おめでたい", "happy event, matter for congratulation"],
    ["思い掛けない", "おもいがけない", "unexpected, casual"],
    ["思い切り", "おもいきり", "with all one's strength (heart), resignation, resolution"],
    ["思い込む", "おもいこむ", "to be under impression that, to be convinced that"],
    ["思いっ切り", "おもいっきり", "very, much, fully"],
    ["重たい", "おもたい", "heavy, massive, serious"],
    ["お休み", "おやすみ", "holiday, absence; (exp.) Good night"],
    ["おやつ", "おやつ", "between meal snack, afternoon refreshment"],
    ["親指", "おやゆび", "thumb"],
    ["オルガン", "オルガン", "organ"],
    ["恩恵", "おんけい", "blessing, benefit"],
    ["温室", "おんしつ", "greenhouse"],
    ["温泉", "おんせん", "spa, hot spring"],
    ["温帯", "おんたい", "temperate zone"],
    ["御中", "おんちゅう", "Messrs."],
    ["女の人", "おんなのひと", "woman"],
    ["～日", "～か", "counter for days"],
    ["～下", "～か", "under ~"],
    ["～化", "～か", "action of making something"],
    ["～科", "～か", "family, group, course"],
    ["～歌", "～か", "song of ~"],
    ["～画", "～が", "picture, painting"],
    ["カーブ", "カーブ", "curve; curve ball (baseball)"],
    ["外～", "がい～", "foreign ~, outside ~"],
    ["～外", "～がい", "out of ~"],
    ["開会", "かいかい", "opening of a meeting"],
    ["会館", "かいかん", "meeting hall, assembly hall"],
    ["改札", "かいさつ", "examination of tickets"],
    ["解散", "かいさん", "breakup, dissolution"],
    ["海水浴", "かいすいよく", "sea bathing, seawater bath"],
    ["回数", "かいすう", "number of times, frequency"],
    ["回数券", "かいすうけん", "book of tickets"],
    ["改正", "かいせい", "revision, amendment, alteration"],
    ["快晴", "かいせい", "good weather"],
    ["解説", "かいせつ", "explanation, commentary"],
    ["改造", "かいぞう", "remodeling"],
    ["開通", "かいつう", "opening, open"],
    ["回転", "かいてん", "rotation, turning"],
    ["解答", "かいとう", "answer, solution"],
    ["回答", "かいとう", "reply, answer"],
    ["外部", "がいぶ", "the outside, external"],
    ["解放", "かいほう", "release, liberation, emancipation"],
    ["開放", "かいほう", "open, throw open, liberalization"],
    ["海洋", "かいよう", "ocean"],
    ["概論", "がいろん", "introduction, general remark"],
    ["却って", "かえって", "on the contrary, rather"],
    ["家屋", "かおく", "house, building"],
    ["係わる", "かかわる", "to concern oneself in, to be involved in"],
    ["書留", "かきとめ", "registered mail"],
    ["書取", "かきとり", "dictation"],
    ["垣根", "かきね", "hedge"],
    ["限り", "かぎり", "limit(s), as far as possible"],
    ["各～", "かく～", "every ~, each ~"],
    ["架空", "かくう", "imaginary, fiction, fanciful"],
    ["各自", "かくじ", "individual, each"],
    ["拡充", "かくじゅう", "expansion"],
    ["学術", "がくじゅつ", "science, learning, scholarship"],
    ["各地", "かくち", "various parts of the country"],
    ["拡張", "かくちょう", "expansion, extension"],
    ["角度", "かくど", "angle"],
    ["学年", "がくねん", "year in school, grade in school"],
    ["格別", "かくべつ", "exceptional"],
    ["確率", "かくりつ", "probability"],
    ["学力", "がくりょく", "scholarship, knowledge"],
    ["掛け算", "かけざん", "multiplication"],
    ["可決", "かけつ", "approval, adoption (e.g., motion, bill), passage"],
    ["火口", "かこう", "crater (of a volcano)"],
    ["下降", "かこう", "decline, descent, fall"],
    ["火山", "かざん", "volcano"],
    ["かしこまりました", "かしこまりました", "Certainly"],
    ["貸し出し", "かしだし", "lending, loaning"],
    ["過失", "かしつ", "error, mistake, negligence"],
    ["果実", "かじつ", "fruit"],
    ["貸間", "かしま", "room to let"],
    ["貸家", "かしや", "house for rent"],
    ["箇所", "かしょ", "place, point, part"],
    ["過剰", "かじょう", "excess, over-"],
    ["かじる", "かじる", "to chew, to bite (at)"],
    ["課税", "かぜい", "taxation"],
    ["下線", "かせん", "underline, underscore"],
    ["カセット", "カセット", "cassette (tape)"],
    ["加速", "かそく", "acceleration"],
    ["加速度", "かそくど", "acceleration"],
    ["塊", "かたまり", "lump, mass, cluster"],
    ["固まる", "かたまる", "to harden, to solidify, to become firm"],
    ["片道", "かたみち", "one-way (trip)"],
    ["傾く", "かたむく", "to incline toward, to slant, to lurch"],
    ["片寄る", "かたよる", "to be one-sided, to incline, to be partial"],
    ["～がち", "～がち", "tend to do ~"],
    ["学科", "がっか", "study subject, course of study"],
    ["学会", "がっかい", "academic conference"],
    ["学級", "がっきゅう", "class"],
    ["担ぐ", "かつぐ", "to shoulder, to carry on shoulder"],
    ["括弧", "かっこ", "parenthesis, brackets"],
    ["活字", "かつじ", "printing type"],
    ["勝手に", "かってに", "arbitrarily,"],
    ["活力", "かつりょく", "vitality, energy"],
    ["仮名", "かな", "kana"],
    ["仮名遣い", "かなづかい", "kana orthography, syllabary spelling"],
    ["加熱", "かねつ", "heating"],
    ["兼ねる", "かねる", "to simultaneously serve two or more functions"],
    ["カバー", "カバー", "cover (e.g., book)"],
    ["過半数", "かはんすう", "majority"],
    ["かび (～がはえる)", "かび (～がはえる)", "mold, mildew"],
    ["被せる", "かぶせる", "to cover (with something)"],
    ["釜", "かま", "iron pot, kettle"],
    ["構いません", "かまいません", "it's all right; one doesn’t mind"],
    ["紙屑", "かみくず", "wastepaper"],
    ["神様", "かみさま", "god"],
    ["剃刀", "かみそり", "razor"],
    ["ガム", "ガム", "chewing gum"],
    ["貨物", "かもつ", "cargo, freight"],
    ["カラー", "カラー", "collar, color"],
    ["からかう", "からかう", "to ridicule, to make fun of"],
    ["空っぽ", "からっぽ", "empty, vacant, hollow"],
    ["かるた", "かるた", "playing cards (POR: carta)"],
    ["枯れる", "かれる", "to wither, to die (plant), to be blasted (plant)"],
    ["カロリー", "カロリー", "calorie"],
    ["可愛がる", "かわいがる", "to love, to be affectionate"],
    ["為替", "かわせ", "money order, exchange"],
    ["瓦", "かわら", "roof tile"],
    ["～刊", "～かん", "~ issued (magazine, newspaper)"],
    ["～間", "～かん", "between, during"],
    ["～巻", "～かん", "volume"],
    ["～館", "～かん", "~ hall, ~ building"],
    ["～感", "～かん", "feeling, sense, impression"],
    ["換気", "かんき", "ventilation"],
    ["感激", "かんげき", "deep emotion, impression, inspiration"],
    ["関西", "かんさい", "south-western half of Japan, including Osaka"],
    ["元日", "がんじつ", "New Year's Day"],
    ["鑑賞", "かんしょう", "appreciation"],
    ["感ずる", "かんずる", "to feel, to sense"],
    ["間接", "かんせつ", "indirect, indirectness"],
    ["観測", "かんそく", "observation"],
    ["寒帯", "かんたい", "frigid zone"],
    ["官庁", "かんちょう", "government office, authorities"],
    ["勘違い", "かんちがい", "misunderstanding, wrong guess"],
    ["缶詰", "かんづめ", "canning, canned goods,"],
    ["乾電池", "かんでんち", "dry cell, battery"],
    ["関東", "かんとう", "eastern half of Japan, including Tokyo"],
    ["観念", "かんねん", "idea, notion; sense"],
    ["乾杯", "かんぱい", "Cheers! (a toast)"],
    ["看板", "かんばん", "sign, signboard"],
    ["看病", "かんびょう", "nursing (a patient)"],
    ["冠", "かんむり", "crown, wreath"],
    ["漢和", "かんわ", "Chinese Character-Japanese (e.g., dictionary)"],
    ["～期", "～き", "~age, ~period"],
    ["～器", "～き", "device, equipment"],
    ["～機", "～き", "machine"],
    ["気圧", "きあつ", "atmospheric pressure"],
    ["着替え", "きがえ", "changing clothes, change of clothes"],
    ["着替える", "きがえる", "to change (one's) clothes"],
    ["機関車", "きかんしゃ", "locomotive, engine"],
    ["飢饉", "ききん", "famine"],
    ["器具", "きぐ", "instrument"],
    ["記号", "きごう", "symbol, code"],
    ["刻む", "きざむ", "to mince, to carve, to engrave"],
    ["儀式", "ぎしき", "ceremony, rite, ritual"],
    ["基準", "きじゅん", "standard, basis, criteria"],
    ["規準", "きじゅん", "standard, basis, criteria"],
    ["起床", "きしょう", "rising, getting out of bed"],
    ["着せる", "きせる", "to put on clothes"],
    ["基礎", "きそ", "foundation, basis"],
    ["基地", "きち", "base"],
    ["きっかけ", "きっかけ", "prompt, trigger, cue"],
    ["ぎっしり", "ぎっしり", "tightly, fully"],
    ["基盤", "きばん", "foundation, basis"],
    ["～気味", "～ぎみ", "slightly ~"],
    ["客席", "きゃくせき", "guest seating"],
    ["客間", "きゃくま", "parlor, guest room"],
    ["ギャング", "ギャング", "gang"],
    ["キャンパス", "キャンパス", "campus"],
    ["休業", "きゅうぎょう", "closure, shutdown, holiday"],
    ["休講", "きゅうこう", "lecture canceled"],
    ["給与", "きゅうよ", "salary"],
    ["休養", "きゅうよう", "rest, break, recreation"],
    ["清い", "きよい", "clear, pure, noble"],
    ["～教", "～きょう", "religion"],
    ["～行", "～ぎょう", "line, row"],
    ["～業", "～ぎょう", "type of business"],
    ["強化", "きょうか", "strengthen, intensify, reinforce"],
    ["境界", "きょうかい", "boundary"],
    ["共産～", "きょうさん～", "communist ~"],
    ["行事", "ぎょうじ", "event, function"],
    ["恐縮", "きょうしゅく", "sorry to trouble"],
    ["教養", "きょうよう", "culture, education, sophistication"],
    ["行列", "ぎょうれつ", "line, procession; matrix (math)"],
    ["漁業", "ぎょぎょう", "fishing (industry)"],
    ["曲線", "きょくせん", "curve"],
    ["規律", "きりつ", "order, rules, law"],
    ["斬る", "きる", "to behead, to murder"],
    ["～きる", "～きる", "nevertheless, to carry through"],
    ["～切れ", "～きれ", "out of ~"],
    ["気を付ける", "きをつける", "to be careful, to pay attention, to take care"],
    ["金魚", "きんぎょ", "goldfish"],
    ["区域", "くいき", "zone, district, area"],
    ["空～", "くう～", "empty ~"],
    ["偶数", "ぐうすう", "even number"],
    ["空想", "くうそう", "daydream, fantasy"],
    ["空中", "くうちゅう", "sky, air"],
    ["クーラー", "クーラー", "air conditioner"],
    ["釘", "くぎ", "nail"],
    ["区切る", "くぎる", "to punctuate, to cut off, to mark off"],
    ["櫛", "くし", "comb"],
    ["くしゃみ", "くしゃみ", "sneeze"],
    ["苦情", "くじょう", "complaint, grievance, grumble"],
    ["苦心", "くしん", "pain, trouble"],
    ["屑", "くず", "waste, scrap"],
    ["崩す", "くずす", "to destroy, to make change (money)"],
    ["薬指", "くすりゆび", "ring finger"],
    ["崩れる", "くずれる", "to collapse, to crumble"],
    ["砕く", "くだく", "to break, to smash"],
    ["砕ける", "くだける", "to break, to be broken"],
    ["くたびれる", "くたびれる", "to get tired, to wear out"],
    ["くだらない", "くだらない", "good-for-nothing, stupid, worthless"],
    ["～口", "～くち", "~ opening; ~ entrance, ~ exit"],
    ["唇", "くちびる", "lip"],
    ["口紅", "くちべに", "lipstick"],
    ["くっつく", "くっつく", "to adhere to, to keep close to"],
    ["くっつける", "くっつける", "to attach"],
    ["くどい", "くどい", "verbose, importunate, heavy (taste)"],
    ["句読点", "くとうてん", "punctuation marks"],
    ["配る", "くばる", "to distribute, to deliver"],
    ["工夫", "くふう", "device, artifice, ingenuity"],
    ["区分", "くぶん", "division, section, classification"],
    ["組合せ", "くみあわせ", "combination"],
    ["組み立てる", "くみたてる", "to assemble, to set up, to construct"],
    ["悔やむ", "くやむ", "to regret, to mourn"],
    ["クリーニング", "クリーニング", "cleaning, dry cleaning, laundry service"],
    ["くるむ", "くるむ", "to be enveloped by, to wrap up"],
    ["くれぐれも", "くれぐれも", "repeatedly, sincerely, earnestly"],
    ["～家", "～け", "~'s family, the house of ~"],
    ["～形", "～けい", "shape of ~"],
    ["～系", "～けい", "~ system, ~ lineage, ~ group"],
    ["稽古", "けいこ", "practice, training, study"],
    ["敬語", "けいご", "honorific language (lit., respect language)"],
    ["蛍光灯", "けいこうとう", "fluorescent lamp"],
    ["形式", "けいしき", "form, formality, format"],
    ["継続", "けいぞく", "continuation"],
    ["毛糸", "けいと", "knitting wool"],
    ["経度", "けいど", "longitude"],
    ["系統", "けいとう", "system, genealogy"],
    ["芸能", "げいのう", "public entertainment, performing arts"],
    ["競馬", "けいば", "horse racing"],
    ["警備", "けいび", "defense, guard, policing, security"],
    ["形容詞", "けいようし", "adjective"],
    ["形容動詞", "けいようどうし", "adjectival noun, quasi-adjective"],
    ["外科", "げか", "surgical department"],
    ["毛皮", "けがわ", "fur, skin, pelt"],
    ["激増", "げきぞう", "sudden increase"],
    ["下車", "げしゃ", "alighting, getting off"],
    ["下旬", "げじゅん", "month (last third of)"],
    ["下水", "げすい", "drainage, sewage, ditch, gutter, sewerage"],
    ["削る", "けずる", "to cut down little by little, to take a percentage"],
    ["桁", "けた", "column, beam, digit"],
    ["下駄", "げた", "(Japanese footwear), wooden clogs"],
    ["血圧", "けつあつ", "blood pressure"],
    ["月給", "げっきゅう", "monthly salary"],
    ["傑作", "けっさく", "masterpiece, best work"],
    ["月末", "げつまつ", "end of the month"],
    ["気配", "けはい", "indication, sign, hint"],
    ["下品", "げひん", "vulgar, indecent, coarse"],
    ["煙い", "けむい", "smoky"],
    ["険しい", "けわしい", "steep, rugged; severe"],
    ["～圏", "～けん", "bloc, sphere, area"],
    ["現～", "げん～", "present, incumbent"],
    ["見学", "けんがく", "tour, study by observation"],
    ["謙虚", "けんきょ", "modesty, humble"],
    ["原稿", "げんこう", "manuscript, copy"],
    ["原産", "げんさん", "place of origin"],
    ["原始", "げんし", "origin, primeval"],
    ["研修", "けんしゅう", "training"],
    ["厳重", "げんじゅう", "strict, severe, firm"],
    ["謙遜", "けんそん", "humble, humility, modesty"],
    ["県庁", "けんちょう", "prefectural office"],
    ["限度", "げんど", "limit, bounds"],
    ["現に", "げんに", "actually, really"],
    ["顕微鏡", "けんびきょう", "microscope"],
    ["懸命", "けんめい", "eagerness, strenuous"],
    ["原理", "げんり", "principle, theory, fundamental truth"],
    ["原料", "げんりょう", "raw materials"],
    ["小～", "こ～", "small ~"],
    ["恋しい", "こいしい", "dear, beloved; to miss"],
    ["高～", "こう～", "high (level) ~"],
    ["～校", "～こう", "counter for school"],
    ["～港", "～こう", "~ port"],
    ["～号", "～ごう", "counter for magazine; the name of ship"],
    ["工員", "こういん", "factory worker"],
    ["強引", "ごういん", "forcible, assertive, pushy"],
    ["公害", "こうがい", "public nuisance, pollution"],
    ["高級", "こうきゅう", "high class; first-rate"],
    ["公共", "こうきょう", "public, community, communal"],
    ["工芸", "こうげい", "industrial arts"],
    ["孝行", "こうこう", "filial piety"],
    ["交差", "こうさ", "cross"],
    ["講師", "こうし", "lecturer"],
    ["工事", "こうじ", "construction work"],
    ["公式", "こうしき", "formula, formality, official"],
    ["口実", "こうじつ", "excuse"],
    ["こうして", "こうして", "like this, with this"],
    ["公衆", "こうしゅう", "the public"],
    ["香水", "こうすい", "perfume"],
    ["功績", "こうせき", "achievements, merit"],
    ["光線", "こうせん", "beam, light ray"],
    ["高層", "こうそう", "tall, high rise"],
    ["構造", "こうぞう", "structure, construction"],
    ["交替", "こうたい", "change, relief, alteration"],
    ["耕地", "こうち", "arable land"],
    ["交通機関", "こうつうきかん", "transportation facilities"],
    ["校庭", "こうてい", "school yard"],
    ["肯定", "こうてい", "positive, affirmation"],
    ["高度", "こうど", "altitude, height; advanced"],
    ["高等", "こうとう", "high class, high grade"],
    ["合同", "ごうどう", "combination, incorporation"],
    ["高等学校", "こうとうがっこう", "senior high school"],
    ["公表", "こうひょう", "official announcement, proclamation"],
    ["鉱物", "こうぶつ", "mineral"],
    ["公務", "こうむ", "official business, public business"],
    ["項目", "こうもく", "item"],
    ["紅葉", "こうよう", "fall colors (of leaves)"],
    ["合理", "ごうり", "rational"],
    ["交流", "こうりゅう", "exchange; alternating current"],
    ["合流", "ごうりゅう", "confluence, merge, join"],
    ["効力", "こうりょく", "effect, efficacy"],
    ["コース", "コース", "course"],
    ["コーラス", "コーラス", "chorus"],
    ["焦がす", "こがす", "to burn, to scorch"],
    ["～国", "～こく", "nation of ~"],
    ["国王", "こくおう", "king"],
    ["国立", "こくりつ", "national"],
    ["ご苦労様", "ごくろうさま", "Thank you for your had work"],
    ["焦げる", "こげる", "to burn, to be burned"],
    ["凍える", "こごえる", "to freeze, to be chilled, to be frozen"],
    ["心当たり", "こころあたり", "having some knowledge of, happening to know"],
    ["心得る", "こころえる", "to understand, to have thorough knowledge"],
    ["腰掛け", "こしかけ", "seat, bench"],
    ["腰掛ける", "こしかける", "to sit (down)"],
    ["五十音", "ごじゅうおん", "the Japanese syllabary"],
    ["こしらえる", "こしらえる", "to make, to manufacture"],
    ["擦る", "こする", "to rub, to chafe, to file, to frost (glass), to strike (match)"],
    ["個体", "こたい", "an individual"],
    ["ごちそうさま", "ごちそうさま", "Thank you for the meal"],
    ["こちらこそ", "こちらこそ", "it is I who should say so"],
    ["小遣い", "こづかい", "pocket money, allowance"],
    ["コック", "コック", "cook; tap, cock"],
    ["こっそり", "こっそり", "stealthily, secretly"],
    ["古典", "こてん", "classics, classic"],
    ["～毎", "～ごと", "every ~, each ~"],
    ["〜 (まる) ごと", "〜 (まる) ごと", "whole ~, all of ~"],
    ["言付ける", "ことづける", "to leave a message"],
    ["言葉遣い", "ことばづかい", "speech, expression, wording"],
    ["こないだ", "こないだ", "the other day, lately, recently"],
    ["御無沙汰", "ごぶさた", "not writing or contacting for a while"],
    ["ゴム", "ゴム", "gum, rubber"],
    ["御免", "ごめん", "declining (something); pardon, sorry"],
    ["ごめんください", "ごめんください", "May I come in, Is anyone here"],
    ["小指", "こゆび", "little finger"],
    ["堪える", "こらえる", "to bear, to endure, to put up with"],
    ["娯楽", "ごらく", "pleasure, amusement, recreation"],
    ["御覧", "ごらん", "(hon.) look, inspection, try"],
    ["コレクション", "コレクション", "collection; correction"],
    ["転がす", "ころがす", "to roll"],
    ["転がる", "ころがる", "to roll, to tumble"],
    ["紺", "こん", "navy blue, deep blue"],
    ["今～", "こん～", "this, current"],
    ["コンクール", "コンクール", "contest (FRE: concours)"],
    ["コンクリート", "コンクリート", "concrete"],
    ["混合", "こんごう", "mixing, mixture"],
    ["コンセント", "コンセント", "consent; power outlet"],
    ["献立", "こんだて", "menu"],
    ["こんばんは", "こんばんは", "good evening"],
    ["サークル", "サークル", "circle, sports club (e.g., at a company)"],
    ["再～", "さい～", "re ~"],
    ["最～", "さい～", "the most ~"],
    ["在学", "ざいがく", "(enrolled) in school"],
    ["再三", "さいさん", "again and again, repeatedly"],
    ["祭日", "さいじつ", "national holiday, festival day"],
    ["催促", "さいそく", "demand, urge (action), press for"],
    ["採点", "さいてん", "marking, grading"],
    ["災難", "さいなん", "calamity, misfortune"],
    ["裁縫", "さいほう", "sewing"],
    ["材木", "ざいもく", "lumber, timber"],
    ["サイレン", "サイレン", "siren"],
    ["逆さ", "さかさ", "reverse, upside down"],
    ["逆様", "さかさま", "reverse, upside down"],
    ["捜す", "さがす", "to search, to seek, to look for"],
    ["遡る", "さかのぼる", "to go back, to date back; ascend"],
    ["酒場", "さかば", "bar, bar-room"],
    ["一昨昨日", "さきおととい", "two days before yesterday, three days ago"],
    ["先程", "さきほど", "a little while ago"],
    ["索引", "さくいん", "index, indices"],
    ["作者", "さくしゃ", "author, artist"],
    ["削除", "さくじょ", "elimination, deletion"],
    ["作成", "さくせい", "creation, preparation, to make"],
    ["作製", "さくせい", "manufacture, production"],
    ["探る", "さぐる", "to search, to look for, investigate"],
    ["囁く", "ささやく", "to whisper, to murmur"],
    ["匙", "さじ", "spoon"],
    ["座敷", "ざしき", "tatami room"],
    ["差し支え", "さしつかえ", "hindrance, impediment"],
    ["差し引き", "さしひき", "deduction, balance"],
    ["刺身", "さしみ", "sliced raw fish"],
    ["(かさを～) さす", "(かさを～) さす", "to open; hold (an umbrella)"],
    ["流石", "さすが", "indeed, truly, as one would expect"],
    ["撮影", "さつえい", "photographing"],
    ["雑音", "ざつおん", "noise (jarring, grating)"],
    ["さっさと", "さっさと", "quickly"],
    ["早速", "さっそく", "at once, immediately, promptly"],
    ["錆", "さび", "rust (color)"],
    ["錆びる", "さびる", "to rust, to become rusty"],
    ["座布団", "ざぶとん", "cushion (Japanese)"],
    ["妨げる", "さまたげる", "to disturb, to prevent"],
    ["さようなら", "さようなら", "good-bye"],
    ["サラリーマン", "サラリーマン", "salaryman; company employee"],
    ["騒がしい", "さわがしい", "noisy"],
    ["さわやか", "さわやか", "fresh, refreshing"],
    ["～山", "～さん", "the name of mountain"],
    ["～産", "～さん", "made in ~"],
    ["三角", "さんかく", "triangle, triangular"],
    ["算数", "さんすう", "arithmetic"],
    ["産地", "さんち", "producing area"],
    ["サンプル", "サンプル", "sample"],
    ["山林", "さんりん", "mountain forest"],
    ["～史", "～し", "history of ~"],
    ["～紙", "～し", "newspaper, type of paper"],
    ["～寺", "～じ", "the name of temple"],
    ["仕上がる", "しあがる", "to be finished"],
    ["明明後日", "しあさって", "two days after tomorrow"],
    ["シーズン", "シーズン", "season (sporting)"],
    ["シーツ", "シーツ", "sheet"],
    ["寺院", "じいん", "temple"],
    ["しいんと (する)", "しいんと (する)", "silent (as the grave), (deathly) quiet"],
    ["自衛", "じえい", "self-defense"],
    ["塩辛い", "しおからい", "salty (taste)"],
    ["司会", "しかい", "host, chairperson"],
    ["四角い", "しかくい", "square"],
    ["仕方がない", "しかたがない", "it can't be helped, it's inevitable"],
    ["～時間目", "～じかんめ", "~th hour, ~th period"],
    ["時間割", "じかんわり", "timetable, schedule"],
    ["〜(日本) 式", "～(にほん) しき", "custom,"],
    ["敷地", "しきち", "site"],
    ["敷く", "しく", "to spread out, to lay out"],
    ["茂る", "しげる", "to grow thick"],
    ["持参", "じさん", "bringing, taking, carrying"],
    ["磁石", "じしゃく", "magnet"],
    ["四捨五入", "ししゃごにゅう", "rounding up (fractions)"],
    ["始終", "しじゅう", "continuously, always, constantly"],
    ["自習", "じしゅう", "self-study"],
    ["静まる", "しずまる", "to quieten down, to calm down"],
    ["姿勢", "しせい", "attitude; posture"],
    ["自然科学", "しぜんかがく", "natural science"],
    ["時速", "じそく", "speed (per hour)"],
    ["子孫", "しそん", "descendant, offspring"],
    ["死体", "したい", "corpse"],
    ["下書き", "したがき", "rough copy, draft"],
    ["自宅", "じたく", "one's own home (same as 自分の家 (じぶんのいえ))"],
    ["下町", "したまち", "old parts of town"],
    ["自治", "じち", "self-government, autonomy"],
    ["室～", "しつ～", "room"],
    ["～室", "～しつ", "counter for room"],
    ["～日", "～じつ", "day"],
    ["実感", "じっかん", "feelings, realization"],
    ["湿気", "しっき", "moisture, humidity, dampness"],
    ["しつこい", "しつこい", "insistent, obstinate"],
    ["実習", "じっしゅう", "practice, training"],
    ["実績", "じっせき", "achievements, actual results"],
    ["執筆", "しっぴつ", "writing"],
    ["実物", "じつぶつ", "an actual thing"],
    ["しっぽ", "しっぽ", "tail (animal)"],
    ["実用", "じつよう", "practical use, utility"],
    ["しつれいしました (かん)", "しつれいしました (かん)", "Excuse me., I'm sorry."],
    ["実例", "じつれい", "example, instance"],
    ["失恋", "しつれん", "broken heart, unrequited love"],
    ["指定", "してい", "designation, specification, assignment"],
    ["私鉄", "してつ", "private railway"],
    ["縛る", "しばる", "to tie, to bind"],
    ["地盤", "じばん", "(the) ground"],
    ["しびれる", "しびれる", "to become numb"],
    ["紙幣", "しへい", "paper money, notes, bills"],
    ["しぼむ", "しぼむ", "to wither, to shrivel"],
    ["絞る", "しぼる", "to press, to wring, to squeeze"],
    ["縞", "しま", "stripe"],
    ["～おしまい (おわり)", "～おしまい (おわり)", "end up ~"],
    ["しみじみ", "しみじみ", "keenly, deeply, heartily"],
    ["氏名", "しめい", "full name"],
    ["締切", "しめきり", "deadline"],
    ["締め切る", "しめきる", "to close, cancel"],
    ["しめた (かん)", "しめた (かん)", "I've got it, all right, fine"],
    ["地面", "じめん", "ground, earth's surface"],
    ["～車", "～しゃ", "~ car"],
    ["～者", "～しゃ", "person"],
    ["～社", "～しゃ", "counter for company"],
    ["ジャーナリスト", "ジャーナリスト", "journalist"],
    ["社会科学", "しゃかいかがく", "social science"],
    ["しゃがむ", "しゃがむ", "to squat"],
    ["蛇口", "じゃぐち", "faucet, tap"],
    ["弱点", "じゃくてん", "weak point, weakness"],
    ["車庫", "しゃこ", "garage, car shed"],
    ["車掌", "しゃしょう", "(train) conductor"],
    ["写生", "しゃせい", "sketching, drawing from nature"],
    ["社説", "しゃせつ", "editorial"],
    ["しゃっくり", "しゃっくり", "hiccough, hiccup"],
    ["シャッター", "シャッター", "shutter"],
    ["しゃぶる", "しゃぶる", "to suck, to chew"],
    ["車輪", "しゃりん", "(car) wheel"],
    ["洒落", "しゃれ", "joke, pun, witticism"],
    ["じゃんけん", "じゃんけん", "rock-scissors-paper game"],
    ["～手", "～しゅ", "~ player, person who does ~"],
    ["～酒", "～しゅ", "kind of alcohol"],
    ["～集", "～しゅう", "collection of ~"],
    ["重～", "じゅう～", "heavy ~"],
    ["集会", "しゅうかい", "meeting, assembly"],
    ["集金", "しゅうきん", "money collection"],
    ["集合", "しゅうごう", "gathering, assembly"],
    ["習字", "しゅうじ", "penmanship"],
    ["修繕", "しゅうぜん", "repair, mending"],
    ["じゅうたん (カーペット)", "じゅうたん (カーペット)", "carpet"],
    ["終点", "しゅうてん", "terminus, last stop (e.g train)"],
    ["重点", "じゅうてん", "important point, lay stress on, emphasis"],
    ["リズム", "リズム", "rhythm"],
    ["リットル", "リットル", "liter"],
    ["リボン", "リボン", "ribbon"],
    ["略す", "りゃくす", "to abbreviate"],
    ["～流", "～りゅう", "fashion, manner, way"],
    ["流域", "りゅういき", "(river) basin"],
    ["両～", "りょう～", "both ~"],
    ["～料", "～りょう", "fare, charge"],
    ["～領", "～りょう", "territory"],
    ["両側", "りょうがわ", "both sides"],
    ["漁師", "りょうし", "fisherman"],
    ["領事", "りょうじ", "consul"],
    ["領収", "りょうしゅう", "receipt, voucher"],
    ["～力", "～りょく", "power of ~"],
    ["臨時", "りんじ", "temporary, special, extraordinary"],
    ["留守番", "るすばん", "care-taking, caretaker, house-watching"],
    ["零点", "れいてん", "zero, no marks"],
    ["冷凍", "れいとう", "freezing, cold storage, refrigeration"],
    ["レインコート", "レインコート", "raincoat"],
    ["レクリェーション", "レクリェーション", "recreation"],
    ["レジャー", "レジャー", "leisure"],
    ["列島", "れっとう", "chain of islands"],
    ["リポート", "リポート", "report, paper"],
    ["煉瓦", "れんが", "brick"],
    ["連合", "れんごう", "union, alliance"],
    ["レンズ", "レンズ", "lens"],
    ["ろうそく", "ろうそく", "candle"],
    ["ローマ字", "ローマじ", "romanization, Roman letters (alphabet)"],
    ["録音", "ろくおん", "(audio) recording"],
    ["ロッカー", "ロッカー", "locker"],
    ["ロビー", "ロビー", "lobby"],
    ["～論", "～ろん", "theory"],
    ["論ずる", "ろんずる", "to argue, to discuss"],
    ["和～", "わ～", "Japanese style"],
    ["～羽", "～わ", "counter for rabbits; birds"],
    ["和英", "わえい", "Japanese-English"],
    ["我～", "わが～", "our ~"],
    ["若々しい", "わかわかしい", "youthful, young"],
    ["詫びる", "わびる", "to apologize"],
    ["和服", "わふく", "Japanese clothes"],
    ["割合に", "わりあいに", "relatively, comparatively"],
    ["割算", "わりざん", "division (math)"],
    ["割と", "わりと", "relatively, comparatively"],
    ["割引", "わりびき", "discount"],
    ["ワンピース", "ワンピース", "one-piece dress"],
    ["アイデア; アイディア", "アイデア; アイディア", "idea"],
    ["あいまい", "あいまい", "vague, ambiguous"],
    ["扇ぐ", "あおぐ", "to fan, to flap"],
    ["青白い", "あおじろい", "pale"],
    ["呆れる", "あきれる", "to be shocked, to be appalled"],
    ["アクセント", "アクセント", "accent"],
    ["あくび", "あくび", "yawn"],
    ["飽くまで", "あくまで", "to the end, to the last, stubbornly"],
    ["明くる～", "あくる～", "next, following"],
    ["明け方", "あけがた", "dawn"],
    ["憧れる", "あこがれる", "to long for, to yearn after"],
    ["朝寝坊", "あさねぼう", "oversleeping, late riser"],
    ["足跡", "あしあと", "footprint"],
    ["足元", "あしもと", "at one's feet"],
    ["味わう", "あじわう", "to taste, to savor"],
    ["あちらこちら", "あちらこちら", "here and there"],
    ["厚かましい", "あつかましい", "impudent, shameless,"],
    ["圧縮", "あっしゅく", "compression, condensation, pressure"],
    ["宛名", "あてな", "address, direction"],
    ["当てはまる", "あてはまる", "to be applicable, to come under (a category)"],
    ["当てはめる", "あてはめる", "to apply, to adapt"],
    ["暴れる", "あばれる", "to act violently, to rage"],
    ["あぶる", "あぶる", "to scorch, to roast"],
    ["あふれる", "あふれる", "to flood, to overflow"],
    ["雨戸", "あまど", "sliding storm door"],
    ["甘やかす", "あまやかす", "to pamper, to spoil"],
    ["余る", "あまる", "to be left over, to be in excess"],
    ["編み物", "あみもの", "knitting"],
    ["編む", "あむ", "to knit"],
    ["危うい", "あやうい", "dangerous, critical"],
    ["怪しい", "あやしい", "suspicious, dubious, doubtful"],
    ["荒い", "あらい", "rough, rude, wild"],
    ["粗い", "あらい", "coarse, rough"],
    ["粗筋", "あらすじ", "outline, synopsis"],
    ["改めて", "あらためて", "another time, again"],
    ["改める", "あらためる", "to change, to reform, to revise"],
    ["有難い", "ありがたい", "grateful, thankful, appreciated"],
    ["あれこれ", "あれこれ", "one thing or another, this and that"],
    ["荒れる", "あれる", "to be stormy, to be rough, to be ruined"],
    ["慌ただしい", "あわただしい", "busy, hurried"],
    ["安易", "あんい", "easy-going"],
    ["アンテナ", "アンテナ", "antenna"],
    ["～位", "～い", "~th place"],
    ["言い出す", "いいだす", "to start talking, to suggest"],
    ["言い付ける", "いいつける", "to tell, to order"],
    ["意義", "いぎ", "meaning, significance"],
    ["生き生き", "いきいき", "vividly, lively"],
    ["いきなり", "いきなり", "all of a sudden"],
    ["幾～", "いく～", "several ~"],
    ["育児", "いくじ", "childcare, nursing"],
    ["幾分", "いくぶん", "somewhat"],
    ["生け花", "いけばな", "flower arrangement"],
    ["以後", "いご", "after this; from now on; hereafter"],
    ["以降", "いこう", "on and after, hereafter"],
    ["イコール", "イコール", "equal"],
    ["勇ましい", "いさましい", "brave, valiant"],
    ["衣食住", "いしょくじゅう", "food, clothing and shelter"],
    ["～いち (にほんいち)", "～いち (にほんいち)", "No. 1 ~ (in)"],
    ["いちいち", "いちいち", "one by one, separately"],
    ["一応", "いちおう", "tentatively, for the time being"],
    ["一段と", "いちだんと", "by far, greater"],
    ["一流", "いちりゅう", "first class, leading"],
    ["一昨日", "いっさくじつ", "day before yesterday"],
    ["一昨年", "いっさくねん", "year before last"],
    ["一斉", "いっせい", "simultaneous, all at once"],
    ["一旦", "いったん", "once, for a moment"],
    ["一定", "いってい", "fixed, settled, regular"],
    ["行っていらっしゃい", "いっていらっしゃい", "have a nice day, see you"],
    ["行ってらっしゃい", "いってらっしゃい", "have a nice day, see you"],
    ["いってまいります", "いってまいります", "(Lit.) I'll go and come back, 'I'm going, see you later'"],
    ["いってきます", "いってきます", "(Lit.) I'll go and come back, 'I'm going, see you later'"],
    ["移転", "いてん", "moving, transfer"],
    ["井戸", "いど", "water well"],
    ["緯度", "いど", "latitude (navigation)"],
    ["威張る", "いばる", "to be proud, to swagger"],
    ["嫌がる", "いやがる", "reluctant, to dislike"],
    ["いよいよ", "いよいよ", "more and more, increasingly, at last"],
    ["煎る", "いる", "to roast"],
    ["炒る", "いる", "to parch, to roast"],
    ["入れ物", "いれもの", "container, case"],
    ["インキ", "インキ", "ink"],
    ["引力", "いんりょく", "gravity"],
    ["ウーマン", "ウーマン", "woman"],
    ["ウール", "ウール", "wool"],
    ["ウエートレス", "ウエートレス", "waitress"],
    ["植木", "うえき", "garden shrubs, trees, potted plant"],
    ["飢える", "うえる", "to starve"],
    ["浮ぶ", "うかぶ", "to float, to rise to surface, to come to mind"],
    ["浮かべる", "うかべる", "to float; to express"],
    ["浮く", "うく", "to float"],
    ["承る", "うけたまわる", "(humble) to hear, to be told, to know"],
    ["受取", "うけとり", "receipt"],
    ["受け持つ", "うけもつ", "to take (be in) charge of"],
    ["薄暗い", "うすぐらい", "dim, gloomy"],
    ["薄める", "うすめる", "to dilute, to water down"],
    ["打合せ", "うちあわせ", "business meeting, previous arrangement"],
    ["打ち消す", "うちけす", "to deny, to negate"],
    ["うどん", "うどん", "udon noodles (Japanese traditional noodles)"],
    ["うなずく", "うなずく", "to nod"],
    ["敬う", "うやまう", "to show respect, to honor"],
    ["裏返す", "うらがえす", "to turn inside out, to turn (something) over"],
    ["裏口", "うらぐち", "backdoor, rear entrance"],
    ["占う", "うらなう", "to predict, to divine"],
    ["恨み", "うらみ", "resentment"],
    ["恨む", "うらむ", "to curse, to feel bitter"],
    ["羨む", "うらやむ", "to envy"],
    ["売上", "うりあげ", "amount sold, proceeds"],
    ["売り切れ", "うりきれ", "sold-out"],
    ["売り切れる", "うりきれる", "to be sold out"],
    ["売行き", "うれゆき", "sales"],
    ["うろうろ", "うろうろ", "loitering, aimless wandering"],
    ["上～", "うわ～", "upper ~"],
    ["運河", "うんが", "canal, waterway"],
    ["うんと", "うんと", "a great deal, very much"],
    ["英文", "えいぶん", "sentence in English"],
    ["英和", "えいわ", "English-Japanese (e.g., dictionary)"],
    ["ええと", "ええと", "let me see, well, er..."],
    ["液体", "えきたい", "liquid, fluid"],
    ["エチケット", "エチケット", "etiquette"],
    ["絵の具", "えのぐ", "colors, paints"],
    ["エプロン", "エプロン", "apron"],
    ["偉い", "えらい", "great, celebrated, remarkable,"],
    ["～園", "～えん", "~ garden (especially man made)"],
    ["宴会", "えんかい", "party, banquet"],
    ["園芸", "えんげい", "horticulture, gardening"],
    ["演劇", "えんげき", "play (theatrical)"],
    ["円周", "えんしゅう", "circumference"],
    ["遠足", "えんそく", "trip, hike, picnic"],
    ["延長", "えんちょう", "extension, prolongation"],
    ["煙突", "えんとつ", "chimney"],
    ["御～", "おん～", "honorific ~"],
    ["追いかける", "おいかける", "to chase or run after someone"],
    ["追い越す", "おいこす", "to pass (e.g., car), to outdistance, to outstrip"],
    ["オイル", "オイル", "oil"],
    ["王女", "おうじょ", "princess"],
    ["応ずる", "おうずる", "to respond, to comply with"],
    ["応接", "おうせつ", "reception"],
    ["応対", "おうたい", "receiving, dealing with"],
    ["往復", "おうふく", "(col) round trip, coming and going, return ticket"],
    ["欧米", "おうべい", "Europe and America, the West"],
    ["応用", "おうよう", "application, put to practical use"],
    ["電波", "でんぱ", "electro-magnetic wave"],
    ["テンポ", "テンポ", "tempo"],
    ["電流", "でんりゅう", "electric current"],
    ["電力", "でんりょく", "electric power"],
    ["問い合わせ", "といあわせ", "inquiry"],
    ["～頭", "～とう", "counter for animals"],
    ["～等", "～とう", "level, place"],
    ["～島", "～とう", "kind of islands"],
    ["銅", "どう", "copper"],
    ["同～", "どう～", "same ~"],
    ["～道", "～どう", "kind of path, road"],
    ["どういたしまして (かん)", "どういたしまして (かん)", "you are welcome, don't mention it"],
    ["統一", "とういつ", "unity, consolidation, uniformity"],
    ["同格", "どうかく", "the same rank, equality, apposition"],
    ["峠", "とうげ", "ridge, (mountain) pass, difficult part"],
    ["統計", "とうけい", "scattering, a scatter, dispersion"],
    ["動作", "どうさ", "action, movements, motions"],
    ["東西", "とうざい", "East and West, whole country"],
    ["当日", "とうじつ", "appointed day, very day"],
    ["投書", "とうしょ", "letter to the editor, letter from a reader, contribution"],
    ["登場", "とうじょう", "entry (on stage)"],
    ["どうせ", "どうせ", "anyhow, in any case, at any rate"],
    ["灯台", "とうだい", "lighthouse"],
    ["盗難", "とうなん", "theft, robbery"],
    ["当番", "とうばん", "being on duty"],
    ["等分", "とうぶん", "division into equal parts"],
    ["透明", "とうめい", "transparency, cleanness"],
    ["灯油", "とうゆ", "lamp oil, kerosene"],
    ["童話", "どうわ", "fairy tale"],
    ["～通り", "～とおり", "in accordance with ~; following ~; ~ street, ~ avenue"],
    ["通り掛かる", "とおりかかる", "to happen to pass by"],
    ["溶かす", "とかす", "to melt, to dissolve"],
    ["尖る", "とがる", "to taper to a point, to become sharp"],
    ["どきどき", "どきどき", "throb, beat (fast)"],
    ["特殊", "とくしゅ", "special, unique"],
    ["特色", "とくしょく", "characteristic, feature"],
    ["特定", "とくてい", "specific, special, particular"],
    ["特売", "とくばい", "special sale"],
    ["溶け込む", "とけこむ", "to melt into; to become a part of"],
    ["退ける", "どける", "to dislodge, to put something out of the way"],
    ["床の間", "とこのま", "alcove"],
    ["～ところ", "～ところ", "about to do ~"],
    ["所々", "ところどころ", "here and there, some parts (of something)"],
    ["都心", "としん", "heart (of city)"],
    ["戸棚", "とだな", "cupboard, cabinet"],
    ["とっくに", "とっくに", "long ago, already, a long time ago"],
    ["どっと", "どっと", "suddenly"],
    ["整う", "ととのう", "to be prepared, to be in order, to be arranged"],
    ["留まる", "とどまる", "to be fixed; to abide, to stay (in the one place)"],
    ["怒鳴る", "どなる", "to shout, to yell"],
    ["殿", "どの", "Mister (mostly in addressing someone on an envelope)"],
    ["飛び込む", "とびこむ", "to jump in, to leap in, to plunge into"],
    ["留まる", "とまる", "to be fixed; to abide, to stay (in the one place)"],
    ["ともかく", "ともかく", "anyhow, at any rate, anyway"],
    ["捕える", "とらえる", "to seize, to capture, to arrest"],
    ["取り入れる", "とりいれる", "to harvest, to take in, to adopt"],
    ["取り消す", "とりけす", "to cancel"],
    ["取り出す", "とりだす", "to take out, to pick out"],
    ["採る", "とる", "to adopt (measure, proposal); to pick (fruit)"],
    ["捕る", "とる", "to take, to catch (fish)"],
    ["丼", "どんぶり", "porcelain bowl, bowl of rice with food on top"],
    ["～内", "～ない", "inside ~"],
    ["内科", "ないか", "internist clinic, internal medicine"],
    ["内線", "ないせん", "phone extension"],
    ["ナイロン", "ナイロン", "nylon"],
    ["長～", "なが～", "long ~"],
    ["仲直り", "なかなおり", "reconciliation, make peace with"],
    ["長引く", "ながびく", "to be prolonged, to drag on"],
    ["中指", "なかゆび", "middle finger"],
    ["仲良し", "なかよし", "intimate friend, bosom buddy"],
    ["慰める", "なぐさめる", "to comfort, to console"],
    ["為す", "なす", "to accomplish, to do"],
    ["謎謎", "なぞなぞ", "riddle"],
    ["傾らか", "なだらか", "gradual, gentle"],
    ["懐かしい", "なつかしい", "dear, desired, missed"],
    ["撫でる", "なでる", "to brush gently, to stroke"],
    ["斜め", "ななめ", "diagonal, oblique"],
    ["なにしろ", "なにしろ", "at any rate, in any case"],
    ["何々", "なになに", "such and such, What?"],
    ["何分", "なにぶん", "by all means, please"],
    ["生意気", "なまいき", "impertinent, impudent"],
    ["並木", "なみき", "roadside tree, row of trees"],
    ["倣う", "ならう", "to imitate, to follow, to emulate"],
    ["南極", "なんきょく", "south pole, Antarctic"],
    ["なんとなく", "なんとなく", "somehow or other, for some reason or another"],
    ["なんとも", "なんとも", "nothing (with neg. verb), quite, not a bit"],
    ["ナンバー", "ナンバー", "number"],
    ["南米", "なんべい", "South America"],
    ["南北", "なんぼく", "south and north"],
    ["匂う", "におう", "to be fragrant, to smell"],
    ["逃がす", "にがす", "to let loose, to set free, to let escape"],
    ["憎い", "にくい", "hateful, abominable, detestable"],
    ["～難い", "～がたい", "hard (difficult) to do ~"],
    ["憎む", "にくむ", "to hate, to detest"],
    ["憎らしい", "にくらしい", "odious, hateful"],
    ["にこにこ", "にこにこ", "smile sweetly, smiley"],
    ["濁る", "にごる", "to become muddy, to get cloudy"],
    ["虹", "にじ", "rainbow"],
    ["日時", "にちじ", "date and time"],
    ["日用品", "にちようひん", "daily necessities"],
    ["日課", "にっか", "daily work, daily routine"],
    ["日程", "にってい", "schedule"],
    ["鈍い", "にぶい", "dull (e.g., a knife), thickheaded, slow (opposite of fast), stupid"],
    ["入社", "にゅうしゃ", "entry to a company"],
    ["女房", "にょうぼう", "wife"],
    ["睨む", "にらむ", "to glare at, to stare; to keep an eye on"],
    ["俄", "にわか", "sudden, abrupt, unexpected"],
    ["縫う", "ぬう", "to sew"],
    ["ねじ", "ねじ", "(a) screw"],
    ["捩る", "ねじる", "to twist"],
    ["ネックレス", "ネックレス", "necklace"],
    ["熱する", "ねっする", "to heat"],
    ["寝間着", "ねまき", "sleep-wear, pajamas"],
    ["寝巻", "ねまき", "sleep-wear, pajamas"],
    ["狙い", "ねらい", "aim"],
    ["狙う", "ねらう", "to aim at"],
    ["～年生", "～ねんせい", "counter for school year"],
    ["年度", "ねんど", "year, fiscal year"],
    ["農産物", "のうさんぶつ", "agricultural produce"],
    ["農村", "のうそん", "agricultural community"],
    ["濃度", "のうど", "concentration, density"],
    ["農薬", "のうやく", "agricultural chemicals"],
    ["能率", "のうりつ", "efficiency"],
    ["のこぎり", "のこぎり", "saw"],
    ["残らず", "のこらず", "completely, without exception"],
    ["上り", "のぼり", "up-train (going to Tokyo), ascent"],
    ["糊", "のり", "glue, paste, starch"],
    ["乗換", "のりかえ", "a transfer (e.g., trains, buses)"],
    ["乗り越し", "のりこし", "riding past (one's station)"],
    ["鈍い", "のろい", "dull (e.g., a knife), slow (opposite of fast), stupid"],
    ["のろのろ", "のろのろ", "slowly, sluggishly"],
    ["呑気", "のんき", "carefree, optimistic, careless"],
    ["はい (かん)", "はい (かん)", "yes"],
    ["灰色", "はいいろ", "grey, ashen"],
    ["俳句", "はいく", "haiku poetry"],
    ["売店", "ばいてん", "shop, stand"],
    ["バイバイ", "バイバイ", "bye bye"],
    ["売買", "ばいばい", "trade, buying and selling"],
    ["這う", "はう", "to creep, to crawl"],
    ["剥す", "はがす", "to tear off, to peel off, to rip off"],
    ["ばからしい", "ばからしい", "absurd"],
    ["秤", "はかり", "scales, weighing machine"],
    ["吐き気", "はきけ", "nausea, sickness in the stomach"],
    ["はきはき", "はきはき", "clearly"],
    ["～泊", "～はく", "counter for staying (e.g., 2 nights)"],
    ["歯車", "はぐるま", "gear, cog-wheel"],
    ["バケツ", "バケツ", "bucket, pail"],
    ["挟まる", "はさまる", "to get between, to be caught in"],
    ["挟む", "はさむ", "to pinch; to hold between; to insert"],
    ["梯子", "はしご", "ladder, stairs"],
    ["始めに", "はじめに", "to begin with, first of all"],
    ["初めに", "はじめに", "to begin with, first of all"],
    ["はじめまして", "はじめまして", "How do you do, I am glad to meet you"],
    ["斜", "はす", "diagonal"],
    ["パターン", "パターン", "pattern"],
    ["肌着", "はだぎ", "underwear"],
    ["果して", "はたして", "as was expected, really"],
    ["鉢", "はち", "a bowl, a pot"],
    ["～発", "～はつ", "counter for bullets"],
    ["×", "ばつ", "cross"],
    ["発揮", "はっき", "exhibition, demonstration, display"],
    ["バック", "バック", "back"],
    ["発想", "はっそう", "idea; way of thinking (same as 考え方 (かんがえかた))"],
    ["発電", "はつでん", "generation (e.g., power)"],
    ["発売", "はつばい", "sale"],
    ["話合い", "はなしあい", "discussion, talk"],
    ["話し掛ける", "はなしかける", "to accost a person, to talk (to someone)"],
    ["話中", "はなしちゅう", "while talking, the line is busy"],
    ["甚だしい", "はなはだしい", "extreme, excessive, terrible"],
    ["花火", "はなび", "fireworks"],
    ["花嫁", "はなよめ", "bride"],
    ["ばね", "ばね", "spring (e.g., coil, leaf)"],
    ["跳ねる", "はねる", "to jump, to leap"],
    ["破片", "はへん", "fragment, broken piece"],
    ["歯磨き", "はみがき", "toothbrushing, toothpaste"],
    ["はめる", "はめる", "(col) to get in, to insert, to put on"],
    ["早口", "はやくち", "fast-talking"],
    ["払い込む", "はらいこむ", "to deposit, to pay in"],
    ["払い戻す", "はらいもどす", "to repay, to pay back"],
    ["針金", "はりがね", "wire"],
    ["張り切る", "はりきる", "to be in high spirits, to be full of vigor"],
    ["反～", "はん～", "anti ~, counter ~"],
    ["反映", "はんえい", "reflection, influence"],
    ["半径", "はんけい", "radius"],
    ["判子", "はんこ", "seal (used for signature)"],
    ["万歳", "ばんざい", "banzai, Long live ~!"],
    ["判事", "はんじ", "judge, justice"],
    ["番地", "ばんち", "house number, address"],
    ["パンツ", "パンツ", "underpants"],
    ["バンド", "バンド", "band"],
    ["半島", "はんとう", "peninsula"],
    ["ハンドル", "ハンドル", "handle, steering wheel"],
    ["～番目", "～ばんめ", "~th"],
    ["非～", "ひ～", "anti~, counter~, an~, non~"],
    ["～費", "～ひ", "cost of ~"],
    ["日当たり", "ひあたり", "exposure to the sun, sunny place"],
    ["日帰り", "ひがえり", "day trip"],
    ["比較的", "ひかくてき", "comparatively; relatively"],
    ["日陰", "ひかげ", "shadow"],
    ["ぴかぴか", "ぴかぴか", "glitter, sparkle"],
    ["引受る", "ひきうける", "to undertake, to take up, to take over"],
    ["引き返す", "ひきかえす", "to turn back, to go back, reverse"],
    ["引算", "ひきざん", "subtraction"],
    ["引き出す", "ひきだす", "to pull out, to take out, to withdraw"],
    ["引き止める", "ひきとめる", "to keep back, to prevent, to hold back"],
    ["卑怯", "ひきょう", "cowardice, meanness, unfairness"],
    ["引分け", "ひきわけ", "a draw (in competition), tie game"],
    ["陽射", "ひざし", "sunlight, rays of the sun"],
    ["肘", "ひじ", "elbow"],
    ["ピストル", "ピストル", "pistol"],
    ["ビタミン", "ビタミン", "vitamin"],
    ["ぴたり", "ぴたり", "exactly; close-fitting"],
    ["引っ掛かる", "ひっかかる", "to be caught in, to be stuck in"],
    ["筆記", "ひっき", "note taking, writing"],
    ["引っ繰り返す", "ひっくりかえす", "to turn over, to overturn, to knock over"],
    ["引っ繰り返る", "ひっくりかえる", "to be overturned, to be upset, to topple over, to be reversed"],
    ["引っ込む", "ひっこむ", "to draw back, to sink, to cave in"],
    ["筆者", "ひっしゃ", "writer, author"],
    ["必需品", "ひつじゅひん", "necessities, essential"],
    ["一～", "ひと～", "one ~"],
    ["人差指", "ひとさしゆび", "index finger"],
    ["一通り", "ひととおり", "general, briefly"],
    ["人通り", "ひとどおり", "pedestrian traffic"],
    ["ひとまず", "ひとまず", "for the present, once, for the time being"],
    ["瞳", "ひとみ", "pupil (of eye)"],
    ["一休み", "ひとやすみ", "a rest"],
    ["独り言", "ひとりごと", "a soliloquy, a monologue, speaking to oneself"],
    ["ひとりでに", "ひとりでに", "by itself, automatically, naturally"],
    ["ビニール", "ビニール", "vinyl"],
    ["皮肉", "ひにく", "cynicism, sarcasm"],
    ["日日", "ひにち", "date"],
    ["捻る", "ひねる", "to twist, to turn"],
    ["日の入り", "ひのいり", "sunset"],
    ["日の出", "ひので", "sunrise"],
    ["響き", "ひびき", "echo, sound"],
    ["響く", "ひびく", "to resound; to affect"],
    ["皮膚", "ひふ", "skin"],
    ["ひゃっかじてん", "ひゃっかじてん", "encyclopedia"],
    ["美容", "びよう", "beauty of figure or form"],
    ["～病", "～びょう", "kind of disease"],
    ["表紙", "ひょうし", "front cover, binding"],
    ["標識", "ひょうしき", "sign, mark"],
    ["標準", "ひょうじゅん", "standard, level"],
    ["標本", "ひょうほん", "example, specimen"],
    ["評論", "ひょうろん", "criticism, critique"],
    ["ビルディング", "ビルディング", "building"],
    ["昼寝", "ひるね", "nap (at home), siesta"],
    ["広さ", "ひろさ", "extent"],
    ["広場", "ひろば", "plaza"],
    ["広々", "ひろびろ", "extensive, spacious"],
    ["ピンク", "ピンク", "pink"],
    ["便箋", "びんせん", "writing paper, stationery"],
    ["瓶詰", "びんづめ", "bottling, bottled"],
    ["～部", "～ぶ", "~ part"],
    ["ファスナー", "ファスナー", "fastener, zipper"],
    ["～風", "～ふう", "~ style"],
    ["風船", "ふうせん", "balloon"],
    ["不運", "ふうん", "unlucky, misfortune, bad luck"],
    ["不規則", "ふきそく", "irregularity, unsteadiness"],
    ["普及", "ふきゅう", "diffusion, spread"],
    ["付近", "ふきん", "neighborhood, vicinity"],
    ["副～", "ふく～", "vice ~"],
    ["副詞", "ふくし", "adverb"],
    ["複写", "ふくしゃ", "copy, duplicate"],
    ["複数", "ふくすう", "plural, multiple"],
    ["膨らます", "ふくらます", "to swell, to expand, to inflate"],
    ["膨らむ", "ふくらむ", "to expand, to swell (out), to become inflated"],
    ["不潔", "ふけつ", "unclean, dirty"],
    ["更ける", "ふける", "to get late, to wear on"],
    ["符号", "ふごう", "sign, mark, symbol"],
    ["夫妻", "ふさい", "man and wife, married couple"],
    ["塞がる", "ふさがる", "to be plugged up, to be shut up"],
    ["塞ぐ", "ふさぐ", "to stuff, to close up, to block (up)"],
    ["ふざける", "ふざける", "to romp, to gambol, to frolic"],
    ["無沙汰", "ぶさた", "neglecting to stay in contact"],
    ["武士", "ぶし", "warrior, samurai"],
    ["部首", "ぶしゅ", "radical (of a kanji character)"],
    ["襖", "ふすま", "sliding screen"],
    ["附属", "ふぞく", "attached, belonging, affiliated"],
    ["蓋", "ふた", "cover, lid, cap"],
    ["物騒", "ぶっそう", "dangerous, disturbed, insecure"],
    ["ぶつぶつ", "ぶつぶつ", "grumbling, complaining in a small voice"],
    ["船便", "ふなびん", "surface mail (ship)"],
    ["部品", "ぶひん", "parts, accessories"],
    ["吹雪", "ふぶき", "snow storm"],
    ["父母", "ふぼ", "father and mother, parents"],
    ["踏切", "ふみきり", "railway crossing, level crossing, starting line, scratch, crossover"],
    ["麓", "ふもと", "the foot, the bottom, the base (of a mountain)"],
    ["フライパン", "フライパン", "fry pan, frying pan"],
    ["ブラウス", "ブラウス", "blouse"],
    ["ぶらさげる", "ぶらさげる", "to hang, to suspend, to swing"],
    ["ブラシ", "ブラシ", "brush"],
    ["プラットホーム", "プラットホーム", "platform"],
    ["～振り", "～ぶり", "after an interval of ~"],
    ["フリー", "フリー", "free"],
    ["振り仮名", "ふりがな", "pronunciation key"],
    ["振り向く", "ふりむく", "to turn one's face, to turn around"],
    ["プリント", "プリント", "print, handout"],
    ["古～", "ふる～", "old ~"],
    ["故郷", "ふるさと", "home town, birthplace"],
    ["古里", "ふるさと", "home town, birthplace"],
    ["振舞う", "ふるまう", "to behave, to conduct"],
    ["ブローチ", "ブローチ", "brooch"],
    ["プログラム", "プログラム", "program"],
    ["風呂敷", "ふろしき", "wrapping cloth, cloth wrapper"],
    ["ふわふわ", "ふわふわ", "light, soft"],
    ["噴火", "ふんか", "eruption"],
    ["分解", "ぶんかい", "analysis, disassembly"],
    ["文芸", "ぶんげい", "literature, art and literature"],
    ["文献", "ぶんけん", "literature, books (reference)"],
    ["噴水", "ふんすい", "water fountain"],
    ["分数", "ぶんすう", "fraction (in math)"],
    ["文体", "ぶんたい", "literary style"],
    ["分布", "ぶんぷ", "distribution"],
    ["文房具", "ぶんぼうぐ", "stationery"],
    ["文脈", "ぶんみゃく", "context"],
    ["分量", "ぶんりょう", "amount, quantity"],
    ["分類", "ぶんるい", "classification"],
    ["閉会", "へいかい", "closure"],
    ["平気", "へいき", "coolness, calmness, unconcern"],
    ["並行", "へいこう", "(going) side by side, concurrent, at the same time"],
    ["平日", "へいじつ", "weekday"],
    ["兵隊", "へいたい", "soldier"],
    ["平凡", "へいぼん", "common, ordinary"],
    ["平野", "へいや", "plain, open field"],
    ["凹む", "へこむ", "to be dented, to be indented"],
    ["へそ", "へそ", "navel, belly-button"],
    ["隔てる", "へだてる", "to be shut out"],
    ["別荘", "べっそう", "villa; vacation home; summer cottage"],
    ["別々", "べつべつ", "separately, individually"],
    ["ベテラン", "ベテラン", "veteran"],
    ["ヘリコプター", "ヘリコプター", "helicopter"],
    ["～遍", "～へん", "time"],
    ["～弁", "～べん", "speech, dialect"],
    ["編集", "へんしゅう", "editing, compilation, editorial (e.g., committee)"],
    ["便所", "べんじょ", "toilet, lavatory"],
    ["ペンチ", "ペンチ", "pliers (lit: pinchers)"],
    ["～歩", "～ほ", "step, pace"],
    ["～ぽい", "～ぽい", "~ish"],
    ["～ほう (ひかく)", "～ほう (ひかく)", "(in comparison)"],
    ["防～", "ぼう～", "~ prevention"],
    ["望遠鏡", "ぼうえんきょう", "telescope"],
    ["方角", "ほうがく", "direction, way"],
    ["箒", "ほうき", "broom"],
    ["方言", "ほうげん", "dialect"],
    ["坊さん", "ぼうさん", "Buddhist priest, monk"],
    ["防止", "ぼうし", "prevention, check"],
    ["方針", "ほうしん", "objective, plan, policy"],
    ["法則", "ほうそく", "law, rule"],
    ["包帯", "ほうたい", "bandage"],
    ["膨大", "ぼうだい", "enormous, extensive"],
    ["包丁", "ほうちょう", "kitchen knife, carving knife"],
    ["方程式", "ほうていしき", "equation"],
    ["防犯", "ぼうはん", "prevention of crime"],
    ["方面", "ほうめん", "direction; area"],
    ["坊や", "ぼうや", "boy"],
    ["放る", "ほうる", "to let go"],
    ["ボーナス", "ボーナス", "bonus"],
    ["朗らか", "ほがらか", "bright, cheerful, melodious"],
    ["牧場", "ぼくじょう", "farm (livestock); pasture land"],
    ["牧畜", "ぼくちく", "stock-farming"],
    ["保健", "ほけん", "health preservation, hygiene, sanitation"],
    ["募集", "ぼしゅう", "recruitment"],
    ["干す", "ほす", "to air, to dry, to drink up"],
    ["ポスター", "ポスター", "poster"],
    ["北極", "ほっきょく", "North Pole"],
    ["坊っちゃん", "ぼっちゃん", "son (of others)"],
    ["解く", "ほどく", "to unfasten"],
    ["掘る", "ほる", "to dig, to excavate"],
    ["彫る", "ほる", "to carve, to chisel"],
    ["ぼろ", "ぼろ", "rag, tattered clothes"],
    ["盆", "ぼん", "Festival of the Dead; tray"],
    ["盆地", "ぼんち", "basin (e.g., between mountains)"],
    ["ほんの～", "ほんの～", "mere, just"],
    ["本部", "ほんぶ", "headquarters"],
    ["本来", "ほんらい", "essentially, naturally, by nature"],
    ["まあまあ", "まあまあ", "okay; so-so; decent"],
    ["毎～", "まい～", "every ~"],
    ["枚数", "まいすう", "the number of flat things"],
    ["毎度", "まいど", "each time, common service-sector greeting"],
    ["枕", "まくら", "pillow"],
    ["曲げる", "まげる", "to bend, to crook, to lean"],
    ["まごまご", "まごまご", "confused, be at a loss"],
    ["摩擦", "まさつ", "friction; rubbing; chafe"],
    ["マスク", "マスク", "mask"],
    ["またぐ", "またぐ", "to straddle"],
    ["待合室", "まちあいしつ", "waiting room"],
    ["待ち合わせる", "まちあわせる", "to rendezvous, to meet at a prearranged place and time"],
    ["街角", "まちかど", "street corner"],
    ["真っ暗", "まっくら", "total darkness"],
    ["真っ黒", "まっくろ", "pitch black"],
    ["真っ青", "まっさお", "deep blue, ghastly pale"],
    ["真っ先", "まっさき", "foremost, beginning"],
    ["真っ白", "まっしろ", "pure white"],
    ["祭る", "まつる", "to deify, to enshrine"],
    ["窓口", "まどぐち", "(ticket) window"],
    ["真似る", "まねる", "to mimic, to imitate"],
    ["まぶた", "まぶた", "eyelids"],
    ["マフラー", "マフラー", "a winter scarf"],
    ["間も無く", "まもなく", "soon, in a short time"],
    ["マラソン", "マラソン", "marathon"],
    ["稀", "まれ", "rare, seldom"],
    ["回り道", "まわりみち", "detour"],
    ["満員", "まんいん", "full house, no vacancy, sold out"],
    ["マンション", "マンション", "multistory apartment house, condominium"],
    ["満点", "まんてん", "perfect score"],
    ["未～", "み～", "not yet ~"],
    ["見上げる", "みあげる", "to look up at, to admire"],
    ["見送る", "みおくる", "to see off; to escort; to let pass"],
    ["見下ろす", "みおろす", "to overlook, to look down on something"],
    ["見掛け", "みかけ", "outward appearance"],
    ["三日月", "みかづき", "crescent moon"],
    ["岬", "みさき", "cape (on coast)"],
    ["みじめ", "みじめ", "sad, pitiful, wretched"],
    ["ミシン", "ミシン", "sewing machine"],
    ["自ら", "みずから", "for one's self, personally"],
    ["水着", "みずぎ", "bathing suit (woman's)"],
    ["店屋", "みせや", "store, shop"],
    ["～みたい", "～みたい", "looks like ~"],
    ["見出し", "みだし", "heading, index"],
    ["道順", "みちじゅん", "itinerary, route"],
    ["みっともない", "みっともない", "shameful, indecent"],
    ["見詰める", "みつめる", "to stare at, to gaze at"],
    ["見直す", "みなおす", "to look over again, to review"],
    ["見慣れる", "みなれる", "to become used to seeing, to be familiar with"],
    ["醜い", "みにくい", "ugly"],
    ["実る", "みのる", "to bear fruit, to ripen"],
    ["身分", "みぶん", "position, status"],
    ["見本", "みほん", "sample"],
    ["見舞う", "みまう", "to ask after (health), to visit"],
    ["未満", "みまん", "less than, insufficient"],
    ["名字", "みょうじ", "surname, family name"],
    ["ミリ (メートル)", "ミリ (メートル)", "milli-"],
    ["民間", "みんかん", "private, civilian"],
    ["民主～", "みんしゅ～", "democratic"],
    ["民謡", "みんよう", "folk song, popular song"],
    ["～向け", "～むけ", "for ~"],
    ["無限", "むげん", "infinite"],
    ["無地", "むじ", "plain"],
    ["矛盾", "むじゅん", "contradiction, inconsistency"],
    ["無数", "むすう", "countless number, infinite number"],
    ["紫", "むらさき", "purple color, violet"],
    ["群れ", "むれ", "crowd, flock, herd"],
    ["姪", "めい", "niece"],
    ["名～", "めい～", "famous ~"],
    ["～名", "～めい", "counter for people"],
    ["名作", "めいさく", "masterpiece"],
    ["名所", "めいしょ", "famous place"],
    ["命ずる", "めいずる", "to command, to appoint"],
    ["迷信", "めいしん", "superstition"],
    ["名物", "めいぶつ", "famous product, special product, speciality"],
    ["銘々", "めいめい", "each, individual"],
    ["恵まれる", "めぐまれる", "to be blessed with, to be rich in"],
    ["巡る", "めぐる", "to go around"],
    ["目指す", "めざす", "to aim at, to have an eye on"],
    ["目覚し", "めざまし", "alarm-clock"],
    ["目下", "めした", "at present, now"],
    ["目印", "めじるし", "mark, sign, landmark"],
    ["目立つ", "めだつ", "to be conspicuous, to stand out"],
    ["めちゃくちゃ", "めちゃくちゃ", "absurd, messed up, wrecked"],
    ["めっきり", "めっきり", "remarkably"],
    ["めでたい", "めでたい", "happy, propitious, joyous"],
    ["メニュー", "メニュー", "menu"],
    ["めまい", "めまい", "dizziness, giddiness"],
    ["目安", "めやす", "criterion, aim"],
    ["免税", "めんぜい", "tax exemption"],
    ["面積", "めんせき", "area"],
    ["面倒臭い", "めんどうくさい", "bother to do, tiresome"],
    ["儲かる", "もうかる", "to be profitable, to yield a profit"],
    ["儲ける", "もうける", "to earn, to have (bear, beget) a child"],
    ["申し訳ない", "もうしわけない", "inexcusable, I am sorry"],
    ["モーター", "モーター", "motor"],
    ["木材", "もくざい", "lumber, timber, wood"],
    ["目次", "もくじ", "table of contents"],
    ["潜る", "もぐる", "to drive, to pass through; to evade, to hide"],
    ["もしかしたら", "もしかしたら", "perhaps, maybe, by some chance"],
    ["もたれる", "もたれる", "to lean against, to lean on"],
    ["モダン", "モダン", "modern"],
    ["餅", "もち", "sticky rice cake"],
    ["～もち", "～もち", "person who has ~"],
    ["モデル", "モデル", "a fashion model"],
    ["元々", "もともと", "originally, by nature, from the start"],
    ["物置", "ものおき", "storage room"],
    ["物語る", "ものがたる", "to tell, to indicate"],
    ["物差し", "ものさし", "ruler, measure"],
    ["物凄い", "ものすごい", "earth-shattering, staggering, to a very great extent"],
    ["モノレール", "モノレール", "monorail"],
    ["紅葉", "もみじ", "(Japanese) maple"],
    ["揉む", "もむ", "to rub, to crumple (up), to wrinkle"],
    ["催し", "もよおし", "event, festivities, function"],
    ["盛る", "もる", "to serve (food); to fill up; to prescribe"],
    ["～問", "～もん", "counter for questions"],
    ["問答", "もんどう", "questions and answers, dialogue"],
    ["～夜", "～や", "counter for nights"],
    ["やかましい", "やかましい", "to be fussy, to be overly critical"],
    ["夜間", "やかん", "at night, nighttime"],
    ["やかん", "やかん", "kettle"],
    ["役者", "やくしゃ", "actor"],
    ["役所", "やくしょ", "government office, public office"],
    ["役人", "やくにん", "government official"],
    ["薬品", "やくひん", "medicine(s), chemical(s)"],
    ["役目", "やくめ", "duty, business"],
    ["火傷", "やけど", "burn, scald"],
    ["夜行", "やこう", "night train, night travel"],
    ["矢印", "やじるし", "directing arrow"],
    ["やたらに", "やたらに", "randomly, recklessly, blindly"],
    ["薬局", "やっきょく", "pharmacy, drugstore"],
    ["やっつける", "やっつける", "to attack (an enemy), to beat, to finish off"],
    ["家主", "やぬし", "landlord"],
    ["やっぱり", "やっぱり", "after all, anyway"],
    ["破く", "やぶく", "to tear"],
    ["やむをえない", "やむをえない", "cannot be helped, unavoidable"],
    ["軟らかい", "やわらかい", "soft, tender, limp"],
    ["遊園地", "ゆうえんち", "amusement park"],
    ["夕刊", "ゆうかん", "evening paper"],
    ["郵送", "ゆうそう", "mailing"],
    ["夕立", "ゆうだち", "(sudden) evening shower (rain)"],
    ["夕日", "ゆうひ", "(in) the evening sun, setting sun"],
    ["悠々", "ゆうゆう", "quiet, calm, leisurely"],
    ["有料", "ゆうりょう", "admission-paid, toll"],
    ["浴衣", "ゆかた", "bathrobe, informal summer kimono"],
    ["行方", "ゆくえ", "one's whereabouts"],
    ["湯気", "ゆげ", "steam"],
    ["輸血", "ゆけつ", "blood transfusion"],
    ["輸送", "ゆそう", "transport, transportation"],
    ["油断", "ゆだん", "negligence, unpreparedness"],
    ["湯飲み", "ゆのみ", "teacup"],
    ["緩い", "ゆるい", "loose, lenient, slow"],
    ["溶岩", "ようがん", "lava"],
    ["用語", "ようご", "term, terminology"],
    ["要旨", "ようし", "gist, essentials, summary"],
    ["幼児", "ようじ", "infant, baby, child"],
    ["容積", "ようせき", "capacity, volume"],
    ["幼稚", "ようち", "infancy, childish, infantile"],
    ["幼稚園", "ようちえん", "kindergarten"],
    ["用途", "ようと", "use, usefulness"],
    ["洋品店", "ようひんてん", "clothes store"],
    ["養分", "ようぶん", "nourishment, nutrient"],
    ["羊毛", "ようもう", "wool"],
    ["漸く", "ようやく", "at last, finally, hardly"],
    ["要領", "ようりょう", "gist, essentials, outline"],
    ["翌～", "よく～", "next ~"],
    ["欲張り", "よくばり", "greedy"],
    ["余計", "よけい", "too much, unnecessary, abundance, surplus, excess, superfluity"],
    ["よこす", "よこす", "to send, to forward; to hand over (e.g., money)"],
    ["余所", "よそ", "another place, somewhere else, strange parts"],
    ["四つ角", "よつかど", "four corners, crossroads"],
    ["酔っ払い", "よっぱらい", "drunkard"],
    ["予備", "よび", "preparation, spare"],
    ["呼び掛ける", "よびかける", "to call out to, to accost, to address (crowd), to appeal"],
    ["呼び出す", "よびだす", "to summon, to call (e.g., phone)"],
    ["蘇る", "よみがえる", "to be resurrected, to be revived"],
    ["慶ぶ", "よろこぶ", "to be delighted, to be glad"],
    ["～等", "～ら", "plural persons"],
    ["来日", "らいにち", "coming to Japan, visit to Japan"],
    ["落第", "らくだい", "failure, dropping out of a class"],
    ["ラッシュアワー", "ラッシュアワー", "rush hour"],
    ["欄", "らん", "column of text (e.g., as in a newspaper)"],
    ["ランチ", "ランチ", "lunch"],
    ["ランニング", "ランニング", "running; tank top"],
    ["乱暴", "らんぼう", "rude, violent, rough"],
    ["理科", "りか", "science"],
    ["利害", "りがい", "advantages and disadvantages, interest"],
    ["段階", "だんかい", "gradation, grade, stage"],
    ["短期", "たんき", "short term"],
    ["炭鉱", "たんこう", "coal mine, coal pit"],
    ["短所", "たんしょ", "defect, weak point; disadvantage"],
    ["たんす", "たんす", "chest of drawers"],
    ["淡水", "たんすい", "fresh water"],
    ["断水", "だんすい", "water outage"],
    ["単数", "たんすう", "singular (number)"],
    ["団地", "だんち", "housing complex"],
    ["断定", "だんてい", "conclusion, decision"],
    ["短編", "たんぺん", "short (e.g., story, film)"],
    ["誓う", "ちかう", "to swear, to vow"],
    ["地下水", "ちかすい", "underground water"],
    ["近々", "ちかぢか", "soon, before long"],
    ["近付ける", "ちかづける", "to bring near, to put close, to let come near"],
    ["近寄る", "ちかよる", "to approach, to draw near"],
    ["力強い", "ちからづよい", "powerful, strong, vigorous"],
    ["ちぎる", "ちぎる", "to cut up fine, to pick (fruit)"],
    ["地質", "ちしつ", "geological features"],
    ["知人", "ちじん", "friend, acquaintance"],
    ["地帯", "ちたい", "area, zone"],
    ["縮む", "ちぢむ", "to shrink, to be contracted"],
    ["縮める", "ちぢめる", "to shorten, to reduce, to shrink"],
    ["縮れる", "ちぢれる", "to be wavy, to be curled"],
    ["チップ", "チップ", "gratuity, tip; chip"],
    ["地点", "ちてん", "site, point on a map"],
    ["地名", "ちめい", "place name"],
    ["茶色い", "ちゃいろい", "brown"],
    ["～着", "～ちゃく", "counter for clothes; finishing place"],
    ["着々", "ちゃくちゃく", "steadily"],
    ["中間", "ちゅうかん", "middle, midway, interim"],
    ["中旬", "ちゅうじゅん", "second third of a month"],
    ["抽象", "ちゅうしょう", "abstract"],
    ["中世", "ちゅうせい", "Middle Ages, medieval times"],
    ["中性", "ちゅうせい", "neuter gender, neutral"],
    ["中途", "ちゅうと", "in the middle, half-way"],
    ["中年", "ちゅうねん", "middle-aged"],
    ["長～", "ちょう～", "long ~"],
    ["～庁", "～ちょう", "office, agency"],
    ["～兆", "～ちょう", "trillion"],
    ["～長", "～ちょう", "leader, head"],
    ["～帳", "～ちょう", "~ book, notebook"],
    ["超過", "ちょうか", "excess, being more than"],
    ["彫刻", "ちょうこく", "carving, engraving, sculpture"],
    ["長所", "ちょうしょ", "strong point, merit; advantage"],
    ["長女", "ちょうじょ", "eldest daughter"],
    ["調整", "ちょうせい", "regulation, adjustment, tuning"],
    ["調節", "ちょうせつ", "regulation, adjustment, control"],
    ["長短", "ちょうたん", "length, long and short, +-"],
    ["頂点", "ちょうてん", "top, summit"],
    ["長男", "ちょうなん", "eldest son"],
    ["長方形", "ちょうほうけい", "rectangle, oblong"],
    ["調味料", "ちょうみりょう", "condiment, seasoning"],
    ["～丁目", "～ちょうめ", "~ district (of a town; city, block)"],
    ["チョーク", "チョーク", "chalk"],
    ["直後", "ちょくご", "immediately following"],
    ["直線", "ちょくせん", "straight line"],
    ["直前", "ちょくぜん", "just before"],
    ["直通", "ちょくつう", "direct connection"],
    ["直流", "ちょくりゅう", "direct current"],
    ["貯蔵", "ちょぞう", "storage, preservation"],
    ["直角", "ちょっかく", "right angle"],
    ["直径", "ちょっけい", "diameter"],
    ["散らかす", "ちらかす", "to scatter around, to leave untidy"],
    ["散らかる", "ちらかる", "to be in disorder"],
    ["塵紙", "ちりがみ", "tissue paper, toilet paper"],
    ["追加", "ついか", "addition, supplement, appendix"],
    ["ついで", "ついで", "opportunity, occasion"],
    ["～通", "～つう", "counter for letters"],
    ["通ずる", "つうずる", "to lead, to run, to open"],
    ["通知", "つうち", "notice, notification"],
    ["通帳", "つうちょう", "bankbook"],
    ["通用", "つうよう", "popular use, circulation"],
    ["通路", "つうろ", "passage, pathway"],
    ["～遣い", "～づかい", "use of ~"],
    ["～付", "～つき", "with ~"],
    ["突き当たり", "つきあたり", "end (e.g., of street)"],
    ["突き当たる", "つきあたる", "to run into, to collide with"],
    ["月日", "つきひ", "time, years, days"],
    ["～続く", "～つづく", "follow, continue, go on"],
    ["突っ込む", "つっこむ", "to plunge into, to stick into"],
    ["務める", "つとめる", "to serve, to act"],
    ["努める", "つとめる", "to try, to aim"],
    ["綱", "つな", "rope"],
    ["繋がり", "つながり", "connection, link, relationship"],
    ["粒", "つぶ", "grain"],
    ["潰す", "つぶす", "to smash, to waste"],
    ["潰れる", "つぶれる", "to be smashed, to go bankrupt"],
    ["つまずく", "つまずく", "to stumble, to trip"],
    ["詰まる", "つまる", "to be blocked, to be packed"],
    ["爪", "つめ", "fingernail or toenail"],
    ["艶", "つや", "gloss, glaze"],
    ["強気", "つよき", "firm, strong"],
    ["～辛い", "～づらい", "hard to do ~"],
    ["釣り合う", "つりあう", "to balance, to be in harmony, to suit"],
    ["吊る", "つる", "to hang"],
    ["吊す", "つるす", "to hang"],
    ["手洗い", "てあらい", "restroom, lavatory"],
    ["低～", "てい～", "low ~"],
    ["定員", "ていいん", "fixed number of regular personnel, capacity (e.g., of boat)"],
    ["定価", "ていか", "established price"],
    ["低下", "ていか", "fall, decline"],
    ["定期券", "ていきけん", "commuter pass, season ticket"],
    ["定休日", "ていきゅうび", "regular holiday"],
    ["停止", "ていし", "suspension, interruption, stoppage"],
    ["停車", "ていしゃ", "stopping (e.g., train)"],
    ["出入り", "でいり", "in and out, coming and going"],
    ["出入口", "でいりぐち", "exit and entrance"],
    ["手入れ", "ていれ", "repairs, maintenance"],
    ["テーマ", "テーマ", "theme, project, topic (GER: Thema)"],
    ["～滴", "～てき", "drop"],
    ["出来上がり", "できあがり", "be finished, ready"],
    ["出来上がる", "できあがる", "to be finished, to be ready"],
    ["的確", "てきかく", "precise, accurate"],
    ["適確", "てきかく", "precise, accurate"],
    ["手首", "てくび", "wrist"],
    ["凸凹", "でこぼこ", "unevenness, roughness, ruggedness"],
    ["手頃", "てごろ", "moderate, handy"],
    ["弟子", "でし", "pupil, disciple, apprentice"],
    ["でたらめ", "でたらめ", "irresponsible utterance, nonsense; random"],
    ["手帳", "てちょう", "notebook"],
    ["鉄橋", "てっきょう", "iron bridge"],
    ["手続き", "てつづき", "procedure, (legal) process, formalities"],
    ["鉄砲", "てっぽう", "gun"],
    ["テニスコート", "テニスコート", "tennis court"],
    ["手拭い", "てぬぐい", "(hand) towel"],
    ["手前", "てまえ", "before, this side"],
    ["出迎え", "でむかえ", "meeting, reception"],
    ["照らす", "てらす", "to shine on, to illuminate"],
    ["照る", "てる", "to shine"],
    ["～点", "～てん", "counter for scores"],
    ["展開", "てんかい", "develop, expansion (opposite of compression)"],
    ["伝記", "でんき", "biography, life story"],
    ["電球", "でんきゅう", "light bulb"],
    ["点数", "てんすう", "marks, points, score"],
    ["伝染", "でんせん", "contagion"],
    ["電池", "でんち", "battery"],
    ["点々", "てんてん", "here and there, little by little"],
    ["転々", "てんてん", "from one to another"],
    ["電柱", "でんちゅう", "telephone pole, telegraph pole, light pole"],
    ["天皇", "てんのう", "Emperor of Japan"]
  ],
  "n1": [
    ["現像", "げんぞう", "developing (film)"],
    ["原則", "げんそく", "principle, general rule"],
    ["見地", "けんち", "point of view"],
    ["現地", "げんち", "actual place, local"],
    ["限定", "げんてい", "limit, restriction"],
    ["原点", "げんてん", "origin (coordinates, starting point)"],
    ["原典", "げんてん", "original, source"],
    ["原爆", "げんばく", "atomic bomb"],
    ["原文", "げんぶん", "the text, original"],
    ["厳密", "げんみつ", "strict, close"],
    ["賢明", "けんめい", "wisdom, intelligence, prudence"],
    ["倹約", "けんやく", "thrift, economy, frugality"],
    ["原油", "げんゆ", "crude oil"],
    ["兼用", "けんよう", "multi-use, combined use"],
    ["権力", "けんりょく", "(political) power, authority, influence"],
    ["言論", "げんろん", "discussion, speech"],
    ["故～", "こ～", "deceased, late"],
    ["語彙", "ごい", "vocabulary, glossary"],
    ["恋する", "こいする", "to fall in love with, to love"],
    ["甲", "こう", "1st in rank; shell"],
    ["～光", "～こう", "light"],
    ["好意", "こうい", "good will, favor, courtesy"],
    ["行為", "こうい", "act, deed, conduct"],
    ["合意", "ごうい", "agreement, consent, mutual understanding"],
    ["工学", "こうがく", "engineering"],
    ["抗議", "こうぎ", "protest, objection"],
    ["合議", "ごうぎ", "consultation, conference"],
    ["皇居", "こうきょ", "Imperial Palace"],
    ["好況", "こうきょう", "prosperous conditions, healthy economy"],
    ["鉱業", "こうぎょう", "mining industry"],
    ["興業", "こうぎょう", "starting a business; industry"],
    ["高原", "こうげん", "tableland, plateau"],
    ["交互", "こうご", "mutual, reciprocal, alternate"],
    ["煌々と", "こうこうと", "brightly"],
    ["考古学", "こうこがく", "archeology"],
    ["工作", "こうさく", "handicraft, maneuvering"],
    ["耕作", "こうさく", "cultivation, farming"],
    ["鉱山", "こうざん", "mine"],
    ["講習", "こうしゅう", "short course, training"],
    ["口述", "こうじゅつ", "verbal statement"],
    ["控除", "こうじょ", "subsidy, deduction"],
    ["交渉", "こうしょう", "negotiation"],
    ["高尚", "こうしょう", "high, noble, refined"],
    ["向上", "こうじょう", "rise, improvement, progress"],
    ["行進", "こうしん", "march, parade"],
    ["香辛料", "こうしんりょう", "spices"],
    ["降水", "こうすい", "rainfall, precipitation"],
    ["洪水", "こうずい", "flood"],
    ["合成", "ごうせい", "synthetic, mixed"],
    ["公然", "こうぜん", "openly"],
    ["抗争", "こうそう", "dispute, resistance"],
    ["構想", "こうそう", "plan, plot, idea, conception"],
    ["後退", "こうたい", "retreat, backspace"],
    ["光沢", "こうたく", "luster, glossy finish (of photographs)"],
    ["公団", "こうだん", "public corporation"],
    ["好調", "こうちょう", "satisfactory, in good shape"],
    ["口頭", "こうとう", "oral"],
    ["講読", "こうどく", "reading"],
    ["購読", "こうどく", "subscription"],
    ["購入", "こうにゅう", "purchase, buy"],
    ["公認", "こうにん", "official recognition, authorization"],
    ["光熱費", "こうねつひ", "cost of fuel and light"],
    ["購買", "こうばい", "purchase, buy"],
    ["好評", "こうひょう", "popularity, favorable reputation"],
    ["交付", "こうふ", "delivering, furnishing (with copies)"],
    ["公募", "こうぼ", "public appeal, public contribution"],
    ["巧妙", "こうみょう", "ingenious, skillful, clever"],
    ["公用", "こうよう", "government business, public use, public expense"],
    ["小売", "こうり", "retail"],
    ["効率", "こうりつ", "efficiency"],
    ["公立", "こうりつ", "public institution"],
    ["護衛", "ごえい", "guard, convoy, escort"],
    ["コーナー", "コーナー", "corner"],
    ["小柄", "こがら", "small, diminutive"],
    ["小切手", "こぎって", "cheque, check"],
    ["国産", "こくさん", "domestic products"],
    ["国定", "こくてい", "state-sponsored, national"],
    ["告白", "こくはく", "confession, acknowledgment"],
    ["国防", "こくぼう", "national defense"],
    ["国有", "こくゆう", "national ownership"],
    ["極楽", "ごくらく", "paradise"],
    ["国連", "こくれん", "U.N., United Nations"],
    ["焦げ茶", "こげちゃ", "dark brown"],
    ["語源", "ごげん", "word root, word derivation, etymology"],
    ["心地", "ここち", "feeling, sensation, mood"],
    ["心得", "こころえ", "knowledge, information"],
    ["心掛け", "こころがけ", "readiness, intention, aim"],
    ["心掛ける", "こころがける", "to bear in mind, to aim to do"],
    ["志", "こころざし", "will, intention, motive"],
    ["志す", "こころざす", "to plan, to intend, to aspire to"],
    ["心強い", "こころづよい", "heartening, reassuring"],
    ["心細い", "こころぼそい", "helpless, hopeless, discouraging"],
    ["試み", "こころみ", "trial, experiment"],
    ["試みる", "こころみる", "to try, to test"],
    ["快い", "こころよい", "pleasant, agreeable"],
    ["誤差", "ごさ", "error"],
    ["ございます (かん)", "ございます (かん)", "to be (polite, to exist)"],
    ["孤児", "こじ", "orphan"],
    ["こじれる", "こじれる", "to get complicated, to grow worse"],
    ["こす (みずを～)", "こす (みずを～)", "to strain, to filter"],
    ["梢", "こずえ", "treetop"],
    ["個性", "こせい", "individuality, personality, idiosyncrasy"],
    ["戸籍", "こせき", "census, family register"],
    ["古代", "こだい", "ancient times"],
    ["こたつ", "こたつ", "table with heater, (originally) charcoal brazier in a floor well"],
    ["こだわる", "こだわる", "to fuss over, to be particular about"],
    ["誇張", "こちょう", "exaggeration"],
    ["こつ (をつかむ)", "こつ (をつかむ)", "secret, trick, hang"],
    ["滑稽", "こっけい", "funny, humorous, comical"],
    ["国交", "こっこう", "diplomatic relations"],
    ["骨董品", "こっとうひん", "curio"],
    ["固定", "こてい", "fixation, fixing (e.g., salary, capital)"],
    ["事柄", "ことがら", "matter, thing, affair, circumstance"],
    ["孤独", "こどく", "isolation, loneliness, solitude"],
    ["ことごとく", "ことごとく", "altogether, entirely"],
    ["言付け", "ことづけ", "to leave a message"],
    ["殊に", "ことに", "especially, above all"],
    ["粉々", "こなごな", "in very small pieces"],
    ["好ましい", "このましい", "nice, likable, desirable"],
    ["碁盤", "ごばん", "Go board"],
    ["個別", "こべつ", "particular case"],
    ["ごまかす", "ごまかす", "to deceive, to falsify, to misrepresent"],
    ["細やか", "こまやか", "meager, modest"],
    ["コマーシャル", "コマーシャル", "a commercial"],
    ["込める", "こめる", "to include, to put into"],
    ["コメント", "コメント", "comment"],
    ["籠もる", "こもる", "to seclude oneself, to be confined in"],
    ["固有", "こゆう", "characteristic, tradition, peculiar"],
    ["暦", "こよみ", "calendar, almanac"],
    ["凝らす", "こらす", "to concentrate, to devote, to peer into"],
    ["ごらんなさい (かん)", "ごらんなさい (かん)", "look, (please) try to do"],
    ["孤立", "こりつ", "isolation, helplessness"],
    ["懲りる", "こりる", "to learn by experience, to be disgusted with"],
    ["凝る", "こる", "to stiffen, to harden"],
    ["根気", "こんき", "patience; perseverance, energy"],
    ["根拠", "こんきょ", "basis, foundation"],
    ["混血", "こんけつ", "mixed race, mixed parentage"],
    ["コンタクト (レンズ)", "コンタクト (レンズ)", "contact; contact lens"],
    ["昆虫", "こんちゅう", "insect, bug"],
    ["根底", "こんてい", "root, basis, foundation"],
    ["混同", "こんどう", "confusion, mixing, merger"],
    ["コントラスト", "コントラスト", "contrast"],
    ["コントロール", "コントロール", "control"],
    ["コンパス", "コンパス", "compass"],
    ["根本", "こんぽん", "foundation, root, base"],
    ["財", "ざい", "fortune, riches"],
    ["再会", "さいかい", "meeting again, reunion"],
    ["災害", "さいがい", "calamity, disaster, misfortune"],
    ["細菌", "さいきん", "bacillus, bacterium, germ"],
    ["細工", "さいく", "work, craftsmanship, trick"],
    ["採掘", "さいくつ", "mining"],
    ["サイクル", "サイクル", "cycle"],
    ["採決", "さいけつ", "vote, roll call"],
    ["再建", "さいけん", "(temple or shrine) rebuilding"],
    ["再現", "さいげん", "reproduction, return, revival"],
    ["財源", "ざいげん", "source of funds, resources, finances"],
    ["在庫", "ざいこ", "stockpile, stock"],
    ["採算", "さいさん", "profit"],
    ["サイズ", "サイズ", "size"],
    ["再生", "さいせい", "playback, regeneration, resuscitation"],
    ["財政", "ざいせい", "economy, financial affairs"],
    ["最善", "さいぜん", "the very best"],
    ["採択", "さいたく", "adoption, selection, choice"],
    ["栽培", "さいばい", "cultivation"],
    ["再発", "さいはつ", "return, relapse, reoccurrence"],
    ["細胞", "さいぼう", "cell"],
    ["採用", "さいよう", "use, adopt"],
    ["遮る", "さえぎる", "to interrupt, to intercept, to obstruct"],
    ["さえずる", "さえずる", "to sing, to chirp, to twitter"],
    ["冴える", "さえる", "to be clear, to be bright, to be skillful"],
    ["竿", "さお", "rod, pole (e.g., for drying laundry)"],
    ["栄える", "さかえる", "to flourish, to prosper, to thrive"],
    ["差額", "さがく", "balance, difference, margin"],
    ["杯", "さかずき", "wine cup"],
    ["逆立ち", "さかだち", "handstand, headstand"],
    ["さきに (いぜん)", "さきに (いぜん)", "before, earlier than, previously"],
    ["詐欺", "さぎ", "fraud, swindle"],
    ["削減", "さくげん", "cut, reduction"],
    ["錯誤", "さくご", "mistake"],
    ["作戦", "さくせん", "military operations, tactics, strategy"],
    ["叫び", "さけび", "shout, scream, outcry"],
    ["捧げる", "ささげる", "to lift up, to give, to offer"],
    ["差し掛かる", "さしかかる", "to come near to, to approach"],
    ["指図", "さしず", "instruction, mandate"],
    ["差し出す", "さしだす", "to present, to submit, to hold out"],
    ["差し支える", "さしつかえる", "to interfere, to hinder"],
    ["授ける", "さずける", "to grant, to award, to teach"],
    ["摩する", "さする", "to rub, to stroke"],
    ["さぞ (さぞや。さぞかし)", "さぞ (さぞや。さぞかし)", "I am sure, certainly, no doubt"],
    ["定まる", "さだまる", "to become settled, to be fixed"],
    ["定める", "さだめる", "to decide, to determine"],
    ["座談会", "ざだんかい", "symposium, round-table discussion"],
    ["雑", "ざつ", "rough, crude"],
    ["雑貨", "ざっか", "miscellaneous goods, general goods"],
    ["殺人", "さつじん", "murder"],
    ["察する", "さっする", "to guess, to sense, to judge"],
    ["雑談", "ざつだん", "chatting, idle talk"],
    ["さっと", "さっと", "suddenly, smoothly"],
    ["さっぱりする", "さっぱりする", "to refresh"],
    ["悟る", "さとる", "to attain enlightenment, to understand"],
    ["最中", "さなか", "in the middle of, midst"],
    ["座標", "ざひょう", "coordinates"],
    ["さほど", "さほど", "not so, not that much"],
    ["サボる", "サボる", "to cut (skip) classes; to loaf on the job; to idle away one's time"],
    ["様", "さま", "state; way (a person does something); Mr. or Mrs."],
    ["寒気", "さむけ", "chill, shiver, cold"],
    ["侍", "さむらい", "samurai"],
    ["さも", "さも", "with gusto, with satisfaction"],
    ["作用", "さよう", "operation, effect, function"],
    ["さらう (こどもを～)", "さらう (こどもを～)", "to kidnap"],
    ["障る", "さわる", "to hinder, to interfere with, to affect"],
    ["酸", "さん", "acid"],
    ["山岳", "さんがく", "mountains"],
    ["参議院", "さんぎいん", "House of Councilors"],
    ["産休", "さんきゅう", "maternity leave"],
    ["サンキュー", "サンキュー", "thank you"],
    ["残金", "ざんきん", "remaining money"],
    ["産後", "さんご", "postpartum, after childbirth"],
    ["残酷", "ざんこく", "cruelty, harshness"],
    ["産出", "さんしゅつ", "yield, produce"],
    ["参照", "さんしょう", "reference, consultation, consultation"],
    ["参上", "さんじょう", "calling on, visiting"],
    ["残高", "ざんだか", "(bank) balance, remainder"],
    ["サンタクロース", "サンタクロース", "Santa Claus"],
    ["桟橋", "さんばし", "wharf, jetty, pier"],
    ["賛美", "さんび", "praise, adoration, glorification"],
    ["山腹", "さんぷく", "hillside, mountainside"],
    ["産婦人科", "さんふじんか", "maternity and gynecology department"],
    ["産物", "さんぶつ", "product, result, fruit"],
    ["山脈", "さんみゃく", "mountain range"],
    ["仕上がり", "しあがり", "finish, end, completion"],
    ["仕上", "しあげ", "end, finishing touches"],
    ["仕上げる", "しあげる", "to finish up, to complete"],
    ["飼育", "しいく", "breeding, raising, rearing"],
    ["強いて", "しいて", "to dare, to insist"],
    ["シート", "シート", "seat; sheet"],
    ["ジーパン", "ジーパン", "jeans"],
    ["仕入れる", "しいれる", "to lay in stock, to replenish stock, to procure"],
    ["強いる", "しいる", "to force, to compel, to coerce"],
    ["潮", "しお", "tide"],
    ["歯科", "しか", "dentistry"],
    ["自我", "じが", "self, ego"],
    ["自覚", "じかく", "self-conscious"],
    ["仕掛", "しかけ", "device, trick, mechanism"],
    ["仕掛ける", "しかける", "to lay, to set, to wage"],
    ["しかしながら", "しかしながら", "however, nevertheless"],
    ["色彩", "しきさい", "color"],
    ["式場", "しきじょう", "ceremonial hall, place of ceremony (e.g., marriage)"],
    ["しきたり", "しきたり", "custom, conventional practice, tradition"],
    ["事業", "じぎょう", "project, enterprise, business"],
    ["軽蔑", "けいべつ", "scorn, disdain"],
    ["経歴", "けいれき", "personal history, career"],
    ["経路", "けいろ", "course, route, channel"],
    ["けがらわしい", "けがらわしい", "filthy, unfair"],
    ["劇団", "げきだん", "troupe, theatrical company"],
    ["激励", "げきれい", "encouragement"],
    ["ゲスト", "ゲスト", "guest"],
    ["獣", "けだもの", "beast, brute"],
    ["決", "けつ", "decision, vote"],
    ["決意", "けつい", "decision, determination"],
    ["結核", "けっかく", "tuberculosis"],
    ["決議", "けつぎ", "resolution, vote, decision"],
    ["結合", "けつごう", "combination, union"],
    ["決算", "けっさん", "balance sheet, settlement of accounts"],
    ["月謝", "げっしゃ", "monthly tuition fee"],
    ["決勝", "けっしょう", "finals (in sports)"],
    ["結晶", "けっしょう", "crystal, crystallization"],
    ["結成", "けっせい", "formation"],
    ["結束", "けっそく", "union, unity"],
    ["げっそり", "げっそり", "being disheartened, losing weight"],
    ["決断", "けつだん", "decision, determination"],
    ["月賦", "げっぷ", "monthly installment"],
    ["欠乏", "けつぼう", "shortage"],
    ["蹴飛ばす", "けとばす", "to kick away, to kick (someone)"],
    ["けなす", "けなす", "to speak ill of"],
    ["煙たい", "けむたい", "smoky, feeling awkward"],
    ["煙る", "けむる", "to smoke (e.g., fire)"],
    ["獣", "けもの", "beast, brute"],
    ["家来", "けらい", "retainer, retinue, servant"],
    ["下痢", "げり", "diarrhea"],
    ["権威", "けんい", "authority, power, influence"],
    ["兼業", "けんぎょう", "holding two jobs at the same time"],
    ["原形", "げんけい", "original form, base form"],
    ["原型", "げんけい", "prototype, model, archetypal"],
    ["権限", "けんげん", "power, authority, jurisdiction"],
    ["現行", "げんこう", "present, current, in operation"],
    ["健在", "けんざい", "in good health, well"],
    ["原作", "げんさく", "original work"],
    ["検事", "けんじ", "public prosecutor"],
    ["原子", "げんし", "atom"],
    ["元首", "げんしゅ", "ruler, sovereign"],
    ["原書", "げんしょ", "original document"],
    ["懸賞", "けんしょう", "offering prizes, winning, reward"],
    ["健全", "けんぜん", "health, soundness, wholesome"],
    ["元素", "げんそ", "element"],
    ["同調", "どうちょう", "sympathy, agree with, alignment"],
    ["到底", "とうてい", "(cannot) possibly"],
    ["動的", "どうてき", "dynamic, kinetic"],
    ["尊い", "とうとい", "precious, valuable, noble"],
    ["貴い", "とうとい", "precious, valuable, noble"],
    ["同等", "どうとう", "equality, equal, same rank"],
    ["堂々", "どうどう", "magnificent, grand, impressive"],
    ["尊ぶ", "とうとぶ", "to value, to prize, to esteem"],
    ["どうにか", "どうにか", "in some way or other, one way or another"],
    ["投入", "とうにゅう", "throw, investment, making (an electrical circuit)"],
    ["導入", "どうにゅう", "introduction, bringing in, leading in"],
    ["当人", "とうにん", "the one concerned, the said person"],
    ["同封", "どうふう", "enclosure (e.g., in a letter)"],
    ["逃亡", "とうぼう", "escape"],
    ["冬眠", "とうみん", "hibernation, winter sleep"],
    ["同盟", "どうめい", "alliance, union, league"],
    ["どうやら", "どうやら", "it seems like, somehow or other"],
    ["動力", "どうりょく", "power, motive power, dynamic force"],
    ["登録", "とうろく", "registration, register, record"],
    ["討論", "とうろん", "discussion; debate"],
    ["遠ざかる", "とおざかる", "to go far off"],
    ["遠回り", "とおまわり", "detour, roundabout way"],
    ["トーン", "トーン", "tone"],
    ["とかく", "とかく", "anyhow, anyway, in any case"],
    ["とがめる", "とがめる", "to blame, to rebuke"],
    ["時折", "ときおり", "sometimes"],
    ["とぎれる", "とぎれる", "to pause, to be interrupted"],
    ["研ぐ", "とぐ", "to sharpen, to grind, to polish"],
    ["特技", "とくぎ", "special talent; skill"],
    ["独裁", "どくさい", "dictatorship, despotism"],
    ["特産", "とくさん", "specialty, special product"],
    ["独自", "どくじ", "original, peculiar, characteristic"],
    ["特集", "とくしゅう", "feature (e.g., newspaper, special edition, report)"],
    ["独占", "どくせん", "monopoly"],
    ["独創", "どくそう", "originality"],
    ["得点", "とくてん", "score, points made"],
    ["特派", "とくは", "send specially, special envoy"],
    ["特有", "とくゆう", "characteristic (of, peculiar (to))"],
    ["とげ (をさす)", "とげ (をさす)", "thorn"],
    ["遂げる", "とげる", "to accomplish, to achieve, to carry out"],
    ["～どころか", "～どころか", "rather, far from"],
    ["年頃", "としごろ", "age, marriageable age, adolescence"],
    ["戸締り", "とじまり", "closing up, locking the doors"],
    ["途上", "とじょう", "en/in route, half way"],
    ["土台", "どだい", "foundation, base, basis"],
    ["途絶える", "とだえる", "to stop, to cease, to come to an end"],
    ["特許", "とっきょ", "special permission, patent"],
    ["特権", "とっけん", "privilege, special right"],
    ["とっさに", "とっさに", "at once"],
    ["突如", "とつじょ", "suddenly, all of a sudden"],
    ["とって", "とって", "handle, grip, knob"],
    ["突破", "とっぱ", "breaking through, breakthrough, penetration"],
    ["土手", "どて", "embankment, bank"],
    ["届", "とどけ", "report, notification, registration"],
    ["滞る", "とどこおる", "to stagnate, to be delayed"],
    ["整える", "ととのえる", "to put in order, to arrange, to adjust; to get ready, to prepare; to raise money"],
    ["止める", "とどめる", "to end, to stop, to cease, to resign"],
    ["唱える", "となえる", "to recite, to chant, to call upon"],
    ["殿様", "とのさま", "feudal lord"],
    ["土俵", "どひょう", "arena"],
    ["扉", "とびら", "door, opening"],
    ["溝", "どぶ", "ditch, drain, gap"],
    ["徒歩", "とほ", "walking, going on foot"],
    ["土木", "どぼく", "public works"],
    ["とぼける", "とぼける", "to play dumb, to feign ignorance, to play innocent, to have a blank facial expression; to play the fool; to be in one's dotage"],
    ["乏しい", "とぼしい", "meager, scarce, hard up, poor"],
    ["富", "とみ", "wealth, fortune"],
    ["富む", "とむ", "to be rich, to become rich"],
    ["共稼ぎ", "ともかせぎ", "working together, (husband and wife) earning a living together"],
    ["伴う", "ともなう", "to accompany, to bring with"],
    ["共働き", "ともばたらき", "dual income (husband and wife both working)"],
    ["ドライ", "ドライ", "dry"],
    ["ドライクリーニング", "ドライクリーニング", "dry cleaning"],
    ["ドライバー", "ドライバー", "driver, screwdriver"],
    ["ドライブイン", "ドライブイン", "drive in"],
    ["トラブル", "トラブル", "trouble (sometimes used as a verb)"],
    ["トランジスター", "トランジスター", "transistor"],
    ["とりあえず", "とりあえず", "at once, first of all, for the time being"],
    ["取扱", "とりあつかい", "treatment, handling, management"],
    ["取り扱う", "とりあつかう", "to treat, to handle, to deal in"],
    ["鳥居", "とりい", "Shinto shrine archway"],
    ["取り替え", "とりかえ", "swap, exchange"],
    ["取り組む", "とりくむ", "to tackle, to engage in a bout, to come to grips with"],
    ["取締り", "とりしまり", "control, crackdown, supervision"],
    ["取り締まる", "とりしまる", "to crack down, to control, to supervise"],
    ["取り調べる", "とりしらべる", "to investigate, to examine"],
    ["取り立てる", "とりたてる", "to collect, to extort"],
    ["取り次ぐ", "とりつぐ", "to act as an agent for, to announce (someone), to convey (a message)"],
    ["取り付ける", "とりつける", "to furnish, to install; to get someone's agreement"],
    ["取り除く", "とりのぞく", "to remove, to take away, to set apart"],
    ["取引", "とりひき", "transactions, dealings, business"],
    ["取り巻く", "とりまく", "to surround, to circle, to enclose"],
    ["取り混ぜる", "とりまぜる", "to mix, to put together"],
    ["取り戻す", "とりもどす", "to take back, to regain"],
    ["取り寄せる", "とりよせる", "to order, to send away for"],
    ["ドリル", "ドリル", "drill"],
    ["副", "とりわけ", "especially, above all"],
    ["とろける", "とろける", "melt; to be enchanted with"],
    ["鈍感", "どんかん", "thickheadedness, stolidity"],
    ["とんだ", "とんだ", "terrible, awful, serious, absolutely not"],
    ["度忘れ", "どわすれ", "lapse of memory, forget for a moment"],
    ["問屋", "とんや", "wholesale store"],
    ["内閣", "ないかく", "cabinet, (government)"],
    ["乃至", "ないし", "from...to, between...and, or"],
    ["内緒", "ないしょ", "secrecy, privacy, secret"],
    ["内心", "ないしん", "innermost thoughts, real intention, inmost heart"],
    ["内蔵", "ないぞう", "internal organ; built-in"],
    ["ナイター", "ナイター", "game under lights (e.g., baseball), night game"],
    ["内部", "ないぶ", "interior, inside, internal"],
    ["内乱", "ないらん", "civil war, domestic conflict"],
    ["内陸", "ないりく", "inland"],
    ["苗", "なえ", "rice seedling"],
    ["なおさら", "なおさら", "all the more, still less"],
    ["流し", "ながし", "sink"],
    ["長々", "ながなが", "long, drawn-out, very long"],
    ["中程", "なかほど", "middle, midway"],
    ["渚", "なぎさ", "water's edge, beach, shore"],
    ["嘆く", "なげく", "to sigh, to lament, to grieve"],
    ["投げ出す", "なげだす", "to abandon, to throw out"],
    ["仲人", "なこうど", "go-between, matchmaker"],
    ["和やか", "なごやか", "mild, calm, harmonious"],
    ["名残", "なごり", "remains, traces, memory"],
    ["情け", "なさけ", "sympathy, compassion"],
    ["情無い", "なさけない", "miserable, pitiable, shameful"],
    ["情深い", "なさけぶかい", "tender-hearted, compassionate"],
    ["詰る", "なじる", "to rebuke, to scold, to tell off"],
    ["名高い", "なだかい", "famous, celebrated, well-known"],
    ["雪崩", "なだれ", "avalanche"],
    ["懐く", "なつく", "to become emotionally attached"],
    ["名付ける", "なづける", "to name"],
    ["何気ない", "なにげない", "casual, unconcerned"],
    ["なにとぞ", "なにとぞ", "please, kindly, by all means"],
    ["なにより", "なにより", "most, best"],
    ["ナプキン", "ナプキン", "napkin"],
    ["名札", "なふだ", "name plate, name tag"],
    ["生臭い", "なまぐさい", "smelling of fish or blood, fish or meat"],
    ["生温い", "なまぬるい", "lukewarm, halfhearted"],
    ["生身", "なまみ", "living flesh, flesh and blood, the quick"],
    ["鉛", "なまり", "lead (the metal)"],
    ["滑らか", "なめらか", "smoothness, glassiness"],
    ["嘗める", "なめる", "to lick; to experience; to make fun of"],
    ["悩ましい", "なやましい", "seductive, melancholy, languid"],
    ["悩ます", "なやます", "to bother, to harass, to molest"],
    ["悩み", "なやみ", "trouble(s), worry, distress"],
    ["並びに", "ならびに", "and"],
    ["成り立つ", "なりたつ", "to consist of; to be practical (logical, feasible, viable), to be concluded, to hold true"],
    ["なるたけ", "なるたけ", "as much as possible, if possible"],
    ["慣れ", "なれ", "practice, experience"],
    ["馴々しい", "なれなれしい", "familiar, make free with"],
    ["～なんか", "～なんか", "in the least ~"],
    ["ナンセンス", "ナンセンス", "nonsense"],
    ["何だか", "なんだか", "a little, somewhat, somehow"],
    ["なんだかんだ", "なんだかんだ", "something or other"],
    ["なんなり", "なんなり", "anything, whatever"],
    ["荷", "に", "load, baggage, cargo"],
    ["似通う", "にかよう", "to resemble closely"],
    ["にきび", "にきび", "pimple, acne"],
    ["賑わう", "にぎわう", "to prosper, to flourish, to be crowded with people"],
    ["憎しみ", "にくしみ", "hatred"],
    ["肉親", "にくしん", "blood relationship, blood relative"],
    ["肉体", "にくたい", "the body, the flesh"],
    ["逃げ出す", "にげだす", "to run away, to escape from"],
    ["西日", "にしび", "westering sun"],
    ["滲む", "にじむ", "to run, to blur, to spread"],
    ["にせ物", "にせもの", "imitation, counterfeit"],
    ["日夜", "にちや", "day and night, always"],
    ["荷造り", "にづくり", "packing, baling, crating"],
    ["担う", "になう", "to carry on shoulder, to bear (burden), to shoulder (gun)"],
    ["鈍る", "にぶる", "to become less capable, to grow dull, to become blunt, to weaken"],
    ["にも関わらず", "にもかかわらず", "in spite of, nevertheless"],
    ["ニュアンス", "ニュアンス", "nuance"],
    ["ニュー", "ニュー", "new"],
    ["入手", "にゅうしゅ", "obtaining, coming to hand"],
    ["入賞", "にゅうしょう", "winning a prize or place (in a contest"],
    ["入浴", "にゅうよく", "bathe, bathing"],
    ["尿", "にょう", "urine"],
    ["認識", "にんしき", "recognition, cognizance"],
    ["妊娠", "にんしん", "conception, pregnancy"],
    ["任務", "にんむ", "duty, mission, task"],
    ["任命", "にんめい", "appointment, nomination, ordination"],
    ["抜かす", "ぬかす", "to omit, to leave out"],
    ["抜け出す", "ぬけだす", "to slip out, to sneak away, to excel"],
    ["主", "ぬし", "owner, master, god"],
    ["沼", "ぬま", "swamp, bog, pond"],
    ["音色", "ねいろ", "tone color, timbre"],
    ["値打ち", "ねうち", "value, worth, price"],
    ["ネガ", "ネガ", "(photographic) negative"],
    ["寝かせる", "ねかせる", "to put to bed, to lay down, to ferment"],
    ["ねじまわし", "ねじまわし", "screwdriver"],
    ["捩れる", "ねじれる", "twist, strain"],
    ["妬む", "ねたむ", "to be jealous, to be envious"],
    ["ねだる", "ねだる", "to nag, to demand"],
    ["熱意", "ねつい", "zeal, enthusiasm"],
    ["熱湯", "ねっとう", "boiling water"],
    ["熱量", "ねつりょう", "calorific value"],
    ["粘り", "ねばり", "stickiness, viscosity"],
    ["粘る", "ねばる", "to be sticky, to be adhesive, to persist, to stick to"],
    ["値引き", "ねびき", "price reduction, discount"],
    ["根回し", "ねまわし", "making necessary arrangements"],
    ["眠たい", "ねむたい", "sleepy"],
    ["練る", "ねる", "to knead, to work over, to polish up"],
    ["念", "ねん", "sense, feeling, desire"],
    ["年賀", "ねんが", "New Year's greetings, New Year's card"],
    ["念願", "ねんがん", "one's heart's desire, earnest petition"],
    ["年号", "ねんごう", "name of an era, year number"],
    ["燃焼", "ねんしょう", "burning, combustion"],
    ["年長", "ねんちょう", "seniority"],
    ["燃料", "ねんりょう", "fuel"],
    ["年輪", "ねんりん", "annual tree ring"],
    ["ノイローゼ", "ノイローゼ", "neurosis (GER: Neurose)"],
    ["農耕", "のうこう", "farming, agriculture"],
    ["農場", "のうじょう", "farm"],
    ["農地", "のうち", "agricultural land"],
    ["納入", "のうにゅう", "payment, supply"],
    ["逃す", "のがす", "to let loose, to set free, to let escape"],
    ["逃れる", "のがれる", "to escape"],
    ["軒並", "のきなみ", "row of houses; uniformly"],
    ["望ましい", "のぞましい", "desirable, hoped for"],
    ["乗っ取る", "のっとる", "to capture, to occupy, to take over"],
    ["のどか", "のどか", "tranquil, calm, quiet"],
    ["罵る", "ののしる", "to speak ill of, to abuse"],
    ["延べ", "のべ", "futures, credit (buying), stretching, total"],
    ["飲み込む", "のみこむ", "to gulp down, to swallow deeply, to understand"],
    ["乗り込む", "のりこむ", "to board, to get into (a car); to march into, to enter"],
    ["刃", "は", "blade, sword"],
    ["～派", "～は", "group, party, section (mil)"],
    ["バー", "バー", "bar"],
    ["把握", "はあく", "grasp, catch, understanding"],
    ["パート", "パート", "part-time job"],
    ["廃棄", "はいき", "disposal, abandon, discarding"],
    ["配給", "はいきゅう", "distribution (e.g., films, rice"],
    ["ばい菌", "ばいきん", "bacteria, germ(s)"],
    ["配偶者", "はいぐうしゃ", "spouse"],
    ["拝啓", "はいけい", "-- a formal greeting used at the beginning of a letter --"],
    ["背景", "はいけい", "background, scenery, setting"],
    ["背後", "はいご", "back, rear"],
    ["廃止", "はいし", "abolition, repeal"],
    ["拝借", "はいしゃく", "(humble) (polite) borrowing"],
    ["排除", "はいじょ", "exclusion, removal, rejection"],
    ["賠償", "ばいしょう", "reparations, indemnity, compensation"],
    ["排水", "はいすい", "drainage"],
    ["敗戦", "はいせん", "defeat, losing a war"],
    ["配置", "はいち", "arrangement (of resources), disposition"],
    ["配布", "はいふ", "distribution"],
    ["配分", "はいぶん", "distribution, allotment"],
    ["敗北", "はいぼく", "defeat (as a verb it means 'to be defeated')"],
    ["倍率", "ばいりつ", "diameter, magnification"],
    ["配慮", "はいりょ", "consideration, concern, forethought"],
    ["配列", "はいれつ", "arrangement, array (programming)"],
    ["破壊", "はかい", "destruction"],
    ["いたわる", "いたわる", "to sympathize with, to console, to care for"],
    ["一概に", "いちがいに", "unconditionally, necessarily"],
    ["著しい", "いちじるしい", "remarkable, considerable"],
    ["一同", "いちどう", "all present, all concerned, all of us"],
    ["一部分", "いちぶぶん", "a part, a portion"],
    ["一別", "いちべつ", "parting"],
    ["一面", "いちめん", "one side, the other hand"],
    ["一目", "いちもく", "a glance, a look, a glimpse"],
    ["一様", "いちよう", "uniform, similar, equal"],
    ["一律", "いちりつ", "even, uniform, equal"],
    ["一連", "いちれん", "a series, a chain, a ream (of paper)"],
    ["一括", "いっかつ", "all together, batch"],
    ["一気", "いっき", "at one push, in one gulp"],
    ["一挙に", "いっきょに", "at a stroke, with a single swoop"],
    ["一見", "いっけん", "a look, a glimpse, glance; first meeting"],
    ["一切", "いっさい", "without exception, the whole"],
    ["一心", "いっしん", "one mind, with rapt attention"],
    ["いっそ", "いっそ", "rather, sooner, might as well"],
    ["一変", "いっぺん", "complete change"],
    ["意図", "いと", "intention, aim, design"],
    ["営む", "いとなむ", "to carry on (e.g., in ceremony), to run a business"],
    ["挑む", "いどむ", "to challenge"],
    ["稲光", "いなびかり", "(flash of) lightning"],
    ["祈り", "いのり", "prayer, supplication"],
    ["いびき", "いびき", "snoring"],
    ["今更", "いまさら", "now, again"],
    ["未だ", "いまだ", "yet, still"],
    ["移民", "いみん", "emigrant, immigrant"],
    ["嫌々", "いやいや", "reluctantly, by no means, unwillingly"],
    ["卑しい", "いやしい", "greedy, vulgar, shabby"],
    ["いやに", "いやに", "awfully, terribly"],
    ["いやらしい", "いやらしい", "unpleasant, disgusting, indecent"],
    ["意欲", "いよく", "will, desire, ambition"],
    ["威力", "いりょく", "power, might, authority"],
    ["衣類", "いるい", "clothes, clothing, garments"],
    ["異論", "いろん", "different opinion, objection"],
    ["印鑑", "いんかん", "stamp, seal"],
    ["陰気", "いんき", "gloom, melancholy"],
    ["隠居", "いんきょ", "retirement; retired person"],
    ["インターチェンジ", "インターチェンジ", "interchange"],
    ["インターナショナル", "インターナショナル", "international"],
    ["インターフォン", "インターフォン", "entry phone, intercom"],
    ["インテリ", "インテリ", "(abbr.) egghead, intelligentsia"],
    ["インフォメーション", "インフォメーション", "information"],
    ["インフレ", "インフレ", "(abbr.) inflation"],
    ["受かる", "うかる", "to pass (examination)"],
    ["受け入れ", "うけいれ", "receiving, acceptance"],
    ["受け入れる", "うけいれる", "to accept, to receive"],
    ["受け継ぐ", "うけつぐ", "to inherit, to succeed"],
    ["受け付ける", "うけつける", "to be accepted, to receive (an application)"],
    ["受け止める", "うけとめる", "to catch, to react to, to take"],
    ["受身", "うけみ", "passive, passive voice"],
    ["受持ち", "うけもち", "charge (of something), matter in one's charge"],
    ["動き", "うごき", "movement, activity, trend"],
    ["埋める", "うずめる", "to bury, to fill"],
    ["嘘つき", "うそつき", "liar"],
    ["うたた寝", "うたたね", "dozing, napping"],
    ["打ち明ける", "うちあける", "to confess, to be open"],
    ["打ち切る", "うちきる", "to stop, to abort, to discontinue, to close"],
    ["打ち消し", "うちけし", "(gram) negation, denial, negative"],
    ["打ち込む", "うちこむ", "to devote oneself to, to shoot into"],
    ["団扇", "うちわ", "fan"],
    ["内訳", "うちわけ", "the items, breakdown, classification"],
    ["写し", "うつし", "copy, duplicate"],
    ["訴え", "うったえ", "lawsuit, complaint"],
    ["うっとうしい", "うっとうしい", "weary, annoying"],
    ["うつむく", "うつむく", "to look downward, to stoop"],
    ["空ろ", "うつろ", "blank, hollow, empty"],
    ["器", "うつわ", "bowl, vessel, container"],
    ["腕前", "うでまえ", "ability, skill, facility"],
    ["雨天", "うてん", "rainy weather"],
    ["促す", "うながす", "to urge, to suggest, to demand"],
    ["うぬぼれ", "うぬぼれ", "pretension, conceit, hubris"],
    ["生まれつき", "うまれつき", "by nature, by birth, native"],
    ["埋め込む", "うめこむ", "to embed, implant"],
    ["梅干し", "うめぼし", "dried plum"],
    ["裏返し", "うらがえし", "inside out, reverse"],
    ["売り出し", "うりだし", "(bargain) sale"],
    ["売り出す", "うりだす", "to put on sale, to market"],
    ["潤う", "うるおう", "to be moist; to profit by"],
    ["浮気", "うわき", "affair, to cheat"],
    ["上回る", "うわまわる", "to exceed"],
    ["植わる", "うわる", "to be planted"],
    ["運営", "うんえい", "management, administration, operation"],
    ["うんざり", "うんざり", "tedious, boring, being fed up with"],
    ["運送", "うんそう", "shipping, freight"],
    ["運賃", "うんちん", "freight rates, shipping expenses, (passenger) fare"],
    ["云々", "うんぬん", "and so on, and so forth"],
    ["運搬", "うんぱん", "transport, carriage"],
    ["運命", "うんめい", "fate"],
    ["運輸", "うんゆ", "transportation"],
    ["運用", "うんよう", "making use of, application, practical use"],
    ["エアメール", "エアメール", "air mail"],
    ["～営", "～えい", "~ run"],
    ["英字", "えいじ", "English letter (character)"],
    ["映写", "えいしゃ", "projection"],
    ["映像", "えいぞう", "reflection, image"],
    ["英雄", "えいゆう", "hero, great man"],
    ["液", "えき", "liquid, fluid"],
    ["閲覧", "えつらん", "inspection, reference, browse"],
    ["獲物", "えもの", "game, spoils, trophy"],
    ["襟", "えり", "neck, collar"],
    ["エレガント", "エレガント", "elegant"],
    ["円滑", "えんかつ", "harmony, smoothness"],
    ["縁側", "えんがわ", "veranda, porch, balcony, open corridor"],
    ["沿岸", "えんがん", "coast, shore"],
    ["婉曲", "えんきょく", "euphemistic, indirect, insinuating"],
    ["演出", "えんしゅつ", "production (erg. play, direction)"],
    ["エンジニア", "エンジニア", "engineer"],
    ["演じる", "えんじる", "to perform, to play (a part), to act"],
    ["演ずる", "えんずる", "to perform, to play (a part), to act"],
    ["沿線", "えんせん", "along railway line"],
    ["縁談", "えんだん", "marriage proposal"],
    ["遠方", "えんぽう", "long way, distant place"],
    ["円満", "えんまん", "harmony, peace, smoothness"],
    ["追い込む", "おいこむ", "to herd, to corner, to drive"],
    ["追い出す", "おいだす", "to expel, to drive out"],
    ["於いて", "おいて", "at, in, on"],
    ["老いる", "おいる", "to age, to grow old"],
    ["応急", "おうきゅう", "emergency"],
    ["黄金", "おうごん", "gold"],
    ["往診", "おうしん", "doctor's visit, house call"],
    ["応募", "おうぼ", "subscription, application"],
    ["おおい (かん)", "おおい (かん)", "hey"],
    ["大方", "おおかた", "almost all, majority"],
    ["大柄", "おおがら", "large build, large pattern"],
    ["おおげさ", "おおげさ", "grandiose, exaggerated"],
    ["大筋", "おおすじ", "outline, summary"],
    ["大空", "おおぞら", "heaven, the sky"],
    ["オートマチック", "オートマチック", "automatic"],
    ["大幅", "おおはば", "full width, large scale, drastic"],
    ["おおまかな", "おおまかな", "rough, approximate"],
    ["大水", "おおみず", "flood"],
    ["公", "おおやけ", "public"],
    ["犯す", "おかす", "to perpetrate, to violate"],
    ["侵す", "おかす", "to invade, to raid, to trespass"],
    ["臆病", "おくびょう", "cowardice, timidity"],
    ["遅らす", "おくらす", "to retard, to delay"],
    ["厳か", "おごそか", "majestic, dignified"],
    ["行い", "おこない", "conduct, behavior, action"],
    ["おごる (ゆうしょくを～)", "おごる (ゆうしょくを～)", "to give (someone) a treat"],
    ["収まる", "おさまる", "to settle into; to be obtained"],
    ["納まる", "おさまる", "to settle into; to be obtained"],
    ["治まる", "おさまる", "to be at peace, to calm down"],
    ["お産", "おさん", "(giving) birth"],
    ["押し切る", "おしきる", "to have one's own way"],
    ["押し込む", "おしこむ", "to push into, to crowd into"],
    ["惜しむ", "おしむ", "to be frugal, to value, to regret"],
    ["押し寄せる", "おしよせる", "to push aside, to advance on"],
    ["雄", "おす", "male (animal)"],
    ["御世辞", "おせじ", "flattery, compliment"],
    ["襲う", "おそう", "to attack"],
    ["遅くとも", "おそくとも", "at the latest"],
    ["恐れ", "おそれ", "fear, horror"],
    ["恐れ入る", "おそれいる", "to be filled with awe, to feel small"],
    ["おだてる", "おだてる", "to flatter"],
    ["落ち込む", "おちこむ", "to get depressed"],
    ["落ち着き", "おちつき", "calm, composure"],
    ["落葉", "おちば", "fallen leaves"],
    ["乙", "おつ", "2nd in rank"],
    ["お使い", "おつかい", "errand"],
    ["おっかない", "おっかない", "frightening, scary"],
    ["お手上げ", "おてあげ", "given in, given up hope"],
    ["おどおど", "おどおど", "coweringly, hesitantly"],
    ["脅す", "おどす", "to threaten, to menace"],
    ["訪れる", "おとずれる", "to visit"],
    ["お供", "おとも", "attendant, companion"],
    ["衰える", "おとろえる", "to become weak, to decline"],
    ["同い年", "おないどし", "of the same age"],
    ["自ずから", "おのずから", "naturally, as a matter of course"],
    ["怯える", "おびえる", "to become frightened"],
    ["おびただしい", "おびただしい", "abundantly, innumerably"],
    ["脅かす", "おびやかす", "to threaten, to coerce"],
    ["帯びる", "おびる", "to bear, to carry, to be entrusted"],
    ["お袋", "おふくろ", "mother"],
    ["覚え", "おぼえ", "memory, sense, experience"],
    ["おまけ", "おまけ", "a discount; something additional"],
    ["お宮", "おみや", "Shinto shrine"],
    ["おむつ", "おむつ", "diaper, nappy"],
    ["思い付き", "おもいつき", "plan, idea, suggestion"],
    ["趣", "おもむき", "flavor, appearance, quaint"],
    ["赴く", "おもむく", "to go, to proceed"],
    ["重んじる", "おもんじる", "to respect, to honor, to esteem, to prize"],
    ["重んずる", "おもんずる", "to honor, to respect, to value"],
    ["親父", "おやじ", "one's father, old man, one's boss"],
    ["及び", "および", "and, as well as"],
    ["及ぶ", "およぶ", "to reach, to extend"],
    ["折", "おり", "chance, occasion"],
    ["檻", "おり", "cage, pen, jail cell"],
    ["オリエンテーション", "オリエンテーション", "orientation"],
    ["折り返す", "おりかえす", "to turn up, to fold back"],
    ["織物", "おりもの", "textile, fabric"],
    ["俺", "おれ", "I (ego) (boastful first-person pronoun)"],
    ["愚か", "おろか", "foolish, stupid"],
    ["おろそか", "おろそか", "neglect, negligence, carelessness"],
    ["おんぶ", "おんぶ", "carrying on one's back (erg. Baby)"],
    ["オンライン", "オンライン", "on-line"],
    ["温和", "おんわ", "gentle, mild, moderate"],
    ["我", "が～", "ego"],
    ["カーペット", "カーペット", "carpet"],
    ["～界", "～かい", "world, circle, kingdom"],
    ["～街", "～がい", "town"],
    ["改悪", "かいあく", "deterioration, changing for the worse"],
    ["海運", "かいうん", "marine transportation"],
    ["外貨", "がいか", "foreign money"],
    ["改革", "かいかく", "reform, reformation, innovation"],
    ["貝殻", "かいがら", "shell"],
    ["外観", "がいかん", "appearance, exterior, facade"],
    ["階級", "かいきゅう", "class, rank, grade"],
    ["海峡", "かいきょう", "channel"],
    ["会見", "かいけん", "interview, conference"],
    ["介護", "かいご", "nursing"],
    ["開催", "かいさい", "holding a meeting, open an exhibition"],
    ["回収", "かいしゅう", "collection, recovery"],
    ["改修", "かいしゅう", "repair, improvement"],
    ["怪獣", "かいじゅう", "monster"],
    ["解除", "かいじょ", "cancellation, release, cancel"],
    ["外相", "がいしょう", "Foreign Minister"],
    ["害する", "がいする", "to harm, to offend"],
    ["概説", "がいせつ", "general statement, outline"],
    ["回送", "かいそう", "forwarding"],
    ["階層", "かいそう", "class, level, stratum, hierarchy"],
    ["開拓", "かいたく", "cultivation, pioneer"],
    ["会談", "かいだん", "conversation, interview"],
    ["改定", "かいてい", "reform"],
    ["改訂", "かいてい", "revision"],
    ["ガイド", "ガイド", "guide"],
    ["街道", "かいどう", "highway"],
    ["該当", "がいとう", "corresponding, answering to, coming under"],
    ["街頭", "がいとう", "in the street"],
    ["ガイドブック", "ガイドブック", "guidebook"],
    ["介入", "かいにゅう", "intervention"],
    ["概念", "がいねん", "general idea, concept, notion"],
    ["開発", "かいはつ", "development, exploitation"],
    ["海抜", "かいばつ", "height above sea level"],
    ["介抱", "かいほう", "nursing, looking after"],
    ["解剖", "かいぼう", "dissection, autopsy"],
    ["外来", "がいらい", "(abbr.) imported, outpatient clinic"],
    ["回覧", "かいらん", "circulation"],
    ["概略", "がいりゃく", "outline, summary, gist"],
    ["海流", "かいりゅう", "ocean current"],
    ["改良", "かいりょう", "improvement, reform"],
    ["回路", "かいろ", "circuit (electric)"],
    ["海路", "かいろ", "sea route"],
    ["省みる", "かえりみる", "to reflect"],
    ["顧みる", "かえりみる", "to look back, to turn around, to review"],
    ["顔付き", "かおつき", "facial expression"],
    ["課外", "かがい", "extracurricular"],
    ["掲げる", "かかげる", "to hoist, to fly (a sail), to float (a flag)"],
    ["かかと", "かかと", "shoe heel"],
    ["書き取る", "かきとる", "to write down, to take dictation"],
    ["掻き回す", "かきまわす", "to stir up, to churn, to disturb"],
    ["かく (はじを)", "かく (はじを)", "to humiliate oneself"],
    ["～画", "～かく", "~ strokes"],
    ["学芸", "がくげい", "arts and sciences, liberal arts"],
    ["格差", "かくさ", "difference, disparity"],
    ["拡散", "かくさん", "scattering, diffusion"],
    ["学士", "がくし", "university graduate"],
    ["各種", "かくしゅ", "every kind, all sorts"],
    ["隔週", "かくしゅう", "every other week"],
    ["確信", "かくしん", "conviction, confidence"],
    ["革新", "かくしん", "reform, innovation"],
    ["学説", "がくせつ", "theory"],
    ["確定", "かくてい", "fixed, decision"],
    ["カクテル", "カクテル", "cocktail"],
    ["獲得", "かくとく", "acquisition, possession"],
    ["楽譜", "がくふ", "score (music, sheet music)"],
    ["確保", "かくほ", "guarantee, insure, secure"],
    ["革命", "かくめい", "revolution"],
    ["確立", "かくりつ", "establishment"],
    ["賭", "かけ", "betting, gambling, a gamble"],
    ["掛～", "かけ～", "credit"],
    ["～掛け", "～かけ", "rack, hanger"],
    ["崖", "がけ", "cliff"],
    ["駆け足", "かけあし", "running fast, double time"],
    ["家計", "かけい", "household economy, family finances"],
    ["駆けっこ", "かけっこ", "(foot) race"],
    ["加工", "かこう", "manufacturing, processing, treatment"],
    ["化合", "かごう", "chemical combination"],
    ["かさばる", "かさばる", "to be bulky"],
    ["かさむ", "かさむ", "to pile up, to increase"],
    ["箇条書", "かじょうがき", "itemized form, itemization"],
    ["頭", "かしら", "head, chief"],
    ["微か", "かすか", "faint, dim, weak"],
    ["霞む", "かすむ", "to grow hazy, to be misty"],
    ["擦る", "かする", "to rub, to chafe"],
    ["火星", "かせい", "Mars"],
    ["化石", "かせき", "fossil, petrifaction, fossilization"],
    ["河川", "かせん", "rivers"],
    ["化繊", "かせん", "synthetic fibers"],
    ["過疎", "かそ", "depopulation"],
    ["片～", "かた～", "single ~"],
    ["片言", "かたこと", "broken (in reference to speaking style, e.g., Japanese)"],
    ["傾ける", "かたむける", "to incline, to tilt, to bend"],
    ["固める", "かためる", "to harden, to freeze, to fortify"],
    ["傍ら", "かたわら", "beside(s, while, nearby"],
    ["花壇", "かだん", "flower bed"],
    ["家畜", "かちく", "domestic animals, livestock, cattle"],
    ["且つ", "かつ", "yet, and"],
    ["がっくり", "がっくり", "heartbroken"],
    ["合唱", "がっしょう", "chorus, singing in a chorus"],
    ["がっしり", "がっしり", "firmly, solidly, tough"],
    ["合致", "がっち", "agreement, concurrence, conforming to"],
    ["がっちり", "がっちり", "solidly built, tightly"],
    ["かつて", "かつて", "once, before, formerly"],
    ["勝手", "かって", "kitchen; one's way, selfishness"],
    ["カット", "カット", "cut, cutting"],
    ["活発", "かっぱつ", "vigor, active"],
    ["合併", "がっぺい", "combination, amalgamation, merger"],
    ["カテゴリー", "カテゴリー", "category"],
    ["叶う", "かなう", "to come true"],
    ["叶える", "かなえる", "to grant (request, wish)"],
    ["金槌", "かなづち", "(iron) hammer"],
    ["かなわない", "かなわない", "be beyond one's power, be unable"],
    ["加入", "かにゅう", "becoming a member, admission"],
    ["予て", "かねて", "previously, already, lately"],
    ["庇う", "かばう", "to protect someone, to&nbsp;&nbsp;cover up for someone"],
    ["株式", "かぶしき", "stock"],
    ["かぶれる", "かぶれる", "to react to; to be influenced by"],
    ["花粉", "かふん", "pollen"],
    ["貨幣", "かへい", "money, currency, coinage"],
    ["構える", "かまえる", "to set up"],
    ["過密", "かみつ", "crowded"],
    ["噛み切る", "かみきる", "to bite off, to gnaw through"],
    ["カムバック", "カムバック", "comeback"],
    ["カメラマン", "カメラマン", "cameraman"],
    ["粥", "かゆ", "rice porridge"],
    ["体付き", "からだつき", "body build, figure"],
    ["絡む", "からむ", "to entangle, to entwine"],
    ["かりに", "かりに", "temporarily; if, for argument's sake"],
    ["カルテ", "カルテ", "clinical records (GER: Karte)"],
    ["ガレージ", "ガレージ", "garage (at house)"],
    ["過労", "かろう", "overwork, strain"],
    ["かろうじて", "かろうじて", "barely, narrowly"],
    ["交す", "かわす", "to exchange"],
    ["代る代る", "かわるがわる", "alternately"],
    ["簡易", "かんい", "simplicity, easiness, quasi-"],
    ["灌漑", "かんがい", "irrigation"],
    ["眼科", "がんか", "ophthalmology"],
    ["眼球", "がんきゅう", "eyeball"],
    ["玩具", "がんぐ", "toy"],
    ["簡潔", "かんけつ", "brevity, concise, simple"],
    ["還元", "かんげん", "resolution, reduction, return"],
    ["看護", "かんご", "nursing"],
    ["漢語", "かんご", "Chinese word, Sino-Japanese word"],
    ["頑固", "がんこ", "stubbornness, obstinacy"],
    ["勧告", "かんこく", "advice, counsel"],
    ["換算", "かんさん", "conversion, change, exchange"],
    ["監視", "かんし", "observation, guarding, surveillance"],
    ["慣習", "かんしゅう", "usual (historical) custom"],
    ["観衆", "かんしゅう", "spectators, audience"],
    ["願書", "がんしょ", "application form"],
    ["干渉", "かんしょう", "interference, intervention"],
    ["頑丈", "がんじょう", "solid, firm, strong"],
    ["感触", "かんしょく", "sense of touch, feeling, sensation"],
    ["肝心", "かんじん", "essential, fundamental, crucial"],
    ["肝腎", "かんじん", "essential, fundamental, crucial"],
    ["関税", "かんぜい", "customs, duty, tariff"],
    ["岩石", "がんせき", "rock"],
    ["感染", "かんせん", "infection, contagion"],
    ["幹線", "かんせん", "main line, trunk line"],
    ["簡素", "かんそ", "simplicity, plain"],
    ["観点", "かんてん", "point of view"],
    ["感度", "かんど", "sensitivity, severity (quake)"],
    ["カンニング", "カンニング", "cunning, cheat"],
    ["元年", "がんねん", "first year (of a specific reign)"],
    ["幹部", "かんぶ", "management, executive"],
    ["完ぺき", "かんぺき", "perfection, completeness, flawless"],
    ["勘弁", "かんべん", "pardon, forgiveness, forbearance"],
    ["感無量", "かんむりょう", "deep feeling, filled with emotion"],
    ["勧誘", "かんゆう", "invitation, canvassing, inducement"],
    ["関与", "かんよ", "participation, taking part in"],
    ["寛容", "かんよう", "forbearance, tolerance, generosity, involvement"],
    ["元来", "がんらい", "originally, naturally"],
    ["観覧", "かんらん", "viewing"],
    ["慣例", "かんれい", "custom, precedent, of convention"],
    ["還暦", "かんれき", "60th birthday"],
    ["貫禄", "かんろく", "presence, dignity"],
    ["緩和", "かんわ", "relief, mitigation"],
    ["議案", "ぎあん", "legislative bill"],
    ["危害", "きがい", "injury, harm, danger"],
    ["企画", "きかく", "planning, project"],
    ["規格", "きかく", "standard, norm"],
    ["着飾る", "きかざる", "to dress up"],
    ["気兼ね", "きがね", "hesitance, diffidence, feeling constraint"],
    ["気軽", "きがる", "cheerful, buoyant, lighthearted"],
    ["危機", "きき", "crisis"],
    ["聞き取り", "ききとり", "listening comprehension"],
    ["効き目", "ききめ", "effect, virtue, efficacy"],
    ["帰京", "ききょう", "returning to Tokyo"],
    ["戯曲", "ぎきょく", "play, drama"],
    ["基金", "ききん", "fund, foundation"],
    ["喜劇", "きげき", "comedy, funny show"],
    ["議決", "ぎけつ", "resolution, decision, vote"],
    ["棄権", "きけん", "abstain from voting, renunciation of a right"],
    ["既婚", "きこん", "married"],
    ["気障", "きざ", "affectation, conceit, snobbery"],
    ["記載", "きさい", "mention, entry"],
    ["兆", "きざし", "sign, omen, indication"],
    ["気質", "きしつ", "character, trait, temperament"],
    ["期日", "きじつ", "fixed date, settlement date"],
    ["きしむ", "きしむ", "to jar, to creak, to grate"],
    ["議事堂", "ぎじどう", "Diet building"],
    ["記述", "きじゅつ", "describing, descriptor"],
    ["気象", "きしょう", "weather, climate"],
    ["傷付く", "きずつく", "to be hurt, to be wounded, to get injured"],
    ["傷付ける", "きずつける", "to wound, to hurt someone's feelings"],
    ["犠牲", "ぎせい", "sacrifice"],
    ["汽船", "きせん", "steamship"],
    ["寄贈", "きぞう", "donation, presentation"],
    ["偽造", "ぎぞう", "forgery, fabrication, counterfeiting"],
    ["貴族", "きぞく", "noble, aristocrat"],
    ["議題", "ぎだい", "topic of discussion, agenda"],
    ["鍛える", "きたえる", "to forge, to train, to discipline"],
    ["気立て", "きだて", "good-natured, kind-hearted"],
    ["来る", "きたる", "to come, to approach,"],
    ["きちっと", "きちっと", "exactly, perfectly"],
    ["几帳面", "きちょうめん", "methodical, punctual, steady"],
    ["きっかり", "きっかり", "exactly, precisely"],
    ["きっちり", "きっちり", "precisely, tightly"],
    ["きっぱり", "きっぱり", "clearly, plainly, distinctly"],
    ["規定", "きてい", "regulation, provisions"],
    ["起点", "きてん", "starting point"],
    ["軌道", "きどう", "orbit; track"],
    ["技能", "ぎのう", "technical skill, ability, capacity"],
    ["規範", "きはん", "model, standard, example"],
    ["気品", "きひん", "grace, elegance"],
    ["気風", "きふう", "character, traits, ethos"],
    ["起伏", "きふく", "undulation"],
    ["規模", "きぼ", "scale, scope, plan, structure"],
    ["気まぐれ", "きまぐれ", "whim, caprice, uneven temper"],
    ["生真面目", "きまじめ", "serious, sincerity"],
    ["期末", "きまつ", "(end of the season or term)"],
    ["きまりわるい", "きまりわるい", "feeling awkward, being ashamed"],
    ["記名", "きめい", "signature, register"],
    ["規約", "きやく", "agreement, rules, code"],
    ["脚色", "きゃくしょく", "dramatization (e.g., film"],
    ["逆転", "ぎゃくてん", "(sudden) change, reversal, turn-around"],
    ["脚本", "きゃくほん", "scenario"],
    ["華奢", "きゃしゃ", "delicate, slender"],
    ["客観", "きゃっかん", "objective"],
    ["キャッチ", "キャッチ", "catch"],
    ["キャリア", "キャリア", "career, career government employee"],
    ["救援", "きゅうえん", "relief, rescue, reinforcement"],
    ["休学", "きゅうがく", "temporary absence from school, suspension"],
    ["究極", "きゅうきょく", "ultimate, final, eventual"],
    ["窮屈", "きゅうくつ", "narrow, tight, formal"],
    ["球根", "きゅうこん", "(plant) bulb"],
    ["救済", "きゅうさい", "relief, aid, rescue"],
    ["給仕", "きゅうじ", "waiter"],
    ["給食", "きゅうしょく", "school lunch, providing a meal"],
    ["休戦", "きゅうせん", "truce, armistice"],
    ["宮殿", "きゅうでん", "palace"],
    ["旧知", "きゅうち", "old friend, old friendship"],
    ["窮乏", "きゅうぼう", "poverty"],
    ["寄与", "きよ", "contribution, service"],
    ["強", "きょう", "strong"],
    ["～狂", "～きょう", "maniac, fan, freak"],
    ["驚異", "きょうい", "wonder, miracle"],
    ["教科", "きょうか", "subject, curriculum"],
    ["協会", "きょうかい", "association, society, organization"],
    ["共学", "きょうがく", "coeducation"],
    ["共感", "きょうかん", "sympathy, response"],
    ["境遇", "きょうぐう", "environment, circumstances"],
    ["教訓", "きょうくん", "lesson, precept, moral instruction"],
    ["強行", "きょうこう", "forcing, enforcement"],
    ["強硬", "きょうこう", "firm, vigorous, stubborn"],
    ["教材", "きょうざい", "teaching materials"],
    ["凶作", "きょうさく", "bad harvest, poor crop"],
    ["業者", "ぎょうしゃ", "trader, merchant"],
    ["教習", "きょうしゅう", "training, instruction"],
    ["郷愁", "きょうしゅう", "nostalgia, homesickness"],
    ["教職", "きょうしょく", "teaching profession"],
    ["興じる", "きょうじる", "to amuse oneself, to make merry"],
    ["強制", "きょうせい", "obligation, compulsion, enforcement"],
    ["行政", "ぎょうせい", "administration"],
    ["業績", "ぎょうせき", "achievement, work, contribution"],
    ["共存", "きょうぞん", "coexistence"],
    ["協定", "きょうてい", "arrangement, pact, agreement"],
    ["郷土", "きょうど", "homeland"],
    ["脅迫", "きょうはく", "threat, coercion"],
    ["業務", "ぎょうむ", "business, duties, work"],
    ["共鳴", "きょうめい", "resonance, sympathy"],
    ["郷里", "きょうり", "birth-place, home town"],
    ["強烈", "きょうれつ", "strong, intense, severe"],
    ["共和", "きょうわ", "republicanism, cooperation"],
    ["局限", "きょくげん", "limit, localize"],
    ["極端", "きょくたん", "extreme, extremity"],
    ["居住", "きょじゅう", "residence"],
    ["拒絶", "きょぜつ", "refusal, rejection"],
    ["漁船", "ぎょせん", "fishing boat"],
    ["漁村", "ぎょそん", "fishing village"],
    ["拒否", "きょひ", "denial, rejection, refusal"],
    ["許容", "きょよう", "permission, pardon"],
    ["清らか", "きよらか", "clean, pure, chaste"],
    ["きらびやか", "きらびやか", "gorgeous, gaudy, dazzling"],
    ["～きり", "～きり", "only"],
    ["義理", "ぎり", "debt of gratitude, obligation"],
    ["切替", "きりかえ", "exchange, conversion, switchover"],
    ["気流", "きりゅう", "atmospheric current"],
    ["切れ目", "きれめ", "break, pause, gap"],
    ["疑惑", "ぎわく", "doubt, misgivings, suspicion"],
    ["極めて", "きわめて", "exceedingly, extremely (written expression)"],
    ["近眼", "きんがん", "nearsightedness"],
    ["緊急", "きんきゅう", "urgent, pressing, emergency"],
    ["近郊", "きんこう", "suburbs, outskirts"],
    ["均衡", "きんこう", "equilibrium, balance"],
    ["禁じる", "きんじる", "to prohibit"],
    ["勤勉", "きんべん", "industry, diligence"],
    ["吟味", "ぎんみ", "examination, careful investigation"],
    ["勤務", "きんむ", "service, duty, work"],
    ["禁物", "きんもつ", "taboo, forbidden thing"],
    ["勤労", "きんろう", "labor, exertion, diligent service"],
    ["クイズ", "クイズ", "quiz"],
    ["食い違う", "くいちがう", "to cross each other, to differ"],
    ["空間", "くうかん", "space, room, airspace"],
    ["空腹", "くうふく", "hunger"],
    ["区画", "くかく", "division, section, area"],
    ["区間", "くかん", "section"],
    ["茎", "くき", "stalk"],
    ["区切り", "くぎり", "an end, a stop, punctuation"],
    ["くぐる", "くぐる", "to pass through; to go around"],
    ["くじ (～をひく)", "くじ (～をひく)", "lottery, lot"],
    ["くじびき", "くじびき", "lottery, drawn lot"],
    ["くすぐったい", "くすぐったい", "ticklish"],
    ["愚痴", "ぐち", "idle complaint, grumble"],
    ["口吟む", "くちずさむ", "to humble"],
    ["嘴", "くちばし", "beak, bill"],
    ["朽ちる", "くちる", "to rot"],
    ["覆す", "くつがえす", "to overturn, to upset, to overthrow"],
    ["くっきり", "くっきり", "distinctly, clearly, boldly"],
    ["屈折", "くっせつ", "bending, indentation, refraction"],
    ["ぐっと", "ぐっと", "firmly, fast, more"],
    ["首飾り", "くびかざり", "necklace"],
    ["首輪", "くびわ", "necklace, choker"],
    ["組み込む", "くみこむ", "to insert, to include, to cut in (printing)"],
    ["組み合わせる", "くみあわせる", "to join together, to combine, to join up"],
    ["蔵", "くら", "warehouse, cellar"],
    ["グレー", "グレー", "grey, gray"],
    ["クレーン", "クレーン", "crane"],
    ["玄人", "くろうと", "expert, professional"],
    ["黒字", "くろじ", "balance (figure) in the black"],
    ["軍艦", "ぐんかん", "warship, battleship"],
    ["軍事", "ぐんじ", "military affairs"],
    ["君主", "くんしゅ", "ruler, monarch"],
    ["群集", "ぐんしゅう", "(social) group, crowd, mob"],
    ["群衆", "ぐんしゅう", "(social) group, crowd, mob"],
    ["軍備", "ぐんび", "armaments, military preparations"],
    ["軍服", "ぐんぷく", "military or naval uniform"],
    ["芸", "げい", "art, accomplishment, performance"],
    ["経過", "けいか", "passage, progress"],
    ["軽快", "けいかい", "lively, casual, light"],
    ["警戒", "けいかい", "warning, admonition, vigilance"],
    ["敬具", "けいぐ", "Sincerely (used at the end of letter)"],
    ["軽減", "けいげん", "reduction, lessening"],
    ["掲載", "けいさい", "appearance (e.g., article in paper)"],
    ["傾斜", "けいしゃ", "inclination, slope, dip"],
    ["形成", "けいせい", "formation"],
    ["形勢", "けいせい", "condition, situation, prospects"],
    ["軽率", "けいそつ", "thoughtless, careless, hasty"],
    ["刑罰", "けいばつ", "judgment, penalty, punishment"],
    ["経費", "けいひ", "expenses, cost, outlay"],
    ["警部", "けいぶ", "police inspector"],
    ["転換", "てんかん", "convert, divert"],
    ["転居", "てんきょ", "moving, changing residence"],
    ["転勤", "てんきん", "transfer (to another office of a company)"],
    ["点検", "てんけん", "inspection, examination, checking"],
    ["電源", "でんげん", "source of electricity, power (e.g., button on TV)"],
    ["天国", "てんごく", "paradise, heaven, Kingdom of Heaven"],
    ["天才", "てんさい", "a genius"],
    ["天災", "てんさい", "natural calamity, disaster"],
    ["展示", "てんじ", "exhibition, display"],
    ["伝説", "でんせつ", "tradition, legend, folklore"],
    ["点線", "てんせん", "dotted line"],
    ["転じる", "てんじる", "to turn, to shift"],
    ["転ずる", "てんずる", "to turn, to shift"],
    ["天体", "てんたい", "heavenly body"],
    ["伝達", "でんたつ", "transmission (e.g., news, communication, delivery)"],
    ["天地", "てんち", "heaven and earth, the universe"],
    ["てんで", "てんで", "(not) at all, altogether, entirely"],
    ["転任", "てんにん", "change of post"],
    ["展望", "てんぼう", "view, outlook, prospect"],
    ["伝来", "でんらい", "ancestral, hereditary, imported"],
    ["転落", "てんらく", "fall, degradation"],
    ["問い合わせる", "といあわせる", "to inquire, to seek information"],
    ["当～", "とう～", "Our ~ (e.g., Hotel, plane, etc.)"],
    ["胴", "どう", "trunk, body, frame"],
    ["同意", "どうい", "agreement, consent; same meaning"],
    ["動員", "どういん", "mobilization"],
    ["同感", "どうかん", "agreement, same opinion, same feeling"],
    ["陶器", "とうき", "pottery, ceramics"],
    ["討議", "とうぎ", "debate, discussion"],
    ["動機", "どうき", "motive, incentive"],
    ["等級", "とうきゅう", "grade, class"],
    ["同級", "どうきゅう", "the same grade, same class"],
    ["同居", "どうきょ", "living together"],
    ["登校", "とうこう", "attendance (at school)"],
    ["統合", "とうごう", "integration, unification, synthesis"],
    ["動向", "どうこう", "trend, tendency, movement, attitude"],
    ["投資", "とうし", "investment"],
    ["同情", "どうじょう", "sympathy, compassion, sympathize"],
    ["道場", "どうじょう", "(arch) dojo, hall used for martial arts training, mandala"],
    ["統制", "とうせい", "regulation, control"],
    ["当選", "とうせん", "being elected, winning the prize"],
    ["逃走", "とうそう", "flight, desertion, escape"],
    ["統率", "とうそつ", "command, generalship, leadership"],
    ["到達", "とうたつ", "reaching, attaining, arrival"],
    ["統治", "とうち", "rule, reign, governing"],
    ["仕切る", "しきる", "to partition, to divide, to mark off"],
    ["資金", "しきん", "funds, capital"],
    ["軸", "じく", "axis, stem, shaft"],
    ["しくじる", "しくじる", "to fail, to fall through, to blunder"],
    ["仕組", "しくみ", "structure, mechanism"],
    ["死刑", "しけい", "death penalty"],
    ["湿気る", "しける", "to be damp, to be moist"],
    ["施行", "しこう", "enforcement, operation"],
    ["思考", "しこう", "thought"],
    ["志向", "しこう", "intention, aim"],
    ["嗜好", "しこう", "taste, liking, preference"],
    ["事項", "じこう", "matter(s), item(s), facts"],
    ["時刻表", "じこくひょう", "timetable, (train) schedule"],
    ["地獄", "じごく", "hell"],
    ["時差", "じさ", "time difference"],
    ["自在", "じざい", "freely, at will"],
    ["視察", "しさつ", "inspection, observation"],
    ["資産", "しさん", "property, fortune, assets"],
    ["支持", "しじ", "support"],
    ["自主", "じしゅ", "independence, autonomy"],
    ["自首", "じしゅ", "surrender, give oneself up"],
    ["刺繍", "ししゅう", "embroidery"],
    ["市場", "しじょう", "(the) market (as a concept)"],
    ["辞職", "じしょく", "resignation"],
    ["雫", "しずく", "drop (of water)"],
    ["システム", "システム", "system"],
    ["沈める", "しずめる", "to sink, to submerge"],
    ["施設", "しせつ", "establishment, facility"],
    ["事前", "じぜん", "prior, beforehand, in advance"],
    ["子息", "しそく", "(hon.) son"],
    ["持続", "じぞく", "continuation, endurance"],
    ["自尊心", "じそんしん", "self-respect, conceit"],
    ["慕う", "したう", "to yearn to adore"],
    ["下心", "したごころ", "secret intention, motive"],
    ["下地", "したじ", "groundwork, foundation"],
    ["親しむ", "したしむ", "to be intimate with, to befriend"],
    ["下調べ", "したしらべ", "preliminary investigation"],
    ["愛想", "あいそう", "sociability"],
    ["間柄", "あいだがら", "relationship"],
    ["合間", "あいま", "interval"],
    ["敢えて", "あえて", "dare (to do), venture (to do), challenge (to do)"],
    ["仰ぐ", "あおぐ", "to look up (to), to respect; to ask for"],
    ["垢", "あか", "dirt, filth"],
    ["赤字", "あかじ", "deficit, go in the red"],
    ["明かす", "あかす", "to reveal; to stay up"],
    ["赤らむ", "あからむ", "to become red, to blush"],
    ["上がり", "あがり", "ascent; income; completion, stop"],
    ["諦め", "あきらめ", "resignation, reconciliation, consolation"],
    ["アクセル", "アクセル", "(abbr.) accelerator"],
    ["あくどい", "あくどい", "gaudy vicious"],
    ["顎", "あご", "chin"],
    ["憧れ", "あこがれ", "yearning, longing, aspiration"],
    ["麻", "あさ", "hemp"],
    ["あざ", "あざ", "birthmark, bruise"],
    ["浅ましい", "あさましい", "shameful, mean, despicable"],
    ["欺く", "あざむく", "to deceive"],
    ["鮮やか", "あざやか", "vivid, clear"],
    ["嘲笑う", "あざわらう", "to sneer at, to ridicule"],
    ["悪しからず", "あしからず", "don't take me wrong, but..., I'm sorry"],
    ["味わい", "あじわい", "flavor, relish"],
    ["焦る", "あせる", "to be in a hurry, to be impatient"],
    ["あせる (こえが～)", "あせる (こえが～)", "to fade, to discolor"],
    ["値", "あたい", "value, price, worth"],
    ["値する", "あたいする", "to be worth, to deserve"],
    ["悪化", "あっか", "deterioration, worsen"],
    ["扱い", "あつかい", "treatment, service"],
    ["呆気ない", "あっけない", "not enough, too quick (short, long, etc.)"],
    ["あっさり", "あっさり", "easily, readily, quickly"],
    ["斡旋", "あっせん", "kind offices, mediation"],
    ["圧倒", "あっとう", "overwhelm, overpower"],
    ["圧迫", "あっぱく", "pressure, coercion, oppression"],
    ["あつらえる", "あつらえる", "to give an order, to place an order"],
    ["圧力", "あつりょく", "stress, pressure"],
    ["当て", "あて", "expectations; depend"],
    ["～宛", "～あて", "for…(e.g., In a letter)"],
    ["当て字", "あてじ", "phonetic-equivalent character, substitute character"],
    ["跡継ぎ", "あとつぎ", "heir, successor"],
    ["後回し", "あとまわし", "putting off, postponing"],
    ["油絵", "あぶらえ", "oil painting"],
    ["アプローチ", "アプローチ", "approach (in golf)"],
    ["あべこべ", "あべこべ", "contrary, opposite, inverse"],
    ["甘える", "あまえる", "to behave like a spoiled child, to fawn on"],
    ["雨具", "あまぐ", "rain gear"],
    ["甘口", "あまくち", "sweet flavor"],
    ["アマチュア", "アマチュア", "amateur"],
    ["網", "あみ", "net"],
    ["操る", "あやつる", "to manipulate, to operate, to pull strings"],
    ["危ぶむ", "あやぶむ", "to fear, to have misgivings, to be doubtful"],
    ["あやふや", "あやふや", "uncertain, vague, ambiguous"],
    ["過ち", "あやまち", "fault, error, indiscretion"],
    ["誤る", "あやまる", "to make a mistake"],
    ["歩み", "あゆみ", "step, progress, history"],
    ["歩む", "あゆむ", "to walk"],
    ["予め", "あらかじめ", "in advance, previously"],
    ["荒らす", "あらす", "to damage; to invade"],
    ["争い", "あらそい", "dispute, quarrel, conflict"],
    ["改まる", "あらたまる", "to be renewed; to be formal"],
    ["荒っぽい", "あらっぽい", "rough, rude"],
    ["アラブ", "アラブ", "Arab"],
    ["霰", "あられ", "hail (e.g., falling ice balls)"],
    ["有り様", "ありさま", "state, condition"],
    ["ありのまま", "ありのまま", "the truth, as it is, frankly"],
    ["ありふれる", "ありふれる", "common, ordinary, routine"],
    ["アルカリ", "アルカリ", "alkali"],
    ["アルミ", "アルミ", "aluminum (Al, aluminum)"],
    ["アワー", "アワー", "hour"],
    ["合わす", "あわす", "to join together, to face, to unite"],
    ["～合せ", "～あわせ", "in all"],
    ["アンコール", "アンコール", "encore"],
    ["暗殺", "あんさつ", "assassination"],
    ["暗算", "あんざん", "mental arithmetic"],
    ["暗示", "あんじ", "hint, suggestion"],
    ["案じる", "あんじる", "to be anxious, to ponder"],
    ["安静", "あんせい", "rest"],
    ["案の定", "あんのじょう", "sure enough, as usual"],
    ["いい加減", "いいかげん", "random, irresponsible"],
    ["言い訳", "いいわけ", "excuse, explanation"],
    ["イェス", "イェス", "yes; Jesus"],
    ["家出", "いえで", "running away from home"],
    ["生かす", "いかす", "to keep something alive; to make use of"],
    ["いかに", "いかに", "how, in what way"],
    ["いかにも", "いかにも", "truly (same as 実に (じつに))"],
    ["異議", "いぎ", "objection, dissent, protest"],
    ["生き甲斐", "いきがい", "something one lives for, very important"],
    ["行き違い", "いきちがい", "misunderstanding, disagreement"],
    ["意気込む", "いきごむ", "to be enthusiastic about"],
    ["育成", "いくせい", "rearing, training, cultivation"],
    ["幾多", "いくた", "many, numerous"],
    ["(花を〜) 生ける, 活ける", "(はなを～) いける", "to arrange (flowers)"],
    ["異見", "いけん", "different opinion, objection"],
    ["意向", "いこう", "intention, idea, inclination"],
    ["移行", "いこう", "switching over to"],
    ["いざ", "いざ", "now, come (now), crucial moment"],
    ["移住", "いじゅう", "migration, immigration"],
    ["衣装", "いしょう", "clothing, costume, outfit"],
    ["いじる", "いじる", "to touch, to tamper with"],
    ["異性", "いせい", "the opposite sex"],
    ["遺跡", "いせき", "historic ruins"],
    ["依存", "いぞん", "dependence, dependent, reliance"],
    ["委託", "いたく", "consign (goods (for sale) to a firm), entrust"],
    ["いたって", "いたって", "very much, exceedingly, extremely"],
    ["出世", "しゅっせ", "promotion, successful career, eminence"],
    ["出題", "しゅつだい", "proposing a question"],
    ["出動", "しゅつどう", "mobilization, action"],
    ["出費", "しゅっぴ", "expenses, disbursements"],
    ["出品", "しゅっぴん", "exhibit, display"],
    ["主導", "しゅどう", "main leadership"],
    ["主任", "しゅにん", "person in charge, responsible official"],
    ["首脳", "しゅのう", "head, leader"],
    ["守備", "しゅび", "defense"],
    ["手法", "しゅほう", "technique"],
    ["樹木", "じゅもく", "trees and shrubs, arbor"],
    ["樹立", "じゅりつ", "establish, create"],
    ["準急", "じゅんきゅう", "local express (train, slower than an express)"],
    ["準じる", "じゅんじる", "to follow, to conform, to apply to"],
    ["～署", "～しょ", "department"],
    ["～症", "～しょう", "disease"],
    ["～証", "～しょう", "proof, certificate"],
    ["～嬢", "～じょう", "young woman"],
    ["上位", "じょうい", "superior, higher order"],
    ["上演", "じょうえん", "art performance"],
    ["城下", "じょうか", "land near the castle"],
    ["消去", "しょうきょ", "elimination, erasing"],
    ["上空", "じょうくう", "sky, high-altitude sky, upper air"],
    ["衝撃", "しょうげき", "shock, crash, impact, ballistic"],
    ["証言", "しょうげん", "evidence, testimony"],
    ["証拠", "しょうこ", "evidence, proof"],
    ["照合", "しょうごう", "check, verification"],
    ["詳細", "しょうさい", "detail, particulars"],
    ["上昇", "じょうしょう", "rising, ascending, climbing"],
    ["昇進", "しょうしん", "promotion"],
    ["称する", "しょうする", "to take the name of, to call oneself"],
    ["情勢", "じょうせい", "state of things, condition, situation"],
    ["消息", "しょうそく", "news, letter, circumstances"],
    ["承諾", "しょうだく", "consent, agreement"],
    ["情緒", "じょうちょ", "emotion, feeling"],
    ["情緒", "じょうしょ", "emotion, feeling"],
    ["象徴", "しょうちょう", "symbol"],
    ["小児科", "しょうにか", "pediatrics"],
    ["使用人", "しようにん", "employee, servant"],
    ["情熱", "じょうねつ", "passion, enthusiasm, zeal"],
    ["譲歩", "じょうほ", "concession, conciliation, compromise"],
    ["条約", "じょうやく", "treaty, pact"],
    ["勝利", "しょうり", "victory, triumph, win"],
    ["上陸", "じょうりく", "landing, disembarkation"],
    ["蒸溜", "じょうりゅう", "distillation"],
    ["奨励", "しょうれい", "encouragement, promotion"],
    ["ショー", "ショー", "show"],
    ["除外", "じょがい", "exception, exclusion"],
    ["職員", "しょくいん", "staff member, personnel"],
    ["植民地", "しょくみんち", "colony"],
    ["職務", "しょくむ", "professional duties"],
    ["諸君", "しょくん", "Gentlemen!, Ladies!"],
    ["助言", "じょげん", "advice, suggestion"],
    ["徐行", "じょこう", "going slowly"],
    ["所在", "しょざい", "whereabouts"],
    ["所持", "しょじ", "possession, owning"],
    ["所属", "しょぞく", "attached to, belong to"],
    ["処置", "しょち", "treatment"],
    ["しょっちゅう", "しょっちゅう", "always, constantly"],
    ["所定", "しょてい", "fixed, prescribed"],
    ["所得", "しょとく", "income"],
    ["処罰", "しょばつ", "punishment"],
    ["初版", "しょはん", "first edition"],
    ["書評", "しょひょう", "book review"],
    ["処分", "しょぶん", "disposal, dealing, punishment"],
    ["庶民", "しょみん", "masses, common people"],
    ["庶務", "しょむ", "general affairs"],
    ["所有", "しょゆう", "one's possessions, ownership"],
    ["調べ", "しらべ", "investigation, inspection"],
    ["自立", "じりつ", "independence, self-reliance"],
    ["記す", "しるす", "to note, to write down"],
    ["指令", "しれい", "orders, instructions, directive"],
    ["～心", "～しん", "mind of ~"],
    ["陣", "じん", "battle formation, camp, encampment"],
    ["進化", "しんか", "evolution, progress"],
    ["人格", "じんかく", "personality, character"],
    ["審議", "しんぎ", "deliberation"],
    ["新婚", "しんこん", "newly-wed"],
    ["審査", "しんさ", "judging, inspection, examination"],
    ["人材", "じんざい", "man of talent"],
    ["紳士", "しんし", "gentleman"],
    ["真実", "しんじつ", "truth, reality"],
    ["信者", "しんじゃ", "believer, devotee"],
    ["真珠", "しんじゅ", "pearl"],
    ["進出", "しんしゅつ", "advancement"],
    ["心情", "しんじょう", "mentality"],
    ["新人", "しんじん", "new face, newcomer"],
    ["神聖", "しんせい", "holiness, sacredness, dignity"],
    ["親善", "しんぜん", "friendship"],
    ["真相", "しんそう", "truth, real situation"],
    ["迅速", "じんそく", "quick, fast, prompt"],
    ["人体", "じんたい", "human body"],
    ["新築", "しんちく", "new building, new construction"],
    ["心中", "しんじゅう", "double suicide"],
    ["進呈", "しんてい", "presentation"],
    ["進展", "しんてん", "progress, development"],
    ["神殿", "しんでん", "temple, sacred place"],
    ["進度", "しんど", "progress"],
    ["振動", "しんどう", "oscillation, vibration"],
    ["新入生", "しんにゅうせい", "new student, first-year student, freshman"],
    ["信任", "しんにん", "trust, confidence, credence"],
    ["神秘", "しんぴ", "mystery"],
    ["辛抱", "しんぼう", "patience, endurance"],
    ["人民", "じんみん", "people, public"],
    ["侵略", "しんりゃく", "aggression, invasion, raid"],
    ["診療", "しんりょう", "medical examination and treatment"],
    ["粋", "すい", "essence"],
    ["水源", "すいげん", "source of river"],
    ["推進", "すいしん", "propulsion, driving force"],
    ["吹奏", "すいそう", "playing wind instruments"],
    ["推測", "すいそく", "guess, conjecture"],
    ["水田", "すいでん", "(water-filled) paddy field"],
    ["推理", "すいり", "reasoning, inference, mystery or detective genre"],
    ["数詞", "すうし", "numeral"],
    ["崇拝", "すうはい", "worship, adoration"],
    ["据え付ける", "すえつける", "to install, to equip, to mount"],
    ["据える", "すえる", "to set, to lay, to place"],
    ["すがすがしい", "すがすがしい", "fresh, refreshing"],
    ["救い", "すくい", "help, aid, relief"],
    ["すくう (みずを～)", "すくう (みずを～)", "to scoop"],
    ["健やか", "すこやか", "vigorous, healthy, sound"],
    ["濯ぐ", "すすぐ", "to rinse, to wash out"],
    ["進み", "すすみ", "progress"],
    ["裾", "すそ", "(trouser) cuff, (skirt) hem, cut edge of a hairdo"],
    ["スタジオ", "スタジオ", "studio"],
    ["スチーム", "スチーム", "steam"],
    ["ストライキ", "ストライキ", "strike"],
    ["スト", "スト", "(abbr.) strike"],
    ["ストロー", "ストロー", "straw"],
    ["ストロボ", "ストロボ", "stroboscope (literally: strobo, strobe lamp, stroboscopic lamp)"],
    ["すばしこい", "すばしこい", "nimble, smart, quick"],
    ["素早い", "すばやい", "fast, quick"],
    ["ずばり", "ずばり", "decisively, unreservedly, frankly"],
    ["スプリング", "スプリング", "spring"],
    ["スペース", "スペース", "space"],
    ["ずぶぬれ", "ずぶぬれ", "soaked, dripping wet"],
    ["スポーツカー", "スポーツカー", "sports car"],
    ["澄ます", "すます", "to clear, to make clear, to listen for"],
    ["清ます", "すます", "to clear, to make clear, to listen for"],
    ["済ます", "すます", "to finish; to settle; to do without"],
    ["すみやか", "すみやか", "speedy"],
    ["スラックス", "スラックス", "slacks"],
    ["ずらっと", "ずらっと", "in a line, in a row"],
    ["ずるずる", "ずるずる", "dragging on, sound of sniffling"],
    ["ずれ", "ずれ", "difference, gap"],
    ["すれちがい", "すれちがい", "chance encounter"],
    ["擦れる", "すれる", "to rub, to chafe"],
    ["すんなり", "すんなり", "pass with no objection, slim, slender"],
    ["生育", "せいいく", "growth, development, breeding"],
    ["成育", "せいいく", "growth, raising"],
    ["成果", "せいか", "results, fruits"],
    ["正解", "せいかい", "correct, right answer, solution"],
    ["正義", "せいぎ", "justice, right, righteousness"],
    ["生計", "せいけい", "livelihood, living"],
    ["政権", "せいけん", "(political) administration, political power"],
    ["星座", "せいざ", "constellation"],
    ["制裁", "せいさい", "restraint, sanctions, punishment"],
    ["政策", "せいさく", "political measures, policy"],
    ["生死", "せいし", "life and death"],
    ["静止", "せいし", "stillness, repose, standing still"],
    ["誠実", "せいじつ", "sincere, honest, faithful"],
    ["成熟", "せいじゅく", "maturity, ripeness"],
    ["青春", "せいしゅん", "youth, springtime of life, adolescent"],
    ["清純", "せいじゅん", "purity, innocence"],
    ["聖書", "せいしょ", "Bible"],
    ["正常", "せいじょう", "normalcy, normality, normal"],
    ["制する", "せいする", "to control, to command"],
    ["整然", "せいぜん", "orderly, regular, well-organized"],
    ["盛装", "せいそう", "be dressed up, wear rich clothes"],
    ["盛大", "せいだい", "grand, prosperous, magnificent"],
    ["清濁", "せいだく", "good and evil, purity and impurity"],
    ["制定", "せいてい", "enactment, establishment, creation"],
    ["静的", "せいてき", "static"],
    ["製鉄", "せいてつ", "iron manufacture"],
    ["晴天", "せいてん", "fine weather"],
    ["正当", "せいとう", "just, due, proper"],
    ["制服", "せいふく", "uniform"],
    ["征服", "せいふく", "conquest, subjugation, overcoming"],
    ["製法", "せいほう", "manufacturing method, recipe, formula"],
    ["精密", "せいみつ", "precise, exact, detailed, minute"],
    ["税務署", "ぜいむしょ", "tax office"],
    ["制約", "せいやく", "limitation, constraints"],
    ["勢力", "せいりょく", "influence, power, might, strength"],
    ["整列", "せいれつ", "stand in a row, form a line"],
    ["セール", "セール", "sale"],
    ["急かす", "せかす", "to hurry, to urge on"],
    ["伜", "せがれ", "son, my son"],
    ["責務", "せきむ", "duty, obligation"],
    ["セクション", "セクション", "section"],
    ["世辞", "せじ", "flattery, compliment"],
    ["世帯", "せたい", "household"],
    ["是正", "ぜせい", "correction, revision"],
    ["世代", "せだい", "generation"],
    ["切開", "せっかい", "opening up, cutting through"],
    ["セックス", "セックス", "sex"],
    ["切実", "せつじつ", "compelling, serious, severe, acute"],
    ["接触", "せっしょく", "touch, contact"],
    ["接続詞", "せつぞくし", "conjunction"],
    ["設置", "せっち", "establishment, institution"],
    ["折衷", "せっちゅう", "compromise, cross, blending, eclecticism"],
    ["設定", "せってい", "establishment, creation"],
    ["説得", "せっとく", "persuasion"],
    ["切ない", "せつない", "painful, trying, sad"],
    ["絶版", "ぜっぱん", "out of print"],
    ["設立", "せつりつ", "establishment, foundation, institution"],
    ["攻め", "せめ", "attack, offense"],
    ["ゼリー", "ゼリー", "jelly"],
    ["セレモニー", "セレモニー", "ceremony"],
    ["世論", "せろん", "public opinion"],
    ["先", "せん", "priority, precedence, previous"],
    ["繊維", "せんい", "fiber, fiber, textile"],
    ["全快", "ぜんかい", "complete recovery of health"],
    ["宣教", "せんきょう", "religious mission"],
    ["宣言", "せんげん", "declaration, proclamation, announcement"],
    ["戦災", "せんさい", "war damage"],
    ["専修", "せんしゅう", "specialization"],
    ["戦術", "せんじゅつ", "tactics"],
    ["センス", "センス", "sense (for music, style, tact, etc.)"],
    ["潜水", "せんすい", "diving"],
    ["全盛", "ぜんせい", "height of prosperity"],
    ["先代", "せんだい", "family predecessor, previous age, previous generation"],
    ["先だって", "せんだって", "recently, the other day"],
    ["先着", "せんちゃく", "first arrival"],
    ["前提", "ぜんてい", "preamble, premise, prerequisite"],
    ["先天的", "せんてんてき", "inherent, congenital, hereditary"],
    ["前途", "ぜんと", "future prospects, outlook, the journey ahead"],
    ["戦闘", "せんとう", "battle, fight, combat"],
    ["潜入", "せんにゅう", "infiltration, sneaking in"],
    ["船舶", "せんぱく", "ship"],
    ["全滅", "ぜんめつ", "annihilation"],
    ["専用", "せんよう", "exclusive use, personal use"],
    ["占領", "せんりょう", "occupation, possession, have a room to oneself"],
    ["善良", "ぜんりょう", "goodness, excellence, virtue"],
    ["戦力", "せんりょく", "war potential"],
    ["前例", "ぜんれい", "precedent"],
    ["相応", "そうおう", "suitability, fitness"],
    ["総会", "そうかい", "general meeting"],
    ["創刊", "そうかん", "launching (e.g., newspaper, first issue)"],
    ["雑木", "ぞうき", "various kinds of small trees, assorted trees"],
    ["早急", "そうきゅう", "urgent"],
    ["早急", "さっきゅう", "urgent"],
    ["増強", "ぞうきょう", "reinforce, increase"],
    ["送金", "そうきん", "remittance, sending money"],
    ["走行", "そうこう", "running a wheeled vehicle (e.g., car, traveling)"],
    ["総合", "そうごう", "synthesis, generalization"],
    ["捜索", "そうさく", "search (esp. for someone or something missing, investigation)"],
    ["蔵相", "ぞうしょう", "Minister of Finance"],
    ["装飾", "そうしょく", "ornament"],
    ["増進", "ぞうしん", "promoting, increase, advance"],
    ["相対", "そうたい", "relative"],
    ["壮大", "そうだい", "magnificent, grand, majestic"],
    ["騒動", "そうどう", "strife, riot, rebellion"],
    ["遭難", "そうなん", "disaster, shipwreck, accident"],
    ["相場", "そうば", "market price, speculation, estimation"],
    ["装備", "そうび", "equipment"],
    ["創立", "そうりつ", "establishment, founding"],
    ["添える", "そえる", "to add to, to attach, to accompany"],
    ["ソース", "ソース", "source"],
    ["即座に", "そくざに", "immediately, right away"],
    ["促進", "そくしん", "promotion, acceleration, encouragement"],
    ["即する", "そくする", "to conform to, to agree with, to be adapted to,"],
    ["束縛", "そくばく", "restraint, restriction, confinement"],
    ["側面", "そくめん", "side, sidelight, lateral"],
    ["損う", "そこなう", "to harm, to hurt"],
    ["そこら", "そこら", "everywhere, somewhere"],
    ["素材", "そざい", "raw materials, subject matter"],
    ["阻止", "そし", "obstruction, check, hindrance"],
    ["訴訟", "そしょう", "litigation, lawsuit"],
    ["育ち", "そだち", "breeding, growth"],
    ["措置", "そち", "measure, step"],
    ["ソックス", "ソックス", "socks"],
    ["素っ気無い", "そっけない", "cold, short, curt, blunt"],
    ["外方", "そっぽ", "look (or turn) the other way"],
    ["備え付ける", "そなえつける", "to provide, to equip, to install"],
    ["備わる", "そなわる", "to be furnished with"],
    ["具わる", "そなわる", "to be furnished with"],
    ["聳える", "そびえる", "to rise, to tower, to soar"],
    ["素朴", "そぼく", "simplicity, artlessness, naivety"],
    ["背く", "そむく", "to run counter to, to go against"],
    ["染まる", "そまる", "to be dyed"],
    ["染める", "そめる", "to dye, to color"],
    ["そらす", "そらす", "to bend, to warp"],
    ["そり (～にのる)", "そり (～にのる)", "sleigh, sled"],
    ["反る", "そる", "to warp, to be warped, to curve"],
    ["それゆえ", "それゆえ", "therefore, for that reason, so"],
    ["ソロ", "ソロ", "solo"],
    ["揃い", "そろい", "set, suit, uniform"],
    ["ぞんざい", "ぞんざい", "rude, careless, slovenly"],
    ["損失", "そんしつ", "loss"],
    ["存続", "そんぞく", "duration, continuance"],
    ["ダース", "ダース", "dozen"],
    ["対応", "たいおう", "dealing with"],
    ["大家", "たいか", "rich family, distinguished family"],
    ["退化", "たいか", "degeneration, retrogression"],
    ["大概", "たいがい", "in general, mainly"],
    ["体格", "たいかく", "physique, constitution"],
    ["大金", "たいきん", "large amount of money"],
    ["待遇", "たいぐう", "treatment, reception"],
    ["対決", "たいけつ", "confrontation, showdown"],
    ["体験", "たいけん", "personal experience"],
    ["対抗", "たいこう", "opposition, antagonism"],
    ["退治", "たいじ", "extermination"],
    ["大衆", "たいしゅう", "general public"],
    ["対処", "たいしょ", "deal with, cope"],
    ["退職", "たいしょく", "retirement (from office)"],
    ["題する", "だいする", "to title"],
    ["態勢", "たいせい", "attitude, conditions, tendency"],
    ["対談", "たいだん", "talk, dialogue"],
    ["大胆", "だいたん", "bold, daring, audacious"],
    ["対等", "たいとう", "equivalent"],
    ["台無し", "だいなし", "mess, spoiled, (come to) nothing"],
    ["滞納", "たいのう", "non-payment, default"],
    ["対比", "たいひ", "contrast, comparison"],
    ["タイピスト", "タイピスト", "typist"],
    ["大部", "たいぶ", "most (e.g., most part, greater, fairly, a good deal, much)"],
    ["大便", "だいべん", "feces"],
    ["代弁", "だいべん", "speak for another"],
    ["待望", "たいぼう", "long-expected, waiting"],
    ["台本", "だいほん", "libretto, scenario"],
    ["タイマー", "タイマー", "timer"],
    ["怠慢", "たいまん", "negligence, carelessness"],
    ["タイミング", "タイミング", "timing"],
    ["タイム", "タイム", "time"],
    ["タイムリー", "タイムリー", "timely, run-batted-in (baseball), RBI"],
    ["対面", "たいめん", "interview, meeting"],
    ["代用", "だいよう", "substitution"],
    ["体力", "たいりょく", "physical strength"],
    ["タイル", "タイル", "tile"],
    ["対話", "たいわ", "conversation, dialogue"],
    ["耐える", "たえる", "to endure, to put up with"],
    ["堪える", "たえる", "to endure, to put up with"],
    ["絶える", "たえる", "to die out, to become extinct"],
    ["断える", "たえる", "to cease, to become extinct"],
    ["打開", "だかい", "solution, breakthrough"],
    ["焚火", "たきび", "(open) fire"],
    ["妥協", "だきょう", "compromise, giving in"],
    ["たくましい", "たくましい", "burly, strong, sturdy"],
    ["巧み", "たくみ", "skill, cleverness"],
    ["丈", "たけ", "length, height"],
    ["打撃", "だげき", "blow, damage; batting (baseball)"],
    ["妥結", "だけつ", "agreement"],
    ["駄作", "ださく", "poor work"],
    ["足し算", "たしざん", "addition"],
    ["多数決", "たすうけつ", "majority rule"],
    ["助け", "たすけ", "assistance"],
    ["携わる", "たずさわる", "to engage, to involve"],
    ["漂う", "ただよう", "to drift about, to float, to hang in air"],
    ["立ち去る", "たちさる", "to leave, to depart"],
    ["立ち寄る", "たちよる", "to stop by, to drop in for a short visit"],
    ["抱っこ", "だっこ", "(child's) hug"],
    ["達者", "たっしゃ", "skillful, in good health"],
    ["脱出", "だっしゅつ", "escape"],
    ["脱する", "だっする", "to escape from, to get out"],
    ["達成", "たっせい", "achievement"],
    ["脱退", "だったい", "secession, withdrawal"],
    ["だったら", "だったら", "if it's the case"],
    ["立て替える", "たてかえる", "to pay in advance, to pay for another"],
    ["建前", "たてまえ", "position; stance one takes in public; principle"],
    ["奉る", "たてまつる", "to offer, to do respectfully"],
    ["だと", "だと", "if it's the case"],
    ["他動詞", "たどうし", "transitive verb (direct object)"],
    ["辿り着く", "たどりつく", "to reach, to make it somehow"],
    ["辿る", "たどる", "to follow (road, to pursue (course), to follow up"],
    ["束ねる", "たばねる", "to tie up in a bundle, to control"],
    ["だぶだぶ", "だぶだぶ", "loose, baggy"],
    ["他方", "たほう", "another side, on the other hand"],
    ["多忙", "たぼう", "busy"],
    ["給う", "たまう", "to receive, to grant"],
    ["魂", "たましい", "soul, spirit"],
    ["溜まり", "たまり", "collected things, gathering place, arrears"],
    ["賜る", "たまわる", "to grant, to bestow"],
    ["保つ", "たもつ", "to keep, to preserve, to sustain"],
    ["たやすい", "たやすい", "easy, simple, light"],
    ["多様", "たよう", "diversity, variety"],
    ["だるい", "だるい", "sluggish, feel heavy (tired), languid"],
    ["弛み", "たるみ", "slack, slackening"],
    ["弛む", "たるむ", "to slacken, to loosen, to relax"],
    ["垂れる", "たれる", "to hang, to droop; to drip"],
    ["タレント", "タレント", "talent, star, personality"],
    ["タワー", "タワー", "tower"],
    ["単一", "たんいつ", "single, simple, sole"],
    ["短歌", "たんか", "31-syllable Japanese poem"],
    ["担架", "たんか", "stretcher, litter"],
    ["短気", "たんき", "quick temper"],
    ["団結", "だんけつ", "unity, union, solidarity"],
    ["探検", "たんけん", "exploration, expedition"],
    ["断言", "だんげん", "assertion, declaration, affirmation"],
    ["短縮", "たんしゅく", "shortening, abbreviation, reduction"],
    ["断然", "だんぜん", "firmly, absolutely, definitely"],
    ["炭素", "たんそ", "carbon (C)"],
    ["短大", "たんだい", "junior college"],
    ["単調", "たんちょう", "monotony, monotone, dullness"],
    ["単独", "たんどく", "sole, single"],
    ["旦那", "だんな", "master (of house), husband (informal)"],
    ["短波", "たんぱ", "short wave"],
    ["蛋白質", "たんぱくしつ", "protein"],
    ["ダンプ", "ダンプ", "dump truck"],
    ["断面", "だんめん", "cross section"],
    ["弾力", "だんりょく", "elasticity, flexibility"],
    ["治安", "ちあん", "public order, security"],
    ["チームワーク", "チームワーク", "teamwork"],
    ["チェンジ", "チェンジ", "change"],
    ["違える", "ちがえる", "to change"],
    ["畜産", "ちくさん", "animal husbandry"],
    ["畜生", "ちくしょう", "beast, brute, damn"],
    ["蓄積", "ちくせき", "accumulation, accumulate, store"],
    ["地形", "ちけい", "landform, geographical features, topography"],
    ["知性", "ちせい", "intelligence"],
    ["乳", "ちち", "milk, breast, loop"],
    ["縮まる", "ちぢまる", "to be shortened, to be contracted, to shrink"],
    ["秩序", "ちつじょ", "order, regularity"],
    ["窒息", "ちっそく", "suffocation"],
    ["知的", "ちてき", "intellectual"],
    ["着手", "ちゃくしゅ", "embarkation, launch"],
    ["着色", "ちゃくしょく", "coloring, coloring"],
    ["着席", "ちゃくせき", "sit down, seat"],
    ["着目", "ちゃくもく", "attention"],
    ["着陸", "ちゃくりく", "landing, touch down"],
    ["着工", "ちゃっこう", "start of (construction) work"],
    ["茶の間", "ちゃのま", "living room (Japanese style)"],
    ["茶の湯", "ちゃのゆ", "tea ceremony"],
    ["ちやほや", "ちやほや", "pamper, make a fuss of, spoil"],
    ["チャンネル", "チャンネル", "a channel"],
    ["宙返り", "ちゅうがえり", "somersault, looping-the-loop"],
    ["中継", "ちゅうけい", "relay, hook-up"],
    ["忠告", "ちゅうこく", "advice, warning"],
    ["中傷", "ちゅうしょう", "slander, libel, defamation"],
    ["中枢", "ちゅうすう", "center, mainstay, nucleus"],
    ["抽選", "ちゅうせん", "lottery, raffle, drawing (of lots)"],
    ["中断", "ちゅうだん", "interruption, suspension, break"],
    ["中毒", "ちゅうどく", "poisoning"],
    ["中腹", "ちゅうふく", "mountain side, halfway up"],
    ["中立", "ちゅうりつ", "neutrality"],
    ["中和", "ちゅうわ", "neutralize, counteract"],
    ["～著", "～ちょ", "written by ~"],
    ["腸", "ちょう", "bowels, intestines"],
    ["蝶", "ちょう", "butterfly"],
    ["超", "ちょう", "super-, ultra-, hyper-"],
    ["調印", "ちょういん", "signature, sign, sealing"],
    ["聴覚", "ちょうかく", "the sense of hearing"],
    ["長官", "ちょうかん", "chief, (government) secretary"],
    ["聴講", "ちょうこう", "lecture attendance, auditing"],
    ["徴収", "ちょうしゅう", "collection, levy"],
    ["聴診器", "ちょうしんき", "stethoscope"],
    ["調停", "ちょうてい", "arbitration, conciliation, mediation"],
    ["重複", "ちょうふく", "duplication, repetition, overlapping, redundancy, restoration"],
    ["長編", "ちょうへん", "long (e.g., novel, film)"],
    ["重宝", "ちょうほう", "convenient, useful"],
    ["調理", "ちょうり", "cooking"],
    ["調和", "ちょうわ", "harmony"],
    ["ちょくちょく", "ちょくちょく", "often, frequently, now and then, occasionally"],
    ["直面", "ちょくめん", "confrontation"],
    ["著書", "ちょしょ", "literary work, book"],
    ["貯蓄", "ちょちく", "savings"],
    ["直感", "ちょっかん", "intuition, instinct"],
    ["著名", "ちょめい", "well-known, noted, celebrated"],
    ["ちらっと", "ちらっと", "at a glance, by accident"],
    ["塵", "ちり", "dust, dirt"],
    ["塵取り", "ちりとり", "dustpan"],
    ["賃金", "ちんぎん", "wages"],
    ["沈殿", "ちんでん", "precipitation, deposition, settlement"],
    ["沈没", "ちんぼつ", "sinking, foundering"],
    ["沈黙", "ちんもく", "silence, reticence"],
    ["陳列", "ちんれつ", "exhibition, display, show"],
    ["追及", "ついきゅう", "investigation, inquiry"],
    ["追跡", "ついせき", "pursuit"],
    ["追放", "ついほう", "exile, banishment"],
    ["費やす", "ついやす", "to spend, to devote, to waste"],
    ["墜落", "ついらく", "falling, crashing"],
    ["痛感", "つうかん", "feeling keenly, fully realizing"],
    ["通常", "つうじょう", "common, normal, usual"],
    ["痛切", "つうせつ", "keen, deep"],
    ["杖", "つえ", "cane"],
    ["使い道", "つかいみち", "use"],
    ["仕える", "つかえる", "to serve, to work for"],
    ["司る", "つかさどる", "to rule, to govern, to administer"],
    ["つかの間", "つかのま", "moment, brief time,"],
    ["月並", "つきなみ", "conventional, trite, common"],
    ["継目", "つぎめ", "joint, seam"],
    ["尽きる", "つきる", "to be used up, to be run out"],
    ["尽くす", "つくす", "to exhaust, to run out; to devote, to serve"],
    ["つくづく", "つくづく", "completely, really"],
    ["作り", "つくり", "make up, structure, physique"],
    ["造り", "つくり", "make up, structure, physique"],
    ["繕う", "つくろう", "to mend, to repair"],
    ["付け加える", "つけくわえる", "to add one thing to another"],
    ["告げる", "つげる", "to inform"],
    ["つじつま (はなしの～)", "つじつま (はなしの～)", "coherence, consistency"],
    ["筒", "つつ", "pipe, tube"],
    ["突く", "つつく", "to thrust, to strike, to attack; to poke, to nudge, to pick at"],
    ["突っ突く", "つっつく", "to prompt someone"],
    ["謹む", "つつしむ", "to be careful, to be chaste or discreet"],
    ["突っ張る", "つっぱる", "to support, to become stiff; to thrust (ones opponent), to stick to (ones opinion), to insist on"],
    ["務まる", "つとまる", "be equal, be fit"],
    ["勤め先", "つとめさき", "place of work"],
    ["努めて", "つとめて", "make an effort!, work hard!"],
    ["津波", "つなみ", "tsunami, tidal wave"],
    ["つねる", "つねる", "to pinch"],
    ["角", "つの", "horn"],
    ["募る", "つのる", "to invite, to solicit help, participation, etc"],
    ["唾", "つば", "saliva, spit, sputum"],
    ["呟く", "つぶやく", "to mutter, to murmur"],
    ["つぶら", "つぶら", "round, rotund"],
    ["つぶる (めを～)", "つぶる (めを～)", "to close the eyes"],
    ["壷", "つぼ", "jar, pot, vase"],
    ["蕾", "つぼみ", "bud, flower bud"],
    ["連なる", "つらなる", "to extend, to stretch out, to stand in a row"],
    ["貫く", "つらぬく", "to go through"],
    ["連ねる", "つらねる", "to link, to join, to put together"],
    ["釣り鐘", "つりがね", "temple bell (for striking)"],
    ["吊り革", "つりかわ", "strap"],
    ["手当", "てあて", "allowance, compensation; treatment"],
    ["定義", "ていぎ", "definition"],
    ["提供", "ていきょう", "offer, program sponsoring"],
    ["提携", "ていけい", "cooperation, tie-up, joint business"],
    ["体裁", "ていさい", "decency, style, form, appearance"],
    ["提示", "ていじ", "presentation, exhibit, suggest, citation"],
    ["ティシュペーパー", "ティシュペーパー", "tissue"],
    ["定食", "ていしょく", "fixed-price lunch, set meal, dinner"],
    ["訂正", "ていせい", "correction, revision"],
    ["停滞", "ていたい", "stagnation, tie-up, congestion, retention"],
    ["邸宅", "ていたく", "mansion, residence"],
    ["定年", "ていねん", "retirement age"],
    ["堤防", "ていぼう", "bank, weir"],
    ["手遅れ", "ておくれ", "being (too); belated treatment"],
    ["でかい", "でかい", "huge"],
    ["手掛かり", "てがかり", "hint, clue, key"],
    ["手掛ける", "てがける", "to handle, to manage, to work with"],
    ["手数", "てかず", "trouble, labor, handling"],
    ["手軽", "てがる", "easy, simple, cheap"],
    ["適応", "てきおう", "adaptation, accommodation, conformity"],
    ["適宜", "てきぎ", "suitability"],
    ["適性", "てきせい", "aptitude"],
    ["できもの", "できもの", "boil, rash"],
    ["手際", "てぎわ", "performance, skill, tact"],
    ["出くわす", "でくわす", "to happen to meet, to come across"],
    ["手順", "てじゅん", "process, procedure, protocol"],
    ["手錠", "てじょう", "handcuffs, manacles"],
    ["手数", "てすう", "trouble, labor, handling"],
    ["デコレーション", "デコレーション", "decoration"],
    ["手近", "てぢか", "near, handy, familiar"],
    ["てっきり", "てっきり", "surely, certainly, beyond doubt"],
    ["鉄鋼", "てっこう", "iron and steel"],
    ["デッサン", "デッサン", "rough sketch (FRE: dessin)"],
    ["徹する", "てっする", "to devote oneself, to believe in"],
    ["てっぺん", "てっぺん", "top, summit, apex"],
    ["鉄棒", "てつぼう", "iron rod, crowbar, horizontal bar (gymnastics)"],
    ["出直し", "でなおし", "adjustment, touch up"],
    ["掌", "てのひら", "the palm"],
    ["手配", "てはい", "arrangement, search (by police)"],
    ["手筈", "てはず", "arrangement, plan, program"],
    ["手引", "てびき", "guidance, guide, introduction"],
    ["手本", "てほん", "model, pattern"],
    ["手回し", "てまわし", "preparations, arrangements"],
    ["手元", "てもと", "(money) on hand or at home, one's purse; usual skill"],
    ["デモンストレーション", "デモンストレーション", "demonstration"],
    ["照り返す", "てりかえす", "to reflect, to throw back light"],
    ["テレックス", "テレックス", "telex, teletypewriter exchange"],
    ["手分け", "てわけ", "division of labor"],
    ["天", "てん", "heaven, sky"],
    ["田園", "でんえん", "country, rural districts"],
    ["天下", "てんか", "the world, whole country"],
    ["転回", "てんかい", "revolution, rotation"],
    ["連休", "れんきゅう", "consecutive holidays"],
    ["レンジ", "レンジ", "range, stove"],
    ["連日", "れんじつ", "every day"],
    ["連帯", "れんたい", "solidarity"],
    ["レンタカー", "レンタカー", "rented car"],
    ["連中", "れんちゅう", "colleagues, company, a lot"],
    ["レントゲン", "レントゲン", "X-ray (lit: Roentgen)"],
    ["連邦", "れんぽう", "commonwealth, federation of states"],
    ["連盟", "れんめい", "league, union, alliance"],
    ["老衰", "ろうすい", "senility, senile decay"],
    ["朗読", "ろうどく", "reading aloud, recitation"],
    ["浪費", "ろうひ", "waste, extravagance"],
    ["労力", "ろうりょく", "labor, effort, trouble"],
    ["ロープウエイ", "ロープウエイ", "ropeway, aerial tram"],
    ["ロープ", "ロープ", "rope"],
    ["ろくな", "ろくな", "satisfactory, decent"],
    ["露骨", "ろこつ", "blunt, outspoken; conspicuous; broad, suggestive"],
    ["ロマンチック", "ロマンチック", "romantic"],
    ["論議", "ろんぎ", "discussion"],
    ["論理", "ろんり", "logic"],
    ["惑星", "わくせい", "planet"],
    ["技", "わざ", "art, technique"],
    ["わざわざ", "わざわざ", "take the trouble (to do), doing something especially rather than incidentally"],
    ["煩わしい", "わずらわしい", "burdensome, troublesome, complicated"],
    ["渡り鳥", "わたりどり", "migratory bird, bird of passage"],
    ["ワット", "ワット", "watt"],
    ["詫び", "わび", "apology"],
    ["和文", "わぶん", "Japanese text, sentence in Japanese"],
    ["藁", "わら", "straw"],
    ["～割", "～わり", "~ percent"],
    ["割当", "わりあて", "allotment, allocation, quota"],
    ["割込む", "わりこむ", "to cut in, to disturb"],
    ["悪者", "わるもの", "bad fellow, rascal"],
    ["我", "われ", "me, oneself, self, ego"],
    ["捗る", "はかどる", "to make progress, to move right ahead (with the work), to advance"],
    ["はかない", "はかない", "short-lived, momentary, ephemeral"],
    ["ばかばかしい", "ばかばかしい", "stupid"],
    ["破棄", "はき", "revocation, annulment, breaking (e.g., treaty)"],
    ["剥ぐ", "はぐ", "to tear off, to peel off, to rip off"],
    ["迫害", "はくがい", "persecution"],
    ["薄弱", "はくじゃく", "feebleness, weakness, weak"],
    ["白状", "はくじょう", "confession"],
    ["漠然", "ばくぜん", "obscure, vague, equivocal"],
    ["爆弾", "ばくだん", "bomb"],
    ["爆破", "ばくは", "blast, explosion, blow up"],
    ["暴露", "ばくろ", "disclosure, exposure, revelation"],
    ["励ます", "はげます", "to encourage, to cheer, to raise (the voice)"],
    ["励む", "はげむ", "to be zealous, to make an effort"],
    ["剥げる", "はげる", "to come off, to be worn off, to fade, to discolor"],
    ["化ける", "ばける", "to disguise, to take the form of"],
    ["派遣", "はけん", "dispatch, send"],
    ["恥", "はじ", "shame, embarrassment"],
    ["弾く", "はじく", "to play (piano, guitar)"],
    ["パジャマ", "パジャマ", "pajamas"],
    ["恥じらう", "はじらう", "to feel shy, to be bashful, to blush"],
    ["恥じる", "はじる", "to feel ashamed"],
    ["橋渡し", "はしわたし", "bridge building', mediation"],
    ["弾む", "はずむ", "to bounce, to be encouraged, to splurge on"],
    ["破損", "はそん", "damage"],
    ["叩く", "はたく", "to strike, to clap, to dust, to beat"],
    ["裸足", "はだし", "barefoot"],
    ["果たす", "はたす", "to accomplish, to fulfill, to carry out, to achieve"],
    ["蜂蜜", "はちみつ", "honey"],
    ["パチンコ", "パチンコ", "pachinko (Japanese pinball)"],
    ["罰", "ばつ", "punishment, penalty"],
    ["発育", "はついく", "(physical) growth, development"],
    ["発芽", "はつが", "germination"],
    ["発掘", "はっくつ", "excavation, exhumation; discovery (e.g., new talent)"],
    ["発言", "はつげん", "utterance, speech, proposal"],
    ["バッジ", "バッジ", "badge"],
    ["発生", "はっせい", "outbreak, spring forth, occurrence"],
    ["仕立てる", "したてる", "to tailor, to make, to prepare"],
    ["下取り", "したどり", "trade in, part exchange"],
    ["下火", "したび", "burning low, waning, declining"],
    ["実", "じつ", "fruit, good result"],
    ["実家", "じっか", "(one's parents') home"],
    ["失格", "しっかく", "disqualification, elimination, incapacity (legal)"],
    ["質疑", "しつぎ", "question"],
    ["失脚", "しっきゃく", "losing one's standing, being overthrown, falling"],
    ["実業家", "じつぎょうか", "industrialist, businessman"],
    ["シック", "シック", "chic"],
    ["じっくり", "じっくり", "deliberately, carefully"],
    ["躾", "しつけ", "discipline, training"],
    ["躾ける", "しつける", "to discipline, to teach manners"],
    ["実践", "じっせん", "practice, put into practice"],
    ["質素", "しっそ", "simplicity, modesty, frugality"],
    ["実態", "じったい", "truth, fact"],
    ["失調", "しっちょう", "lack of harmony, imbalance"],
    ["嫉妬", "しっと", "jealousy"],
    ["実費", "じっぴ", "actual expense, cost price"],
    ["指摘", "してき", "pointing out, identification"],
    ["自転", "じてん", "rotation, spin"],
    ["助動詞", "じょどうし", "auxiliary verb"],
    ["淑やか", "しとやか", "graceful"],
    ["萎びる", "しなびる", "to shrivel, to fade"],
    ["シナリオ", "シナリオ", "scenario"],
    ["しなやか", "しなやか", "supple, flexible, elastic"],
    ["屎尿", "しにょう", "human waste"],
    ["地主", "じぬし", "landlord"],
    ["凌ぐ", "しのぐ", "to outdo, to surpass; to endure"],
    ["芝", "しば", "lawn"],
    ["始発", "しはつ", "first train"],
    ["耳鼻科", "じびか", "otolaryngology"],
    ["私物", "しぶつ", "private property, personal effects"],
    ["しぶとい", "しぶとい", "tenacious, stubborn"],
    ["司法", "しほう", "administration of justice"],
    ["始末", "しまつ", "disposal; cleaning up afterwards"],
    ["染みる", "しみる", "to soak; pierce"],
    ["使命", "しめい", "mission, errand, message"],
    ["地元", "じもと", "local"],
    ["視野", "しや", "field of vision, outlook"],
    ["弱", "じゃく", "delicate, supple"],
    ["社交", "しゃこう", "social life"],
    ["ジャズ", "ジャズ", "jazz"],
    ["謝絶", "しゃぜつ", "refusal"],
    ["社宅", "しゃたく", "company owned house"],
    ["若干", "じゃっかん", "some, few, number of"],
    ["三味線", "しゃみせん", "three-stringed Japanese guitar"],
    ["斜面", "しゃめん", "slope, slanting surface, bevel"],
    ["砂利", "じゃり", "gravel, ballast, pebbles"],
    ["洒落る", "しゃれる", "to joke, to play on words; stylish"],
    ["ジャンパー", "ジャンパー", "jacket, jumper"],
    ["ジャンプ", "ジャンプ", "jump"],
    ["ジャンボ", "ジャンボ", "jumbo"],
    ["ジャンル", "ジャンル", "genre"],
    ["主", "しゅ", "owner, master, god"],
    ["種", "しゅ", "seed; variety"],
    ["私有", "しゆう", "private ownership"],
    ["～宗", "～しゅう", "sect"],
    ["収益", "しゅうえき", "earnings, proceeds, returns"],
    ["修学", "しゅうがく", "learning"],
    ["周期", "しゅうき", "cycle, period"],
    ["衆議院", "しゅうぎいん", "Lower House, House of Representatives"],
    ["就業", "しゅうぎょう", "employment, starting work"],
    ["従業員", "じゅうぎょういん", "employee, worker"],
    ["集計", "しゅうけい", "totalization, aggregate"],
    ["襲撃", "しゅうげき", "attack, charge, raid"],
    ["収支", "しゅうし", "income and expenditure"],
    ["終始", "しゅうし", "from beginning to end; consistent(ly)"],
    ["修士", "しゅうし", "Masters degree program"],
    ["従事", "じゅうじ", "engaging, pursuing, following"],
    ["終日", "しゅうじつ", "all day"],
    ["充実", "じゅうじつ", "fullness, perfection"],
    ["収集", "しゅうしゅう", "gathering up, collection"],
    ["十字路", "じゅうじろ", "crossroads"],
    ["執着", "しゅうじゃく", "attachment, adhesion, tenacity"],
    ["執着", "しゅうちゃく", "attachment, adhesion, tenacity"],
    ["柔軟", "じゅうなん", "flexible"],
    ["重複", "じゅうふく", "duplication, repetition, overlapping"],
    ["収容", "しゅうよう", "accommodation; seating; custody"],
    ["従来", "じゅうらい", "up to now, so far, traditional"],
    ["守衛", "しゅえい", "security guard, doorkeeper"],
    ["主演", "しゅえん", "starring, playing the leading part"],
    ["主観", "しゅかん", "subjectivity, subject, ego"],
    ["修行", "しゅぎょう", "pursuit of knowledge, training, ascetic practice"],
    ["塾", "じゅく", "after-school (cram) school"],
    ["祝賀", "しゅくが", "celebration, congratulations"],
    ["宿命", "しゅくめい", "fate, destiny, predestination"],
    ["手芸", "しゅげい", "handicrafts"],
    ["主権", "しゅけん", "sovereignty"],
    ["主催", "しゅさい", "organization, sponsorship, to host"],
    ["取材", "しゅざい", "coverage, collecting data"],
    ["趣旨", "しゅし", "object, meaning"],
    ["種々", "しゅじゅ", "variety"],
    ["主食", "しゅしょく", "staple food"],
    ["主人公", "しゅじんこう", "protagonist"],
    ["主体", "しゅたい", "subject, main constituent"],
    ["主題", "しゅだい", "subject, theme, motif"],
    ["出演", "しゅつえん", "leading performer, stage appearance"],
    ["出血", "しゅっけつ", "bleeding"],
    ["出現", "しゅつげん", "appearance, arrival"],
    ["出産", "しゅっさん", "childbirth"],
    ["出社", "しゅっしゃ", "come to work"],
    ["出生", "しゅっしょう", "birth"],
    ["出生", "しゅっせい", "birth"],
    ["微量", "びりょう", "minuscule amount, extremely small quantity"],
    ["昼飯", "ひるめし", "lunch (mid-day meal)"],
    ["比例", "ひれい", "proportion"],
    ["疲労", "ひろう", "fatigue, weariness"],
    ["敏感", "びんかん", "sensibility, susceptibility, sensitive (to)"],
    ["貧困", "ひんこん", "poverty, lack"],
    ["品質", "ひんしつ", "quality"],
    ["貧弱", "ひんじゃく", "poor, meager, insubstantial"],
    ["品種", "ひんしゅ", "breed, type, variety"],
    ["ヒント", "ヒント", "hint"],
    ["頻繁", "ひんぱん", "frequency"],
    ["貧乏", "びんぼう", "poverty, destitute, poor"],
    ["ファイト", "ファイト", "fight"],
    ["ファイル", "ファイル", "file; portfolio"],
    ["ファン", "ファン", "fan"],
    ["不意", "ふい", "sudden, abrupt, unexpected"],
    ["フィルタ", "フィルタ", "filter"],
    ["封", "ふう", "seal"],
    ["封鎖", "ふうさ", "blockade, freezing (funds)"],
    ["風車", "ふうしゃ", "windmill"],
    ["風習", "ふうしゅう", "custom"],
    ["風俗", "ふうぞく", "manners, customs; sex industry"],
    ["ブーツ", "ブーツ", "boots"],
    ["風土", "ふうど", "natural features, climate"],
    ["ブーム", "ブーム", "boom"],
    ["フォーム", "フォーム", "foam; form"],
    ["部下", "ぶか", "one's subordinate"],
    ["不可欠", "ふかけつ", "indispensable, essential"],
    ["ぶかぶか", "ぶかぶか", "too big, baggy"],
    ["不吉", "ふきつ", "ominous, sinister, bad luck, ill omen"],
    ["不況", "ふきょう", "recession, depression, slump"],
    ["布巾", "ふきん", "dish cloth"],
    ["複合", "ふくごう", "composite, complex"],
    ["福祉", "ふくし", "welfare, well-being"],
    ["覆面", "ふくめん", "mask, veil, disguise"],
    ["膨れる", "ふくれる", "to swell (out), to be inflated, to bulge"],
    ["不景気", "ふけいき", "business recession, hard times, depression"],
    ["耽る", "ふける", "to indulge in, to give oneself up to, to be absorbed in"],
    ["老ける", "ふける", "to age"],
    ["富豪", "ふごう", "wealthy person, millionaire"],
    ["布告", "ふこく", "edict, ordinance, proclamation"],
    ["ブザー", "ブザー", "buzzer"],
    ["負債", "ふさい", "debt, liabilities"],
    ["不在", "ふざい", "absence"],
    ["ふさわしい", "ふさわしい", "appropriate"],
    ["不順", "ふじゅん", "irregularity, unseasonableness"],
    ["負傷", "ふしょう", "injury, wound"],
    ["侮辱", "ぶじょく", "insult, contempt, slight"],
    ["不審", "ふしん", "suspicious, doubt, infidelity"],
    ["不振", "ふしん", "dullness, slump, stagnation"],
    ["武装", "ぶそう", "arms, armament, armed"],
    ["札", "ふだ", "token, label; ticket, card; charm, talisman"],
    ["負担", "ふたん", "burden; load"],
    ["不調", "ふちょう", "bad condition, disorder, slump"],
    ["復活", "ふっかつ", "revival (e.g., musical), restoration"],
    ["物議", "ぶつぎ", "public discussion (criticism)"],
    ["復旧", "ふっきゅう", "restoration, restitution, rehabilitation"],
    ["復興", "ふっこう", "revival, renaissance, reconstruction"],
    ["物資", "ぶっし", "goods, materials"],
    ["仏像", "ぶつぞう", "Buddhist image (statue)"],
    ["物体", "ぶったい", "object"],
    ["沸騰", "ふっとう", "boiling, seething"],
    ["不当", "ふとう", "injustice, impropriety, unfair"],
    ["不動産", "ふどうさん", "real estate"],
    ["無難", "ぶなん", "safety, security"],
    ["赴任", "ふにん", "(proceeding to) new appointment"],
    ["腐敗", "ふはい", "decay, depravity"],
    ["不評", "ふひょう", "bad reputation, disgrace, unpopularity"],
    ["不服", "ふふく", "dissatisfaction, discontent, disapproval"],
    ["普遍", "ふへん", "universality, ubiquity, omnipresence"],
    ["踏まえる", "ふまえる", "to be based on, to have origin in"],
    ["踏み込む", "ふみこむ", "to step into (someone else's territory, to break into, to raid"],
    ["不明", "ふめい", "unknown, ambiguous"],
    ["部門", "ぶもん", "class, group, category, department, field, branch"],
    ["扶養", "ふよう", "support, maintenance"],
    ["ふらふら", "ふらふら", "unsteady on one's feet, totter, dizzy"],
    ["ぶらぶら", "ぶらぶら", "dangle heavily, sway to and fro, stroll idly"],
    ["振り返る", "ふりかえる", "to turn head, to turn around, to look back"],
    ["振り出し", "ふりだし", "outset, starting point, drawing or issuing (draft)"],
    ["不良", "ふりょう", "badness, delinquent, failure"],
    ["浮力", "ふりょく", "buoyancy"],
    ["武力", "ぶりょく", "armed might, military power, the sword, force"],
    ["ブル", "ブル", "bull"],
    ["震わせる", "ふるわせる", "to be shaking, to be trembling"],
    ["無礼", "ぶれい", "impolite, rude"],
    ["付録", "ふろく", "appendix, supplement"],
    ["フロント", "フロント", "front"],
    ["憤慨", "ふんがい", "indignation, resentment"],
    ["文化財", "ぶんかざい", "cultural assets, cultural property"],
    ["分業", "ぶんぎょう", "division of labor, specialization, assembly-line production"],
    ["文語", "ぶんご", "written language, literary language"],
    ["分散", "ぶんさん", "dispersion, decentralization, variance (statistics)"],
    ["分子", "ぶんし", "numerator, molecule"],
    ["紛失", "ふんしつ", "losing something"],
    ["噴出", "ふんしゅつ", "spewing, gushing, spouting"],
    ["文書", "ぶんしょ", "document, writing"],
    ["紛争", "ふんそう", "dispute, trouble, strife"],
    ["ふんだん", "ふんだん", "plentiful, abundant, lavish"],
    ["分担", "ぶんたん", "apportionment, sharing"],
    ["奮闘", "ふんとう", "hard struggle, strenuous effort"],
    ["分配", "ぶんぱい", "division, sharing"],
    ["分母", "ぶんぼ", "denominator"],
    ["粉末", "ふんまつ", "fine powder"],
    ["分離", "ぶんり", "separation, detachment, segregation"],
    ["分裂", "ぶんれつ", "split, division, break up"],
    ["ペア", "ペア", "pair, pear"],
    ["兵器", "へいき", "arms, weapons, ordinance"],
    ["閉口", "へいこう", "shut mouth"],
    ["閉鎖", "へいさ", "closing, closure, shutdown"],
    ["兵士", "へいし", "soldier"],
    ["平常", "へいじょう", "normal, usual"],
    ["平方", "へいほう", "square (e.g., meter, square)"],
    ["並列", "へいれつ", "arrangement, parallel, abreast"],
    ["ベース", "ベース", "base, bass"],
    ["辟易", "へきえき", "wince, shrink back, succumbing to, being frightened"],
    ["ぺこぺこ", "ぺこぺこ", "fawn, be very hungry"],
    ["ベスト", "ベスト", "best; vest"],
    ["ベストセラー", "ベストセラー", "best-seller"],
    ["隔たる", "へだたる", "to be distant"],
    ["縁", "へり", "edge"],
    ["へりくだる", "へりくだる", "to deprecate oneself and praise the listener"],
    ["弁解", "べんかい", "explanation, justification, excuse"],
    ["変革", "へんかく", "change, reform(the) Reformation"],
    ["返還", "へんかん", "return, restoration"],
    ["便宜", "べんぎ", "convenience, accommodation"],
    ["偏見", "へんけん", "prejudice, narrow view"],
    ["弁護", "べんご", "defense, pleading, advocacy"],
    ["返済", "へんさい", "repayment"],
    ["弁償", "べんしょう", "compensation, reparation, reimbursement"],
    ["変遷", "へんせん", "change, transition, vicissitudes"],
    ["返答", "へんとう", "reply"],
    ["変動", "へんどう", "change, fluctuation"],
    ["弁論", "べんろん", "discussion, debate, argument"],
    ["穂", "ほ", "ear (of plant), head (of plant)"],
    ["保育", "ほいく", "nursing, nurturing, rearing"],
    ["ボイコット", "ボイコット", "boycott"],
    ["ポイント", "ポイント", "point"],
    ["法案", "ほうあん", "bill (law)"],
    ["防衛", "ぼうえい", "defense, protection, self-defense"],
    ["防火", "ぼうか", "fire prevention, fire fighting, fire proof"],
    ["崩壊", "ほうかい", "collapse, decay (physics), crumbling"],
    ["妨害", "ぼうがい", "disturbance, obstruction, interference"],
    ["法学", "ほうがく", "law, jurisprudence"],
    ["封建", "ほうけん", "feudalistic"],
    ["豊作", "ほうさく", "abundant harvest, bumper crop"],
    ["方策", "ほうさく", "plan, policy"],
    ["奉仕", "ほうし", "attendance, service"],
    ["方式", "ほうしき", "form, method, system"],
    ["放射", "ほうしゃ", "radiation, emission"],
    ["放射能", "ほうしゃのう", "radioactivity"],
    ["報酬", "ほうしゅう", "remuneration, recompense, reward"],
    ["放出", "ほうしゅつ", "release, emit"],
    ["報じる", "ほうじる", "to inform, to report"],
    ["報ずる", "ほうずる", "to inform, to report"],
    ["紡績", "ぼうせき", "spinning"],
    ["呆然", "ぼうぜん", "dumbfounded, overcome with surprise"],
    ["放置", "ほうち", "leave as is, leave alone, neglect"],
    ["膨張", "ぼうちょう", "expansion, swelling, increase"],
    ["法廷", "ほうてい", "courtroom"],
    ["報道", "ほうどう", "coverage, report"],
    ["冒頭", "ぼうとう", "beginning, start, outset"],
    ["暴動", "ぼうどう", "insurrection, riot, uprising"],
    ["褒美", "ほうび", "reward, prize"],
    ["暴風", "ぼうふう", "storm, windstorm, gale"],
    ["葬る", "ほうむる", "to bury, to entomb"],
    ["放り込む", "ほうりこむ", "to throw into"],
    ["放り出す", "ほうりだす", "to throw out, to give up, to abandon"],
    ["暴力", "ぼうりょく", "violence"],
    ["飽和", "ほうわ", "saturation"],
    ["ホース", "ホース", "hose"],
    ["ポーズ", "ポーズ", "pause"],
    ["ホール", "ホール", "hall; hole"],
    ["保温", "ほおん", "retaining warmth, keeping heat in, heat insulation"],
    ["捕獲", "ほかく", "capture, seizure"],
    ["保管", "ほかん", "custody, safekeeping, storage"],
    ["補給", "ほきゅう", "supply, supplying, replenishment"],
    ["補強", "ほきょう", "reinforcement"],
    ["募金", "ぼきん", "fund-raising, collection of funds"],
    ["牧師", "ぼくし", "pastor, minister, clergyman"],
    ["捕鯨", "ほげい", "whaling"],
    ["惚ける", "ぼける", "to grow senile, to fade"],
    ["保険", "ほけん", "insurance, guarantee"],
    ["母校", "ぼこう", "alma mater"],
    ["母国", "ぼこく", "one's home country (same as 自分の国 (じぶんのくに))"],
    ["誇る", "ほこる", "to boast of, to be proud of"],
    ["綻びる", "ほころびる", "to come apart at the seams, to smile broadly"],
    ["干し～", "ほし～", "dried ~"],
    ["ポジション", "ポジション", "position"],
    ["干し物", "ほしもの", "dried washing (clothes"],
    ["保守", "ほしゅ", "conservative, maintaining"],
    ["補充", "ほじゅう", "supplementation, replenishment, replenishing"],
    ["補助", "ほじょ", "assistance, support, auxiliary"],
    ["舗装", "ほそう", "pavement, road surface"],
    ["補足", "ほそく", "supplement, complement"],
    ["墓地", "ぼち", "cemetery, graveyard"],
    ["発作", "ほっさ", "fit, attack"],
    ["没収", "ぼっしゅう", "forfeited"],
    ["発足", "ほっそく", "starting, inauguration"],
    ["ポット", "ポット", "pot"],
    ["ほっぺた", "ほっぺた", "cheek"],
    ["ぼつぼつ", "ぼつぼつ", "gradually, here and there, spots"],
    ["没落", "ぼつらく", "ruin, fall, collapse"],
    ["解ける", "ほどける", "to come untied, to come apart"],
    ["施す", "ほどこす", "to give, to conduct, to perform"],
    ["ほとり", "ほとり", "vicinity of lake; river"],
    ["ぼやく", "ぼやく", "to grumble, to complain"],
    ["ぼやける", "ぼやける", "to become dim, to become blurred"],
    ["保養", "ほよう", "health preservation, recuperation, recreation"],
    ["捕虜", "ほりょ", "prisoner of war"],
    ["ボルト", "ボルト", "volt; bolt"],
    ["滅びる", "ほろびる", "to be ruined, to perish, to be destroyed"],
    ["滅ぼす", "ほろぼす", "to destroy, to overthrow, to ruin"],
    ["本格", "ほんかく", "propriety, full-scale"],
    ["本館", "ほんかん", "main building"],
    ["本気", "ほんき", "seriousness, truth, sanctity"],
    ["本国", "ほんごく", "one's own country"],
    ["本質", "ほんしつ", "essence, true nature, reality"],
    ["本体", "ほんたい", "substance, body, trunk"],
    ["本音", "ほんね", "(one's) real intention, motive"],
    ["本能", "ほんのう", "instinct"],
    ["本場", "ほんば", "home, best place, genuine"],
    ["ポンプ", "ポンプ", "pump"],
    ["本文", "ほんぶん", "text (of document), body (of letter)"],
    ["本名", "ほんみょう", "real name"],
    ["マーク", "マーク", "mark"],
    ["マイ～", "マイ～", "my ~, one's own ~"],
    ["マイクロフォン", "マイクロフォン", "microphone"],
    ["埋蔵", "まいぞう", "buried property, treasure trove"],
    ["舞う", "まう", "to dance, to flutter about, to revolve"],
    ["真上", "まうえ", "just above, right overhead"],
    ["前売", "まえうり", "advance sale, booking"],
    ["前置き", "まえおき", "preface, introduction"],
    ["任す", "まかす", "to entrust, to leave to a person"],
    ["負かす", "まかす", "to defeat"],
    ["賄う", "まかなう", "to give board to, to provide meals, to pay"],
    ["紛らわしい", "まぎらわしい", "confusing, misleading, ambiguous"],
    ["紛れる", "まぎれる", "to be diverted, to slip into"],
    ["真心", "まごころ", "sincerity, devotion"],
    ["まごつく", "まごつく", "to be confused, to be flustered"],
    ["誠", "まこと", "truth, faith, fidelity"],
    ["誠に", "まことに", "indeed, really (very polite), absolutely"],
    ["まさしく", "まさしく", "surely, no doubt, evidently"],
    ["勝る", "まさる", "to excel, to surpass, to out-rival"],
    ["～増し", "～増し", "~increase"],
    ["交える", "まじえる", "to mix, to converse with, to cross (swords)"],
    ["真下", "ました", "right under, directly below"],
    ["まして", "まして", "still more, still less (with neg. verb), to say nothing of"],
    ["交わる", "まじわる", "to cross, to intersect, to mingle with,"],
    ["麻酔", "ますい", "anesthesia"],
    ["またがる (うまを～)", "またがる (うまを～)", "to straddle"],
    ["待ち合わせ", "まちあわせ", "appointment"],
    ["待ち遠しい", "まちどおしい", "looking forward to"],
    ["待ち望む", "まちのぞむ", "to look anxiously for, to wait eagerly for"],
    ["まちまち", "まちまち", "various, different"],
    ["末期", "まっき", "deathbed, hour of death"],
    ["真っ二つ", "まっぷたつ", "in two equal parts"],
    ["まと", "まと", "mark, target"],
    ["纏まり", "まとまり", "conclusion, settlement, consistency"],
    ["纏め", "まとめ", "settlement, conclusion"],
    ["免れる", "まぬがれる", "to escape from, to be exempted"],
    ["招き", "まねき", "invitation"],
    ["瞬き", "まばたき", "wink, twinkling (of stars), flicker (of light)"],
    ["麻痺", "まひ", "paralysis, palsy, numbness"],
    ["～まみれ", "～まみれ", "covered with (by, in) ~"],
    ["眉", "まゆ", "eyebrow"],
    ["鞠", "まり", "ball"],
    ["丸ごと", "まるごと", "in its entirety, whole, wholly"],
    ["まるっきり", "まるっきり", "completely, perfectly, just as if"],
    ["丸々", "まるまる", "completely"],
    ["丸める", "まるめる", "to make round, to round off, to roll up"],
    ["満月", "まんげつ", "full moon"],
    ["満場", "まんじょう", "unanimous, whole audience"],
    ["真ん前", "まんまえ", "right in front, under the nose"],
    ["真ん丸い", "まんまるい", "perfectly circular"],
    ["真ん円い", "まんまるい", "perfectly round"],
    ["～味", "～み", "~ cast (sense of taste)"],
    ["見合い", "みあい", "formal marriage interview"],
    ["見合わせる", "みあわせる", "to exchange glances; to postpone"],
    ["見落とす", "みおとす", "to overlook, to fail to notice"],
    ["未開", "みかい", "savage land, backward region, uncivilized"],
    ["味覚", "みかく", "taste, palate, sense of taste"],
    ["幹", "みき", "(tree) trunk"],
    ["見苦しい", "みぐるしい", "unsightly, ugly"],
    ["見込み", "みこみ", "prospects, expectation, hope"],
    ["未婚", "みこん", "unmarried"],
    ["未熟", "みじゅく", "inexperience, unskilled, immature"],
    ["微塵", "みじん", "particle, atom"],
    ["水気", "みずけ", "moisture, dampness"],
    ["ミスプリント", "ミスプリント", "misprint"],
    ["みすぼらしい", "みすぼらしい", "shabby, seedy"],
    ["ミセス", "ミセス", "Mrs."],
    ["見せびらかす", "みせびらかす", "to show off, to flaunt"],
    ["見せ物", "みせもの", "show, exhibition"],
    ["溝", "みぞ", "ditch, drain, gutter, gap"],
    ["満たす", "みたす", "to satisfy, to ingratiate, to fill, to fulfill"],
    ["乱す", "みだす", "to throw out of order, to disarrange, to disturb"],
    ["乱れる", "みだれる", "to get confused, to be disordered, to be disturbed"],
    ["未知", "みち", "not yet known"],
    ["身近", "みぢか", "near oneself, close to one, familiar"],
    ["導く", "みちびく", "to be guided, to be shown"],
    ["密集", "みっしゅう", "crowd, close formation, dense"],
    ["密接", "みっせつ", "connected, close, intimate"],
    ["密度", "みつど", "density"],
    ["見積もり", "みつもり", "estimation, quotation"],
    ["未定", "みてい", "not yet fixed, undecided, pending"],
    ["見通し", "みとおし", "perspective, unobstructed view, prospect"],
    ["見なす", "みなす", "to consider as, to regard"],
    ["源", "みなもと", "source, origin"],
    ["見習う", "みならう", "to follow another's example"],
    ["身なり", "みなり", "personal appearance"],
    ["峰", "みね", "peak, ridge"],
    ["身の上", "みのうえ", "one's future, one's welfare, one's personal history"],
    ["見逃す", "みのがす", "to miss, to overlook, to leave at large"],
    ["身の回り", "みのまわり", "one's personal appearance, personal belongings"],
    ["見計らう", "みはからう", "to choose at one's own discretion"],
    ["見晴らし", "みはらし", "view"],
    ["身振り", "みぶり", "gesture"],
    ["脈", "みゃく", "pulse"],
    ["ミュージック", "ミュージック", "music"],
    ["未練", "みれん", "lingering affection, attachment, regret(s)"],
    ["見渡す", "みわたす", "to look out over, to survey (scene), to take an extensive view of"],
    ["民宿", "みんしゅく", "private house providing lodging and meals to tourists"],
    ["民族", "みんぞく", "people, race"],
    ["民俗", "みんぞく", "folk customs"],
    ["無意味", "むいみ", "nonsense, no meaning"],
    ["ムード", "ムード", "mood"],
    ["無口", "むくち", "reticence"],
    ["婿", "むこ", "son-in-law"],
    ["無効", "むこう", "invalid, no effect, unavailable"],
    ["無言", "むごん", "silence"],
    ["無邪気", "むじゃき", "innocence, simple-mindedness"],
    ["むしる", "むしる", "to pluck, to pick, to tear"],
    ["結び", "むすび", "ending, conclusion, union"],
    ["結び付き", "むすびつき", "connection, relation"],
    ["結び付く", "むすびつく", "to be connected or related, to join together"],
    ["結び付ける", "むすびつける", "to combine, to join, to tie on, to attach with a knot"],
    ["無線", "むせん", "wireless, radio"],
    ["無駄遣い", "むだづかい", "waste money on, squander money on"],
    ["無断", "むだん", "without permission, without notice"],
    ["無知", "むち", "ignorance"],
    ["無茶", "むちゃ", "absurd, unreasonable"],
    ["無茶苦茶", "むちゃくちゃ", "confused, jumbled, mixed up, unreasonable"],
    ["空しい", "むなしい", "vacant, futile, vain"],
    ["無念", "むねん", "chagrin, regret"],
    ["無能", "むのう", "inefficiency, incompetence"],
    ["無闇に", "むやみに", "unreasonably, absurdly, at random"],
    ["無用", "むよう", "useless, needlessness, unnecessariness"],
    ["斑", "むら", "unevenness, inconsistency, irregularity"],
    ["群がる", "むらがる", "to swarm, to gather"],
    ["無論", "むろん", "of course, naturally"],
    ["名産", "めいさん", "noted product"],
    ["名称", "めいしょう", "name"],
    ["命中", "めいちゅう", "a hit"],
    ["明白", "めいはく", "obvious, clear"],
    ["名簿", "めいぼ", "register of names"],
    ["名誉", "めいよ", "honor, credit, prestige"],
    ["明瞭", "めいりょう", "clarity"],
    ["明朗", "めいろう", "bright, clear, cheerful"],
    ["メーカー", "メーカー", "manufacturer"],
    ["目方", "めかた", "weight"],
    ["恵み", "めぐみ", "blessing"],
    ["恵む", "めぐむ", "to bless, to show mercy to"],
    ["目覚しい", "めざましい", "brilliant, remarkable"],
    ["目覚める", "めざめる", "to wake up"],
    ["召す", "めす", "to call, to send for, to put on"],
    ["雌", "めす", "female (animal)"],
    ["目付き", "めつき", "look, expression of the eyes, eyes"],
    ["滅亡", "めつぼう", "downfall, collapse, destruction"],
    ["メディア", "メディア", "media"],
    ["目途", "めど", "goal, outlook"],
    ["目盛", "めもり", "scale, gradations"],
    ["メロディー", "メロディー", "melody"],
    ["面会", "めんかい", "interview"],
    ["免除", "めんじょ", "exemption, exoneration, discharge"],
    ["面する", "めんする", "to face on, to look out on to"],
    ["面目", "めんぼく", "face, honor, reputation"],
    ["面目", "めんもく", "face, honor, reputation"],
    ["～網", "～もう", "~ network"],
    ["設ける", "もうける", "to create, to establish"],
    ["申し入れる", "もうしいれる", "to propose, to suggest"],
    ["申込", "もうしこみ", "application, request, proposal"],
    ["申出", "もうしで", "request, claim, report"],
    ["申し出る", "もうしでる", "to report to, to tell, to suggest"],
    ["申し分", "もうしぶん", "objection, shortcomings"],
    ["盲点", "もうてん", "blind spot"],
    ["猛烈", "もうれつ", "violent, vehement, rage"],
    ["モーテル", "モーテル", "motel"],
    ["もがく", "もがく", "to struggle, to wriggle, to be impatient"],
    ["目録", "もくろく", "catalogue, catalog, list"],
    ["目論見", "もくろみ", "a plan, a scheme, intention"],
    ["模型", "もけい", "model, dummy, marquette"],
    ["模索", "もさく", "groping (for)"],
    ["もしかして", "もしかして", "perhaps, possibly"],
    ["もしくは", "もしくは", "or, otherwise"],
    ["もたらす", "もたらす", "to bring, to take, to bring about"],
    ["持ち切り", "もちきり", "hot topic, talk of the town"],
    ["目下", "もっか", "at present, now"],
    ["専ら", "もっぱら", "wholly, solely, entirely"],
    ["もてなす", "もてなす", "to entertain, to make welcome"],
    ["もてる", "もてる", "to be well liked, to be popular"],
    ["モニター", "モニター", "(computer) monitor"],
    ["物好き", "ものずき", "(idle) curiosity"],
    ["物足りない", "ものたりない", "unsatisfied, unsatisfactory"],
    ["もはや", "もはや", "already, now"],
    ["模範", "もはん", "model, example"],
    ["模倣", "もほう", "imitation, copying"],
    ["もめる", "もめる", "to disagree, to dispute"],
    ["股", "もも", "thigh, femur"],
    ["腿", "もも", "thigh, femur"],
    ["催す", "もよおす", "to hold (a meeting), to give (a dinner)"],
    ["漏らす", "もらす", "to let leak, to reveal"],
    ["盛り上がる", "もりあがる", "to rouse, to swell, to rise"],
    ["漏る", "もる", "to leak, to run out"],
    ["漏れる", "もれる", "to leak out, to escape, to filter out"],
    ["脆い", "もろい", "brittle, fragile, tender-hearted"],
    ["もろに", "もろに", "completely, altogether, bodily"],
    ["矢", "や", "arrow"],
    ["野外", "やがい", "fields, outskirts, open air, suburbs"],
    ["～薬", "～やく", "medicine"],
    ["夜具", "やぐ", "bedding"],
    ["役職", "やくしょく", "post, managerial position, official position"],
    ["役場", "やくば", "town hall"],
    ["やけに", "やけに", "sure, very"],
    ["屋敷", "やしき", "mansion"],
    ["養う", "やしなう", "to rear, to maintain, to cultivate"],
    ["野心", "やしん", "ambition, aspiration"],
    ["安っぽい", "やすっぽい", "cheap-looking, tawdry"],
    ["休める", "やすめる", "to rest, to suspend, to give relief"],
    ["野生", "やせい", "wild"],
    ["奴", "やつ", "(vulg.) fellow, guy, chap"],
    ["闇", "やみ", "darkness, shady, illegal"],
    ["病む", "やむ", "to fall ill, to be ill"],
    ["ややこしい", "ややこしい", "puzzling, tangled, complicated, complex"],
    ["やりとおす", "やりとおす", "to carry through, to achieve, to complete"],
    ["やりとげる", "やりとげる", "to accomplish"],
    ["和らげる", "やわらげる", "to soften, to moderate, to relieve"],
    ["ヤング", "ヤング", "young"],
    ["～油", "～ゆ", "~ oil"],
    ["優位", "ゆうい", "predominance, ascendancy, superiority"],
    ["憂鬱", "ゆううつ", "depression, melancholy"],
    ["有益", "ゆうえき", "beneficial, profitable"],
    ["優越", "ゆうえつ", "supremacy, predominance, being superior to"],
    ["勇敢", "ゆうかん", "bravery, heroism, gallantry"],
    ["夕暮れ", "ゆうぐれ", "evening, (evening) twilight"],
    ["融資", "ゆうし", "financing, loan"],
    ["有する", "ゆうする", "to own, to be endowed with"],
    ["優勢", "ゆうせい", "superiority, superior power, predominance"],
    ["優先", "ゆうせん", "preference, priority"],
    ["誘導", "ゆうどう", "guidance, leading, inducement"],
    ["融通", "ゆうずう", "adaptability, versatility, finance"],
    ["優美", "ゆうび", "grace, refinement, elegance"],
    ["有望", "ゆうぼう", "good prospects, full of hope, promising"],
    ["遊牧", "ゆうぼく", "nomadism"],
    ["夕焼け", "ゆうやけ", "sunset"],
    ["有力", "ゆうりょく", "influence, prominence; potent"],
    ["幽霊", "ゆうれい", "ghost, specter, phantom"],
    ["誘惑", "ゆうわく", "temptation, allurement, lure"],
    ["故", "ゆえ", "reason, cause, circumstances"],
    ["歪む", "ゆがむ", "to warp, to be crooked, to be distorted"],
    ["揺さぶる", "ゆさぶる", "to shake, to jolt, to rock, to swing"],
    ["濯ぐ", "ゆすぐ", "to rinse, to wash out"],
    ["ゆとり", "ゆとり", "reserve, affluence, time (to spare)"],
    ["ユニーク", "ユニーク", "unique"],
    ["ユニフォーム", "ユニフォーム", "uniform"],
    ["指差す", "ゆびさす", "to point at"],
    ["弓", "ゆみ", "bow"],
    ["揺らぐ", "ゆらぐ", "to swing, to sway, to shake"],
    ["緩む", "ゆるむ", "to become loose, to slacken"],
    ["緩める", "ゆるめる", "to loosen, to slow down"],
    ["緩やか", "ゆるやか", "lenient"],
    ["要因", "よういん", "primary factor, main cause"],
    ["溶液", "ようえき", "solution"],
    ["用件", "ようけん", "business"],
    ["養護", "ようご", "protection, nursing, protective care"],
    ["用紙", "ようし", "a form"],
    ["様式", "ようしき", "style, form, pattern"],
    ["要する", "ようする", "to demand, to require, to take"],
    ["要請", "ようせい", "claim, demand, request, application"],
    ["様相", "ようそう", "aspect"],
    ["用品", "ようひん", "articles, supplies, parts"],
    ["洋風", "ようふう", "western style"],
    ["用法", "ようほう", "directions, rules of use"],
    ["要望", "ようぼう", "demand for, request"],
    ["余暇", "よか", "leisure, leisure time, spare time"],
    ["予感", "よかん", "presentiment, premonition"],
    ["余興", "よきょう", "side show, entertainment"],
    ["預金", "よきん", "deposit, bank account"],
    ["欲", "よく", "greed, wants"],
    ["抑圧", "よくあつ", "restraint, oppression, suppression"],
    ["浴室", "よくしつ", "bathroom, bath"],
    ["抑制", "よくせい", "control, restraint, suppression"],
    ["欲深い", "よくふかい", "greedy"],
    ["欲望", "よくぼう", "desire, appetite"],
    ["避ける", "よける", "to avoid (physical contact with; to ward off, to avert"],
    ["予言", "よげん", "prediction, promise, prognostication"],
    ["横綱", "よこづな", "sumo grand champion"],
    ["汚れ", "よごれ", "dirt, filth"],
    ["よし (かん)", "よし (かん)", "all right!"],
    ["良し", "よし", "all right!"],
    ["善し悪し", "よしあし", "good or bad, merits or demerits, quality"],
    ["余所見", "よそみ", "looking away, looking aside"],
    ["余地", "よち", "place, room, margin"],
    ["よって (よりどころ)", "よって (よりどころ)", "therefore, consequently"],
    ["与党", "よとう", "government party, (ruling) party in power, government"],
    ["呼び止める", "よびとめる", "to flag down"],
    ["夜更し", "よふかし", "staying up late, keeping late hours"],
    ["夜更け", "よふけ", "late at night"],
    ["余程", "よほど", "very, much, to a large extent, quite"],
    ["読み上げる", "よみあげる", "to read out loud (and clearly), to call a roll"],
    ["～寄り", "～より", "near to ~ (e.g., North by East)"],
    ["寄り掛かる", "よりかかる", "to lean against, to recline on, to lean on, to rely on"],
    ["世論", "よろん", "public opinion"],
    ["弱る", "よわる", "to weaken, to be troubled, to be emaciated"],
    ["来場", "らいじょう", "attendance"],
    ["ライス", "ライス", "rice"],
    ["酪農", "らくのう", "dairy farming"],
    ["落下", "らっか", "fall, drop, come down"],
    ["楽観", "らっかん", "optimism"],
    ["ランプ", "ランプ", "lamp; ramp"],
    ["濫用", "らんよう", "abuse, misuse, using to excess"],
    ["リード", "リード", "lead; reed"],
    ["理屈", "りくつ", "theory, reason"],
    ["利子", "りし", "interest (bank)"],
    ["利潤", "りじゅん", "profit, returns"],
    ["理性", "りせい", "reason, sense"],
    ["利息", "りそく", "interest (bank)"],
    ["立体", "りったい", "solid body"],
    ["立方", "りっぽう", "cube"],
    ["立法", "りっぽう", "legislation, lawmaking"],
    ["利点", "りてん", "advantage, point in favor"],
    ["略奪", "りゃくだつ", "pillage, looting, robbery"],
    ["略語", "りゃくご", "abbreviation, acronym"],
    ["流通", "りゅうつう", "circulation of money or goods, distribution"],
    ["領域", "りょういき", "area, territory, region"],
    ["了解", "りょうかい", "comprehension, consent, understanding"],
    ["領海", "りょうかい", "territorial waters"],
    ["両極", "りょうきょく", "both extremities, north and south poles"],
    ["良好", "りょうこう", "favorable, satisfactory"],
    ["良識", "りょうしき", "good sense"],
    ["良質", "りょうしつ", "good quality, superior quality"],
    ["了承", "りょうしょう", "acknowledgment, understanding"],
    ["良心", "りょうしん", "conscience"],
    ["領地", "りょうち", "territory"],
    ["領土", "りょうど", "territory, possession"],
    ["両立", "りょうりつ", "compatibility, coexistence, standing together"],
    ["旅客", "りょかく", "passenger"],
    ["旅券", "りょけん", "passport"],
    ["履歴", "りれき", "personal history, background, log"],
    ["理論", "りろん", "theory"],
    ["林業", "りんぎょう", "forestry"],
    ["類", "るい", "kind, class, family"],
    ["類推", "るいすい", "analogy"],
    ["類似", "るいじ", "analogous"],
    ["ルーズ", "ルーズ", "loose"],
    ["冷酷", "れいこく", "cruelty, coldheartedness, ruthless"],
    ["冷蔵", "れいぞう", "refrigeration"],
    ["冷淡", "れいたん", "coolness, indifference"],
    ["レース", "レース", "race; lace"],
    ["レギュラー", "レギュラー", "regular"],
    ["レッスン", "レッスン", "lesson"],
    ["レディー", "レディー", "lady"],
    ["レバー", "レバー", "lever; liver"],
    ["恋愛", "れんあい", "love, romance"],
    ["バッテリー", "バッテリー", "battery"],
    ["バット", "バット", "bat, vat"],
    ["発病", "はつびょう", "attack, to become sick"],
    ["初耳", "はつみみ", "something heard for the first time"],
    ["果て", "はて", "the end, the extremity, the limit(s)"],
    ["果てる", "はてる", "to end, to be finished, to be exhausted"],
    ["ばてる", "ばてる", "to be exhausted, to be worn out"],
    ["パトカー", "パトカー", "patrol car"],
    ["甚だ", "はなはだ", "very, greatly, exceedingly"],
    ["華々しい", "はなばなしい", "brilliant, magnificent, spectacular"],
    ["花びら", "はなびら", "(flower) petal"],
    ["華やか", "はなやか", "brilliant, gorgeous, florid"],
    ["阻む", "はばむ", "to keep someone from doing, to stop, to oppose"],
    ["浜", "はま", "beach, seashore"],
    ["浜辺", "はまべ", "beach, foreshore"],
    ["はまる", "はまる", "to get into, to go into, to fit, to be fit for, to suit"],
    ["早める", "はやめる", "to hasten, to quicken, to accelerate"],
    ["腹立ち", "はらだち", "anger"],
    ["原っぱ", "はらっぱ", "open field, empty lot, plain"],
    ["はらはら", "はらはら", "feel nervous"],
    ["ばらまく", "ばらまく", "to disseminate, to scatter"],
    ["張り紙", "はりがみ", "notice, poster"],
    ["遥か", "はるか", "far, far-away, distant"],
    ["破裂", "はれつ", "explosion, rupture, break off"],
    ["腫れる", "はれる", "to swell (from inflammation, to become swollen)"],
    ["繁栄", "はんえい", "prospering, prosperity, flourishing"],
    ["版画", "はんが", "art print"],
    ["ハンガー", "ハンガー", "(coat) hanger"],
    ["反感", "はんかん", "antipathy, revolt, animosity"],
    ["反響", "はんきょう", "echo, reverberation, repercussion"],
    ["パンク", "パンク", "puncture, bursting; punk"],
    ["反撃", "はんげき", "counterattack, counteroffensive, counterblow"],
    ["判決", "はんけつ", "judicial decision, judgment, sentence, decree"],
    ["反射", "はんしゃ", "reflection, reverberation"],
    ["繁盛", "はんじょう", "prosperity, flourishing, thriving"],
    ["繁殖", "はんしょく", "breed, multiply, propagation"],
    ["反する", "はんする", "to be inconsistent with, to oppose, to contradict"],
    ["判定", "はんてい", "judgment, decision, award, verdict"],
    ["万人", "ばんにん", "all people, everybody, 10000 people"],
    ["晩年", "ばんねん", "(one's) last years"],
    ["反応", "はんのう", "reaction, response"],
    ["万能", "ばんのう", "all-purpose, almighty, omnipotent"],
    ["半端", "はんぱ", "fragment, fraction, incompleteness"],
    ["反発", "はんぱつ", "repelling, rebound, oppose"],
    ["反乱", "はんらん", "rebellion, revolt, uprising"],
    ["氾濫", "はんらん", "overflowing, flood"],
    ["美", "び", "beauty"],
    ["ひいては", "ひいては", "not only…but also, in addition to, consequently"],
    ["ビールス", "ビールス", "virus"],
    ["控室", "ひかえしつ", "waiting room"],
    ["控える", "ひかえる", "to hold back; to make notes"],
    ["悲観", "ひかん", "pessimism, disappointment"],
    ["引き上げる", "ひきあげる", "to withdraw, to leave, to pull out"],
    ["率いる", "ひきいる", "to lead, to spearhead (a group), to command (troops)"],
    ["引き起こす", "ひきおこす", "to cause"],
    ["引下げる", "ひきさげる", "to pull down, to lower, to reduce, to withdraw"],
    ["引きずる", "ひきずる", "to drag along, to pull, to prolong"],
    ["引取る", "ひきとる", "to take back; to adopt; to leave"],
    ["否決", "ひけつ", "rejection, negation, voting down"],
    ["日頃", "ひごろ", "normally, habitually"],
    ["久しい", "ひさしい", "long, long-continued, old (story)"],
    ["悲惨", "ひさん", "tragedy, disaster; misery, wretched, pitiful"],
    ["ビジネス", "ビジネス", "business"],
    ["比重", "ひじゅう", "specific gravity"],
    ["美術", "びじゅつ", "art, fine arts"],
    ["秘書", "ひしょ", "(private) secretary"],
    ["微笑", "びしょう", "smile"],
    ["歪む", "ひずむ", "to warp, to be distorted"],
    ["密か", "ひそか", "secret, private, surreptitious"],
    ["浸す", "ひたす", "to soak, to dip, to drench"],
    ["ひたすら", "ひたすら", "nothing but, earnestly, intently"],
    ["左利き", "ひだりきき", "left-handedness, sake drinker, left-hander"],
    ["引っ掻く", "ひっかく", "to scratch"],
    ["必修", "ひっしゅう", "required (subject)"],
    ["びっしょり", "びっしょり", "wet through, drenched"],
    ["必然", "ひつぜん", "inevitable, necessary"],
    ["匹敵", "ひってき", "comparing with, rival, equal"],
    ["一息", "ひといき", "a breath, a pause, an effort"],
    ["人影", "ひとかげ", "man's shadow, soul"],
    ["人柄", "ひとがら", "personality, character"],
    ["人気", "ひとけ", "sign of life"],
    ["一頃", "ひところ", "once, some time ago"],
    ["人質", "ひとじち", "hostage"],
    ["一筋", "ひとすじ", "a line, earnestly, blindly, straightforwardly"],
    ["人目", "ひとめ", "glimpse, public gaze"],
    ["日取り", "ひどり", "fixed date, appointed day"],
    ["雛", "ひな", "young bird, chick, doll"],
    ["雛祭", "ひなまつり", "Girls' (dolls') Festival"],
    ["日向", "ひなた", "sunny place, in the sun"],
    ["非難", "ひなん", "blame, attack, criticism"],
    ["避難", "ひなん", "taking refuge, finding shelter"],
    ["日の丸", "ひのまる", "the Japanese flag"],
    ["火花", "ひばな", "spark"],
    ["ひび (かべの～)", "ひび (かべの～)", "crack, fissure, flaw"],
    ["悲鳴", "ひめい", "shriek, scream"],
    ["冷やかす", "ひやかす", "to banter, to make fun of, to jeer at, to cool, to refrigerate"],
    ["日焼け", "ひやけ", "sunburn"],
    ["標語", "ひょうご", "motto, slogan, catchword"],
    ["描写", "びょうしゃ", "depiction, description, portrayal"],
    ["ひょっと", "ひょっと", "possibly, accidentally"],
    ["びら", "びら", "handout, leaflet"],
    ["平たい", "ひらたい", "flat, even, level"],
    ["びり", "びり", "last on the list, at the bottom"],
    ["比率", "ひりつ", "ratio, proportion, percentage"]
  ]
};

// Hiragana / Katakana alphabet charts — [Character, Romaji] pairs.
// Covers base gojuon (46), voiced/dakuten + handakuten (25), and combined/youon sounds (33) = 104 each.
const KANA_DATA = {
  "hiragana": [
    ["あ", "a"],
    ["い", "i"],
    ["う", "u"],
    ["え", "e"],
    ["お", "o"],
    ["か", "ka"],
    ["き", "ki"],
    ["く", "ku"],
    ["け", "ke"],
    ["こ", "ko"],
    ["さ", "sa"],
    ["し", "shi"],
    ["す", "su"],
    ["せ", "se"],
    ["そ", "so"],
    ["た", "ta"],
    ["ち", "chi"],
    ["つ", "tsu"],
    ["て", "te"],
    ["と", "to"],
    ["な", "na"],
    ["に", "ni"],
    ["ぬ", "nu"],
    ["ね", "ne"],
    ["の", "no"],
    ["は", "ha"],
    ["ひ", "hi"],
    ["ふ", "fu"],
    ["へ", "he"],
    ["ほ", "ho"],
    ["ま", "ma"],
    ["み", "mi"],
    ["む", "mu"],
    ["め", "me"],
    ["も", "mo"],
    ["や", "ya"],
    ["ゆ", "yu"],
    ["よ", "yo"],
    ["ら", "ra"],
    ["り", "ri"],
    ["る", "ru"],
    ["れ", "re"],
    ["ろ", "ro"],
    ["わ", "wa"],
    ["を", "wo"],
    ["ん", "n"],
    ["が", "ga"],
    ["ぎ", "gi"],
    ["ぐ", "gu"],
    ["げ", "ge"],
    ["ご", "go"],
    ["ざ", "za"],
    ["じ", "ji"],
    ["ず", "zu"],
    ["ぜ", "ze"],
    ["ぞ", "zo"],
    ["だ", "da"],
    ["ぢ", "ji"],
    ["づ", "zu"],
    ["で", "de"],
    ["ど", "do"],
    ["ば", "ba"],
    ["び", "bi"],
    ["ぶ", "bu"],
    ["べ", "be"],
    ["ぼ", "bo"],
    ["ぱ", "pa"],
    ["ぴ", "pi"],
    ["ぷ", "pu"],
    ["ぺ", "pe"],
    ["ぽ", "po"],
    ["きゃ", "kya"],
    ["きゅ", "kyu"],
    ["きょ", "kyo"],
    ["しゃ", "sha"],
    ["しゅ", "shu"],
    ["しょ", "sho"],
    ["ちゃ", "cha"],
    ["ちゅ", "chu"],
    ["ちょ", "cho"],
    ["にゃ", "nya"],
    ["にゅ", "nyu"],
    ["にょ", "nyo"],
    ["ひゃ", "hya"],
    ["ひゅ", "hyu"],
    ["ひょ", "hyo"],
    ["みゃ", "mya"],
    ["みゅ", "myu"],
    ["みょ", "myo"],
    ["りゃ", "rya"],
    ["りゅ", "ryu"],
    ["りょ", "ryo"],
    ["ぎゃ", "gya"],
    ["ぎゅ", "gyu"],
    ["ぎょ", "gyo"],
    ["じゃ", "ja"],
    ["じゅ", "ju"],
    ["じょ", "jo"],
    ["びゃ", "bya"],
    ["びゅ", "byu"],
    ["びょ", "byo"],
    ["ぴゃ", "pya"],
    ["ぴゅ", "pyu"],
    ["ぴょ", "pyo"]
  ],
  "katakana": [
    ["ア", "a"],
    ["イ", "i"],
    ["ウ", "u"],
    ["エ", "e"],
    ["オ", "o"],
    ["カ", "ka"],
    ["キ", "ki"],
    ["ク", "ku"],
    ["ケ", "ke"],
    ["コ", "ko"],
    ["サ", "sa"],
    ["シ", "shi"],
    ["ス", "su"],
    ["セ", "se"],
    ["ソ", "so"],
    ["タ", "ta"],
    ["チ", "chi"],
    ["ツ", "tsu"],
    ["テ", "te"],
    ["ト", "to"],
    ["ナ", "na"],
    ["ニ", "ni"],
    ["ヌ", "nu"],
    ["ネ", "ne"],
    ["ノ", "no"],
    ["ハ", "ha"],
    ["ヒ", "hi"],
    ["フ", "fu"],
    ["ヘ", "he"],
    ["ホ", "ho"],
    ["マ", "ma"],
    ["ミ", "mi"],
    ["ム", "mu"],
    ["メ", "me"],
    ["モ", "mo"],
    ["ヤ", "ya"],
    ["ユ", "yu"],
    ["ヨ", "yo"],
    ["ラ", "ra"],
    ["リ", "ri"],
    ["ル", "ru"],
    ["レ", "re"],
    ["ロ", "ro"],
    ["ワ", "wa"],
    ["ヲ", "wo"],
    ["ン", "n"],
    ["ガ", "ga"],
    ["ギ", "gi"],
    ["グ", "gu"],
    ["ゲ", "ge"],
    ["ゴ", "go"],
    ["ザ", "za"],
    ["ジ", "ji"],
    ["ズ", "zu"],
    ["ゼ", "ze"],
    ["ゾ", "zo"],
    ["ダ", "da"],
    ["ヂ", "ji"],
    ["ヅ", "zu"],
    ["デ", "de"],
    ["ド", "do"],
    ["バ", "ba"],
    ["ビ", "bi"],
    ["ブ", "bu"],
    ["ベ", "be"],
    ["ボ", "bo"],
    ["パ", "pa"],
    ["ピ", "pi"],
    ["プ", "pu"],
    ["ペ", "pe"],
    ["ポ", "po"],
    ["キャ", "kya"],
    ["キュ", "kyu"],
    ["キョ", "kyo"],
    ["シャ", "sha"],
    ["シュ", "shu"],
    ["ショ", "sho"],
    ["チャ", "cha"],
    ["チュ", "chu"],
    ["チョ", "cho"],
    ["ニャ", "nya"],
    ["ニュ", "nyu"],
    ["ニョ", "nyo"],
    ["ヒャ", "hya"],
    ["ヒュ", "hyu"],
    ["ヒョ", "hyo"],
    ["ミャ", "mya"],
    ["ミュ", "myu"],
    ["ミョ", "myo"],
    ["リャ", "rya"],
    ["リュ", "ryu"],
    ["リョ", "ryo"],
    ["ギャ", "gya"],
    ["ギュ", "gyu"],
    ["ギョ", "gyo"],
    ["ジャ", "ja"],
    ["ジュ", "ju"],
    ["ジョ", "jo"],
    ["ビャ", "bya"],
    ["ビュ", "byu"],
    ["ビョ", "byo"],
    ["ピャ", "pya"],
    ["ピュ", "pyu"],
    ["ピョ", "pyo"]
  ]
};
